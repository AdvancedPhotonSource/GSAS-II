...
try:
    ...
    ):  # TODO: I think this may use a fair amount of memory; delay import?
...
try:
    ...
    blue.reverse()
    green = [tuple(1.0 - np.array(item)) for item in _Old_Paired_data["green"]]
    green.reverse()
    red = [tuple(1.0 - np.array(item)) for item in _Old_Paired_data["red"]]
    red.reverse()
    Old_Paired_data_r = {"blue": blue, "green": green, "red": red}
    oldpaired_r = mpl.colors.LinearSegmentedColormap(
        "GSPaired_r", Old_Paired_data_r, N=256
    )
    try:
...
class G2PlotNoteBook(wx.Panel):
    ...
    def __init__(self, parent, id=-1, G2frame=None):
        ...
        self.nb = wx.aui.AuiNotebook(
            self, style=wx.aui.AUI_NB_DEFAULT_STYLE ^ wx.aui.AUI_NB_CLOSE_ON_ACTIVE_TAB
        )
        sizer = wx.BoxSizer()
        ...
        self.allowZoomReset = (
            True  # this indicates plot should be updated not initialized
        )
    ...
    def FindPlotTab(self, label, Type, newImage=True, publish=None):
        ...
        try:
            ...
            if Type in ("mpl", "3d"):
                ...
                if (
                    len(Axes) > 1 and Plot.get_aspect() == "auto"
                ):  # aspect will be equal for image plotting
    ...
    def add3D(self, name=""):
        ...
        if mplv[0] == 3:
            if mplv == [3, 0, 3] or mplv[1] >= 3:
                ...
                G2G.G2MessageBox(
                    self,
                    "3D plots with Matplotlib 3.1.x and 3.2.x are distorted, use MPL 3.0.3 or 3.3. You have "
                    + mpl.__version__,
    ...
    def InvokeTreeItem(self, pid):
        ...
        if pid:
            self.G2frame.GPXtree.SelectItem(pid)
    ...
    def __init__(self, plotCanvas, publish=None, Arrows=True):
        ...
        if wx.__version__.startswith("4.2"):
            self.SetToolBitmapSize(
                wx.Size(28, 20)
            )  # seems needed in wx4.2, packs icons closer
        self.AddToolBarTool("Key press", "Select key press", "key.ico", self.OnKey)
        self.AddToolBarTool(
            "Help on", "Show help on this plot", "help.ico", self.OnHelp
        )
        ...
        if Arrows:
            for direc in (
                ...
                self.arrows[direc] = self.AddToolBarTool(
                    sprfx + direc, prfx + direc, fil, self.OnArrow
                )
        if publish:
            self.AddToolBarTool(
                "Publish plot",
    ...
    def OnKey(self, event):
        ...
        if parent.Choice:
            ...
            dlg = wx.SingleChoiceDialog(
                parent, "Select a keyboard command", "Key press list", choices
            )
...
def PlotFPAconvolutors(G2frame, NISTpk, conv2T=None, convI=None, convList=None):
    ...
    Plot.set_title(f"Peak convolution functions @ 2theta={cntr:.3f}")
    ...
    for conv in convList:
        ...
        FFT = FP.best_irfft(f)
        if f[1].real > 0:
            FFT = np.roll(FFT, int(len(FFT) / 2.0))
        FFT /= FFT.max()
        ttArr = np.linspace(
            -NISTpk.twotheta_window_fullwidth_deg / 2,
            NISTpk.twotheta_window_fullwidth_deg / 2,
            len(FFT),
        )
        ttmin = min(ttmin, ttArr[np.argmax(FFT > 0.005)])
        ttmax = max(ttmax, ttArr[::-1][np.argmax(FFT[::-1] > 0.005)])
...
def onLegendPick(event):
    ...
    if canvas.timer:  # clear previous highlight
        ...
        return
    if (
        event.artist in plot.get_legend().get_lines()
    ):  # is this an artist item in the legend?
        lbl = event.artist.get_label()
    elif (
        event.artist in plot.get_legend().get_texts()
    ):  # is this a text item in the legend?
    ...
    for l in plot.get_lines():  # noqa: E741
        if lbl == l.get_label():
            ...
            # GSASIIpath.IPyBreak()
            canvas.timer.lineinfo = (
                l,
                l.get_marker(),
                l.get_markersize(),
                l.get_markeredgewidth(),
            )
    ...
    for refl in HKLref:
        ...
        if H[izone] == Data["Layer"]:
            ...
            if Type in ["|DFsq|/sig", "|DFsq|>sig", "|DFsq|>3sig"]:
                if A > 0.0:
                    Plot.add_artist(
                        Circle(xy, radius=A, ec="g", fc="w", picker=True, gid=hid)
                    )
                else:
                    Plot.add_artist(
                        Circle(xy, radius=-A, ec="r", fc="w", picker=True, gid=hid)
                    )
            else:
                if A > 0.0 and A > B:
                ...
                if B:
                    Plot.add_artist(
                        Circle(xy, radius=B, ec="b", fc="w", picker=True, gid=hid)
                    )
                    if A < B:
                        Plot.add_artist(Circle(xy, radius=A, ec="g", fc="w"))
                    radius = C
                    if radius > 0:
                        if A > B:
                            if refl[3 + Super] < 0:
                                Plot.add_artist(
                                    Circle(
                                        xy, radius=radius, ec=(0.0, 1, 0.0, 0.1), fc="g"
                                    )
                                )
                            else:
                                Plot.add_artist(
                                    Circle(xy, radius=radius, fc="g", ec="g")
                                )
                        elif refl[3 + Super] < 0:
                            Plot.add_artist(
                                Circle(
                                    xy, radius=radius, fc=(1.0, 0.0, 0.0, 0.1), ec="r"
                                )
                            )
                        else:
...
def Plot1DSngl(G2frame, newPlot=False, hklRef=None, Super=0, Title=False):
    ...
    def OnMotion(event):
        ...
        if xpos:  # avoid out of frame mouse position
            Xpos = xpos
            if Page.qaxis:
                dT = np.fabs(2.0 * np.pi / limx[0] - 2.0 * np.pi / limx[1]) / 100.0
                Xpos = 2.0 * np.pi / xpos
                found = hklRef[np.where(np.fabs(hklRef.T[4 + Super] - Xpos) < dT / 2.0)]
            else:
                dT = np.fabs(limx[1] - limx[0]) / 100.0
                found = hklRef[np.where(np.fabs(hklRef.T[4 + Super] - xpos) < dT / 2.0)]
            s = ""
            if len(found):
            ...
            try:
                ...
                G2frame.G2plotNB.status.SetStatusText(
                    "Select " + Title + Name + " pattern first", 1
                )
    ...
    def Draw():
        ...
        if "HKLF" in Name:
            ...
            Fosq, sig, Fcsq = (
                hklRef.T[8 + Super],
                np.nan_to_num(np.sqrt(hklRef.T[8 + Super])),
                hklRef.T[9 + Super],
            )
        ...
        if not Page.vaxis:
            ...
            XD = (
                np.vstack((X, X, np.zeros_like(X) - Ymax / 10.0, Y - Z - Ymax / 10.0))
                .reshape((2, 2, -1))
                .T
            )
        ...
        xylim = np.array(
            [
                [np.min(X), np.max(X)],
                [np.min(Y - Z - Ymax / 10.0), np.max(np.concatenate((Y, Z)))],
            ]
        )
        dxylim = np.array([xylim[0][1] - xylim[0][0], xylim[1][1] - xylim[1][0]]) / 20.0
        xylim[0, 0] -= dxylim[0]
        xylim[0, 1] += dxylim[0]
        xylim[1, 0] -= dxylim[1]
        xylim[1, 1] += dxylim[1]
...
def Plot3DSngl(G2frame, newPlot=False, Data=None, hklRef=None, Title=False):  # noqa: ARG001
    ...
    def OnKeyBox(event):
        ...
        if mode in [
            ...
            Pix = GL.glReadPixels(
                0, 0, size[0], size[1], GL.GL_RGB, GL.GL_UNSIGNED_BYTE
            )
    ...
    Name = G2frame.GPXtree.GetItemText(G2frame.PatternId)
    if (
        Title and Title in G2frame.GetPhaseData()
    ):  # NB: save image as e.g. jpeg will fail if False; MyDir is unknown
    ...
    B4mat = np.concatenate(
        (
            np.concatenate((Bmat, [[0], [0], [0]]), axis=1),
            [
                [0, 0, 0, 1],
            ],
        ),
    ...
    def FillHKLRC():
        ...
        for i, refl in enumerate(hklRef):
            if Data["Shell"][1]:
                if not (
                    Data["Shell"][0] <= 0.5 / refl[4 + Super] <= Data["Shell"][0] + 0.1
                ):
                    continue
            H = refl[:3]
            if "HKLF" in Name:
        ...
        RF2 = 100.0
        if sumFo and sumDF:
            RF = 100.0 * sumDF / sumFo
            RF2 = 100.0 * sumDF2 / sumFo2
        return HKL, zip(list(R), C, strict=False), RF, RF2
    ...
    def GetTruePosition(xy):
        ...
        for _i, ref in enumerate(hklRef):
            ...
            try:
                ...
                XY = [int(X), int(Y)]
                if np.allclose(xy, XY, atol=10) and Zmax > Z:
                    Zmax = Z
    ...
    def SetRotation(newxy):
        ...
        dxy = newxy - oldxy
        if dxy[0] == dxy[1] == 0:
            return  # on Mac motion can be less than a full pixel!
        drawingData["oldxy"] = list(newxy)
        V = np.array([dxy[1], dxy[0], 0.0])
    ...
    def OnMouseWheel(event):
        if event.ShiftDown():
            return
        drawingData["cameraPos"] += event.GetWheelRotation() / 120.0
    ...
    def RenderBox(x, y, z):
        ...
        for line, color in zip(uEdges, uColors, strict=False):
    ...
    def RenderUnitVectors(x, y, z, labxyz=None):
        ...
        GL.glTranslate(x, y, z)
        GL.glBegin(GL.GL_LINES)
        for line, color in list(zip(uEdges, uColors, strict=False))[:3]:
            GL.glColor3ubv(color)
    ...
    def RenderDots(XYZ, RC):
        ...
        XYZ = np.array(XYZ)
        GL.glPushMatrix()
        for xyz, rc in zip(XYZ, RC, strict=False):
            x, y, z = xyz
            r, c = rc
            GL.glColor3ubv(c)
            GL.glPointSize(r * 50)
    ...
    def Draw(caller=""):  # noqa: ARG001
        ...
        Zclip = drawingData["Zclip"] * cPos / 20.0
        if Data["Zone"]:
            Zclip = 0.002
        Q = drawingData["Quaternion"]
        ...
        if Data["Zone"]:
            G2frame.G2plotNB.status.SetStatusText(
                ...
                % (
                    ...
                    super2,
                    RF2,
                    str(list(drawingData["viewPoint"][0])),
                ),
                1,
            )
        elif Data["Shell"][1]:
            G2frame.G2plotNB.status.SetStatusText(
                ...
                % (Data["Type"], Name, len(HKL), RF, super2, RF2, Data["Shell"][0]),
                1,
            )
        else:
            G2frame.G2plotNB.status.SetStatusText(
                "Plot type = %s for %s; N = %d, RF = %6.2f%%, RF%s = %6.2f%%"
                % (Data["Type"], Name, len(HKL), RF, super2, RF2),
                1,
            )
        ...
        GLU.gluPerspective(20.0, aspect, cPos - Zclip, cPos + Zclip)
        ...
        matRot = np.concatenate(
            (
                np.concatenate((matRot, [[0], [0], [0]]), axis=1),
                [
                    [0, 0, 0, 1],
                ],
            ),
    ...
    choice = [
        ...
        "d: Fo-Fc",
        "w: DF/sig",
        "i: toggle intensity scaling",
    ]
    cb = wx.ComboBox(
        G2frame.G2plotNB.status,
...
def PlotDeltSig(G2frame, kind, PatternName=None):
    ...
    G2frame.G2plotNB.RegisterRedrawRoutine(
        ...
        PlotDeltSig,
        (G2frame, kind, G2frame.GPXtree.GetItemText(G2frame.PatternId)),
    )
    Page.Choice = None
    ...
    if kind == "PWDR":
        ...
        sumWdelt = np.sum(
            (xye[1][xmin:xmax] - xye[3][xmin:xmax]) ** 2 * xye[2][xmin:xmax]
        )
        print(
            " Nobs = %d, Rp = %.4f%%, Rwp = %.4f%%"
            % (Nobs, 100.0 * sumDelt / sumObs, 100.0 * np.sqrt(sumWdelt / sumWobs))
        )
    elif kind == "HKLF":
        ...
        print(
            " Nobs = %d, RF = %.4f%%, RF2 = %.4f%%, wRF2 = %.4f%%"
            % (
                Nobs,
                100.0 * sumDelt / sumFobs,
                100.0 * sum2Delt / sumF2obs,
                100.0 * np.sqrt(sumWdelt / sumWobs),
            )
        )
    G2frame.G2plotNB.status.DestroyChildren()  # get rid of special stuff on status bar
    DS.sort()
    EDS = np.zeros_like(DS)
    DX = np.linspace(0.0, 1.0, num=len(DS), endpoint=True)
    ...
    low1 = np.searchsorted(EDS, -1.0)
    hi1 = np.searchsorted(EDS, 1.0)
...
def PlotISFG(G2frame, data, newPlot=False, plotType="", peaks=None):
    ...
    def OnPick(event):
        ...
        if not ind[0]:  # a limit line - allow it to drag
            ...
            Peaks["Peaks"].append(
                [
                    xy[0],
                    (xy[1] - Peaks["Background"][1][1] * xy[0]) / 4.7,
                    0.085,
                    "",
                    "O",
                    "O",
    ...
    if Peaks is None:
        if G2frame.Contour:
            ...
            Page.Choice = (
                ...
                "c: contour on",
                "g: toggle grid",
                "w: toggle waterfall colors (slow!)",
                "m: toggle multiplot",
                "s: color scheme",
                "f: select data",
            )
    ...
    for N, Pattern in enumerate(PlotList):
        ...
        if G2frame.Contour and len(PlotList) > 1:
            ...
            ifCalc = False
            X = xye[0] + Page.Offset[0] * 0.005 * N
            Y = xye[1] + Page.Offset[1] * 0.01 * N
            if (
                xye.shape[0] > 2 and len(PlotList) == 1
            ):  # PDF calc present - single plot only
                ifCalc = True
                ...
                XYlist.append(list(zip(X, Y, strict=False)))
    #            if G2frame.Legend:
    ...
    if G2frame.Contour and len(PlotList) > 1:
        ...
        Img = Plot.imshow(
            ...
            extent=[ContourX[0], ContourX[-1], ContourY[0], ContourY[-1]],
            aspect="auto",
            origin="lower",
        )
        ...
        if Peaks is None:
            ...
            if XYlist.shape[0] > 1:
                if G2frame.Waterfall:
                    for xylist in XYlist:
                        ...
                        segs = np.reshape(
                            np.hstack((xylist[:-1], xylist[1:])), (-1, 2, 2)
                        )
                ...
                elif ifCalc:  # obs, calc & diff
                    Plot.plot(
                        XYlist[0][0], XYlist[0][1], color="b", marker="+", linewidth=0
                    )
                    Plot.plot(
                        XYlist[1][0][ibeg:ifin], XYlist[1][1][ibeg:ifin], color="g"
                    )
                    Plot.plot(
                        XYlist[2][0][ibeg:ifin], XYlist[2][1][ibeg:ifin], color="r"
                    )
                else:
                    ...
                    if G2frame.Legend:
                        ...
                        labels = [
                            os.path.splitext(i[3 : i.find("Azm")].strip())[0]
                            for i in choices
                        ]
                        numlines = len(labels)
                        # create an empty labeled line for each label with color from color map
                        for i, lbl in enumerate(labels):
                            color = acolor(int(0.5 + acolor.N * i / (numlines - 1.0)))
                            lgndlist.append(
                                mpl.lines.Line2D([], [], color=color, label=lbl)
                            )
                        Plot.legend(handles=lgndlist, loc="best")
            elif G2frame.Waterfall:
                colorRange = XYlist[0].T[1]
                segs = np.reshape(
                    np.hstack((XYlist[0][:-1], XYlist[0][1:])), (-1, 2, 2)
                )
                line = mplC.LineCollection(segs, cmap=acolor, norm=normcl)
                line.set_array(colorRange)
                Plot.add_collection(line)
            ...
            if plotType == "G(R)" and numbDen:
                Xb = [0.0, 5.0]
                Yb = [0.0, -20.0 * np.pi * numbDen]
                Plot.plot(Xb, Yb, color="k", dashes=(5, 5))
                Plot.set_xlim([0.0, PDFdata["Rmax"]])
            elif plotType == "g(r)":
                Plot.set_xlim([0.0, PDFdata["Rmax"]])
            elif plotType == "F(Q)":
                Xb = [0.0, 5.0]
            ...
            for peak in Peaks["Peaks"]:
                G2frame.Lines.append(
                    Plot.axvline(peak[0], color="r", picker=True, pickradius=2.0)
                )
            Xb = [0.0, peaks["Limits"][1]]
            Yb = [0.0, Xb[1] * peaks["Background"][1][1]]
            Plot.plot(Xb, Yb, color="k", dashes=(5, 5))
    #    elif G2frame.Legend:
...
def PlotCalib(G2frame, Inst, XY, Sigs, newPlot=False):
    ...
    for ixy, xyw in enumerate(XY):
        ...
        if W:
            ...
            Plot.plot(Q, -bin, "b+")
        Plot.axhline(0.0, color="r", linestyle="--")
    Plot.legend(loc="best")
...
def PlotXY(
    ...
    def OnKeyPress(event):
        if event.key == "u":
            if Page.Offset[1] < 100.0:
                Page.Offset[1] += 1.0
        elif event.key == "d":
            if Page.Offset[1] > 0.0:
                Page.Offset[1] -= 1.0
        elif event.key == "l":
            Page.Offset[0] -= 1.0
        elif event.key == "r":
    ...
    def Draw():
        ...
        for ixy, xy in enumerate(XY):
            ...
            if lines:
                ...
                if len(names):
                    Plot.plot(
                        X + dX, Y + dY, colors[ixy % NC], picker=False, label=names[ixy]
                    )
                else:
        ...
        if len(vertLines):
            for ixy, X in enumerate(vertLines):
                ...
                for x in X:
                    Plot.axvline(
                        x + dX, color=colors[ixy % NC], dashes=(5, 5), picker=False
                    )
        if XY2 is not None and len(XY2):
            for ixy, xy in enumerate(XY2):
                X, Y = xy
                dX = Page.Offset[0] * (ixy + 1) * Xmax / 500.0
                dY = Page.Offset[1] * (ixy + 1) * Ymax / 100.0
                if points2:
                    if len(names2):
                        Plot.scatter(
                            X,
                            Y,
                            marker="+",
                            color=colors[(ixy + 1) % NC],
                            picker=False,
                            label=names2[ixy],
                        )
                    else:
                        Plot.scatter(
                            X, Y, marker="+", color=colors[(ixy + 1) % NC], picker=False
                        )
                elif len(names2):
                    Plot.plot(
                        X + dX,
...
#### PlotXYZvect ################################################################################
def PlotXYZvect(
    G2frame, X, Y, Z, R, labelX=r"X", labelY=r"Y", labelZ=r"Z", Title="", PlotName=None
):
...
def Plot3dXYZ(
    ...
    PHI = np.linspace(0.0, 360.0, int(nY), True)
    PSI = np.linspace(0.0, 180.0, int(nX), True)
    X = Zmul * np.outer(npcosd(PHI), npsind(PSI)) / 2.0
    Y = Zmul * np.outer(npsind(PHI), npsind(PSI)) / 2.0
    Z = Zmul * np.outer(np.ones(np.size(PHI)), npcosd(PSI)) / 2.0
    ...
    if np.any(X) and np.any(Y) and np.any(Z):
        ...
        if True:
            ...
            xyzlim = np.array(
                [Plot.get_xlim3d(), Plot.get_ylim3d(), Plot.get_zlim3d()]
            ).T
            XYZlim = [min(xyzlim[0]), max(xyzlim[1])]
            Plot.set_xlim3d(XYZlim)
            Plot.set_ylim3d(XYZlim)
...
#### PlotAAProb ################################################################################
def PlotAAProb(
    G2frame, resNames, Probs1, Probs2, Title="", thresh=None, pickHandler=None
):
    "Needs a description"

    def OnMotion(event):
        xpos, ypos = event.xdata, event.ydata
        if xpos and xpos > 1.0:
            if 0 <= xpos < len(resNames):
                resName = resNames[int(xpos + 0.5) - 1]
            else:
                resName = ""
            SetCursor(Page)
            try:
...
def PlotStrain(G2frame, data, newPlot=False):
    ...
    for N, item in enumerate(data["d-zero"]):
        ...
        Plot.plot(
            [0.0, 360.0], [item["Dcalc"], item["Dcalc"]], colors[5], dashes=(5, 5)
        )
    if not newPlot:
        Page.toolbar.push_current()
        Plot.set_xlim(xylim[0])
...
def PlotNamedFloatHBarGraph(
    G2frame, Xvals, Ynames, Xlabel="Value", Ylabel="", Title="", PlotName=None
):
    ...
    def OnPageChanged(event):  # noqa: ARG001
        ...
        if PlotText == PlotName:
            PlotNamedFloatHBarGraph(
                G2frame, Xvals, Ynames, Xlabel, Ylabel, Title, PlotText
            )
    ...
    colors = ["blue" if x >= 0.0 else "red" for x in Xvals]
    Page.Choice = None
    Plot.set_title(Title)
...
def PlotSASDSizeDist(G2frame):
    ...
    if data["Size"]["logBins"]:
        ...
        Plot.set_xlim([np.min(2.0 * Bins) / 2.0, np.max(2.0 * Bins) * 2.0])
    Plot.bar(
        2.0 * Bins - Dbins, BinMag, 2.0 * Dbins, facecolor="white", edgecolor="green"
    )  # plot diameters
    #    colors=['b','r','c','m','k']
    colors = [
        "xkcd:blue",
    ...
    if "Size Calc" in data:
        ...
        for i in range(len(Rbins)):
            if len(Rbins[i]):
                Plot.plot(
                    2.0 * Rbins[i], Dist[i], color=colors[i % NC]
                )  # plot diameters
    Page.canvas.draw()
...
def PlotSASDPairDist(G2frame):
    ...
    if len(data["Pair"]["Distribution"]):
        ...
        Plot.bar(
            Bins - Dbins, BinMag, Dbins, facecolor="white", edgecolor="green"
        )  # plot diameters
...
def PlotPowderLines(G2frame, indexFrom=""):
    ...
    xmax = peaks[-1][0]
    delt = xmax - xmin
    xlim = [max(0, xmin - delt / 20.0), min(180.0, xmax + delt / 20.0)]
    Plot.set_xlim(xlim)
    Page.canvas.draw()
    Page.toolbar.push_current()
...
def PlotPeakWidths(G2frame, PatternName=None):
    ...
    def OnKeyPress(event):
        if event.key == "g":
            ...
            dlg = wx.FileDialog(
                G2frame,
                "Choose CSV file to write",
                G2G.GetExportPath(G2frame),
            ...
            if "C" in Parms["Type"][0] or "B" in Parms["Type"][0]:
                Write2csv(
                    ...
                    [
                        "Q",
                        "Gauss-def",
                        "Lorenz-def,total-def",
                        "Gauss-fit",
                        "Lorenz-fit,total-fit",
                    ],
                    header=True,
                )
                for vals in zip(Q, Y, Z, W, Yf, Zf, Wf, strict=False):
                    Write2csv(fp, vals)
            else:
                Write2csv(
                    fp,
                ...
                for vals in zip(Q, S, G, W, Sf, Gf, Wf, strict=False):
                    Write2csv(fp, vals)
            fp.close()
    ...
    G2frame.G2plotNB.RegisterRedrawRoutine(
        G2frame.G2plotNB.lastRaisedPlotTab,
        PlotPeakWidths,
        (G2frame, G2frame.GPXtree.GetItemText(G2frame.PatternId)),
    )
    ...
    if "T" in Parms["Type"][0]:  #'T'OF
        ...
        for peak in peaks:
            ...
            Ap.append(peak[4])
            Bp.append(peak[6])
            Qp.append(2.0 * np.pi * difC / peak[0])
        ...
        EtoQ = 4.0 * np.pi * npsind(tth / 2.0) / 12.3986
        Q = EtoQ * X
        Z = np.ones_like(X)
        ...
        Xmin, Xmax = limits[1]
        X = np.linspace(Xmin, Xmax, num=101, endpoint=True)
        Q = 4.0 * np.pi * npsind(X / 2.0) / lam
        Z = np.ones_like(X)
        data = G2mth.setPeakparms(Parms, Parms2, X, Z)
        s = np.sqrt(data[isig]) * np.pi / 18000.0  # var -> sig(radians)
        g = data[igam] * np.pi / 18000.0  # centideg -> radians
        G = G2pwd.getgamFW(g, s)  # /2.  #delt-theta from TCH fxn
        Y = sq8ln2 * s / nptand(X / 2.0)
        Z = g / nptand(X / 2.0)
        W = G / nptand(X / 2.0)
        Plot.plot(Q, Y, color="r", label="Gaussian")
        Plot.plot(Q, Z, color="g", label="Lorentzian")
        ...
        for did in [isig, igam]:
            if np.any(fit[did] < 0.0):
                negWarn = True
        sf = np.sqrt(fit[isig]) * np.pi / 18000.0
        gf = fit[igam] * np.pi / 18000.0
        Gf = G2pwd.getgamFW(gf, sf)  # /2.
        Yf = sq8ln2 * sf / nptand(X / 2.0)
        Zf = gf / nptand(X / 2.0)
        Wf = Gf / nptand(X / 2.0)
        Plot.plot(Q, Yf, color="r", dashes=(5, 5), label="Gaussian fit")
        Plot.plot(Q, Zf, color="g", dashes=(5, 5), label="Lorentzian fit")
        ...
        for peak in peaks:
            ...
            try:
                s = math.sqrt(peak[isig]) * math.pi / 18000.0
            except ValueError:
                s = 0.01
            g = peak[igam] * math.pi / 18000.0
            G = G2pwd.getgamFW(g, s)  # /2.
            Yp.append(sq8ln2 * s / tand(peak[0] / 2.0))
            Zp.append(g / tand(peak[0] / 2.0))
            Wp.append(G / tand(peak[0] / 2.0))
        if len(peaks):
            Plot.plot(Xp, Yp, "+", color="r", label="G peak")
            Plot.plot(Xp, Zp, "+", color="g", label="L peak")
            Plot.plot(Xp, Wp, "+", color="b", label="G+L peak")
    ...
    if negWarn:
        Plot.set_title(
            "WARNING: profile coefficients yield negative peak widths; peaks may be skipped in calcuations"
        )
...
def PlotDeform(G2frame, general, atName, atType, deform, UVmat, radial, neigh):
    ...
    if not new:
        if not Page.IsShown():
            Page.Show()
    PHI = np.linspace(0.0, 360.0, 31, True)
    PSI = np.linspace(0.0, 180.0, 31, True)
    ...
    for shc in SHC:
        p = (
            ...
            * SHC[shc][0]
            * SHC[shc][2] ** 3
            * G2lat.KslCalc(shc, RAP[1], RAP[2]).reshape((31, 31))
        )
        P += p**2
    if not np.any(P):
        P = np.ones((31, 31))
    #    P *= P
    color = np.array(general["Color"][general["AtomTypes"].index(atType)]) / 255.0
    Plot.plot_surface(
        X * P, Y * P, Z * P, rstride=1, cstride=1, color=color, linewidth=1
    )
    for atm in neigh[0]:
        x, y, z = np.inner(atm[3], UVmat)
        color = np.array(general["Color"][general["AtomTypes"].index(atm[1])]) / 255.0
        Plot.plot_surface(
            X + x, Y + y, Z + z, rstride=1, cstride=1, color=color, linewidth=1
        )
    xyzlim = np.array([Plot.get_xlim3d(), Plot.get_ylim3d(), Plot.get_zlim3d()]).T
    XYZlim = [min(xyzlim[0]), max(xyzlim[1])]
    Plot.set_xlim3d(XYZlim)
...
def PlotSizeStrainPO(G2frame, data, hist=""):
        ...
        if event.xdata and event.ydata:  # avoid out of frame errors
            ...
            if r <= 1.0:
                if "Eq. area" in plotType:
                    r, p = 2.0 * npasind(np.sqrt(r) * sq2), npatan2d(xpos, ypos)
                else:
                    r, p = 2.0 * npatand(np.sqrt(r)), npatan2d(xpos, ypos)
                if p < 0.0:
                    p += 360.0
                ipf = lut(r * np.pi / 180.0, p * np.pi / 180.0)
                p = 90.0 - p
                if p < 0.0:
                    p += 360.0
    ...
    plotDict = {
        "Mustrain": "Mustrain",
        "Size": "Size",
        "Preferred orientation": "Pref.Ori.",
    ...
    PHI = np.linspace(0.0, 360.0, 40, True)
    PSI = np.linspace(0.0, 180.0, 40, True)
    ...
    if plotType in ["Mustrain", "Size"]:
        ...
        if coeff[0] == "isotropic":
            ...
            XYZ = np.dstack((X, Y, Z))
            XYZ = np.nan_to_num(
                np.apply_along_axis(uniaxCalc, 2, XYZ, iso, aniso, axes)
            )
            ...
            if np.any(Shkl):
                XYZ = np.dstack((X, Y, Z))
                XYZ = np.nan_to_num(
                    np.apply_along_axis(genMustrain, 2, XYZ, SGData, Amat, Shkl)
                )
                X, Y, Z = np.dsplit(XYZ, 3)
                X = X[:, :, 0]
                Y = Y[:, :, 0]
        ...
        if coeff[0] == "MD":
            ...
            Plot.legend(loc="best")
            Plot.set_title(
                "Axial distribution for HKL=" + str(PH) + " in " + phase + "\n" + hist
            )
        ...
        RefSets = []
        for ir, refSet in enumerate(refSets):
            r, beta, phi = G2lat.HKL2SpAng(
                refSet, cell[:6], SGData
        ...
        Rmd = np.array(Rmd)
        Rmd = np.where(Rmd < 0.0, 0.0, Rmd)
        if "Eq. area" in plotType:
            x, y = (
                np.sin(Beta / 2.0) * np.cos(Phi) / sq2,
                np.sin(Beta / 2.0) * np.sin(Phi) / sq2,
            )
        else:
            x, y = np.tan(Beta / 2.0) * np.cos(Phi), np.tan(Beta / 2.0) * np.sin(Phi)
        npts = 101
        X, Y = np.meshgrid(np.linspace(1.0, -1.0, npts), np.linspace(-1.0, 1.0, npts))
        R, P = np.sqrt(X**2 + Y**2).flatten(), npatan2d(Y, X).flatten()
        P = np.where(P < 0.0, P + 360.0, P)
        if "Eq. area" in plotType:
            R = np.where(R <= 1.0, 2.0 * npasind(R * sq2), 0.0)
        else:
            R = np.where(R <= 1.0, 2.0 * npatand(R), 0.0)
        Z = np.zeros_like(R)
        try:
            sfac = 0.1
            while True:
                try:
                    ...
                    sfac *= 1.05
            Z = [
                lut(ri * np.pi / 180.0, p * np.pi / 180.0)
                for ri, p in zip(list(R), list(P), strict=False)
            ]
        #            print ('IVP for histogramn: %s: interpolate sfactor: %.2f'%(hist,sfac))
            ...
            G2G.G2MessageBox(
                G2frame,
                "IVP interpolate error: scipy needs to be 0.11.0 or newer",
                "IVP error",
            )
            return
...
def PlotTexture(G2frame, data, Start=False):  # noqa: ARG001
        ...
        if event.xdata and event.ydata:  # avoid out of frame errors
            ...
            if "Inverse" in SHData["PlotType"]:
                ...
                if z <= 1.0:
                    z = np.sqrt(z)
                    if "equal" in G2frame.Projection:
                        r, p = 2.0 * npasind(z * sq2), npatan2d(ypos, xpos)
                    else:
                        r, p = 2.0 * npatand(z), npatan2d(ypos, xpos)
                    pf = G2lat.polfcal(
                        ODFln,
                        SamSym[textureData["Model"]],
                        np.array(
                            [
                                r,
                            ]
    ...
    if "Axial" in SHData["PlotType"]:
        ...
        if "Inverse" in SHData["PlotType"]:
            X, Y = np.meshgrid(
                np.linspace(1.0, -1.0, npts), np.linspace(-1.0, 1.0, npts)
            )
            R, P = np.sqrt(X**2 + Y**2).flatten(), npatan2d(Y, X).flatten()
            if "equal" in G2frame.Projection:
                R = np.where(R <= 1.0, 2.0 * npasind(R * sq2), 0.0)
            ...
            PSI = PSI.flatten() * 6.0
            GAM = GAM.flatten() * 12.0
            P = G2lat.polfcal(ODFln, SamSym[textureData["Model"]], PSI, GAM).reshape(
                (31, 31)
            )
            GAM = np.linspace(0.0, 360.0, 31, True)
            PSI = np.linspace(0.0, 180.0, 31, True)
            ...
            if "det Angles" in textureData and textureData["ShoDet"]:
                ...
                if "equal" in G2frame.Projection:
                    Rdet = npsind(Rdet / 2.0) / sq2
                else:
                    Rdet = nptand(Rdet / 2.0)
                Xdet = list(npcosd(Pdet) * Rdet)
                Ydet = list(npsind(Pdet) * Rdet)
                for i, [x, y] in enumerate(zip(Xdet, Ydet, strict=False)):
                    Plot.plot(
                        x,
                        -y,
                        "k+",
...
def ModulationPlot(G2frame, data, atom, ax, off=0):
    ...
    if len(Spos):
        ...
        wave += G2mth.posFourier(
            tau, np.array(scof), np.array(ccof)
        )  # does all the Fourier terms together
    if mapData["Flip"]:
        Title = "Charge flip"
    else:
        Title = MapType
    ...
    ix = ix % rhoSize[:3]
    rho = np.roll(
        np.roll(np.roll(Map["rho"], ix[0], axis=0), ix[1], axis=1), ix[2], axis=2
    )
    ix = rhoSize[:3] // 2
    ib = 4
    hdx = [
        ...
        2,
        2,
    ]  # this needs to be something for an offset correction on atom positions
    if Ax == "x":
        Doff = (hdx[0] + Off) * 0.005
        slab = np.sum(
            np.sum(rho[:, ix[1] - ib : ix[1] + ib, ix[2] - ib : ix[2] + ib, :], axis=2),
            axis=1,
        )
        Plot.plot(tau, wave[0])
    elif Ax == "y":
        Doff = (hdx[1] + Off) * 0.005
        slab = np.sum(
            np.sum(rho[ix[0] - ib : ix[0] + ib, :, ix[2] - ib : ix[2] + ib, :], axis=2),
            axis=0,
        )
        Plot.plot(tau, wave[1])
    elif Ax == "z":
        Doff = (hdx[2] + Off) * 0.005
        slab = np.sum(
            np.sum(rho[ix[0] - ib : ix[0] + ib, ix[1] - ib : ix[1] + ib, :, :], axis=1),
            axis=0,
        )
    ...
    if "delt" in MapType:
        Plot.contour(
            Slab[:, :21], 20, extent=(0.0, 2.0, -0.5 + Doff, 0.5 + Doff), cmap=acolor
        )
    else:
        Plot.contour(Slab[:, :21], 20, extent=(0.0, 2.0, -0.5 + Doff, 0.5 + Doff))
...
def PlotCovariance(G2frame, Data, Cube=False):
        ...
        if event.key == "s":
            ...
            for start in range(0, llen, 8):  # split matrix into batches of 7 columns
                ...
                for idx in range(start, stop):
                    covFile.write(f"{Page.varyList[idx]:^12}\t")
                covFile.write("\n\n")
                for line in range(llen):
                    covFile.write(f"{Page.varyList[line]:>12}\t")
                    for idx in range(start, stop):
                        covFile.write(f"{Page.covArray[line][idx]: 12.6f}\t")
                    covFile.write("\n")
                covFile.write("\n\n\n")
            covFile.close()
        elif event.key == "c":
    ...
    title = G2obj.StripUnicode(
        " for\n" + Data["title"], ""
    )  # matplotlib 1.x does not like unicode
    Page.newAtomDict = Data.get("newAtomDict", {})
    G2frame.G2plotNB.status.DestroyChildren()  # get rid of special stuff on status bar
    Page.Choice = [
        "c: toggle v-cov cube plot",
        "s: to change colors",
        "p: to save covariance as text file",
    ]
    Page.keyPress = OnPlotKeyPress
...
def PlotTorsion(
    ...
    tMin = np.min(torsion)
    tMax = np.max(torsion)
    torsion = 3.0 * (torsion - tMin) / (tMax - tMin)
    X = np.linspace(0.0, 360.0, num=45)
...
def PlotRama(G2frame, phaseName, Rama, RamaName, Names=None, PhiPsi=None, Coeff=None):
    ...
    if RamaName == "All" or "-1" in RamaName:
        if Coeff:
            ...
            Z = np.array(
                [
                    -G2mth.calcRamaEnergy(x, y, Coeff)[1]
                    for x, y in zip(X.flatten(), Y.flatten(), strict=False)
                ]
            )
...
#### PlotSeq ################################################################################
def PlotSelectedSequence(
    G2frame, ColumnList, TableGet, SelectX, fitnum=None, fitvals=None
):
...
def OnStartMask(G2frame):
    ...
    if G2frame.MaskKey == "f":
        ...
        if Masks["Frames"]:
            ...
            G2frame.MaskKey = "f"
        Page.figure.suptitle(
            "Defining Frame mask (use right-mouse to end)", color="g", fontweight="bold"
        )
        Page.canvas.draw()
        ...
        Page.figure.suptitle(
            "Defining Polygon mask (use right-mouse to end)",
            color="r",
            fontweight="bold",
        )
        Page.canvas.draw()
        ...
        Page.figure.suptitle(
            "Left-click to create an arc mask", color="r", fontweight="bold"
        )
        Page.canvas.draw()
        ...
        Page.figure.suptitle(
            "Left-click to create an x-line mask", color="r", fontweight="bold"
        )
        Page.canvas.draw()
        ...
        Page.figure.suptitle(
            "Left-click to create an y-line mask", color="r", fontweight="bold"
        )
        Page.canvas.draw()
        ...
        Page.figure.suptitle(
            "Left-click to create a ring mask", color="r", fontweight="bold"
        )
        Page.canvas.draw()
...
def OnStartNewDzero(G2frame):
    ...
    G2frame.GetStatusBar().SetStatusText(
        "Add strain ring active - LB pick d-zero value", 0
    )
...
def ShowSpotMaskInfo(G2frame, Page):
    if G2frame.MaskKey == "s":
        Page.figure.suptitle(
            f"Multiple spot mode on (size={G2frame.spotSize}), press s or right-click to end",
            color="r",
            fontweight="bold",
        )
    else:
        Page.figure.suptitle(
            f"New spot size={G2frame.spotSize}", color="r", fontweight="bold"
        )
    Page.canvas.draw()
...
def ComputeArc(angI, angO, wave, azm0=0, azm1=362):
    ...
    aR = [
        azm0,
        azm1,
        max(3, int(0.5 + azm1 - azm0)),
    ]  # number of points should be at least 3
    if azm1 - azm0 > 180:
        aR[2] //= 2  # for more than 180 degrees, steps can be 2 deg.
    Azm = np.linspace(*aR)
    for azm in Azm:
...
def PlotImage(G2frame, newPlot=False, event=None, newImage=True):  # noqa: ARG001
    ...
    def OnImMotion(event):
        ...
        if (
            ...
            scalex = 1000.0 / pixelSize[0]  # microns --> 1/mm
            ...
            if item and G2frame.GPXtree.GetItemText(G2frame.PickId) == "Image Controls":
                if "Text" in str(item):
                    ...
                    if Data["det2theta"]:
                        xcent += Data["distance"] * nptand(
                            -Data["tilt"] * npsind(Data["rotation"]) + Data["det2theta"]
                        )
                    xpos = event.xdata - xcent
                    ypos = event.ydata - ycent
                    tth, azm = G2img.GetTthAzm(event.xdata, event.ydata, Data)
                    if "Lazm" in str(item) or (
                        "Uazm" in str(item) and not Data["fullIntegrate"]
                    ):
                ...
                if G2frame.StrainKey:
                    ...
                    G2frame.G2plotNB.status.SetStatusText(
                        "Polygon/frame mask pick - LB next point, RB close polygon", 1
                    )
                else:
                    G2frame.G2plotNB.status.SetStatusText(
                        "Radius=%.3fmm, 2-th=%.3fdeg, dsp=%.3fA, Q=%.5fA-1, azm=%.2fdeg, I=%6d"
                        % (radius, tth, dsp, Q, azm, Int),
                        1,
                    )
        ...
        if treeItem == "Masks":
            ...
            if event.key == "s":  # turn multiple spot mode on/off
                ...
                if dlg.ShowModal() == wx.ID_OK:
                    ...
                    print(f"Spot size set to {G2frame.spotSize} mm")
                    ShowSpotMaskInfo(G2frame, Page)
                dlg.Destroy()
            elif event.key == "t":
                try:  # called from menu?
                    ...
                    G2G.G2MessageBox(
                        G2frame.G2plotNB,
                        f'You must use the "{event.key}" key from the keyboard',
                        "Keyboard only",
                    )
                    return
                ...
                artist = Circle(
                    spot[:2], radius=spot[2] / 2, fc="none", ec="r", picker=True
                )
            ...
            if event.key in [
                ...
                dlg = wx.MessageDialog(
                    G2frame,
                    "Are you sure you want to change the center?",
                    "Center change",
    ...
    def OnImPick(event):
        ...
        def OnDragIntBound(event):
            ...
            ellI = G2img.GetEllipse(
                dspI, Data
            )  # =False if dsp didn't yield an ellipse (ugh! a parabola or a hyperbola)
            dspO = wave / (2.0 * sind(IOtth[1] / 2.0))
            ellO = G2img.GetEllipse(dspO, Data)  # Ditto & more likely for outer ellipse
            Azm = np.arange(LRAzim[0], LRAzim[1] + 1.0) - AzmthOff
            if ellI:
                xyI = []
                for azm in Azm:
        ...
        def OnDragMask(event):
            ...
            if pickType == "Spot":
                ...
                if event.button == 1:
                    ...
                    angI = Masks["Rings"][itemNum][0] - t / 2.0
                    angO = Masks["Rings"][itemNum][0] + t / 2.0
                ...
                if event.button == 1:
                    ...
                    azm[0] += off
                    azm[1] -= off
                (x1, y1), (x2, y2) = ComputeArc(
                    np.squeeze(angI), np.squeeze(angO), wave, *azm
                )
                ...
                a = Page.figure.gca().axhline(
                    Ypos, color="g", linewidth=1, linestyle=(0, (5, 5))
                )
                Page.figure.gca().draw_artist(a)
            elif pickType.startswith("Yline"):
                a = Page.figure.gca().axvline(
                    Xpos, color="g", linewidth=1, linestyle=(0, (5, 5))
                )
                Page.figure.gca().draw_artist(a)
        ...
        if G2frame.GPXtree.GetItemText(G2frame.PickId) == "Image Controls":
            ...
            if pickType == "Spot":
                ...
                print(f"picktype {pickType} should not happen!")
                GSASIIpath.IPyBreak()
            saveLinestyle = [p.get_linestyle() for p in pl]
            for p in pl:
                p.set_linestyle("dotted")  # set line as dotted
            Page.canvas.draw()  # refresh without dotted line & save bitmap
            savedplot = Page.canvas.copy_from_bbox(Page.figure.gca().bbox)
            G2frame.cid = Page.canvas.mpl_connect("motion_notify_event", OnDragMask)
            for p, s in zip(pl, saveLinestyle, strict=False):
                p.set_linestyle(s)  # set back to original
    ...
    def OnImRelease(event):
        ...
        if (
            G2frame.itemPicked is None
            and treeItem == "Image Controls"
            and len(G2frame.ImageZ)
        ):
            ...
            if Ypos and not Page.toolbar.AnyActive():  # make sure zoom/pan not selected
                if event.button == 1:
                    ...
                    if I and J:
                        xpos += 0.5  # shift to pixel center
                        ypos += 0.5
                    ...
                    if G2img.ImageCalibrate(G2frame, Data):
                        G2frame.GetStatusBar().SetStatusText(
                            "Calibration successful - Show ring picks to check", 0
                        )
                        print("Calibration successful")
                    else:
                        G2frame.GetStatusBar().SetStatusText(
                            "Calibration failed - Show ring picks to diagnose", 0
                        )
            ...
            Xpos, Ypos = [event.xdata, event.ydata]
            if (
                not Xpos or not Ypos or Page.toolbar.AnyActive()
            ):  # got point out of frame or zoom/pan selected
                return
            if G2frame.MaskKey == "s":
                if event.button == 3:
                    ToggleMultiSpotMask(G2frame)
                    ...
                    if G2frame.ShiftDown:  # force user selection
                        ...
                        result = G2img.FitImageSpots(
                            G2frame.ImageZ, ImMax, ind, pixelSize, nxy, G2frame.spotSize
                        )
                        Xpos, Ypos, sig = result
                    spot = [Xpos, Ypos, sig]
                    Masks["Points"].append(spot)
                    artist = Circle(
                        (Xpos, Ypos), radius=spot[2] / 2, fc="none", ec="r", picker=True
                    )
                    # GSASIIpath.IPyBreak()
                    Page.figure.gca().add_artist(artist)
                    artist.itemNumber = len(Masks["Points"]) - 1
                ...
                if event.button == 1:
                    ...
                    Masks["Arcs"].append([tth, [azm - a / 2.0, azm + a / 2.0], t])
                    G2imG.UpdateMasks(G2frame, Masks)
                G2frame.MaskKey = ""
                wx.CallAfter(PlotImage, G2frame, newImage=True)
                return
            ...
            if not len(R):
                del StrSta["d-zero"][-1]
                G2frame.ErrorDialog(
                    "Strain peak selection",
                    "WARNING - No points found for this ring selection",
                )
            ...
            if G2frame.ifGetRing:  # delete a calibration ring pick
                ...
                rings = Data["ring"]
                for ring in rings:
                    if np.allclose(ring, xypos, 0.01, 0):
                        rings.remove(ring)
                ...
                if Data["binType"] == "Q":
                    wave = Data["wavelength"]
                    IOtth = [
                        4.0 * math.pi * sind(Data["IOtth"][0] / 2.0) / wave,
                        4.0 * math.pi * sind(Data["IOtth"][1] / 2.0) / wave,
                    ]
                ...
                G2imG.UpdateMasks(G2frame, Masks)
            elif (pickType.startswith("Ring") and treeItem == "Masks") or (
                pickType.startswith("Arc") and treeItem == "Masks"
            ):
                ...
                itemNum = G2frame.itemPicked.itemNumber
                Masks["Xlines"][itemNum] = int(0.5 + 1000.0 * Ypos / pixelSize[1])
                G2imG.UpdateMasks(G2frame, Masks)
            elif pickType.startswith("Yline"):
                itemNum = G2frame.itemPicked.itemNumber
                Masks["Ylines"][itemNum] = int(0.5 + 1000.0 * Xpos / pixelSize[0])
                G2imG.UpdateMasks(G2frame, Masks)
    ...
    xylim = []
    new, plotNum, Page, Plot, lim = G2frame.G2plotNB.FindPlotTab(
        "2D Powder Image", "mpl", newImage=newImage
    )
    ...
    try:
        if G2frame.GPXtree.GetItemText(G2frame.PickId) in [
            ...
            if Data.get("linescan", [False, 0.0])[0]:
                Page.Choice += [
                    "s: toggle sqrt plot line scan",
                    "n: toggle log plot line scan",
                ]
            Page.keyPress = OnImPlotKeyPress
        elif G2frame.GPXtree.GetItemText(G2frame.PickId) in [
            "Masks",
        ]:
            Page.Choice = [
                " key press",
            ...
            Page.Choice.append(
                "s: start multiple spot mask mode"
            )  # this must be the last choice
            Page.keyPress = OnImPlotKeyPress
        elif G2frame.GPXtree.GetItemText(G2frame.PickId) in [
            "Stress/Strain",
        ]:
    ...
    sizexy = Data["size"]
    pixelSize = Data["pixelSize"]
    Xmax = sizexy[0] * pixelSize[0] / 1000.0
    Ymax = sizexy[1] * pixelSize[1] / 1000.0
    xlim = (0, Xmax)
    ylim = (Ymax, 0)
    ...
    try:
        ...
        if (
            "linescan" in Data
            and Data["linescan"][0]
            and G2frame.GPXtree.GetItemText(G2frame.PickId)
            in [
                "Image Controls",
            ]
        ):
        ...
        if G2frame.PickId and G2frame.GPXtree.GetItemText(G2frame.PickId) in [
            ...
            for ellipse in Data["ellipses"]:  # what about hyperbola?
                ...
                if width > 0:  # ellipses
                    ...
                    Plot.text(
                        cent[0], cent[1], "+", color=col, ha="center", va="center"
                    )
        if G2frame.PickId and G2frame.GPXtree.GetItemText(G2frame.PickId) in [
            "Stress/Strain",
        ]:
        ...
        try:
            for tth, rColor, rWidth, rStype in G2frame.PhaseRing2Th:
                (x1, y1), (x2, y2) = ComputeArc(
                    tth - 0.1 / 2.0, tth + 0.1 / 2.0, Data["wavelength"]
                )
                Plot.plot(
        ...
        for iring, ring in enumerate(Masks["Rings"]):  # drawing spot masks
            if ring:
                tth, thick = ring
                (x1, y1), (x2, y2) = ComputeArc(
                    tth - thick / 2.0, tth + thick / 2.0, Data["wavelength"]
                )
        ...
        for iarc, arc in enumerate(Masks["Arcs"]):  # drawing arc masks
            if arc:
                ...
                arcList = []
                arcList.append(
                    Plot.plot(x2, y2, "r", picker=True, pickradius=3)[0]
                )  # 'inner'
                arcList[-1].itemNumber = iarc
                arcList[-1].itemType = "ArcInner"
                arcList.append(
                    Plot.plot(x1, y1, "r", picker=True, pickradius=3)[0]
                )  # 'outer'
                arcList[-1].itemNumber = iarc
                arcList[-1].itemType = "ArcOuter"
                arcList.append(
                    Plot.plot(
                        [x1[0], x2[0]], [y1[0], y2[0]], "r", picker=True, pickradius=3
                    )[0]
                )  # 'lower'
                arcList[-1].itemNumber = iarc
        ...
        for ipoly, polygon in enumerate(Masks["Polygons"]):
            ...
            if polygon[0] != polygon[-1]:
                print(f"Closing polygon {ipoly}")
                polygon.append(polygon[0][:])
            xl, yl = np.hsplit(np.array(polygon), 2)
            G2frame.polyList.append(Plot.plot(xl, yl, "r")[0])  # line
            for i, (x, y) in enumerate(zip(xl[:-1], yl[:-1], strict=False)):
                artist = Plot.plot(x, y, "r+", picker=True, pickradius=10)[
                    0
                ]  # point (plus sign)
        ...
        if Masks["Frames"]:
            ...
            for i, (x, y) in enumerate(zip(xl[:-1], yl[:-1], strict=False)):
                artist = Plot.plot(x, y, "g+", picker=True, pickradius=10)[
                    0
                ]  # point (plus sign)
        ...
        if G2frame.PickId and G2frame.GPXtree.GetItemText(G2frame.PickId) in [
            ...
            for i, xline in enumerate(Masks.get("Xlines", [])):
                ypos = xline * pixelSize[1] / 1000.0  # pixel to mm
                a = Plot.axhline(
                    ypos,
                    color="g",
                    picker=True,
            ...
            for i, yline in enumerate(Masks.get("Ylines", [])):
                xpos = yline * pixelSize[0] / 1000.0  # pixel to mm
                a = Plot.axvline(
                    xpos,
                    color="g",
                    picker=True,
...
def PlotIntegration(G2frame, newPlot=False, event=None):  # noqa: ARG001
    ...
    Img = Plot.imshow(
        ...
        vmax=Imax,
        interpolation="nearest",
        extent=[ysc[0], ysc[-1], xsc[-1], xsc[0]],
        aspect="auto",
    )
    Page.figure.colorbar(Img)
    #    if Data['ellipses']:
...
def PlotRawImage(G2frame, image, label, newPlot=False):  # noqa: ARG001
    ...
    Plot = Page.figure.add_axes([0.0, 0.0, 1.0, 1.0])  # fill page & don't show
    Plot.axis("off")
    Plot.imshow(image)
    Page.canvas.draw()
...
def PlotTRImage(G2frame, tax, tay, taz, newPlot=False):
    ...
    new, plotNum, Page, Plot, lim = G2frame.G2plotNB.FindPlotTab(
        "2D Transformed Powder Image", "mpl"
    )
    if not new:
        if not newPlot:
            xylim = copy.copy(lim)
...
def PlotStructure(G2frame, data, firstCall=False, pageCallback=None):
    ...
    def FindPeaksBonds(XYZ):
        rFact = data["Drawing"].get(
            "radiusFactor", 0.85
        )  # data['Drawing'] could be empty!
        Bonds = [[] for x in XYZ]
        for i, xyz in enumerate(XYZ):
            ...
            for j in IndB[0]:
                Bonds[i].append(Dx[j] / 2.0)
                Bonds[j].append(-Dx[j] / 2.0)
        return Bonds
    ...
    def OnKeyBox(event):
        ...
        if mode in [
            ...
            if projFile:
                ...
                dlg = wx.FileDialog(
                    G2frame,
                    "Choose graphics save file",
                    G2G.GetExportPath(G2frame),
                    wildcard="Graphics file (*." + mode + ")|*." + mode,
    ...
    def OnKey(event):  # on key UP!!
        ...
        if key in ["C"]:
            drawingData["viewPoint"] = [np.array([0.5, 0.5, 0.5]), [0, 0]]
            ...
            if key == "Q":
                ...
                key == "Y" and len(G2phG.ranDrwDict["atomList"]) > 0
                ...
                if G2phG.ranDrwDict["opt"] == 1:
                    ...
                    data["Drawing"]["Atoms"][i][cx + 6] = (
                        0,
                        0,
                        0,
                    )  # mark atoms to be deleted in black & delete later
                    ...
                    G2phG.ranDrwDict["2call"](event=None, selection=[i])
            ...
            if len(G2phG.ranDrwDict["atomList"]) == 0:  # no more atoms left
                ...
                for i in G2phG.ranDrwDict["delAtomsList"]:
                    del data["Drawing"]["Atoms"][i]
    ...
    def OnKeyPressed(
        event,
    ):  # On key down for repeating operation - used to change tau...
        try:
            keyCode = event.GetKeyCode()
            if keyCode > 255:
                keyCode = 0
        ...
        if key in ["+", "-", "=", "0"]:
            if generalData["Modulated"]:
                ...
                SeqId = G2gd.GetGPXtreeItemId(
                    G2frame, G2frame.root, "Sequential results"
                )
                PF2 = False
                if not SeqId:
                    SeqId = G2gd.GetGPXtreeItemId(
                ...
                try:  # this is pythonic cheating; only works if seq data is applicable, otherwise there's errors
                    ...
                    if (
                        cellA is None
                    ):  # happens if no D11 in parmDict or no cell in PDFfit
                        cellA = G2lat.cell2A(data["General"]["Cell"][1:7])
                    cell = G2lat.A2cell(cellA)
                    Vol = G2lat.calc_V(cellA)
                    Amat, Bmat = G2lat.cell2AB(
                        cell
                    )  # Amat - crystal to cartesian, Bmat - inverse
                    Gmat, gmat = G2lat.cell2Gmat(cell)
                    A4mat = np.concatenate(
                        (
                            np.concatenate((Amat, [[0], [0], [0]]), axis=1),
                            [
                                [0, 0, 0, 1],
                            ],
                        ),
                    ...
                    if key in ["=", "-"]:  # meaning '+','-'
                        ...
                        newxy = event.GetPosition()
                        SetCursorStatus(
                            newxy, drawingData.get("showSlice", False) in [1, 3]
                        )
            ...
            for i, peak in enumerate(peakList):
                ...
                if np.allclose(xy, XY, atol=10) and Zmax > Z:
                    ...
                    try:  # to see if selection in previously selected (Indx)
                        Indx.remove(
                            i
                        )  # get exception if Indx doesn't exist or i not in Indx
                        ClearSelectedAtoms()
                        for Id in Indx:
    ...
    def OnMouseUp(event):  # noqa: ARG001
        ...
        if (
            rbObj
            and rbObj.get("needsFill")
            and "FillUnitCell" in G2frame.testRBObjSizers
        ):
            if not G2frame.testRBObjSizers.get("fillMode", False):
                return
            if rbObj.get("FillInProgress"):
                return
    ...
    def OnMouseWheel(event):
        ...
        if page:
            if G2frame.phaseDisplay.GetPageText(page) == "Draw Options":
                ...
                G2frame.phaseDisplay.cameraSlider.SetScaledValue(
                    drawingData["cameraPos"]
                )
        Draw("wheel")
    ...
    def SetSelectedAtoms(ind, Add=False):
        ...
        if page:
            if G2frame.phaseDisplay.GetPageText(page) in (
                ...
                for widget in G2frame.phaseDisplay.GetPage(page).GetChildren():
                    if hasattr(widget, "GetSelectedRows"):
                        break
                else:
                    return
    ...
    def GetRoll(newxy, rhoshape):
        ...
        roll = np.where(dxy > 0.5, 1, np.where(dxy < -0.5, -1, 0))
        return roll

    def SetMapRoll(newxy):
        rho = generalData["Map"]["rho"]
        roll = GetRoll(newxy, rho.shape)
        generalData["Map"]["rho"] = np.roll(
    ...
    def Set4DMapRoll(newxy):
        ...
        if len(rho):
            ...
            generalData["4DmapData"]["rho"] = np.roll(
                np.roll(np.roll(rho, roll[0], axis=0), roll[1], axis=1), roll[2], axis=2
            )
    ...
    def RenderBox():
        GL.glLightfv(GL.GL_LIGHT0, GL.GL_AMBIENT, [0.4, 0.4, 0.4, 1])
        GL.glEnable(GL.GL_COLOR_MATERIAL)
        GL.glLineWidth(2)
    ...
    def RenderUnitVectors(x, y, z):
        GL.glLightfv(GL.GL_LIGHT0, GL.GL_AMBIENT, [0.7, 0.7, 0.7, 1])
        GL.glEnable(GL.GL_COLOR_MATERIAL)
        GL.glLineWidth(2)
        GL.glEnable(GL.GL_BLEND)
        GL.glBlendFunc(GL.GL_SRC_ALPHA, GL.GL_ONE_MINUS_SRC_ALPHA)
    ...
    def RenderViewPlane(plane, Z, width, height):
        ...
        GL.glBegin(GL.GL_POLYGON)
        for vertex, evertex in zip(plane, eBox, strict=False):
    ...
    def RenderMoment(x, y, z, Moment, color, slice=20):
        Dx = np.inner(Amat, Moment / ABC) / 2.0
        Z = np.sqrt(np.sum(Dx**2))
        if Z:
            GL.glMaterialfv(GL.GL_FRONT_AND_BACK, GL.GL_DIFFUSE, color)
            GL.glPushMatrix()
            ...
            GLU.gluDisk(q, 0.0, 0.1, slice, 1)
            GLU.gluQuadricOrientation(q, GLU.GLU_OUTSIDE)
            GLU.gluCylinder(q, 0.1, 0.1, 2.0 * Z, slice, 2)
            GL.glTranslate(0, 0, 2 * Z)
            GLU.gluQuadricOrientation(q, GLU.GLU_INSIDE)
            GLU.gluDisk(q, 0.1, 0.2, slice, 1)
            GLU.gluQuadricOrientation(q, GLU.GLU_OUTSIDE)
            GLU.gluCylinder(q, 0.2, 0.0, 0.4, slice, 2)
            GL.glPopMatrix()
    ...
    def RenderLabel2(x, y, z, label, r, color, matRot, offset=wx.RealPoint(0.0, 0.0)):  # noqa: B008, ARG001
        ...
        """
        figure = mplfig.Figure(figsize=(1.0, 1.0), facecolor=(0.0, 0.0, 0.0, 0.0))
        figure.clf()
        canvas = hcCanvas(figure)
        ax0 = figure.add_subplot(111)
        ax0.text(0.0, 0.0, label)
        ax0.axis("off")
        figure.subplots_adjust(
            bottom=0.0, top=1.0, left=0.0, right=1.0, wspace=0.0, hspace=0.0
        )
        ...
        for i, xyz in enumerate(rhoXYZ):
            if not Rok[i]:
                ...
                if cLevel < 1.0:
                    alpha = min(
                        1.0,
                        (abs(rho[I, J, K]) / mapData["rhoMax"] - cLevel)
                        / (1.0 - cLevel),
                    )
                if rho[I, J, K] < 0.0:
                    XYZ.append(xyz)
    ...
    def distances2Peaks(
        x, y, z, PeakDistRadius, mapPeaks, peakMax, radius, Amat, matRot
    ):
    ...
    def distances2Atoms(x, y, z, atomsExpandRadius, atomsdistRadius, Amat, matRot):
        ...
        radDict = dict(zip(*G2phG.getAtomRadii(data), strict=False))
        Names = []
        Atoms = []
        Radii = []
        Color = []
        for atomB in data["Atoms"]:
            ...
            color = (
                np.array(
                    generalData["Color"][generalData["AtomTypes"].index(atomB[ct])]
                )
                / 255.0
            )
            result = G2spc.GenAtom(xyzB, SGData, False, 6 * [0.0], True)
            for item in result:
    ...
    def Draw(caller="", Fade=None, NPkey=False):
        ...
        #            rho = D4mapData['rho'][:,:,:,int(G2frame.tau*10)]   #pick current tau 3D slice
            ...
            rho = mapData["rho"]
        if len(rho):
            VP = drawingData["viewPoint"][0] - np.array([0.5, 0.5, 0.5])
            ...
            rho = np.roll(
                np.roll(np.roll(rho, Vsteps[0], axis=0), Vsteps[1], axis=1),
                Vsteps[2],
                axis=2,
            )
            ...
            Nc = max(len(rhoXYZ), 1)
            rcube = 2000.0 * Vol / (ForthirdPI * Nc)
            rmax = math.exp(math.log(rcube) / 3.0) ** 2
            radius = min(drawingData.get("mapSize", 10.0) ** 2, rmax)
        ...
        aspect = float(VS[0]) / float(VS[1])
        cPos = drawingData["cameraPos"]
        Zclip = drawingData["Zclip"] * cPos / 200.0
        Q = drawingData["Quaternion"]
        ...
        GS[0][2] = GS[2][0] = math.sqrt(GS[0][0] * GS[2][2])
        GS[1][2] = GS[2][1] = math.sqrt(GS[1][1] * GS[2][2])
        ellipseProb = G2lat.criticalEllipse(drawingData["ellipseProb"] / 100.0)
        ...
        for iat, atom in enumerate(drawingData["Atoms"]):
            ...
            if not atmFade[iat]:
                continue
            atColor = atmFade[iat] * np.array(CL) / 255.0
            ...
            if (
                iat in Ind
                and (G2frame.phaseDisplay.GetPageText(getSelection()) != "Map peaks")
                and not NPkey
            ):
                atColor = np.array(Gr) / 255.0
                bndColor = atColor
            #            color += [.25,]
            radius = 0.5
            if atom[cs] != "":
            ...
            if "balls" in atom[cs]:
                ...
                if "Q" in atom[ct]:
                    ...
                    if SpnData is not None:
                        ...
                        QA = G2mth.invQ(
                            SpnData["Orient"][0]
                        )  # rotate about chosen axis
                        QB = G2mth.make2Quat(symAxis, np.array([0, 0, 1.0]))[
                            0
                        ]  # position obj polar axis
                        QP = G2mth.AVdeg2Q(
                            360.0 / Npsi, np.array([0, 0, 1.0])
                        )  # this shifts by 1 azimuth pixel
                        Q = G2mth.prodQQ(QB, QA)
                        Q = G2mth.prodQQ(Q, QP)
                        PSI, GAM = np.mgrid[0:Npsi, 0:Ngam]  # [azm,pol]
                        PSI = PSI.flatten() * 360.0 / Npsi  # azimuth 0-360 ncl
                        GAM = GAM.flatten() * 180.0 / Ngam  # polar 0-180 incl
                        Rp, PSIp, GAMp = G2mth.RotPolbyQ(np.ones_like(PSI), PSI, GAM, Q)
                        SpnData["hide"] = SpnData.get(
                            "hide", [False for i in range(len(SpnData["atType"]))]
                        )
                        for ish, nSH in enumerate(SpnData["nSH"]):
                            if not SpnData["hide"][ish]:
                                if nSH > 0:
                                    ...
                                    if useAtColor:
                                        atcolor = atColor[ish]
                                    P = G2lat.SHarmcal(SytSym, SHC, PSIp, GAMp).reshape(
                                        (Npsi, Ngam)
                                    )
                ...
                elif atom[ci] in deformationData:
                    defCtrls = deformationData[-atom[ci]]
                    defParms = deformationData[atom[ci]]
                    SytSym = G2spc.SytSym(atom[cx : cx + 3], SGData)[0]
                    if (
                        defCtrls.get("showDef", False)
                        and defCtrls["Radial"] == "Slater"
                    ):
                        ...
                        SHC = defParms[0][1]
                        SHC = {
                            item.replace("D", "C"): SHC[item]
                            for item in SHC
                            if item not in ["Ne", "kappa"]
                        }
                        ...
                        PSI = PSI.flatten() * 360.0 / Npsi  # azimuth 0-360 ncl
                        GAM = GAM.flatten() * 180.0 / Ngam  # polar 0-180 incl
                        Rp, PSIp, GAMp = G2mth.RotPolbyM(
                            np.ones_like(PSI), PSI, GAM, UVMat
                        )
                        P = G2lat.SHarmcal(SytSym, SHC, PSIp, GAMp).reshape(
                            (Npsi, Ngam)
                        )
                ...
                elif atom[ct] == "H" and not drawingData["showHydrogen"]:
                    pass
                else:
        ...
        if (
            not FourD and len(rhoXYZ) and drawingData["showMap"]
        ):  # no green dot map for 4D - it's wrong!
            RenderMap(rho, rhoXYZ, indx, Rok)
        try:
            disSum = (
                drawingData["VPPeakDistRad"]
        ...
        if (pageName in ("Draw Atoms", "Draw Options")) and disSum > 0:
            ...
            if PeakDistRadius > 0:
                distances2Peaks(
                    x, y, z, PeakDistRadius, mapPeaks, radius, peakMax, Amat, matRot
                )
            # find and display atoms within atomsExpandRadius A
        ...
        if (
            len(testRBObj)
            and pageName == "RB Models"
            and "OnOrien" not in G2frame.testRBObjSizers
        ):
            # plot a test rigid body as ball & [green] sticks when adding the RB into cell
            XYZ = G2mth.UpdateRBXYZ(
                Bmat, testRBObj["rbObj"], testRBObj["rbData"], testRBObj["rbType"]
            )[0]
            ...
            for ind, [x, y, z] in enumerate(XYZ):
                ...
                if (
                    "RBhighLight" in testRBObj and testRBObj["RBhighLight"] == ind
                ):  # highlighted atom is green
                    RenderSphere(x, y, z, radius, Gr, Fade)
                else:
                    RenderSphere(x, y, z, radius, color / 255.0, Fade)
                if testRBObj["rbType"] != "Spin":
                    RenderBonds(x, y, z, rbBonds[ind], 0.03, Gr)
                RenderLabel(x, y, z, name, 0.2, wxOrange, matRot)
            RenderRBtriplet(
                testRBObj["rbObj"]["Orig"][0],
                testRBObj["rbObj"]["Orient"][0],
                Bmat,
                testRBObj["rbObj"].get("symAxis"),
            )
        if len(mcsaModels) > 1 and pageName == "MC/SA":  # skip the default MD entry
            for ind, [x, y, z] in enumerate(mcsaXYZ):
                ...
                RenderSphere(x, y, z, 0.2, color / 255.0)
                RenderBonds(x, y, z, mcsaBonds[ind], 0.03, Gr / 255.0)
        ...
        if drawingData.get(
            ...
            SXYZ = np.reshape(
                np.inner(SXYZ, invModel[:3, :3].T) + VP[nxs, nxs, :], (-1, 3)
            )
            if FourD:
                SXYZT = np.vstack((SXYZ.T, np.inner(SXYZ, modQ) + G2frame.tau)).T
                Zslice = np.reshape(
                    map_coordinates(
                        rho, (SXYZT % 1.0 * rho.shape).T, order=1, mode="wrap"
                    ),
                    (npts, npts),
                )
            else:
                Zslice = np.reshape(
                    map_coordinates(
                        rho, (SXYZ % 1.0 * rho.shape).T, order=1, mode="wrap"
                    ),
                    (npts, npts),
                )
            Z = np.where(Zslice <= Rmax, Zslice, Rmax)
            ZU = np.flipud(Z)
            figure = mplfig.Figure(figsize=(6, 6), facecolor=(1.0, 1.0, 1.0, 0.5))
            canvas = hcCanvas(figure)
            figure.clf()
            ax0 = figure.add_subplot(111)
            if drawingData.get("showSlice") in [
                1,
            ]:
            ...
            if drawingData.get("showSlice") in [2, 3]:
                ...
                ax0.imshow(
                    ZU, aspect="equal", cmap=acolor, alpha=0.7, interpolation="bilinear"
                )
                if drawingData.get("showSlice") in [
                    3,
                ]:
    ...
    drawingData = data["Drawing"]
    if not drawingData:
        return None  # nothing setup, nothing to draw
    ...
    if "Quaternion" not in drawingData:
        drawingData["Quaternion"] = [0.0, 0.0, 0, 1.0]
    ...
    if len(mcsaModels) > 1:
        ...
        mcsaTypes = []
        neqv = 0
        for xyz, atyp in zip(XYZs, Types, strict=False):
        ...
        cent = np.fix(np.sum(mcsaXYZ + 2.0, axis=0) / nuniq) - 2
...
def PlotBeadModel(G2frame, Atoms, defaults, PDBtext):
    ...
    def SetLights():
        ...
        GL.glEnable(GL.GL_LIGHT0)
        GL.glLightModeli(GL.GL_LIGHT_MODEL_TWO_SIDE, 0)
        GL.glLightfv(GL.GL_LIGHT0, GL.GL_AMBIENT, [1, 1, 1, 0.8])
        GL.glLightfv(GL.GL_LIGHT0, GL.GL_DIFFUSE, [1, 1, 1, 1])
    ...
    def Draw(caller=""):  # noqa: ARG001
        ...
        if sys.platform == "darwin":
            ...
            GL.glViewport(0, 0, VS[0], VS[1])
        GLU.gluPerspective(50.0, aspect, 1.0, 500.0)
        GLU.gluLookAt(0, 0, cPos, 0, 0, 0, 0, 1, 0)
        SetLights()
    ...
    def OnKeyBox(event):  # noqa: ARG001
        ...
        if mode in [
            ...
            for iatm, xyz in enumerate(XYZ):
                PDB.write(
                    "ATOM   %4d  CA  ALA A%4d    %8.3f%8.3f%8.3f  1.00  0.00\n"
...
def PlotRigidBody(G2frame, rbType, AtInfo, rbData, defaults):
    ...
    def Draw(caller=""):  # noqa: ARG001
        ...
        if sys.platform == "darwin":
            ...
            GL.glViewport(0, 0, VS[0], VS[1])
        GLU.gluPerspective(20.0, aspect, 1.0, 500.0)
        GLU.gluLookAt(0, 0, cPos, 0, 0, 0, 0, 1, 0)
        SetLights()
...
def PlotLayers(G2frame, Layers, laySeq, defaults, firstCall=False):
    ...
    def getAtoms():
        ...
        XYZ = newXYZ
        na = max(int(8.0 / cell[0]), 1)
        nb = max(int(8.0 / cell[1]), 1)
        indA = range(-na, na)
        indB = range(-nb, nb)
    ...
    G2frame.G2plotNB.status.SetStatusText(
        " Layers plotted: " + str(text).replace("'", "")[1:-1], 1
    )
    Page.canvas.Bind(wx.EVT_MOUSEWHEEL, OnMouseWheel)
    Page.canvas.Bind(wx.EVT_LEFT_DOWN, OnMouseDown)
    Page.canvas.Bind(wx.EVT_RIGHT_DOWN, OnMouseDown)
...
def PlotClusterXYZ(G2frame, YM, XYZ, CLuDict, Title="", PlotName="cluster"):
    ...
    if CLuDict["plots"] == "Distances":
        Page.ImgObj = Plot.imshow(
            YM, interpolation="nearest", vmin=Imin, vmax=Imax, origin="lower"
        )
        cax = inset_axes(
            Plot,
            width="5%",
        ...
        if CLuDict["CLuZ"] is not None:
            ...
            ax3.plot(100.0 * CLuDict["PCA"][:10] / np.sum(CLuDict["PCA"]))
