...
def GetAtomInfo(El, ifMag=False):
    ...
    AtomInfo.update(
        dict(
            zip(["Drad", "Arad", "Vdrad", "Hbrad"], atmdata.AtmSize[ElS], strict=False)
        )
    )
    AtomInfo["Symbol"] = El
...
def ClosedFormFF(Z, SQ, k, N):
    ...
    if k == 0:
        if N == 1:
            ...
            return (
                720.0
                * (7.0 * Z2**3 - 35.0 * K2 * Z2**2 + 21.0 * Z2 * K2**2 - K2**3)
                / K2pZ2**7
            )
        elif N == 8:
            return (
        ...
        if N == 2:
            ...
            return (
                1920.0 * K * Z * (7.0 * Z2**2 - 14.0 * K2 * Z2 + 3.0 * K2**2) / K2pZ2**7
            )
        elif N == 8:
            return (
                5760.0
...
def SetupGeneral(data, dirname):
    ...
    if (
        "Algolrithm" in generalData.get("MCSA controls", {})
        or "MCSA controls" not in generalData
    ):
        generalData["MCSA controls"] = {
            "Data source": "",
            "Annealing": [0.7, 0.1, 250],
            "dmin": 2.8,
            "Algorithm": "log",
            "fast parms": [0.8, 0.6],
            "log slope": 0.9,
    ...
    if generalData["Modulated"]:
        if "Super" not in generalData:
            generalData["Super"] = 1
            generalData["SuperVec"] = [[0.0, 0.0, 0.0], False, 1]
            generalData["SSGData"] = {}
    ...
    if "Resolution" in generalData["Map"]:
        generalData["Map"]["GridStep"] = generalData["Map"]["Resolution"] / 2.0
        generalData["Flip"]["GridStep"] = generalData["Flip"]["Resolution"] / 2.0
    ...
    for _iat, atom in enumerate(atomData):
        ...
        if generalData["AtomTypes"].count(atom[ct]):
            ...
            if atom[ct] in generalData["Isotope"]:
                ...
                generalData["AtomMass"].append(
                    Info["Isotopes"][generalData["Isotope"][atom[ct]]]["Mass"]
                )
            else:
                ...
                for iSh in range(nSh):
                    ...
                    if Info["Symbol"] not in generalData["AtomTypes"]:
                        ...
                        if Info["Symbol"] in generalData["Isotope"]:
                            if (
                                generalData["Isotope"][Info["Symbol"]]
                                not in generalData["Isotopes"][Info["Symbol"]]
                            ):
                                isotope = list(
                                    generalData["Isotopes"][Info["Symbol"]].keys()
                                )[-1]
                            ...
                            if (
                                ...
                                generalData["Isotope"][Info["Symbol"]] = isotope
                            generalData["AtomMass"].append(Info["Mass"])
                        generalData["NoAtoms"][Info["Symbol"]] = (
                            atom[cx + 3] * atom[cs + 1] * Srb["Natoms"][iSh]
                        )
                        generalData["Color"].append(Info["Color"])
                    else:
                        generalData["NoAtoms"][Info["Symbol"]] += (
