...
def SetConfigValue(parmdict: dict) -> None:
    ...
    for var in parmdict:
        ...
        if isinstance(parmdict[var], tuple):
            ...
            if parmdict[var][1] is None:
                continue
            if parmdict[var][1] == "":
                continue
            if parmdict[var][0] == parmdict[var][1]:
                continue
...
def HowIsG2Installed():
    ...
    try:
        ...
        if os.path.samefile(os.path.dirname(g2repo.common_dir), path2GSAS2):
            ...
            rev = "-rev"
        if (
            g2URL in g2repo.remote().urls
            or g2URL.replace("https://github.com/", "git@github.com:")
            in g2repo.remote().urls
        ):
            return "github" + rev
...
def GetVersionNumber():
    ...
    if HowIsG2Installed().startswith("git"):
        ...
        try:
            g2repo = openGitRepo(path2GSAS2)
            tag, vers, gnum = g2repo.git.describe(
                "--tags", "--match", "[0-9]*[0-9]", "--exclude", "*.*"
            ).split("-")
            if tag.isnumeric():
                return int(tag)
        except:
...
def getG2VersionInfo():
    ...
    if HowIsG2Installed().startswith("git"):
        ...
        age = delta.total_seconds() / (60 * 60 * 24.0)
        msg = ""
        rc = None
...
def getGitHubVersion():
    ...
    if "version info" not in cfg:
        if GetConfigValue("debug"):
            print(f"no saved version number in {cfgfile}")
        return None, None
    return cfg["version info"].get("lastVersionTag"), cfg["version info"].get(
        "lastVersionNumber"
    )
...
def gitTestGSASII(verbose=True, g2repo=None):
    ...
    if g2repo is None:
        if not os.path.exists(path2GSAS2):
            if verbose:
                print(f"Warning: Directory {path2GSAS2} not found")
            return -1
...
def gitCheckForUpdates(fetch=True, g2repo=None):
    ...
    if fetch:
        try:
            ...
            prevremotecommits = [
                i.hexsha
                for i in head.commit.iter_items(g2repo, f"{head.path}..{tracking.path}")
            ]
            g2repo.remote().fetch()
            fetched = True
    ...
    try:
        ...
        localcommits = [
            i.hexsha
            for i in head.commit.iter_items(g2repo, f"{tracking.path}..{head.path}")
        ]
        remotecommits = [
            i.hexsha
            for i in head.commit.iter_items(g2repo, f"{head.path}..{tracking.path}")
        ]
        if prevremotecommits is None:
            prevremotecommits = remotecommits
        if fetch and len(prevremotecommits) != len(remotecommits):
            print(
                f"Fetch of new GSAS-II versions uploaded {len(remotecommits) - len(prevremotecommits)} new updates."
            )
...
def getGitBinaryReleases(cache=False):
    ...
    while tries < 5:  # this has been known to fail, so retry
        tries += 1
        releases = requests.get(url=f"{G2binURL}/releases", headers=BASE_HEADER).json()
        try:
            ...
            time.sleep(
                10
            )  # this does not seem to help when GitHub is not letting the queries through
...
def InstallGitBinary(tarURL, instDir, nameByVersion=False, verbose=True):
    ...
    try:
        ...
        for f in tarobj.getmembers():  # loop over files in repository
            ...
            if sys.platform == "win32" and f.name.startswith("._"):
                continue  # clean up Mac cruft for Windows
        ...
        if verbose:
            print(f"Binary files created in {os.path.split(newfil)[0]}")
...
def rawGitHubURL(
    dirlist,
    filename,
    orgName=gitTutorialOwn,
    repoName=gitTutorialRepo,
    branchname="main",
):
...
def downloadDirContents(
    dirlist, targetDir, orgName=gitTutorialOwn, repoName=gitTutorialRepo
):
    ...
    for fil in filList:
        ...
        if r.status_code == 200:
            ...
            print(
                f"Warning: {fil} is likely a subdirectory of directory {'/'.join(dirlist)!r}"
            )
        else:
...
def exceptHook(*args):
    ...
    try:
        ...
        if sys.platform.startswith("win"):
            IPython.core.ultratb.FormattedTB(call_pdb=False, color_scheme="NoColor")(
                *args
            )
        else:
            IPython.core.ultratb.FormattedTB(call_pdb=False, color_scheme="LightBG")(
                *args
            )
        from IPython.core import getipython
    ...
    try:
        ipshell(
            msg, local_ns=frame.f_locals, global_ns=frame.f_globals
        )  # newest (IPython >= 8)
...
def InvokeDebugOpts():
    ...
    if any("SPYDER" in name for name in os.environ):
        print(
            "Running from Spyder, keeping breakpoint() active & skipping exception trapping"
        )
...
def SetBinaryPath(showConfigMsg=False):
    ...
    try:
        ...
        if showConfigMsg:
            print(
                f"GSAS-II binaries co-located with GSAS-II: {os.path.dirname(__file__)}"
            )
...
def LoadConfig(printInfo=True):
        ...
        try:
            ...
            for i in config.__dict__:
                if i.startswith("__") and i.endswith("__"):
                    continue
                if isinstance(config.__dict__[i], types.ModuleType):
                    continue
    ...
    capsDict = {
        key.lower(): key for key in config_example.__dict__ if not key.startswith("__")
    }
...
def condaInstall(packageList):
    ...
    try:
        print(
            f"Preparing to install package(s): {' ,'.join(packageList)}"
...
def addCondaPkg():
    ...
    if sys.platform == "win32":
        ...
        script = (
            "source "
            + os.path.join(os.path.dirname(os.environ["CONDA_PYTHON_EXE"]), "activate")
            + " base; "
        )
    ...
    if out is not None and GetConfigValue("debug"):
...
def getIconFile(imgfile):
    ...
    if os.path.exists(
        os.path.join(path2GSAS2, imgfile)
    ):  # patch 3/2024 for svn dir organization
...
if os.path.exists(os.path.expanduser("~/.G2local/")):
    ...
    if files:
        print("*" * 75)
        print(
            "Warning: the following source files are locally overridden in "
...
def postURL(
    URL, postdict, getcookie=None, usecookie=None, timeout=None, retry=2, mode="get"
):
    ...
    while repeat:
        ...
        try:
            if timeout is not None:
                r = reqopt(URL, params=postdict, cookies=usecookie, timeout=timeout)
            else:
...
if __name__ == "__main__":
    ...
    for arg in sys.argv[1:]:
        if (
            "--git-fetch" in arg
        ):  # pulls latest updates from server but does not apply them
            ...
            "--git-reset" in arg
        ):  # restores locally changed GSAS-II files to distributed versions also updates
            gitUpdate = True
            if preupdateType:
                print(f"previous option conflicts with {arg}")
                help = True
            ...
            try:
                ...
                print(
                    f"invalid version specified ({regressversion}) for GitHub regression"
                )
                help = True
                break
            if updateType:
...
""")
    ...
    if updateType == "tags":
        ...
        taglist = [
            tag.split('"')[0]
            for tag in releases.text.split(
                "AdvancedPhotonSource/GSAS-II/releases/tag/"
            )[1:]
        ]
    ...
    if updateType == "fetch":
        ...
        try:
            ...
            if not g2repo.head.is_detached:
                ...
                prevremotecommits = [
                    i.hexsha
                    for i in head.commit.iter_items(
                        g2repo, f"{head.path}..{tracking.path}"
                    )
                ]
            g2repo.remote().fetch()
            if not g2repo.head.is_detached:
