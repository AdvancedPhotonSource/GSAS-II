...
def InitVars():
    ...
    global dependentParmList, arrayList, invarrayList, indParmList, consNum, symGenList  # noqa: PLW0603
    ...
    global holdParmList, holdParmType  # noqa: PLW0603
    holdParmList = []
    holdParmType = {}
    global droppedSym  # noqa: PLW0603
    droppedSym = []
    global constrVarList, indepVarList, depVarList  # noqa: PLW0603
    constrVarList = []
    indepVarList = []
    depVarList = []
    global constrParms  # noqa: PLW0602
...
def GroupConstraints(constrDict):
    ...
    for i, constrI in enumerate(constrDict):
        if i in assignedlist:
            continue  # already in a group, skip
        ...
        while changes:  # loop until we can't find anything to add to the current group
            ...
            for j, constrJ in enumerate(constrDict):
                if j in assignedlist:
                    continue  # already in a group, skip
                if (
                    len(set(VarKeys(constrJ)) & groupset) > 0
                ):  # true if this needs to be added
                    changes = True
                    grouplist.append(j)
...
def GenerateConstraints(
        ...
        if cdict is not None and cdict != warninfo["shown"]:
            ...
            if "_vary" in cdict:
                warninfo["msg"] += (
                    prefix
                    + "new var expression: "
                    + _FormatConstraint(cdict, cdict.get("_name", "New Var"))
                )
            else:
                warninfo["msg"] += (
                    prefix + "constraint equation: " + _FormatConstraint(cdict, val)
                )
        if warninfo["msg"]:
            warninfo["msg"] += "\n"
        warninfo["msg"] += "  " + msg

    global dependentParmList, arrayList, invarrayList, indParmList, consNum  # noqa: PLW0602, PLW0603
    # lists of parameters used for error reporting
    global undefinedVars  # parameters that are used in equivalences but are not defined  # noqa: PLW0603
    undefinedVars = []
    global groupErrors  # parameters in constraints that cause grouping errors  # noqa: PLW0603
    groupErrors = []
    global saveVaryList  # noqa: PLW0603
    ...
    global constrVarList  # noqa: PLW0603
    constrVarList = []
    for cnum, (cdict, fixVal) in enumerate(zip(constrDict, fixedList, strict=False)):  # noqa: B007
        constrVarList += [
            i for i in cdict if i not in constrVarList and not i.startswith("_")
        ]

    # Process the equivalences; If there are conflicting parameters, move them into constraints
    warning = CheckEquivalences(
        constrDict, varyList, fixedList, parmDict, seqHistNum=seqHistNum
    )
    ...
    for cnum, (cdict, fixVal) in enumerate(zip(constrDict, fixedList, strict=False)):
        ...
        for var in VarKeys(cdict):  # assemble warning info
            if cdict[var] == 0:  # zero multiplier
                if var not in zeroList:
                    zeroList.append(var)
                if var not in dropList:
                    dropList.append(var)
            elif (
                var in holdParmList
            ):  # hold invalid in New Var, drop from constraint eqn
                holdList.append(var)
                if fixVal is None:  # hold in a newvar is not allowed
                    problem = True
                elif var not in dropList:
                    dropList.append(var)
            elif ":*:" in var:  # wildcard still present should be treated as undefined
                if var not in undefinedVars:
                    undefinedVars.append(var)
                noWildcardList.append(var)
                problem = True
            elif (
                parmDict is not None and var not in parmDict
            ):  # not defined, constraint will not be used
                if var not in undefinedVars:
                    undefinedVars.append(var)
                notDefList.append(var)
                if seqHistNum is None:
                    if (
                        ":dAx:" in var or ":dAy:" in var or ":dAz:" in var
                    ):  # coordinates from undefined atoms
                        if fixVal is None:
                            problem = True  # invalid in New Var
                        elif var not in dropList:
                            dropList.append(var)  # ignore in constraint eqn
                    else:
                        problem = True
            elif varyList is None:
                valid += 1
            elif (
                var not in varyList and fixVal is not None
            ):  # unvaried, constraint eq. only
                if var not in unvariedParmsList:
                    unvariedParmsList.append(var)
        ...
        if noVaryList and cdict.get(
            "_vary", True
        ):  # true for constr eq & varied New Var
        ...
        for l, m in (  # noqa: E741
            (zeroList, "have zero multipliers"),  # show warning
        ...
        if valid == 0:  # no valid entries
            if seqHistNum is None:
                warn(
                    "Ignoring constraint, contains no usable parameters",
                    cdict,
                    prefix="\nUnused ",
                )
            ...
            len(dropList) > 0
        ):  # mix of valid and problematic items, drop problem vars, but keep rest
            ...
            for var in dropList:  # do cleanup
                ...
                del cdict[var]
            if float(value) != float(fixedList[cnum]):
                fixedList[cnum] = float(np.round(value, 12))
            if GSASIIpath.GetConfigValue("debug"):
                warn(
                    "revised as: "
                    + _FormatConstraint(constrDict[cnum], fixedList[cnum])
                )
    ...
    for i in invalidParms:
        StoreHold(i, "Used in invalid constraint")
    if warning:
        warning += "\n"
    ...
    for group, depPrmList in zip(groups, parmlist, strict=False):
        ...
        for rel in group:
            ...
            if unvaried:  # something is not varied: skip group & remove all parameters from varyList
                for var in depPrmList:
                    StoreHold(var, "Unexpected: mixed use")
                if GSASIIpath.GetConfigValue("debug"):
                    print(
                        "Unexpected: Constraint group ignored (some parameters unvaried)"
                    )
                    for rel in group:
                        print(
                            "   " + _FormatConstraint(constrDict[rel], fixedList[rel])
        ...
        if (
            not hasNewVar
        ):  # constraint equations; all remaining entries varied, vary generated
            for i in range(len(depPrmList)):
                if i >= len(
                    group
                ):  # tag generated degrees of freedom with names and vary them
            ...
            for i, rel in enumerate(group):  # noqa: B007
                if fixedList[rel] is None:
    ...
    for varlist, mapvars, multarr in zip(
        dependentParmList, indParmList, arrayList, strict=False
    ):  # process all constraints
        for mv in mapvars:
            if type(mv) is float or type(mv) is int:
                continue
...
def CheckEquivalences(constrDict, varyList, fixedList, parmDict=None, seqHistNum=None):
    ...
    global depVarList  # parameters used in equivalences as dependent parameters  # noqa: PLW0603
    global indepVarList  # parameters used in equivalences as independent parameters  # noqa: PLW0603
    global constrVarList  # parameters used in other constraints  # noqa: PLW0602

    # lists of parameters used for error reporting
    global undefinedVars  # parameters that are used in equivalences but are not defined  # noqa: PLW0602
    global convVarList  # parameters in equivalences that will be converted to constraints  # noqa: PLW0603
    convVarList = []  # parameters in equivalences to be made into constraints
    global multdepVarList  # noqa: PLW0603
    ...
    for cnum, (varlist, mapvars, multarr, invmultarr) in enumerate(  # noqa: B007
        zip(dependentParmList, indParmList, arrayList, invarrayList, strict=False)
    ):
    ...
    seenOnce = []
    for cnum, (varlist, multarr) in enumerate(  # noqa: B007
        zip(dependentParmList, arrayList, strict=False)
    ):
        if multarr is not None:
            continue  # equivalences only
        for v in varlist:
    ...
    while changed:
        ...
        for cnum, (varlist, mapvars, multarr, invmultarr) in enumerate(  # noqa: B007
            ...
            if cnum in convertList:
                convVarList += [
                    v
                    for v in mapvars + varlist
                    if v not in convVarList and type(v) is not float
                ]
                continue
            ...
            for v in mapvars:
                if v in constrVarList + convVarList:
                    ...
                    msg = True
                    warnEqv(
                        "Independent parameter " + str(v) + " used in constraint", cnum
                    )
                    if cnum not in convertList:
                        convertList.append(cnum)
            for v in varlist:
                if v in multdepVarList:
                    ...
                    msg = True
                    warnEqv(
                        "Dependent parameter " + str(v) + " used in constraint", cnum
                    )
                    if cnum not in convertList:
                        convertList.append(cnum)
            if msg:
    ...
    global unvariedParmsList  # noqa: PLW0603
    unvariedParmsList = []  # parameters in equivalences that are not varied
    # scan equivalences: look for holds
    for cnum, (varlist, mapvars, multarr, invmultarr) in enumerate(
        zip(dependentParmList, indParmList, arrayList, invarrayList, strict=False)
    ):
        if multarr is not None:
            continue  # not an equivalence
        if cnum in convertList:
            continue
        ...
        for v in varlist + mapvars:
            if varyList is None or v in varyList:
                ...
                gotNotVary = True
                if v not in unvariedParmsList:
                    unvariedParmsList.append(v)
        ...
        for v, m in zip(varlist, invmultarr, strict=False):
            if parmDict is not None and v not in parmDict:
                if v not in undefinedVars:
                    undefinedVars.append(v)
                if v not in dropVarList:
                    dropVarList.append(v)
                drop += 1
            elif m == 0:
                warnEqv(
                    "Parameter " + str(v) + " has a zero multiplier, dropping", cnum
                )
                if v not in dropVarList:
                    dropVarList.append(v)
                drop += 1
        ...
        for mv in mapvars:
            ...
            if parmDict is not None and mv not in parmDict:
                ...
                # hold them
                if mv not in undefinedVars:
                    undefinedVars.append(mv)
    ...
    for cnum, varlist in enumerate(dependentParmList):
        ...
        for dep, mult in zip(varlist, invarrayList[cnum], strict=False):
            constrDict += [{indvar: -1.0, dep: 1.0 / mult[0]}]
            fixedList += ["0.0"]
        # msg += '\n  to constraint(s):'
...
def StoreHold(var, holdType=None):
    ...
    by symmetry.
    """
    global holdParmList, holdParmType  # noqa: PLW0602
    if var not in holdParmList:
        holdParmList.append(var)
        if holdType:
            holdParmType[var] = holdType
...
def StoreEquivalence(independentVar, dependentList, symGen=True):
    ...
    global dependentParmList, arrayList, invarrayList, indParmList, symGenList  # noqa: PLW0602
    mapList = []
    multlist = []
    allfloat = True
...
def EvaluateMultipliers(constList, *dicts):
    ...
    prmDict = {}
    for d in dicts:
        prmDict.update(d)  # combine all passed parameter dicts
    problemList = ""
    ...
    global arrayList, invarrayList  # noqa: PLW0602
    for i, (a, valList) in enumerate(zip(arrayList, invarrayList, strict=False)):
        if a is not None:
            continue  # ignore if not equiv
        try:
            valList.shape  # noqa: B018
            ...
            if term[1].histogram == "*":
                if seqhst is None:
                    msg = (
                        "Parameter "
                        + str(terms[0][1])
                        + " contains a wildcard, which are used only sequential refinements. Constraint ignored."
                    )
    ...
    if constrLst[-1] == "e":
        ...
        if terms[0][1] in constrVarList + convVarList:
            msg = (
                "Parameter "
                + str(terms[0][1])
                + " used in constraint. To be recast as constraint."
            )
            return False, msg, "Recast as constraint."
        varList = []
        for m, v in terms:  # noqa: B007
            if v in constrVarList + convVarList:
        ...
        for m, v in terms:  # noqa: B007
            if v in holdParmList and (
                "User supplied" in holdParmType.get(v, "")
                or "symmetry" in holdParmType.get(v, "")
                or "rigid body" in holdParmType.get(v, "")
            ):
                if s:
                    s += ", "
                s += str(v)
        ...
        if gotNotVary and gotVary:  # mix of varied and unvaried parameters
            if msg:
                msg += ". "
            msg += (
                "Unvaried parameter(s): "
                + s
                + "; remainder set Hold. All parameters fixed."
            )
        ...
        v = terms[0][1]
        if v in undefinedVars:
            return False, "Parameter is undefined", "Ignored"
        if v in unvariedParmsList:
            return False, "Parameter is not refined", "Ignored"
    else:
            ...
            if constrLst[-1] == "f":  # new var expressions;
                if m == 0:
                    ...
                    (
                        v in holdParmList
                        and constrLst[-2]
                        and "User supplied" in holdParmType.get(v, "")
                    )
                    or "symmetry" in holdParmType.get(v, "")
                    or "rigid body" in holdParmType.get(v, "")
                ):
...
def VarRemapShow(varyList=None, inputOnly=False, linelen=60):
    ...
    for v in sorted(holdParmList):
        if v in depVars:
            continue
        if v in holdParmType:
            v += "\t" + holdParmType[v]  # noqa: PLW2901
        if first:
    ...
    if droppedSym:
        for varlist, mapvars, multarr, invmultarr in droppedSym:  # noqa: B007
            for i, mv in enumerate(mapvars):  # noqa: B007
                if len(varlist) == 1:
                    s1 = "   " + str(mv) + " is equivalent to "
                else:
                    s1 = "   " + str(mv) + " is equivalent to parameters: "
    ...
    for varlist, mapvars, invmultarr in zip(
        dependentParmList, indParmList, invarrayList, strict=False
    ):
        varied = False
...
def getInvConstraintEq(var, varyList):
    ...
    for varlist, mapvars, invmultarr in zip(
        ...
        return vList, mList
    if GSASIIpath.GetConfigValue("debug"):
        print("getInvConstraintEq: not found: ", var)
...
def GetDroppedSym(seqmode, seqhistnum):
    ...
    symerr = []
    symhelp = []
    global droppedSym  # noqa: PLW0602
...
def Dict2Deriv(varyList, derivDict, dMdv):
    ...
    """
    global dependentParmList, arrayList, invarrayList, indParmList, invarrayList  # noqa: PLW0602
    for varlist, mapvars, multarr, invmultarr in zip(
        ...
        for i, name in enumerate(mapvars):
            ...
            if name not in varyList:
                continue  # skip if independent var not varied
            if multarr is None:
                if debug:
                    print("start dMdv for", name, dMdv[varyList.index(name)])
                for v, m in zip(varlist, invmultarr, strict=False):
                    if m[0] == 0:
                        continue
...
def Map2Dict(parmDict, varyList):
    ...
    for item in holdParmList:
        if varyList is not None and item in varyList:
    ...
    global dependentParmList, arrayList, invarrayList, indParmList  # noqa: PLW0602
    for varlist, mapvars, multarr, invmultarr in zip(
        dependentParmList, indParmList, arrayList, invarrayList, strict=False
    ):
...
def normParms(parmDict):
    ...
    for varlist, mapvars, multarr, invmultarr in zip(
        ...
        if multarr is None or invmultarr is None:
            continue  # unexpected
        for i, s in enumerate(mapvars):
            try:
                s = float(s)  # noqa: PLW2901
            except:  # noqa: E722
                continue
            sumcons = 0.0
...
def Dict2Map(parmDict):
    ...
    for varlist, mapvars, invmultarr in zip(
        ...
        if invmultarr is None:  # is this needed?
            if GSASIIpath.GetConfigValue("debug"):
                print(
                    "Why does this constraint have None for invmultarr?",
                    varlist,
                    mapvars,
                )
            continue
...
def _FillArray(sel, d, collist):
    ...
    try:
        ...
        raise Exception("Initial constraints singular")  # noqa: B904
    for i in range(m, n):
...
if __name__ == "__main__":
    ...
    msg, warning, groups, parmlist = GenerateConstraints(
