...
BraggBrentanoParms = [
    ...
    ("filament_length", 12.0, "X-ray tube line focus length (mm)"),
    ("sample_length", 12.0, "Illuminated sample length in axial direction (mm)"),
    ("receiving_slit_length", 12.0, "Length of receiving slit in axial direction (mm)"),
    ("LAC_cm", 0.0, "Linear absorption coef. adjusted for packing density (cm-1)"),
    ("sample_thickness", 1.0, "Depth of sample (mm)"),
...
IBmonoParms = [
    (
        "src_mono_mm",
        119,
        "Distance from xray line source to monochromator crystal (mm)",
    ),
    ("focus_mono_mm", 217, "Distance from monochromator crystal to focus slit (mm)"),
    (
        "passband_mistune",
        -0.145,
        "Offset from the tuning of the IBM to the center of the reference line of the spectrum, in units of its bandwidth",
    ),
    (
        "mono_src_proj_mn",
        51,
        "Projection width of line-focus xray source on the monochromator along beam direction (microns), sets bandwidth",
    ),
    (
        "passband_shoulder",
        0.087,
        "Width of transition region from flat top to tails, in units of the bandwidth",
    ),
    (
        "two_theta_mono",
        27.27,
        "The full diffraction angle of the IBM crystal, e.g. double 2theta-Bragg for the mono (deg)",
    ),
    (
        "mono_slit_attenuation",
        0.03,
        "Attenuation of Cu K alpha2 lines due to focal slit",
    ),
]
...
def SetMonoWave():
    ...
    if IBmono and len(parmDict["wave"]) == 6:
        for key in "wave", "int", "lwidth":
            if 5 in parmDict[key]:
                del parmDict[key][5]
            if 4 in parmDict[key]:
                del parmDict[key][4]
...
def FillParmSizer():
    ...
    text = wx.StaticText(
        prmPnl, wx.ID_ANY, "label", style=wx.ALIGN_CENTER, size=(170, -1)
    )  # make just a bit bigger than largest item in column
...
def MakeTopasFPASizer(G2frame, FPdlg, SetButtonStatus):
    ...
    def PlotTopasFPA(event):
        ...
        startInd = center_bin_idx - (
            pkPts // 2
        )  # this should be the aligned start of the new data
    ...
    for lbl, prm, defVal in zip(
        ...
        for i in parmDict["wave"].keys():
            ...
            if prm == "wave":
                ctrl = G2G.ValidatedTxtCtrl(
                    FPdlg, parmDict[prm], i, size=(90, -1), nDig=(10, 6)
                )
            else:
                ctrl = G2G.ValidatedTxtCtrl(
                    FPdlg, parmDict[prm], i, size=(90, -1), nDig=(10, 3)
                )
    ...
    btnsizer.Add(
        wx.StaticText(FPdlg, wx.ID_ANY, "Detector type"),
        (0, 0),
        (2, 1),
    ...
    btnsizer.Add(
        wx.StaticText(FPdlg, wx.ID_ANY, "Incident Beam Mono"),
        (0, 3),
        (2, 1),
        wx.ALIGN_CENTER | wx.ALL,
        5,
    ...
    FPdlg.SetSize(
        (max(dx, px + 20), min(750, dy + py + 30))
    )  # 20 for scroll bar, 30 for a bit of room at bottom
...
def XferFPAsettings(InpParms):
    ...
    if (
        IBmono
    ):  # kludge: apply mono_slit_attenuation since it is not part of NIST FPA code
        norm = [
            InpParms["mono_slit_attenuation"]
            if (1.5443 < InpParms["wave"][i] < 1.5447)
            else 1.0
            for i in keys
        ]
    ...
    source_lor_widths_m = (
        1.0e-10 * 1.0e-3 * np.array([InpParms["lwidth"][i] for i in keys])
    )
    source_gauss_widths_m = 1.0e-10 * 1.0e-3 * np.array([0.001 for i in keys])

    NISTparms["emission"] = {
        "emiss_wavelengths": source_wavelengths_m,
        "emiss_intensities": source_intensities,
        "emiss_gauss_widths": source_gauss_widths_m,
        "emiss_lor_widths": source_lor_widths_m,
        "crystallite_size_gauss": 1.0e-9 * InpParms.get("Size_G", 1e6),
        "crystallite_size_lor": 1.0e-9 * InpParms.get("Size_L", 1e6),
    }
    if IBmono:
        NISTparms["emission"]["a_mono"] = InpParms["src_mono_mm"] * 10**-3
        NISTparms["emission"]["b_mono"] = InpParms["focus_mono_mm"] * 10**-3
        NISTparms["emission"]["ibm_source_width"] = (
            InpParms["mono_src_proj_mn"] * 10**-6
        )
        for i in ("passband_mistune", "passband_shoulder", "two_theta_mono"):
            NISTparms["emission"][i] = InpParms[i]
    elif (
        InpParms.get("source_width", 0) > 0 and InpParms.get("tube-tails_rel-I", 0) > 0
    ):
        NISTparms["tube_tails"] = {
            "main_width": 1e-3 * InpParms.get("source_width", 0.0),
            "tail_left": -1e-3 * InpParms.get("tube-tails_L-tail", 0.0),
            "tail_right": 1e-3 * InpParms.get("tube-tails_R-tail", 0.0),
            "tail_intens": InpParms.get("tube-tails_rel-I", 0.0),
        }

    if InpParms["filament_length"] == InpParms["receiving_slit_length"]:  # workaround:
        InpParms["receiving_slit_length"] *= (
            1.00001  # avoid bug when slit lengths are identical
        )
    NISTparms["axial"] = {
        "axDiv": "full",
        "slit_length_source": 1e-3 * InpParms["filament_length"],
    ...
    if InpParms.get("SiPSD_th2_angular_range", 0) > 0 and DetMode == "BBPSD":
        PSDdetector_length_mm = (
            np.arcsin(np.pi * InpParms["SiPSD_th2_angular_range"] / 180.0)
            * InpParms["Rs"]
        )  # mm
        NISTparms["si_psd"] = {
            "si_psd_window_bounds": (0.0, PSDdetector_length_mm / 1000.0)
        }

    if InpParms.get("Specimen_Displacement"):
        NISTparms["displacement"] = {
            "specimen_displacement": 1e-3 * InpParms["Specimen_Displacement"]
        }

    if InpParms.get("receiving_slit_width"):
        NISTparms["receiver_slit"] = {
            "slit_width": 1e-3 * InpParms["receiving_slit_width"]
        }
...
def MakeSimSizer(G2frame, dlg):
    ...
    def _onOK(event):
        ...
        # compute "obs" pattern
        ttArr = np.arange(
            max(0.5, simParms["minTT"] - simParms["calcwid"] / 1.5),
            simParms["maxTT"] + simParms["calcwid"] / 1.5,
            simParms["step"],
        )
        intArr = np.zeros_like(ttArr)
        peaklist = np.linspace(
            simParms["minTT"], simParms["maxTT"], simParms["npeaks"], endpoint=True
        )
        ...
        for num, twoth_peak in enumerate(peaklist):
            try:
                center_bin_idx, peakObj = doFPAcalc(
                    NISTpk, ttArr, twoth_peak, simParms["calcwid"], simParms["step"]
                )
            except Exception as err:
                if msg:
                    msg += "\n"
            ...
            pkPts = len(peakObj.peak)
            minPtsHM = min(
                minPtsHM, sum(peakObj.peak >= 0.5 * pkMax)
            )  # points above half-max
            maxPtsHM = max(
                maxPtsHM, sum(peakObj.peak >= 0.5 * pkMax)
            )  # points above half-max
            startInd = center_bin_idx - (
                pkPts // 2
            )  # this should be the aligned start of the new data
    ...
    def FitFPApeaks(ttArr, intArr, peaklist, maxPtsHM):
        ...
        histId = G2frame.AddSimulatedPowder(
            ttArr, intArr, "NIST Fundamental Parameters simulation", Lam1, Lam2
        )
        controls = G2frame.GPXtree.GetItemPyData(
            G2gd.GetGPXtreeItemId(G2frame, G2frame.root, "Controls")
        )
        controldat = controls.get(
            "data",
            {
                "deriv type": "analytic",
                "min dM/M": 0.001,
        ...
        peakData["sigDict"] = G2pwd.DoPeakFit(
            "LSQ",
            peakData["peaks"],
            bkg,
            limits[1],
    ...
    def _onReadFPA(event):
        ...
        try:
            ...
            G2G.G2MessageBox(
                dlg, f"Error reading file {filename}:{err}\n", "Bad dict input"
            )
        # GSASIIpath.IPyBreak()
        SetButtonStatus()
    ...
    for key, defVal, text in (
        ("minTT", 3.0, "Location of first peak in 2theta (deg)"),
        ("maxTT", 123.0, "Location of last peak in 2theta (deg)"),
        ("step", 0.01, "Pattern step size (deg 2theta)"),
        ("npeaks", 13, "Number of peaks"),
        ("calcwid", 2.0, "Range to compute each peak (deg 2theta)"),
    ):
    ...
    txt = wx.StaticText(
        dlg,
        wx.ID_ANY,
        "If you use this, please cite: " + G2G.GetCite("Fundamental parameter fitting"),
        size=(350, -1),
    )
    txt.Wrap(340)
...
if __name__ == "__main__":
    ...
    frm.plotFrame = wx.Frame(
        None, -1, "GSASII Plots", size=size, style=wx.DEFAULT_FRAME_STYLE ^ wx.CLOSE_BOX
    )
