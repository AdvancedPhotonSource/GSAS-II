...
obsInCaption = (
    True  # include the observed, calc,... items in the plot caption (PlotPatterns)
)
...
def ReplotPattern(G2frame, newPlot, plotType, PatternName=None, PickName=None):  # noqa: ARG001
    ...
    if PatternName:
        ...
        if pId:
            ...
            if GSASIIpath.GetConfigValue("debug"):
                print("PatternName not found", PatternName)
            return
    if PickName == PatternName:
        ...
        if pId:
            ...
            if GSASIIpath.GetConfigValue("debug"):
                print("PickName not found", PickName)
            return
...
def plotVline(Page, Plot, Lines, Parms, pos, color, pick, style="dotted"):
    ...
    if Page.plotStyle["qPlot"]:
        Lines.append(
            Plot.axvline(
                2.0 * np.pi / G2lat.Pos2dsp(Parms, pos),
                color=color,
                picker=pick,
                pickradius=2.0,
                linestyle=style,
        ...
        Lines.append(
            Plot.axvline(pos, color=color, picker=pick, pickradius=2.0, linestyle=style)
        )
...
def PlotPatterns(
    ...
    def PublishPlot(event):  # noqa: ARG001
        ...
        if msg:
            msg = (
                "Publication export is only available under limited plot settings\n"
                + msg
            )
    ...
    def OnPlotKeyPress(event):
        try:  # one way to check if key stroke will work on plot
            ...
            G2frame.G2plotNB.status.SetStatusText(
                "Select " + plottype + " pattern first", 1
            )
            return
        newPlot = False
        if event.key == "w":
            ...
            Page.plotStyle["WgtDiagnostic"] = not Page.plotStyle.get(
                "WgtDiagnostic", False
            )
            newPlot = True
        elif (
            event.key == "b"
            and plottype not in ["SASD", "REFD"]
            and not Page.plotStyle["logPlot"]
            and not Page.plotStyle["sqrtPlot"]
        ):
            ...
            if Page.plotStyle["sqrtPlot"]:
                Page.plotStyle["delOffset"] = 0.02 * np.sqrt(YmaxS)
                Page.plotStyle["refOffset"] = -0.1 * np.sqrt(YmaxS)
                Page.plotStyle["refDelt"] = 0.1 * np.sqrt(YmaxS)
            else:
                Page.plotStyle["delOffset"] = 0.02 * YmaxS
            ...
            if dlg.ShowModal() == wx.ID_OK:
                ...
                G2frame.ContourColor = GSASIIpath.GetConfigValue(
            ...
            if event.key == "d":
                ...
                r = selected[0] + 1
                if r >= grid.NumberRows:
                    r = None
            else:
                if not selected:
                    selected = [grid.NumberRows]
                r = selected[0] - 1
                if r < 0:
                    r = None
            if r is not None:
            ...
            if G2frame.Contour:
                G2frame.Cmin += (G2frame.Cmax - G2frame.Cmin) / 5.0
            elif Page.plotStyle["Offset"][0] < 100.0:
                Page.plotStyle["Offset"][0] += 10.0
        elif event.key == "D":
            if G2frame.Contour:
                G2frame.Cmin -= (G2frame.Cmax - G2frame.Cmin) / 5.0
            elif Page.plotStyle["Offset"][0] > -100.0:
            ...
            if Page.excludeMode:
                try:  # fails from key menu
                    ...
                    G2G.G2MessageBox(
                        G2frame,
                        'To create an excluded region, after clicking on "OK", move to the beginning of the region and press the "e" key. Then move to the end of the region and press "e" again',
                        "How to exclude",
                    )
                    return
                if G2frame.Weight:
                ...
                if abs(Page.startExclReg - event.xdata) < 0.1:
                    return
            ...
            and G2frame.SinglePlot
            and not (
                Page.plotStyle["logPlot"]
                or Page.plotStyle["sqrtPlot"]
                or G2frame.Contour
            )
        ):
            ...
            try:
                xpos = event.xdata
                if xpos is None:
                    return  # avoid out of frame mouse position
                ...
                AttributeError
            ):  # invoked when this is called from dialog rather than key press
                xpos = (
                    Pattern[1][0][-1] + Pattern[1][0][0]
                ) / 2  # set to middle of pattern
            if not Pattern[0]["Magnification"]:
                Pattern[0]["Magnification"] = [[None, 1.0]]
            Pattern[0]["Magnification"] += [[xpos, 2.0]]
            ...
            plotOpt["CSVfile"] = G2G.askSaveFile(
                G2frame, "", ".csv", "Comma separated variable file"
            )
            if plotOpt["CSVfile"]:
                plotOpt["saveCSV"] = True
        else:
    ...
    def OnMotion(event):
        ...
        if Page.excludeMode and Page.savedplot:
            if (
                event.xdata is None
                or G2frame.GPXtree.GetItemText(G2frame.GPXtree.GetSelection())
                != "Limits"
            ):  # reset if out of bounds or not on limits
                Page.savedplot = None
        ...
        if event.button and G2frame.Contour and G2frame.TforYaxis:
            ...
            ylabs = [np.where(i >= 0, Temps[int(i)], " ") for i in ytics]
        ...
        try:
            ...
            limx = Plot.get_xlim()
            dT = tolerance = np.fabs(limx[1] - limx[0]) / 100.0
            if Page.plotStyle["qPlot"] and "PWDR" in plottype:
                ...
                if "T" in Parms["Type"][0]:  # TOF
                    ...
                    dT = tolerance * wave * 90.0 / (np.pi**2 * cosd(xpos / 2))
            ...
            if G2frame.Contour:  # PWDR only
                try:
                    pNum = int(ypos + 0.5)
                    indx = abs(
                        PlotList[pNum][1][0] - xpos
                    ).argmin()  # closest point to xpos
                    val = f"int={ma.getdata(PlotList[pNum][1][1])[indx]:.3g}"
                    if "T" in Parms["Type"][0]:
                        statLine = "TOF=%.3f d=%.5f Q=%.5f %s pattern ID=%d, %s" % (
                            xpos,
                            dsp,
                            q,
                            val,
                        ...
                        statLine = "2-theta=%.3f d=%.5f Q=%.5f %s pattern ID=%d, %s" % (
                            xpos,
                            dsp,
                            q,
                            val,
            ...
            if pickIdText in tickMarkList and len(G2frame.HKL):
                ...
                findx = np.where(
                    np.fabs(np.array(G2frame.HKL).T[indx] - xpos) < dT / 2.0
                )
                found = G2frame.HKL[findx]
                if len(G2frame.Extinct):
                    G2frame.Extinct = np.array(G2frame.Extinct)
                    f2 = G2frame.Extinct[
                        np.where(np.fabs(G2frame.Extinct.T[indx] - xpos) < dT / 2.0)
                    ]
    ...
    def onMovePeak(event):  # noqa: ARG001
        ...
        if len(selectedPeaks) != 1:
            ...
            dlg = G2G.G2SingleChoiceDialog(
                G2frame, "Select peak to move", "select peak", choices
            )
            try:
                if dlg.ShowModal() == wx.ID_OK:
        ...
        if len(selectedPeaks) != 1:
            return
        G2frame.itemPicked = G2frame.Lines[
            selectedPeaks[0] + 2
        ]  # 1st 2 lines are limits
        G2frame.G2plotNB.Parent.Raise()
        OnPickPwd(None)
    ...
    def OnPickPwd(event):
        ...
        def OnDragMarker(event):
            ...
            if G2frame.itemPicked is None:
                return  # not sure why this happens, if it does
        ...
        def OnDragLabel(event):
            ...
            try:
                ...
                if Page.plotStyle["sqrtPlot"]:
                    data[0]["HKLmarkers"][k1][k2][0] = (
                        np.sign(coords[1]) * coords[1] ** 2
                    )
                else:
        ...
        def OnDragTickmarks(event):
            ...
            if event.ydata is None:
                return  # ignore if cursor out of window
            if Page.tickDict is None:
                return  # not sure why this happens, if it does
            Page.canvas.restore_region(savedplot)
            if Page.pickTicknum:
                refDelt = (
                    -(event.ydata - Page.plotStyle["refOffset"]) / Page.pickTicknum
                )
        ...
        def OnDragDiffCurve(event):
            ...
            Page.figure.gca().draw_artist(
                G2frame.itemPicked
            )  #  Diff curve only found in 1-window plot
        ...
        if (
            event is None
        ):  # called from a menu command rather than by click on mpl artist
            mouse = 1
            pick = G2frame.itemPicked
            ind = np.array([0])
        elif str(event.artist).startswith(
            "Text"
        ):  # respond to a right or left click on a HKL Label
            ...
            G2frame.itemPicked = pick
            pick.set_alpha(0.3)  # grey out text
        ...
        if (
            ...
            if ind.all() != [0] and ObsLine[0].get_label() in str(pick):
                ...
                for _pId, phase in enumerate(
                    ...
                    rgb_light = [(2 + i) / 3.0 for i in rgb]
                ...
                G2frame.cid = Page.canvas.mpl_connect(
                    "motion_notify_event", OnDragTickmarks
                )
                G2frame.itemPicked = pick
                return
                ...
                G2frame.cid = Page.canvas.mpl_connect(
                    "motion_notify_event", OnDragDiffCurve
                )
            else:
            ...
            G2frame.PickId
            and G2frame.GPXtree.GetItemText(G2frame.PickId) == "Unit Cells List"
        ):
            ...
            if DifLine[0] is G2frame.itemPicked:  # pick of difference curve
                ...
                if event is not None and event.mouseevent.button == 3:
                    ...
                    if ph not in Phases:
                        print(
                            f"Tickmark label ({ph}) not in Reflection Lists, very odd"
                        )
                        return
                    ...
                    if Page.plotStyle["qPlot"]:
                        if "T" in Parms["Type"][0]:  # TOF
                            q = 2.0 * np.pi / G2lat.Pos2dsp(Parms, xpos)
                            dT = Parms["difC"][1] * 2 * np.pi * tolerance / q**2
                        elif "E" in Parms["Type"][0]:  # energy dispersive x-rays
                            pass  # for now
                        else:  # 'C' or  'B' in Parms['Type'][0] or 'PKS' in Parms['Type'][0]:
                    ...
                    found_indices = np.where(
                        np.fabs(Phases[ph]["RefList"][:, indx] - xtick) < dT / 2.0
                    )
                    ...
                    for _i, refl in enumerate(Phases[ph]["RefList"][found_indices]):
                        ...
                        if Page.plotStyle["sqrtPlot"]:
                            data[0]["HKLmarkers"][ph][key][0] = (
                                np.sign(ytick) * ytick**2
                            )
                        else:
                            data[0]["HKLmarkers"][ph][key][0] = ytick
                        # add reflection if not already present
                        for hkl in data[0]["HKLmarkers"][ph][key][1]:
                            if sum(np.abs(refl[:n] - hkl)) == 0:
                                break
                        else:
                            data[0]["HKLmarkers"][ph][key][1].append(
                                tuple(np.rint(refl[:n]).astype(int))
                            )
                        ...
                        ytick -= (
                            ...
                            * 3
                            * (font / 8.0)
                            * (Plot.get_ylim()[1] - Plot.get_ylim()[0])
                            / 100
                        )
                    wx.CallAfter(
                        PlotPatterns, G2frame, plotType=plottype, extraKeys=extraKeys
            ...
            G2frame.PickId
            and G2frame.GPXtree.GetItemText(G2frame.PickId) == "Background"
        ):
            ...
            if pick.get_marker() == "D":
                ...
                backDict = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, G2frame.PatternId, "Background")
                )[1]
                d2 = [
                    (x - xy[0]) ** 2 + (y - xy[1]) ** 2
                    for x, y in backDict["FixedPoints"]
                ]
                ...
                if mode == "Move":
                    ...
                    savedplot = Page.canvas.copy_from_bbox(axis.bbox)
                    G2frame.cid = Page.canvas.mpl_connect(
                        "motion_notify_event", OnDragMarker
                    )
    ...
    def OnRelease(event):
        ...
        ):  # ignore drag if cursor is outside of plot
        ...
        if not G2frame.PickId:
            if GSASIIpath.GetConfigValue("debug"):
                print("Ignoring drag, G2frame.PickId is not set")
            return

        if G2frame.GPXtree.GetItemText(G2frame.PickId) == "Background" and event.xdata:
            if (
                Page.toolbar.AnyActive()
            ):  # prevent ops. if a toolbar zoom button pressed
            ...
            if (
                ...
                or G2frame.Weight
                or G2frame.Contour
                or not G2frame.SinglePlot
            ):
                return
            backDict = G2frame.GPXtree.GetItemPyData(
                G2gd.GetGPXtreeItemId(G2frame, G2frame.PatternId, "Background")
            )[1]
            if "FixedPoints" not in backDict:
                backDict["FixedPoints"] = []
            try:
                Parms, Parms2 = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(
                        G2frame, G2frame.PatternId, "Instrument Parameters"
        ...
        if G2frame.itemPicked is None:
            ...
            return
        if (
            DifLine[0] is G2frame.itemPicked
        ):  # respond to dragging of the difference curve
            data = G2frame.GPXtree.GetItemPyData(G2frame.PickId)
            ypos = event.ydata
        ...
        if (
            ...
            and xpos
            and phNum is not None
        ):
            # dragging tickmarks
            if phNum:
                Page.plotStyle["refDelt"] = (
                    -(event.ydata - Page.plotStyle["refOffset"]) / phNum
                )
            else:  # 1st row of refl ticks
                Page.plotStyle["refOffset"] = event.ydata
        elif (
            G2frame.GPXtree.GetItemText(G2frame.PickId)
            in ["Peak List", "Limits", "Unit Cells List"]
            and xpos
        ):
            lines = []
            ...
            if lineNo in [0, 1] or lineNo in exclLines:
                ...
                id2 = lineNo % 2
                if Page.plotStyle["qPlot"] and "PWDR" in plottype:
                    limits[Id][id2] = G2lat.Dsp2pos(Parms, 2.0 * np.pi / xpos)
                ...
                if event.button == 3:
                    ...
                    if Page.plotStyle["qPlot"]:
                        tbl[lineNo - 2 - nxcl][0] = G2lat.Dsp2pos(
                            Parms, 2.0 * np.pi / xpos
                        )
                    elif Page.plotStyle["dPlot"]:
            ...
            if Id:
                ...
                if "line" not in pick:  # avoid data points, etc.
                    if pick in Page.phaseList:
                        ...
                        if num:
                            Page.plotStyle["refDelt"] = (
                                -(event.ydata - Page.plotStyle["refOffset"]) / num
                            )
                        else:  # 1st row of refl ticks
                            Page.plotStyle["refOffset"] = event.ydata
    ...
    def onSetPlotLim(event):  # noqa: ARG001
        ...
        def applyLims(event):  # noqa: ARG001
            ...
            for var in "xlims", "ylims", "dylims", "cylims":
                for i in range(2):
                    if not G2frame.UseLimits[var][i]:
                        continue
                    try:
        ...
        dlg = wx.Dialog(
            G2frame.plotFrame, style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER
        )
        vbox = wx.BoxSizer(wx.VERTICAL)
        vbox.Add(wx.StaticText(dlg, wx.ID_ANY, "Set Plot limits"), 0, wx.ALL)
        ...
        for lbl, key in lblkeys:
            ...
            for i in range(2):
                cbox.append(
                    G2G.G2CheckBox(
                        dlg, "", G2frame.UseLimits[key], i, OnChange=onChecked
                    )
                )
                dbox.append(
                    G2G.ValidatedTxtCtrl(
                        dlg, G2frame.FixedLimits[key], i, typeHint=float
                    )
                )
                gsizer.Add(cbox[-1])
                gsizer.Add(dbox[-1])
    ...
    def refPlotUpdate(Histograms, cycle=None, restore=False):
        ...
        if Page.plotStyle["qPlot"]:
            X = (
                2.0 * np.pi / G2lat.Pos2dsp(Parms, xye0)
            )  # might want to consider caching this
            Ibeg = np.searchsorted(X, 2.0 * np.pi / G2lat.Pos2dsp(Parms, limits[1][0]))
            Ifin = np.searchsorted(X, 2.0 * np.pi / G2lat.Pos2dsp(Parms, limits[1][1]))
        elif Page.plotStyle["dPlot"]:
        ...
        if Page.plotStyle["sqrtPlot"]:
            ...
            Y = np.where(xye[1] >= 0.0, np.sqrt(xye[1]), -np.sqrt(-xye[1]))
            Z = np.where(xye[3] >= 0.0, np.sqrt(xye[3]), -np.sqrt(-xye[3]))
            W = np.where(xye[4] >= 0.0, np.sqrt(xye[4]), -np.sqrt(-xye[4]))
    ...
    def onHKLabelConfig(event):  # noqa: ARG001
        ...
        sizer.Add(
            wx.StaticText(dlg, label="Configure Reflection (hkl) Labels"),
            0,
            wx.ALIGN_CENTER_HORIZONTAL | wx.RIGHT | wx.LEFT | wx.BOTTOM,
            15,
        )
        hsizer = wx.FlexGridSizer(cols=2, hgap=2, vgap=2)
        sizer.Add(hsizer)
        hsizer.Add(wx.StaticText(dlg, label="Label Orientation: "))
        data[0]["HKLconfig"]["Orientation"] = data[0]["HKLconfig"].get("Orientation", 0)
        choice = G2G.G2ChoiceButton(
            dlg,
            ["Horizontal", "Vertical"],
            data[0]["HKLconfig"],
            "Orientation",
        ...
        data[0]["HKLconfig"]["Font"] = data[0]["HKLconfig"].get("Font", "8")
        choice = G2G.G2ChoiceButton(
            dlg,
            ["6", "8", "10", "12", "14", "16"],
            None,
        ...
        data[0]["HKLconfig"]["alpha"] = data[0]["HKLconfig"].get("alpha", 2)
        choice = G2G.G2ChoiceButton(
            dlg,
            ["0", "25%", "50%", "75%", "100%"],
            data[0]["HKLconfig"],
    ...
    if (
        "PKS" in plottype
    ):  # This is probably not used anymore; PlotPowderLines seems to be called directly
    ...
    if (
        G2frame.PickId
        and plottype not in ["SASD", "REFD"]
        and "PWDR" in G2frame.GPXtree.GetItemText(G2frame.PickId)
    ):
        publish = PublishPlot
    else:
        publish = None
    new, plotNum, Page, Plot, limits = G2frame.G2plotNB.FindPlotTab(
        "Powder Patterns", "mpl", publish=publish
    )
    if (
        G2frame.ifSetLimitsMode
        and G2frame.GPXtree.GetItemText(G2frame.GPXtree.GetSelection()) == "Limits"
    ):
        # note mode
        ...
        Page.figure.text(0.02, 0.93, msg, fontsize=14, fontweight="bold")

    Page.excludeMode = False  # True when defining an excluded region
    Page.savedplot = None
    # patch
    if "Offset" not in Page.plotStyle and plotType in [
        "PWDR",
        "SASD",
        "REFD",
    ]:  # plot offset data
        Ymax = max(data[1][1])
        Page.plotStyle.update(
            {
                "Offset": [0.0, 0.0],
                "delOffset": 0.02 * Ymax,
                "refOffset": -0.1 * Ymax,
    ...
    if plotType == "PWDR":
        try:
            ...
            if G2frame.lastPlotType != Parms["Type"][1]:
                ...
                Ymax = max(data[1][1])
                if Page.plotStyle["sqrtPlot"]:
                    Page.plotStyle["delOffset"] = 0.02 * np.sqrt(Ymax)
                    Page.plotStyle["refOffset"] = -0.1 * np.sqrt(Ymax)
                    Page.plotStyle["refDelt"] = 0.1 * np.sqrt(Ymax)
                else:
                    Page.plotStyle["delOffset"] = 0.02 * Ymax
                    Page.plotStyle["refOffset"] = -0.1 * Ymax
                    Page.plotStyle["refDelt"] = 0.1 * Ymax
                newPlot = True
        ...
        if plottingItem not in Histograms:
            ...
            if len(histoList) != 0:
                ...
                G2frame.PatternId = G2gd.GetGPXtreeItemId(
                    G2frame, G2frame.root, plottingItem
                )
                data = G2frame.GPXtree.GetItemPyData(G2frame.PatternId)
    ...
    if not G2frame.PickId:
        print(
            "No plot, G2frame.PickId,G2frame.PatternId=",
            G2frame.PickId,
            G2frame.PatternId,
        )
        return None
        ...
        Histograms, Phases = G2frame.GetUsedHistogramsAndPhasesfromTree()
        Phases = G2frame.GPXtree.GetItemPyData(
        ...
        G2frame.Bind(
            wx.EVT_MENU, onMoveDiffCurve, id=G2frame.dataWindow.moveDiffCurve.GetId()
        )
        G2frame.Bind(
            wx.EVT_MENU, onMoveTopTick, id=G2frame.dataWindow.moveTickLoc.GetId()
        )
        G2frame.Bind(
            wx.EVT_MENU, onMoveTickSpace, id=G2frame.dataWindow.moveTickSpc.GetId()
        )
        G2frame.Bind(
            wx.EVT_MENU, onSetPlotLim, id=G2frame.dataWindow.setPlotLim.GetId()
        )
        G2frame.Bind(
            wx.EVT_MENU, onPlotFormat, id=G2frame.dataWindow.setPlotFmt.GetId()
        )
    ...
    if not hasattr(Page, "phaseColors"):
        Page.phaseColors = {}
    ...
    try:
        if not refineMode:
            ...
            wx.CallAfter(
                G2frame.G2plotNB.RegisterRedrawRoutine(
                    G2frame.G2plotNB.lastRaisedPlotTab,
                    ReplotPattern,
                    (G2frame, newPlot, plotType),
                    kwargs,
                )
            )
    ...
    if G2frame.Contour:
        Page.Choice = (
            ...
            "D: lower contour min",
            "U: raise contour min",
            "o: reset contour limits",
            "g: toggle grid",
            "i: interpolation method",
            "S: color scheme",
            "c: contour off",
            "e: toggle temperature for y-axis",
            "s: toggle sqrt plot",
            "w: toggle w(Yo-Yc) contour plot",
            "h: toggle channel # plot",
            "q: toggle Q plot",
        ...
        Page.Choice = [
            ...
            "b: toggle subtract background",
            "c: contour on",
            "x: toggle excluded regions",
            "T: toggle plot title",
            "f: toggle full-length ticks",
            "g: toggle grid",
            "X: toggle cumulative chi^2",
            "m: toggle multidata plot",
            "n: toggle log(I)",
        ]
        if obsInCaption:
            Page.Choice += [
                "o: remove obs, calc,... from legend",
            ]
        else:
            Page.Choice += [
                "o: add obs, calc,... to legend",
            ]
        if ifLimits:
            Page.Choice += [
                "e: create excluded region",
                "s: toggle sqrt plot",
                "w: toggle (Io-Ic)/sig plot",
        ...
        Page.Choice = [
            " key press",
            "b: toggle subtract background file",
            "g: toggle grid",
            "m: toggle multidata plot",
            "n: toggle semilog/loglog",
            "q: toggle S(q) plot",
            "w: toggle (Io-Ic)/sig plot",
            "+: toggle obs line plot",
        ]
        ...
        while Id:
            ...
            ParmList.append(
                G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, pid, "Instrument Parameters")
                )[0]
            )
            SampleList.append(
                G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, pid, "Sample Parameters")
                )
            )
            LimitsList.append(
                G2frame.GPXtree.GetItemPyData(
    ...
    if timeDebug:
        print(
            "plot build time: %.3f for %dx%d patterns"
            % (time.time() - time0, len(PlotList[0][1][1]), len(PlotList))
        )
    ...
    if G2frame.SubBack:
        Title += " - background"
    if Page.plotStyle["qPlot"] or (
        plottype in ["SASD", "REFD"] and not G2frame.Contour and not ifLimits
    ):
        xLabel = r"$Q, \AA^{-1}$"
    ...
    if "T" in ParmList[0]["Type"][0] or (
        Page.plotStyle["Normalize"] and not G2frame.SinglePlot
    ):
        if Page.plotStyle["sqrtPlot"]:
    ...
    )  # wants only default as defined in previous line!!
    ...
    if G2frame.Contour:  # detect unequally spaced points in a contour plot
        for N, Pattern in enumerate(PlotList):  # noqa: B007
            ...
            if not X0:
                ...
                unequalArrays = True
            if (
                Page.plotStyle["qPlot"]
                or Page.plotStyle["dPlot"]
                or ("T" in ParmList[0]["Type"][0] and not Page.plotStyle["chanPlot"])
            ...
            # check to see if the average step size changes across the selected patterns
                ...
                not unequalArrays
                and abs(avgStep - (X[-1] - X[0]) / (len(X) - 1)) > 0.05 * avgStep
            ):
                unequalArrays = True
    ...
    for N, Pattern in enumerate(PlotList):
        ...
        if Pattern[1] is None:
            continue  # skip over uncomputed simulations
        xye = np.array(ma.getdata(Pattern[1]))  # strips mask = X,Yo,W,Yc,Yb,Yd
        ...
        if G2frame.PickId:  # when is this not true?
            ...
            for excl in limits[2:]:
                xye0 = ma.masked_inside(
                    xye[0], excl[0], excl[1], copy=False
                )  # excluded region mask
            if unequalArrays:
                xye0 = ma.masked_outside(
                    xye[0], limits[1][0], limits[1][1], copy=False
                )  # now mask for limits
                Lmask = ma.getmask(xye0)  # limits applied
                ExMask[N] = ExMask[N][~Lmask]  # drop points outside limits
            elif not G2frame.Contour:
                xye0 = ma.masked_outside(
                    xye0, limits[1][0], limits[1][1], copy=False
                )  # now mask for limits
        else:
        ...
        if Page.plotStyle["qPlot"] and "PWDR" in plottype and not ifLimits:
            X = ma.array(2.0 * np.pi / G2lat.Pos2dsp(Parms, xye0.data), mask=xye0.mask)
        elif Page.plotStyle["dPlot"] and "PWDR" in plottype and not ifLimits:
        ...
        if (
            ...
            if (
                ("C" in ParmList[0]["Type"][0] and Page.plotStyle["dPlot"])
                or ("B" in ParmList[0]["Type"][0] and Page.plotStyle["dPlot"])
                or ("E" in ParmList[0]["Type"][0] and Page.plotStyle["dPlot"])
        ...
        if "PWDR" in plottype:
            ...
            if Page.plotStyle["sqrtPlot"]:
                olderr = np.seterr(invalid="ignore")  # get around sqrt(-ve) error
                Y = (
                    np.where(YI >= 0.0, np.sqrt(YI), -np.sqrt(-YI))
                    + NoffY * Ymax / 100.0
                )
                np.seterr(invalid=olderr["invalid"])
            elif Page.plotStyle.get("WgtDiagnostic", False):
        ...
        if ifpicked and not G2frame.Contour:  # draw limit & excluded region lines
            lims = limits[1:]
            if Page.plotStyle["qPlot"] and "PWDR" in plottype and not ifLimits:
                lims = 2.0 * np.pi / G2lat.Pos2dsp(Parms, lims)
            elif Page.plotStyle["dPlot"] and "PWDR" in plottype and not ifLimits:
                lims = G2lat.Pos2dsp(Parms, lims)
            # limit lines
            Lines.append(
                Plot.axvline(
                    lims[0][0], color="g", dashes=(5, 5), picker=True, pickradius=3.0
                )
            )
            Lines.append(
                Plot.axvline(
                    lims[0][1], color="r", dashes=(5, 5), picker=True, pickradius=3.0
                )
            )
            ...
            for i, item in enumerate(lims[1:]):
                Lines.append(
                    Plot.axvline(
                        item[0], color="m", dashes=(5, 5), picker=True, pickradius=3.0
                    )
                )
                Lines.append(
                    Plot.axvline(
                        item[1], color="m", dashes=(5, 5), picker=True, pickradius=3.0
                    )
                )
        ...
        if G2frame.Contour:
            ...
            if (
                ifpicked
            ):  # plotting of "master" pattern (histogram selected in data tree)
                ZI = copy.copy(xye[3])  # Yc
                if Page.plotStyle["sqrtPlot"]:
                    olderr = np.seterr(invalid="ignore")  # get around sqrt(-ve) error
                    Z = np.where(ZI >= 0.0, np.sqrt(ZI), -np.sqrt(-ZI))
                    np.seterr(invalid=olderr["invalid"])
                elif (
                    "PWDR" in plottype
                    and G2frame.SinglePlot
                ...
                if "PWDR" in plottype:
                    if Page.plotStyle["sqrtPlot"]:
                        olderr = np.seterr(
                            invalid="ignore"
                        )  # get around sqrt(-ve) error
                        W = np.where(
                            xye[4] >= 0.0, np.sqrt(xye[4]), -np.sqrt(-xye[4])
                        )  # yb
                        np.seterr(invalid=olderr["invalid"])
                        D = np.where(xye[5], (Y - Z), 0.0) - Page.plotStyle["delOffset"]
                    elif Page.plotStyle.get("WgtDiagnostic", False):
                    ...
                    if np.any(W > 0.0):
                        ...
                        lims = [np.min(np.trim_zeros(YB)) / 2.0, np.max(Y) * 2.0]
                    if all(np.isfinite(lims)):
                ...
                BackLine = None
                DifLine = [None]
                if "PWDR" in plottype and len(
                    limits[2:]
                ):  # compute mask for excluded regions
                    Emask = copy.deepcopy(ma.getmask(X))
                    for excl in limits[2:]:
                        Emask += ma.getmask(
                            ma.masked_inside(xye[0], excl[0], excl[1], copy=False)
                        )
                ...
                if G2frame.Weight:
                    ...
                    if plottype in ["SASD", "REFD"]:
                        ...
                        if "PWDR" in plottype and len(limits[2:]):
                            DZ = ma.array(
                                DZ, mask=Emask
                            )  # weighted difference is always masked
                    DifLine = Plot1.plot(
                        X,
                        DZ,
                        pwdrCol["Diff_color"],
                ...
                if Page.plotStyle["logPlot"]:
                        ...
                        if G2frame.ErrorBars:
                            if Page.plotStyle["sqPlot"]:
                                Plot.errorbar(
                                    ...
                                    yerr=X**4
                                    * Sample["Scale"][0]
                                    * np.sqrt(1.0 / (Pattern[0]["wtFactor"] * xye[2])),
                    ...
                    ymax = 1.0
                    if (
                        Page.plotStyle["Normalize"]
                        and Y.max() != 0
                        and not G2frame.SinglePlot
                    ):
                        ymax = Y.max()
                    if G2frame.SubBack:
                        if "PWDR" in plottype:
                    ...
                    Plot.axhline(0.0, color="k", label="_zero")
                        ...
                        try:
                            ...
                            selectedPeaks = list(
                                set(
                                    [
                                        row
                                        for row, col in G2frame.reflGrid.GetSelectedCells()
                                    ]
                ...
                if Page.plotStyle["logPlot"]:
                    ...
                    Plot.set_ylim(
                        bottom=np.min(np.trim_zeros(Y)) / 2.0, top=np.max(Y) * 2.0
                    )
    ...
    props = dict(
        boxstyle="round,pad=0.15", facecolor="#ccc", alpha=(4 - alpha) / 4.0, ec="#ccc"
    )
    markHKLs = (
        G2frame.GPXtree.GetItemText(G2frame.PickId) == "Reflection Lists"
        or "PWDR" in G2frame.GPXtree.GetItemText(G2frame.PickId)
    ...
    for ph, markerlist in data[0].get("HKLmarkers", {}).items():
        if Page.plotStyle["logPlot"] or not markHKLs:
            continue
        if ph not in Page.phaseColors:
            continue
        ...
        for key, (ypos, hkls) in markerlist.items():
            ...
            xpos = float(key)
            if Page.plotStyle["qPlot"]:
                xpos = 2.0 * np.pi / G2lat.Pos2dsp(Parms, xpos)
            elif Page.plotStyle["dPlot"]:
    ...
    if G2frame.PickId and not G2frame.Contour:
        ...
        orange = [255 / 256.0, 128 / 256.0, 0.0]
        ...
        if G2frame.GPXtree.GetItemText(G2frame.PickId) in [
            ...
            if len(peaks):  # are there any peaks?
                for i, peak in enumerate(peaks[0]):
                    if Page.plotStyle["qPlot"]:
                        x = 2.0 * np.pi / G2lat.Pos2dsp(Parms, peak[0])
                    elif Page.plotStyle["dPlot"]:
            ...
            for hkl in G2frame.HKL:
                ...
                if Page.plotStyle["qPlot"]:
                    Plot.axvline(
                        2.0 * np.pi / G2lat.Pos2dsp(Parms, hkl[-2]),
                        color=clr,
                        dashes=dash,
                        lw=1.5,
                    )
                    ...
                    Plot.axvline(
                        G2lat.Pos2dsp(Parms, hkl[-2]), color=clr, dashes=dash, lw=1.5
                    )
                else:
            ...
            for hkl in G2frame.Extinct:  # plot extinct reflections
                ...
                if Page.plotStyle["qPlot"]:
                    ...
                    Plot.axvline(
                        G2lat.Pos2dsp(Parms, hkl[-2]), color=clr, dashes=(3, 3), lw=2
                    )
                else:
            ...
            for pId, phase in enumerate(Page.phaseList):
                ...
                pos = Page.plotStyle["refOffset"] - pId * Page.plotStyle[
                    "refDelt"
                ] * np.ones_like(peak)
                if Page.plotStyle["qPlot"]:
                ...
                if not Page.plotStyle.get("flTicks", False):  # short tick-marks
                    ...
                    if len(xtick) > 0:
                        ...
                        Page.tickDict[phase] = Plot.plot(
                            xtick[0], 0, "|", mew=0.5, ms=l, label=phase, color=plcolor
                        )[0]
            ...
            if handles:
                labels = dict(
                    zip(legends, handles, strict=False)
    ...
    if G2frame.Contour:
        ...
        if unequalArrays:
            if G2frame.Weight:
                ...
                Vmin = min(
                    [
                        ma.array(i, mask=m).min()
                        for i, m in zip(ContourZ, ExMask, strict=False)
                    ]
                )  # don't count excluded points in limits
                # Vmax = max([i.max() for i in ContourZ])
                Vmax = max(
                    [
                        ma.array(i, mask=m).max()
                        for i, m in zip(ContourZ, ExMask, strict=False)
                    ]
                )
            ...
            Plot.xaxis.set_label_coords(0.5, -0.07)
        else:
            if G2frame.Weight:
                Vmin = np.min(ContourZ)
                Vmax = np.max(ContourZ)
            Page.Img = Plot.imshow(
                ContourZ,
                cmap=acolor,
                vmin=Vmin,
                vmax=Vmax,
        ...
        if x is not None and len(ypList) > 1:
            ...
            for ph in ypList:
                ...
                if G2frame.SubBack:
                    ...
                    y = ypList[ph] + yb
                if Page.plotStyle["sqrtPlot"]:
                    y = np.where(y >= 0.0, np.sqrt(y), -np.sqrt(-y))
                Plot.plot(
                    x,
                    y,
                    pcolor,
    ...
    if not newPlot:
        ...
        if G2frame.Contour:  # for contour plots expand y-axis to include all histograms
            G2frame.xylim = (G2frame.xylim[0], (0.0, len(PlotList)))
        if "PWDR" in plottype:
...
def PublishRietveldPlot(G2frame, Pattern, Plot, Page, reuse=None):
    ...
    def Initialize():
        ...
        for key in "xmin", "xmax", "ymin", "ymax":
            if not plotOpt[key + "_use"]:
                plotOpt[key] = lims[key]
    ...
    def CopyRietveld2Grace(Pattern, Plot, Page, plotOpt, filename):
        ...
        def ClosestColorNumber(color):
            ...
            if not hasattr(mpcls, "to_rgba"):
                mpcls = mpcls.ColorConverter()
            return (
                np.sum(
                    (
                        [np.array(mpcls.to_rgb(c)) for c in colorlist]
                        - np.array(color[:3])
                    )
                    ** 2,
                    axis=1,
...
@{0} errorbar off\n"""
        ...
        xmar = (0.15, 1.2)
        ymar = (0.15, 0.9)
        top2bottom = 4.0  # 4 to 1 spacing for top to bottom boxes

        # scaling for top box
        fp.write("@g{0} hidden false\n@with g{0}\n@legend {1}\n".format(0, "on"))
        fp.write(f"@legend {xmar[1] - 0.2}, {ymar[1] - 0.05}\n")
        fp.write(
            f"@world xmin {Plot.get_xlim()[0]}\n@world xmax {Plot.get_xlim()[1]}\n"
        )
        fp.write(
            f"@world ymin {Plot.get_ylim()[0]}\n@world ymax {Plot.get_ylim()[1]}\n"
        )
        fp.write(f"@view xmin {xmar[0]}\n@view xmax {xmar[1]}\n")
        fp.write(
        ...
        fp.write(
            '@{0}axis label "{1}"\n@{0}axis label char size {2}\n'.format(
                "y", ylbl, float(plotOpt["labelSize"]) / 8.0
            )
        )
        fp.write(
            "@{0}axis label place spec\n@{0}axis label place {1}, {2}\n".format(
                "y", 0.0, 0.1
            )
        )
        # ======================================================================
        ...
        for _i, l in enumerate(Plot.lines):  # noqa: E741
            ...
            s = "@ line {0}, {1}, {0}, {2}\n"
            fp.write(
                s.format(l.get_data()[0][0], Plot.get_ylim()[0], Plot.get_ylim()[1])
            )
            s = "@ line linewidth 2\n@ line linestyle 2\n@ line color 1\n@ line arrow 0\n@line def\n"
            fp.write(s)
        for l in Plot.texts:  # noqa: E741
            ...
            fp.write(s.format(l.get_text(), float(plotOpt["labelSize"]) / 8.0))
        datnum = -1
        # ======================================================================
        # plot data
        for l in Plot.lines:  # noqa: E741
            ...
            glinetyp = 1
            if lbl == "obs":
                gsiz = float(plotOpt["markerSiz"]) / 8.0
            ...
            fp.write("@type xy\n")
            if lbl == "zero":
                fp.write(f"{Plot.get_xlim()[0]} {0}\n")
                fp.write(f"{Plot.get_xlim()[1]} {0}\n")
            elif not ma.any(l.get_xdata().mask):
                for x, y in zip(l.get_xdata(), l.get_ydata(), strict=False):
                    fp.write(f"{x} {y}\n")
            else:
                for x, y, m in zip(
                    l.get_xdata(), l.get_ydata(), l.get_xdata().mask, strict=False
                ):
                    if not m:
                        fp.write(f"{x} {y}\n")
            fp.write("&\n")
            fp.write(
                linedef.format(
                    "s" + str(datnum), glbl, gc, gsym, lineWid, gsiz, gmw, glinetyp
                )
            )
        # ======================================================================
        ...
        for l in Plot.lines:  # noqa: E741
            ...
            gc = ClosestColorNumber(c)
            siz = (
                float(plotOpt["tickSiz"])
                * (Plot.get_ylim()[1] - Plot.get_ylim()[0])
                / (100 * 6)
            )  # 1% for siz=6
            mkwid = float(plotOpt["tickWid"])
            if sum(c) == 4.0:
                continue  # white: ignore
            if not plotOpt["Show"].get(lbl, True):
                continue
            if plotOpt["legend"].get(lbl):
        ...
        fp.write(f"@world ymin {ysig.min()}\n@world ymax {ysig.max()}\n")
        fp.write(f"@view xmin {xmar[0]}\n@view xmax {xmar[1]}\n")
        fp.write(
            f"@view ymin {ymar[0]}\n@view ymax {(1.0 / top2bottom) * (ymar[1] - ymar[0]) + ymar[0]}\n"
        )
        ...
        fp.write(
            '@{0}axis label "{1}"\n@{0}axis label char size {2}\n'.format(
                "x", xlbl, float(plotOpt["labelSize"]) / 8.0
            )
        )
        fp.write(
            '@{0}axis label "{1}"\n@{0}axis label char size {2}\n'.format(
                "y", r"\f{Symbol}D/s", float(plotOpt["labelSize"]) / 8.0
            )
        )
        xticks = Plot.get_xaxis().get_majorticklocs()
        # come up with a "nice" tick interval for (o-c)/sig, since I am not sure
        # if this can be defaulted
        ytick = (ysig.max() - ysig.min()) / 5.0
        l10 = np.log10(ytick)
        if l10 < 0:
        ...
        for x, y, m in zip(savedX, ysig, savedX_mask, strict=False):
            if not m:
                fp.write(f"{x} {y}\n")
        fp.write("&\n")
...
{note2}"""
        ...
        if sum(
            Pattern[1][0].mask
        ):  # are there are excluded points? If so, add them too
        ...
        for i in range(1, 6):
            dest_wks.from_list(
                col=i - 1, data=valueList[i].tolist(), lname=colNamesList[i - 1]
            )
        ...
        for i in range(7, tickPosIdx):
            ...
            plot.color = 4 + j
            refLegendText = (
                refLegendText + "\\l(" + str(4 + k) + ") " + lblList[i] + " "
            )
        ...
        label.text = (
            "\\l(1) %(1)\\l(2) %(2)\\l(3) %(3)\\l(4) %(4) %(CRLF)" + refLegendText
        )
    ...
    def CopyRietveld2Igor(Pattern, Plot, Page, plotOpt, filename, G2frame):
        ...
        for _i, l in enumerate(Plot.lines):  # noqa: E741
            ...
            if lbl == "obs":
                ...
                gsiz = 5 * float(plotOpt["markerSiz"]) / 8.0
                marker = plotOpt["markerSym"]
                gsym = igor_symbols.get(marker, 12)
                gmw = float(plotOpt["markerWid"])
                markerSettings.append(
                    "mode({0})=3,marker({0})={1},msize({0})={2},mrkThick({0})={3}".format(
                        "Intensity", gsym, gsiz, gmw
                    )
                )
            elif lbl in ("calc", "bkg", "zero", "diff"):
                markerSettings.append(
                    "mode({0})=0, lsize({0})={1}".format(
                        InameDict[lbl], plotOpt["lineWid"]
                    )
                )
            else:
                continue
...
""")
        fp.write(f"X ModifyGraph gfSize={int(fontsize + 0.5)}\n")
...
""".format(
        ...
        for _i, l in enumerate(Plot.lines):  # noqa: E741
            ...
            if l in Page.tickDict.values():
                c = plotOpt["colors"].get(lbl, mpl.colors.to_rgba(l.get_color()))
                if sum(c) == 4.0:
                    continue  # white is invisible
                ticknum += 1
                phasename = f"tick{ticknum}"
                if plotOpt["legend"].get(lbl):
                    legends.append((phasename, plotOpt["phaseLabels"].get(lbl, lbl)))
                tickpos = l.get_ydata()[0]
                fp.write(
                    f"""X //  reflection tickmark for phase {lbl}
WAVES /D/O {phasename}X, {phasename}
BEGIN
"""
                )
                for x in l.get_xdata():
                    fp.write(f"{x} {tickpos}\n")
                fp.write(
                    """END
...
""".format(phasename, lbl, *[j * 65535 for j in c[0:3]])
        ...
        for _i, l in enumerate(Plot.lines):  # noqa: E741
            ...
            if "magline" not in lbl:
                continue
            j += 1
            fp.write(f"WAVES /D/O mag{j}X, mag{j}\nBEGIN\n")
            fp.write(
                " {0} {1}\n {0} {2}\n".format(
        ...
        for l in Plot.texts:  # noqa: E741
            ...
            if l.xycoords[0] == "data":
                ...
                xpos = Plot.get_xlim()[1] * 0.95
            fp.write(
                f"X SetDrawEnv xcoord=bottom, ycoord= abs,textyjust=2,fsize={fontsize}\n"
            )
            fp.write(f'X DrawText {xpos},2,"{l.get_text()}"\n')
        ...
        for nam, txt in legends:
            if s:
                s += r"\r"
            s += rf"\\s({nam}) {txt}"
        fp.write(f'X Legend/C/N=text0/J "{s}"\n')
        fp.close()
        print("file", filename, "written")
    ...
    def CopyRietveld2csv(Pattern, Plot, Page, filename):
        ...
        for i, l in enumerate(Plot.lines):  # noqa: B007, E741
            ...
            if "magline" in lbl:
                ...
                if sum(c) == 4.0:
                    continue  # skip over any "white" entries
                if lbl == "zero":
                    continue
        ...
        if hasattr(Pattern[1][0], "mask") and sum(
            Pattern[1][0].mask
        ):  # are there are excluded points? If so, add them too
            lblList.append("excluded")
            valueList.append(1 * Pattern[1][0].mask)
    ...
    def onSave(event):  # noqa: ARG001
        ...
        CopyRietveldPlot(
            G2frame, Pattern, Plot, Page, hcfigure, plotWidgets["phaseList"]
        )
    ...
    def OnLoadPlotOpt(event):  # noqa: ARG001
        ...
        fdlg = wx.FileDialog(
            dlg,
            "Choose saved plot options file",
            filroot,
            style=wx.FD_OPEN,
            wildcard="plot options file(*.pltopts)|*.pltopts",
        )
    ...
    if reuse:
        ...
        if plotOpt["initNeeded"]:
            Initialize()
        GetColors()
        dlg = wx.Dialog(
            G2frame.plotFrame,
            title="Publication plot creation",
    ...
    w = G2G.G2ChoiceButton(
        dlg, txtChoices, None, None, plotOpt, "labelSize", RefreshPlot, size=(50, -1)
    )
    ...
    w = G2G.G2ChoiceButton(
        dlg, symChoices, None, None, plotOpt, "markerSym", RefreshPlot, size=(40, -1)
    )
    sizebox.Add(w, 0, wx.ALL | wx.ALIGN_CENTER)
    sizebox.Add(wx.StaticText(dlg, wx.ID_ANY, " size"), 0, wx.ALL)
    w = G2G.G2ChoiceButton(
        dlg, sizChoices, None, None, plotOpt, "markerSiz", RefreshPlot, size=(50, -1)
    )
    sizebox.Add(w, 0, wx.ALL | wx.ALIGN_CENTER)
    sizebox.Add(wx.StaticText(dlg, wx.ID_ANY, " width"), 0, wx.ALL)
    w = G2G.G2ChoiceButton(
        dlg, lwidChoices, None, None, plotOpt, "markerWid", RefreshPlot, size=(50, -1)
    )
    sizebox.Add(w, 0, wx.ALL | wx.ALIGN_CENTER)
    sizebox.Add((1, 1), 1, wx.EXPAND, 1)
    sizebox.Add(wx.StaticText(dlg, wx.ID_ANY, " Line widths"), 0, wx.ALL)
    w = G2G.G2ChoiceButton(
        dlg, lwidChoices, None, None, plotOpt, "lineWid", RefreshPlot, size=(50, -1)
    )
    sizebox.Add(w, 0, wx.ALL | wx.ALIGN_CENTER)
    sizebox.Add((1, 1), 1, wx.EXPAND, 1)
    sizebox.Add(wx.StaticText(dlg, wx.ID_ANY, " Tick size"), 0, wx.ALL)
    w = G2G.G2ChoiceButton(
        dlg, sizChoices, None, None, plotOpt, "tickSiz", RefreshPlot, size=(50, -1)
    )
    sizebox.Add(w, 0, wx.ALL | wx.ALIGN_CENTER)
    sizebox.Add(wx.StaticText(dlg, wx.ID_ANY, " width"), 0, wx.ALL)
    w = G2G.G2ChoiceButton(
        dlg, lwidChoices, None, None, plotOpt, "tickWid", RefreshPlot, size=(50, -1)
    )
    sizebox.Add(w, 0, wx.ALL | wx.ALIGN_CENTER)
    sizebox.Add((1, 1), 1, wx.EXPAND, 1)
...
Note that the OriginPro connection export requires Origin 2021 or later."""
    ...
    for lbl in plotWidgets["phaseList"]:
        if lbl not in plotOpt["phaseLabels"]:
            plotOpt["phaseLabels"][lbl] = lbl
        val = G2G.ValidatedTxtCtrl(
            gpanel,
            plotOpt["phaseLabels"],
            lbl,
            size=(110, -1),
    ...
    vlbox.Add(
        wx.StaticText(dlg, wx.ID_ANY, "plot limit overrides"),
        0,
    ...
    hbox.Add(txt, 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL)
    val = G2G.ValidatedTxtCtrl(
        dlg, plotOpt, "width", xmin=3.0, xmax=20.0, nDig=(5, 1), size=(45, -1)
    )
    hbox.Add(val, 0, wx.ALL)
    txt = wx.StaticText(dlg, wx.ID_ANY, " Height (in):")
    hbox.Add(txt, 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL)
    val = G2G.ValidatedTxtCtrl(
        dlg, plotOpt, "height", xmin=3.0, xmax=20.0, nDig=(5, 1), size=(45, -1)
    )
    hbox.Add(val, 0, wx.ALL)
    ...
    pval = G2G.ValidatedTxtCtrl(dlg, plotOpt, "dpi", xmin=60, xmax=1600, size=(40, -1))
    ...
    CopyRietveldPlot(
        G2frame, Pattern, Plot, Page, figure, plotWidgets["phaseList"]
    )  # preview plot
    figure.canvas.draw()
...
def CopyRietveldPlot(G2frame, Pattern, Plot, Page, figure, phaseList):  # noqa: ARG001
    ...
    figure.subplots_adjust(
        ...
        bottom=int(plotOpt["labelSize"]) / 150.0,
        right=0.98,
        top=1.0 - int(plotOpt["labelSize"]) / 200.0,
        hspace=0.0,
    )
    ...
    if (
        mpl.__version__.split(".")[0] == "1"
    ):  # deal with older matplotlib, which puts too many ticks
    ...
    for key in (
        ...
        "ymin",
        "ymax",
    ):  # apply limit overrides where use flag is set
        if plotOpt[key + "_use"]:
            lims[key] = plotOpt[key]
    ...
    for _i, l in enumerate(Plot.lines):  # noqa: E741
        ...
        if "magline" in lbl:  # magnification lines
            ...
            if l.get_marker() != "None":
                continue  # ignore 2nd tickmark artist
            if not plotOpt["Show"].get(lbl, True):
                continue  # remove partial w/tickmarks
    ...
    for l in Plot.texts:  # noqa: E741
        ...
        if lbl[1:] in phaseList:  # reflection markers
            if not plotOpt["Show"].get(lbl[1:], True):
                continue  # remove marker w/tickmarks
            p = l.get_bbox_patch()
            props = {
...
def changePlotSettings(G2frame, Plot):
    ...
    w = G2G.ValidatedTxtCtrl(
        dlg, plotOpt, "title", OnLeave=RefreshPlot, size=(200, -1), notBlank=False
    )
    hbox.Add(w, 0, wx.ALL | wx.ALIGN_CENTER)
    vbox.Add(hbox, 0, wx.ALL | wx.EXPAND)
...
def uneqImgShow(figure, ax, Xlist, Ylist, cmap, vmin, vmax, Ylbls=None):
    ...
    def midPoints(x):
        ...
        return np.concatenate(
            [[1.5 * x[0] - x[1] / 2], (x[:-1] + x[1:]) / 2, [1.5 * x[-1] - x[-2] / 2]]
        )
    ...
    meshlist = []
    for i, (X, Y) in enumerate(zip(Xlist, Ylist, strict=False)):
...
def initPartialOpts(phaseColors):
    ...
    for p in phaseColors:
        ...
        partialOpts[p]["style"] = partialOpts[p].get(
            "style", 2
...
def configPartialDisplay(G2frame, phaseColors, RefreshPlot):
    ...
    mainSizer.Add(
        wx.StaticText(
            dlg,
            wx.ID_ANY,
            "Set options for display of Phase partials\n(profiles for each phase)",
    ...
    for p in phaseColors:
        ...
        ch = G2G.G2ChoiceButton(
            dlg, ltypeChoices, partialOpts[p], "style", None, None, StyleChange
        )
        gsizer.Add(ch, 0, wx.ALIGN_CENTER)
