...
def GetEdfData(filename, imageOnly=False):
    ...
    for line in lines:
        ...
        if line:
            ...
            break
    if any(pcent):
        cent[0] = pcent[0] * pixSize[0] / 1000.0
        cent[1] = pcent[1] * pixSize[1] / 1000.0
    File.seek(fileSize - imSize)
    if dType == "UnsignedShort":
        image = np.array(
            np.frombuffer(File.read(imSize), dtype=np.int16), dtype=np.int32
        )
    else:
        image = np.array(
            np.frombuffer(File.read(imSize), dtype=np.int32), dtype=np.int32
        )
