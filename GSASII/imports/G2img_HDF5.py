...
class HDF5_Reader(G2obj.ImportImage):
    ...
    def Reader(self, filename, ParentFrame=None, **kwarg):
        ...
        try:
            ...
            if not self.buffer.get("imagemap"):
                ...
                self.buffer["selectedImages"] = list(
                    range(len(self.buffer["imagemap"]))
                )
                if ParentFrame and len(self.buffer["imagemap"]) > nsel and nsel >= 0:
                    ...
                    dlg = G2G.G2MultiChoiceDialog(
                        ParentFrame, "Select images to read", "Choose images", choices
                    )
                    dlg.Layout()
                    dlg.SendSizeEvent()
                ...
                if len(self.buffer["selectedImages"]) == 0:
                    ...
                    fp.close()
                    return False
            self.buffer["selectedImages"] = self.buffer.get(
                "selectedImages", list(range(len(self.buffer["imagemap"])))
            )
            # get the first selected image
            while imagenum <= len(self.buffer["imagemap"]):
                if imagenum - 1 in self.buffer["selectedImages"]:
                    del self.buffer["selectedImages"][
                        self.buffer["selectedImages"].index(imagenum - 1)
                    ]
                    break
                imagenum += 1
            else:  # unexpected!
        ...
        if GSASIIpath.GetConfigValue("debug"):
            print(f"Read image #{imagenum} ({tag} section {sec}) from file {filename}")
        ...
        def func(name, dset):  # noqa: ARG001
            ...
            if isinstance(dset, h5py.Dataset):
                ...
                if len(dims) < 2:
                    ...
                    size = dims[2:]
                    self.buffer["imagemap"] += [
                        (dset.name, i, size) for i in range(dims[1])
                    ]
                elif len(dims) == 3:
                    size = dims[1:]
                    self.buffer["imagemap"] += [
                        (dset.name, i, size) for i in range(dims[0])
                    ]
                elif len(dims) == 2:
                    size = dims
                    ...
                    print(
                        "Skipping entry " + str(dset.name) + ". Shape is " + str(dims)
                    )
    ...
    def readDataset(self, fp, imagenum=1, name=None, num=None):
        ...
        if name is None:
            name, num, size = self.buffer["imagemap"][
                imagenum - 1
            ]  # look up image in map
            quick = False
        else:
            quick = True
        dset = fp[name]
        ...
        try:
            ...
            if "DetSizeX" in misc and "DetSizeY" in misc:
                pixelsize = [
                    misc[f"DetSize{i}"][j] * misc[f"DetPixelSize{i}"][j]
                    for i in ("X", "Y")
                ]
        ...
        data = {
            ...
            "wavelength": 0.15,
            "distance": 1000.0,
            "center": [sizexy[0] * 0.1, sizexy[1] * 0.1],
            "size": sizexy,
            "det2theta": 0.0,
        }
        for item in self.Comments:
            ...
            if "wavelength" in name and "spread" not in name:
                ...
                data["pixelSize"][0] = float(val) * 1000.0
            elif "y_pixel_size" in name:
                data["pixelSize"][1] = float(val) * 1000.0
