...
def ReportProblems(result, Rvals, varyList):
    ...
    for i, (v1, v2, corr) in enumerate(Hcorr):
        if corr > 0.95:
            ...
            stars = "   "
        m = f" {stars} {varyList[v1]} and {varyList[v2]} (@{100.0 * corr:.2f}%)"
        G2fil.G2Print(m, mode="warn")
        if i == 5:
            msg += "\n" + m
...
def AllPrmDerivs(
    ...
    origDiffs = G2stMth.errRefine(
        [], HistoPhases, parmDict, {}, [], calcControls, pawleyLookup, None
    )
    ...
    for i, prm in enumerate(origParms):
        ...
        if "UVmat" in nam:
            continue
        if hId != "*" and h not in ("", hId):
            continue
        if (
            type(parmDict[prm]) is bool
            or type(parmDict[prm]) is str
            or type(parmDict[prm]) is int
        ):
            continue
        if type(parmDict[prm]) is not float and type(parmDict[prm]) is not np.float64:
            ...
            continue
        if prm in latIgnoreLst:
            continue  # remove unvaried lattice params
        if re.match(r"\d:\d:D[012][012]", prm):
            continue  # don't need Dij terms
        if nam in ["Vol", "Gonio. radius"]:
            continue
        if nam.startswith("dA") and nam[2] in ["x", "y", "z"]:
            continue
        ...
        derivCalcs[prm] = (
            (chiLow - chiStart) / delta,
            0.5 * (chiLow - chiHigh) / delta,
            (chiStart - chiHigh) / delta,
        )
    ...
    while True:
        ...
        if np.any(np.isnan(values)):
            raise G2obj.G2Exception(
                "ERROR - nan found in LS parameters - use Calculate/View LS parms to locate"
            )
        ...
        if "Jacobian" in Controls["deriv type"]:
            ...
            if refPlotUpdate is not None:
                refPlotUpdate(Histograms)  # update plot after completion
        elif "analytic Hessian" in Controls["deriv type"]:
            Lamda = Controls.get("Marquardt", -3)
            maxCyc = Controls["max cyc"]
            result = G2mth.HessianLSQ(
                G2stMth.errRefine,
                values,
                Hess=G2stMth.HessRefine,
                ftol=Ftol,
                xtol=Xtol,
                maxcyc=maxCyc,
                Print=ifPrint,
            ...
            if "lastShifts" in result[2]:
                Rvals["lastShifts"] = dict(
                    zip(varyList, result[2]["lastShifts"], strict=False)
                )
            ...
            if refPlotUpdate is not None:
                refPlotUpdate(Histograms)  # update plot
        ...
        Rvals["DelChi2"] = result[2].get("DelChi2", -1.0)
        ...
        Rvals["RestraintSum"] = Histograms.get("RestraintSum", 0.0)
        Rvals["RestraintTerms"] = Histograms.get("RestraintTerms", 0)
        Rvals["Rwp"] = np.sqrt(Rvals["chisq"] / Histograms["sumwYo"]) * 100.0  # to %
        Rvals["GOF"] = np.sqrt(
            Rvals["chisq"]
            / (Histograms["Nobs"] + Rvals["RestraintTerms"] - len(varyList))
        )
        printFile.write(
            " Number of function calls: %d No. of observations: %d No. of parameters: %d User rejected: %d Sp. gp. extinct: %d\n"
            % (
                result[2]["nfev"],
                Histograms["Nobs"],
                len(varyList),
        ...
        if ncyc:
            printFile.write(
                " Refinement time = %8.3fs, %8.3fs/cycle, for %d cycles\n"
        ...
        if (
            "None" in str(type(result[1])) and ifSeq
        ):  # this bails out of a sequential refinement on singular matrix
            IfOK = False
            covMatrix = []
            ...
            if "Hessian" in Controls["deriv type"]:
                ...
                for i, val in enumerate(np.flipud(result[2]["psing"])):
                    if val:
                        G2fil.G2Print(
                            "Bad parameter: " + varyList[num - i], mode="warn"
                        )
            else:
                Ipvt = result[2]["ipvt"]
                for i, ipvt in enumerate(Ipvt):
                    if not np.sum(result[2]["fjac"], axis=1)[i]:
                        G2fil.G2Print(
                            "Bad parameter: " + varyList[ipvt - 1], mode="warn"
                        )
            break
        IfOK = True
        ...
        try:
            ...
            Lastshft = (
                result[0] - values
            )  # NOT last shift since values is starting set before current refinement
            ...
            if "Hessian" in Controls["deriv type"]:
                ...
                if SVD0 == -1:
                    ...
                    G2fil.G2Print(
                        "**** Refinement failed - other problem ****", mode="error"
                    )
                elif SVD0 > 0:
                    G2fil.G2Print(
                        f"**** Refinement failed with {SVD0} SVD singularities ****",
                        mode="error",
                    )
                else:
...
def Refine(
    ...
    (
        ...
        Phases, restraintDict, rbIds, pFile=printFile, symHold=symHold
    )
    ...
    hapVary, hapDict, controlDict = G2stIO.GetHistogramPhaseData(
        Phases, Histograms, Controls=calcControls, pFile=printFile
    )
    ...
    try:
        covData = {}
        IfOK, Rvals, result, covMatrix, sig, Lastshft = RefineCore(
            Controls,
            Histograms,
            Phases,
            restraintDict,
        ...
        if IfOK:
            if len(covMatrix):  # empty for zero cycle refinement
                ...
                G2stIO.PrintIndependentVars(
                    parmDict, varyList, sigDict, pFile=printFile
                )
                ...
                G2stIO.SetPhaseData(
                    parmDict, sigDict, Phases, rbIds, covData, restraintDict, printFile
                )
                G2stIO.SetISOmodes(parmDict, sigDict, Phases, printFile)
                G2stIO.SetHistogramPhaseData(
                    parmDict,
                    sigDict,
                    Phases,
                    Histograms,
                ...
                G2stIO.SetHistogramData(
                    parmDict, sigDict, Histograms, calcControls, pFile=printFile
                )
                # check for variables outside their allowed range, reset and freeze them
                frozen = dropOOBvars(
                    varyList, parmDict, sigDict, Controls, parmFrozenList
                )
                # covData['depSig'] = G2stIO.PhFrExtPOSig  # created in G2stIO.SetHistogramData, no longer used?
                covData["depSigDict"] = {
                    i: (parmDict[i], sigDict[i]) for i in parmDict if i in sigDict
                }
                if len(frozen):
                    if "msg" in Rvals:
                        Rvals["msg"] += "\n"
            ...
            G2stIO.SaveUsedHistogramsAndPhases(
                GPXfile,
                Histograms,
                Phases,
                rigidbodyDict,
    ...
    s = G2mv.VarRemapSumm()
    if s:
        Rvals["contrSumm"] = f"Constraints: {s}"
...
def DoNoFit(GPXfile, key):
    ...
    (
        ...
        MFtables,
        maxSSwave,
    ) = G2stIO.GetPhaseData(Phases, restraintDict, rbIds, Print=False)
    calcControls["atomIndx"] = atomIndx
    calcControls["Natoms"] = Natoms
    calcControls["FFtables"] = FFtables
    calcControls["EFtables"] = EFtables
    ...
    calcControls["maxSSwave"] = maxSSwave
    hapVary, hapDict, controlDict = G2stIO.GetHistogramPhaseData(
        Phases, Histograms, Controls=calcControls, Print=False
    )
    calcControls.update(controlDict)
    histVary, histDict, histDict1, controlDict = G2stIO.GetHistogramData(
...
def DoLeBail(GPXfile, dlg=None, cycles=10, refPlotUpdate=None, seqList=None):
    ...
    try:
        ...
        G2stIO.SaveUsedHistogramsAndPhases(
            GPXfile, Histograms, Phases, rigidbodyDict, covData, [], True
        )
...
def phaseCheck(phaseVary, Phases, histogram):
    ...
    NewVary = []
    for phase in Phases:
        if histogram not in Phases[phase]["Histograms"]:
            continue
...
def SeqRefine(GPXfile, dlg, refPlotUpdate=None):
    ...
    (
        ...
        Phases, restraintDict, rbIds, Print=False, pFile=printFile, seqHistName="All"
    )
    ...
    for ihst, histogram in enumerate(histNames):
        ...
        if histogram not in Histograms:
            ...
            msg = f"refining with invalid histogram {histogram}"
        ...
        hapVary, hapDict, controlDict = G2stIO.GetHistogramPhaseData(
            Phases, Histo, Controls=calcControls, Print=False
        )
        calcControls.update(controlDict)
        histVary, histDict, histDict1, controlDict = G2stIO.GetHistogramData(
            Histo, False
        ...
        if Controls["Copy2Next"]:  # update with parms from last histogram
            ...
            for phase in Phases:
                if (
                    Phases[phase]["Histograms"][histogram].get("LeBail", False)
                    and lasthist
                ):
                    oldFsqs = Histograms[lasthist]["Reflection Lists"][phase][
                        "RefList"
                    ].T[8:10]  # assume no superlattice!
                    newRefs = Histograms[histogram]["Reflection Lists"][phase][
                        "RefList"
                    ]
                    if len(newRefs) == len(oldFsqs.T):
                        ...
                        print(
                            f"ERROR - mismatch in reflection list length bewteen {lasthist} and {histogram}; no copy done"
                        )
        ...
        try:
            ...
            G2fil.G2Print(
                f" *** ERROR - your constraints are internally inconsistent for histogram {hId}***"
            )
        ...
        try:
            ...
            if not IfOK:
                G2fil.G2Print(
                    "***** Sequential refinement failed at histogram " + histogram,
                    mode="warn",
                )
                break
            sigDict = dict(zip(varyList, sig, strict=False))
            ...
            G2stMth.ApplyRBModels(parmDict, Phases, rigidbodyDict, True)
            SeqResult[histogram]["RBsuDict"] = G2stMth.computeRBsu(
                parmDict, Phases, rigidbodyDict, covMatrix, varyList, sig
            )
            ...
            G2stIO.SetHistogramData(
                parmDict, sigDict, Histo, None, ifPrint, printFile, seq=True
            )
            # check for variables outside their allowed range, reset and freeze them
            ...
            if len(frozen) > 0:
                ...
                for p in frozen:
                    if p not in varyList:
                        print(
                            f"Frozen Warning: {p} not in varyList. This should not happen!"
                        )
                        continue
                    i = varyList.index(p)
            ...
            SeqResult[histogram]["depParmDict"] = {
                i: (parmDict[i], sigDict[i]) for i in sigDict if i not in varyList
            }

            G2stIO.SaveUpdatedHistogramsAndPhases(
                GPXfile,
                Histo,
                Phases,
                rigidbodyDict,
    ...
    SeqResult["histNames"] = [
        itm
        for itm in G2stIO.GetHistogramNames(
            GPXfile,
            [
                "PWDR",
            ],
    ...
    if postFrozenCount:
        msgs["Frozen"] = (
            f"Ending refinement with {postFrozenCount} Frozen variables ({postFrozenCount - preFrozenCount} added now)\n"
        )
        printFile.write("\n" + msgs["Frozen"])
    printFile.close()
    G2fil.G2Print(
...
def RetDistAngle(DisAglCtls, DisAglData, dlg=None):
    ...
    if len(DisAglData.get("covData", {})):
        ...
        # RB additions (which may not be present)
    ...
    Radii = dict(
        zip(
            DisAglCtls["AtomTypes"],
            zip(DisAglCtls["BondRadii"], DisAglCtls["AngleRadii"], strict=False),
            strict=False,
        )
    )
    indices = (-2, -1, 0, 1, 2)
    ...
    for Oatom in targAtoms:
        AtomLabels[Oatom[0]] = Oatom[1]
    # are any atoms generated by a rigid body? If so prepare the parmDict copies
    RBuse = any(i[0] in RBlist for i in origAtoms + targAtoms)
    multiParmDict = False
    if RBuse and covData:
        multiParmDict, changedParmDict = G2mth.setupRBDistDerv(
            parmDict, varyList, varySig, rigidbodyDict, Phases
        )
    DistArray = {}
    AngArray = {}
    for iO, Oatom in enumerate(origAtoms):
        oRBdist = Oatom[0] in RBlist  # is atom in a RB?
        OxyzNames = []
        if covData:
            OxyzNames = [
                pfx + "dAx:%d" % (Oatom[0]),
        ...
        for Tatom in targAtoms:
            ...
            for [Txyz, Top, Tunit, _Spn] in G2spc.GenAtom(
                Tatom[3:6], SGData, False, Move=False
            ):
                Dx = (Txyz - np.array(Oatom[3:6])) + Units
                dx = np.inner(Amat, Dx)
                dist = ma.masked_less(np.sqrt(np.sum(dx**2, axis=0)), 0.5)
                IndB = ma.nonzero(ma.masked_greater(dist - BsumR, 0.0))
                if not np.any(IndB):
                    continue
                for indb in IndB:
                    for i in range(len(indb)):
                        if str(dx.T[indb][i]) in IndBlist:
                            continue
                        ...
                        if (oRBdist or tRBdist) and multiParmDict:
                            ...
                            if sig < 1e-9:
                                sig = 0.0  # very small values are roundoff
                            RBdist = True
                        elif len(Xvcov):
                            pdpx = G2mth.getDistDerv(
                                Oatom[3:6], Tatom[3:6], Amat, unit, Top, SGData
                            )
                        ...
                        Dist.append(
                            [
                                ...
                                tunit,
                                Top,
                                ma.getdata(dist[indb])[i],
                                sig,
                            ]
                        )
                        VectRB.append(RBdist)
                        if (Dist[-1][-2] - AsumR) <= 0.0:
                            VectA.append(
                                [
                                    OxyzNames,
                                    np.array(Oatom[3:6]),
        ...
        if dlg is not None:
            dlg.Update(iO, newmsg=f"Atoms done={iO} of {len(origAtoms)}")
    ...
    if len(SGtable) > 1:
        for i, item in enumerate(SGtable[::2]):
            if 2 * i + 1 == len(SGtable):
                line = f" {item.ljust(30)}"
            else:
                line = f" {item.ljust(30)} {SGtable[2 * i + 1].ljust(30)}"
            MyPrint(line)
    else:
        MyPrint(f" ( 1)    {SGtable[0]}")  # triclinic case
    Cell = DisAglData["Cell"]
...
def DisAglTor(DATData):
    ...
    if DATData["Natoms"] == 4:  # torsion
        ...
        G2fil.G2Print(
            " Torsion angle for {} atom sequence: {} = {}".format(
                DATData["Name"],
                str(atmSeq).replace("'", "")[1:-1],
                G2mth.ValEsd(Tors, sig),
            )
        )
        ...
        G2fil.G2Print(
            " Angle in {} for atom sequence: {} = {}".format(
                DATData["Name"],
                str(atmSeq).replace("'", "")[1:-1],
                G2mth.ValEsd(Ang, sig),
            )
        )
        G2fil.G2Print(" NB: Atom sequence determined by selection order")
        ...
        G2fil.G2Print(
            " Distance in {} for atom sequence: {} = {}".format(
                DATData["Name"],
                str(atmSeq).replace("'", "")[1:-1],
...
def BestPlane(PlaneData):
    ...
    for i, xyz in enumerate(XYZ):
        G2fil.G2Print(
            " %6s%10.3f%10.3f%10.3f" % (Atoms[i][1].ljust(6), xyz[0], xyz[1], xyz[2])
        )
    G2fil.G2Print(
        f"\n Best plane RMS X ={Evec[Order[2]]:8.3f}, Y ={Evec[Order[1]]:8.3f}, Z ={Evec[Order[0]]:8.3f}"
    )
