...
class ExportPhaseCSV(G2fil.ExportBaseclass):
    ...
    def Writer(self, hist, phasenam, mode="w"):
        ...
        if mode == "w":
            ...
            if aniso:
                lbllist += ["U11", "U22", "U33", "U12", "U13", "U23"]
    ...
    def Exporter(self, event=None):
        ...
        for phasenam in self.phasenam:
            ...
            self.Write(
                '\n"Space group:","'
                + str(phasedict["General"]["SGData"]["SpGrp"].strip())
                + '"'
            )
            ...
            for defsig, val in zip(
                3 * [-0.00001]
                + 3 * [-0.001]
                + [-0.01],  # sign values to use when no sigma
                cellList,
                strict=False,
            ):
                txt = G2mth.ValEsd(val, defsig)
...
class ExportMultiPowderCSV(G2fil.ExportBaseclass):
    ...
    def Exporter(self, event=None):
        ...
        self.filename = os.path.join(
            self.dirname, os.path.splitext(self.filename)[0] + self.extension
        )
            ...
            if phasDict.get("Super", False):
                ...
                for refItem in refList:
                    if "T" in phasDict["Type"]:
                        ...
                        g = gam / 100.0
                        s = np.sqrt(max(sig, 0.0001)) / 100.0
                        FWHM = G2pwd.getgamFW(g, s)
                        self.Write(
                            fmt.format(
                                h,
...
class ExportSingleCSV(G2fil.ExportBaseclass):
    ...
    def Writer(self, hist, filename=None):
        ...
        if hist.data["data"][1].get("Super", False):
            ...
            WriteList(
                self,
                ("h", "k", "l", "d-sp", "F_obs", "F_calc", "phase", "mult", "Icorr"),
            )
...
class ExportStrainCSV(G2fil.ExportBaseclass):
    ...
    def Exporter(self, event=None):
        ...
        WriteList(
            self,
            ("Dset", "Dcalc", "e11", "sig(e11)", "e12", "sig(e12)", "e22", "sig(e22)"),
        )
        fmt = 2 * "{:.5f}," + 6 * "{:.0f},"
        fmt1 = "{:.5f}"
        fmt2 = "{:.2f},{:.5f},{:.5f}"
        ...
        for item in StrSta["d-zero"]:
            WriteList(
                self, ("Azm", "dobs", "dcalc", "Dset=" + fmt1.format(item["Dset"]))
            )
