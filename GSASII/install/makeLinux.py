...
if __name__ == "__main__" and sys.platform.startswith("linux"):
    ...
    if "XDG_DATA_HOME" in os.environ and os.path.exists(
        ...
        mfile = os.path.join(
            os.environ.get("XDG_DATA_HOME"), "applications", "GSASII.desktop"
        )
    elif "HOME" in os.environ and os.path.exists(
        os.path.join(os.environ.get("HOME"), ".local/share/applications")
    ):
        mfile = os.path.join(
            os.environ.get("HOME"), ".local/share/applications", "GSASII.desktop"
        )
    ...
    os.chmod(
        G2script,  # make the .py file executable and readable
    ...
    G2start = os.path.abspath(
        os.path.normpath(os.path.join(path2repo, "..", "RunGSASII.sh"))
    )
    ...
    if os.path.exists(activate):
        fp.write(f"source {activate}\n")
    fp.write(f"{pythonexe} {G2script} $*\n")
    fp.close()
    os.chmod(
        G2start,  # make the .py file executable and readable
        stat.S_IXUSR | stat.S_IRUSR | stat.S_IRGRP | stat.S_IXGRP | stat.S_IXOTH,
    )
    ...
    for f, t in zip((dfile, mfile), ("Desktop", "Menu"), strict=False):
        ...
        try:
            with open(f, "w") as fp:
                fp.write(
                    desktop_template.format(terminal, G2start + add2script, G2icon)
                )
            os.chmod(
                dfile,
                stat.S_IWUSR
    ...
    if not os.path.exists:
        ...
        G2reset = os.path.normpath(
            os.path.join(path2repo, "..", "Reset2FreshGSASII.sh")
        )
        ...
        if os.path.exists(activate):
            fp.write(f"source {activate}\n")
        fp.write(
            'read -p "Reset any local changes and install latest GSAS-II version? (Y/[N]): " confirm\n[[ $confirm == [yY] || $confirm == [yY][eE][sS] ]] || exit 1\n'
        )
        ...
        print(
            f"Created {G2reset!r} to reset GSAS-II installation when all else fails..."
        )
