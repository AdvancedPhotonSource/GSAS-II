...
{G2G.GetCite("Bilbao: k-SUBGROUPSMAG", wrap=70, indent=5)}""")
    ...
    postdict = {
        ...
        "crystalsystem": "0",
        "landau": land,
        "eleccion": "subgrmag1_k",
        "inicio": "nostandard",
        "celtodas": celtodas,
        "limite": limite,
        "list": "Submit",
        "listado": "lista",
        "starmagnetica": starmag,
        "pointgroup": "0",
        "polarity": "0",
        "sub": "1",
        "super": "",
...
def parseBilbaoCheckLattice(page):
    ...
    for c in cellopts:
        cells = (
            c.split("pre")[1].split("<")[0].replace(">", "").split("\n")
...
def GetStdSGset(SGData=None, oprList=None):
    ...
    print(
        "Using Bilbao Crystallographic Server utility IDENTIFY GROUP. "
        + "Please cite:\n"
        + G2G.GetCite("Bilbao: k-SUBGROUPSMAG", wrap=70, indent=5)
    )
    ...
    xform = re.split(r"parclose\.png", re.split(r"paropen\.png", page)[1])[
        0
    ]  # pull out section w/Xform matrix
...
def GetSupergroup(SGnum, dlg=None):
    ...
    if dlg:
        dlg.Update(0, newmsg="Waiting for initial web response")
    ...
    if dlg:
        dlg.SetRange(2 + len(xforms))
    for i, line in enumerate(xforms):
        click = line[-1]
        print(SGnum, click)
        out1 = GSASIIpath.postURL(
            Site, {"gnum": SGnum, "show": "show", "click": click}, timeout=timeout
        )
        if not out1:
            return None
        ...
        #    fp.write(out1)
        if dlg:
            dlg.Update(2 + i, newmsg=f"processing {line[1]}, #{i + 1} of {len(xforms)}")
        mvlist = []
...
def BilbaoSymSearch1(
    sgnum, phase, maxdelta=2, angtol=None, pagelist=None, keepCell=False
):
...
{G2G.GetCite("Bilbao: PSEUDO", wrap=70, indent=5)}\n""")
    ...
    for i, atom in enumerate(phase["Atoms"], 1):
        ...
        postdict["stru"] += (
            f"{el:4s} {i} - {atom[cx]:.5f} {atom[cx + 1]:.5f} {atom[cx + 2]:.5f}\n"
        )
    #    if GSASIIpath.GetConfigValue('debug'): print(postdict["stru"])
    savedcookies = {}
    page0 = GSASIIpath.postURL(
        bilbaoSite + pseudosym, postdict, getcookie=savedcookies, timeout=timeout
    )
    if not page0:
        return None
...
def scanBilbaoSymSearch1(page0, postdict):
    ...
    if postdict["what"] == "pseudocell":
        form = (
            page0.split("Lattice Pseudosymmetry")[1]
            .split("<form")[1]
            .split("</form")[0]
        )
        ...
        valsdict["idcell"] = (
            form.split('name="idcell"')[1].split('value="')[1].split('"')[0]
        )
        ...
        for line in form.split("<tr")[2:]:
            ...
            row += (
                items[4].split("<pre>")[1].split("<")[0].split("\n")
...
def BilbaoLowSymSea1(valsdict, row, savedcookies, pagelist=None):
    ...
    if GSASIIpath.GetConfigValue("debug"):
        print(f"processing cell #{num}")
    postdict["lattice"] = num
    page1 = GSASIIpath.postURL(
        bilbaoSite + pseudosym, postdict, usecookie=savedcookies, timeout=timeout
    )
    if not page1:
        return None, None, None, None
    ...
    if pagelist is not None:
        pagelist[lbl] = page1
    if "Possible lattices:" not in page1:
        return 3 * [None]
    if "<form" not in page1:
        return 3 * [None]
...
def BilbaoLowSymSea2(num, valsdict, row, savedcookies, pagelist=None):
    ...
    if GSASIIpath.GetConfigValue("debug"):
        print(f"processing cell #{num} supergroup {row[1]}")
    page1 = GSASIIpath.postURL(
        bilbaoSite + pseudosym, postdict, usecookie=savedcookies, timeout=timeout
    )
    if page1 is None:
    ...
    if len(superdat) >= 2:
        return lbl, superdat[1].split("<pre>")[1].split("</pre")[0].replace(
            "\t", " "
        ).split("\n")
    return lbl, None


def BilbaoSymSearch2(
    valsdict, csdict, rowdict, savedcookies, pagelist=None, dlg=None, ophsnam="?"
):
    ...
    for i, num in enumerate(sorted(csdict)):
        ...
        if csdict[num]:
            ...
            postdict["cs"] = num
            page1 = GSASIIpath.postURL(
                bilbaoSite + pseudosym,
                postdict,
                usecookie=savedcookies,
                timeout=timeout,
            )
            if pagelist is not None:
                pagelist[num] = page1
            if page1 is None:
                ...
                if len(superdat) >= 2:
                    ...
                    structures[num] = (
                        f"coordinates inconsistent with symmetry {rowdict[num][0]}"
                    )
    return structures
...
def find2SearchAgain(pagelist, req="@"):
    ...
    for key, page in pagelist.items():
        if key == 0:
            continue
        if req and req not in key:
            continue
        if page is None:
            continue
        if "<form" not in page:
            continue
        if "Continue to search for pseudosymmetry" not in page:
            continue
        for f in page.split("<form")[1:]:
            if "Continue to search for pseudosymmetry" not in f:
                continue
            d = {}
...
{G2G.GetCite("Bilbao: PSEUDOLATTICE", wrap=70, indent=5)}""")
    ...
    structure = r0.text[
        r0.text.lower().find("<pre>") + 5 : r0.text.lower().find("</pre>")
    ].strip()
...
def test():
    ...
    results, baseList = GetNonStdSubgroupsmag(
        SGData, ("0", "0", "0", " ", " ", " ", " ", " ", " ", " ")
    )
    print(results)
    if results:
