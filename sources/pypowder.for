...
Cf2py depend(NPTS) PRFUNC
      ...
      INTEGER*4 NPTS
      REAL*4 TTHETA,ALP,BET,SIG,GAM,SPH
      REAL*4 DTT(0:NPTS-1),PRFUNC(0:NPTS-1),DPRDT,ALPART,BEPART,SIGPART
     1  GAMPART,SLPART,HLPART
      DO I=0,NPTS-1
        CALL PSVFCJEXPO(DTT(I)*100.,TTHETA*100.,ALP,BET,SIG,GAM,SPH/2.0,
     1    SPH/2.0,PRFUNC(I),DPRDT,ALPART,BEPART,SIGPART,GAMPART,
     1    SLPART,HLPART)
      END DO
      RETURN
      END
...
Cf2py depend(NPTS) SLPART
      ...
      INTEGER*4 NPTS
      REAL*4 TTHETA,ALP,BET,SIG,GAM,SHL,HPART
      REAL*4 DTT(0:NPTS-1),DPRDT(0:NPTS-1),ALPART(0:NPTS-1),
     1   BEPART(0:NPTS-1),SIGPART(0:NPTS-1),GAMPART(0:NPTS-1),
     1   SLPART(0:NPTS-1),PRFUNC(0:NPTS-1)
      DO I=0,NPTS-1
        CALL PSVFCJEXPO(DTT(I)*100.,TTHETA*100.,ALP,BET,SIG,GAM,SHL/2.,
     1     SHL/2.,PRFUNC(I),DPRDT(I),ALPART(I),BEPART(I),SIGPART(I),
     1     GAMPART(I),SPART,HPART)
        SLPART(I) = SPART
        DPRDT(I) = DPRDT(I)*100.
      END DO
      RETURN
