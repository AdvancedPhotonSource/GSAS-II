...
def SamAbs(data, tax, tay, muT):
    ...
    if "Cylind" in data["SampleShape"]:
        muR = (
            muT * (1.0 + npsind(tay) ** 2 / 2.0) / (npcosd(tax))
        )  # adjust for additional thickness off sample normal
...
def FitEllipse(xy):
    def ellipse_center(p):
        ...
        b, c, d, f, a = p[1] / 2.0, p[2], p[3] / 2.0, p[4] / 2.0, p[0]
        num = b * b - a * c
        x0 = (c * d - b * f) / num
        y0 = (a * f - b * d) / num
...
def FitDetector(rings, varyList, parmDict, Print=True, covar=False):
    ...
    def ellipseCalcD(B, xyd, varyList, parmDict):
        ...
        phi = parms["phi"] - 90.0  # get rotation of major axis from tilt axis
        tth = 2.0 * npasind(parms["wave"] / (2.0 * dsp))
        ...
        tbm = nptand((tth - parms["tilt"]) / 2.0)
        tbp = nptand((tth + parms["tilt"]) / 2.0)
        d = parms["dist"] + dxy
        ...
        R0 = np.sqrt((vplus + vminus) ** 2 - (fplus + fminus) ** 2) / 2.0  # +minor axis
        R1 = (vplus + vminus) / 2.0  # major axis
        zdis = (fplus - fminus) / 2.0
        Robs = np.sqrt((x - parms["det-X"]) ** 2 + (y - parms["det-Y"]) ** 2)
        rsqplus = R0**2 + R1**2
        rsqminus = R0**2 - R1**2
        R = rsqminus * npcosd(2.0 * phi0 - 2.0 * phi) + rsqplus
        Q = (
            np.sqrt(2.0)
            * R0
            * R1
            * np.sqrt(R - 2.0 * zdis**2 * npsind(phi0 - phi) ** 2)
        )
        P = 2.0 * R0**2 * zdis * npcosd(phi0 - phi)
        Rcalc = (P + Q) / R
        M = (Robs - Rcalc) * 25.0  # why 25? does make "chi**2" more reasonable
        return M
    ...
    result = leastsq(
        ellipseCalcD,
        p0,
        args=(rings.T, varyList, parmDict),
        full_output=True,
        ftol=1.0e-8,
    )
    chisq = np.sum(result[2]["fvec"] ** 2) / (
        rings.shape[0] - len(p0)
...
def FitMultiDist(rings, varyList, parmDict, Print=True, covar=False):
    ...
    def ellipseCalcD(B, xyd, varyList, parmDict):
        ...
        phi = parms["phi"] - 90.0  # get rotation of major axis from tilt axis
        tth = 2.0 * npasind(parms["wavelength"] / (2.0 * dsp))
        ...
        P = 2.0 * R0**2 * zdis * npcosd(phi0 - phi)
        Rcalc = (P + Q) / R
        return (Robs - Rcalc) * 25.0  # why 25? does make "chi**2" more reasonable
...
def ImageLocalMax(image, w, Xpix, Ypix):
    ...
    if (w2 < xpix < sizex - w2) and (w2 < ypix < sizey - w2) and image[ypix, xpix]:
        ...
        return (
            ...
            ypix,
            np.ravel(ZMax)[Zmax],
            max(0.0001, np.ravel(ZMin)[Zmin]),
        )  # avoid neg/zero minimum
    else:
        return 0, 0, 0, 0
...
def makeRing(dsp, ellipse, pix, reject, scalex, scaley, image, mul=1):
    ...
    def ellipseC():
        ...
        if radii[0] < 0:  # hyperbola
            theta = npacosd(1.0 / np.sqrt(1.0 + (radii[0] / radii[1]) ** 2))
            if debug:
        ...
        return (
            np.pi
            * apb
            * (1 + 3 * (amb / apb) ** 2 / (10 + np.sqrt(4 - 3 * (amb / apb) ** 2)))
        )

    cent, phi, radii = ellipse
    cphi = cosd(phi - 90.0)  # convert to major axis rotation
    sphi = sind(phi - 90.0)
    ring = []
    ...
    for i in range(0, C, 1):  # step around ring in 1mm increments
        a = 360.0 * i / C
        if radii[1] < 0:
            x = radii[1] * coshd(a - phi + 90.0)  # major axis
            y = radii[0] * sinhd(a - phi + 90.0)
        else:
            x = radii[1] * cosd(a - phi + 90.0)  # major axis
...
def GetEllipse(dsp, data):
    ...
    tth = 2.0 * asind(data["wavelength"] / (2.0 * dsp))
    dist = data["distance"]
...
def GetDetectorXY(dsp, azm, data):
    ...
    dxyz0 = np.inner(np.array([0.0, 0.0, 1.0]), MN)  # tilt detector normal
    dxyz0 += np.array([0.0, 0.0, dist])  # translate to distance
    dxyz0 = np.inner(dxyz0, makeMat(data["det2theta"], 1).T)  # rotate on 2-theta
    dxyz1 = np.inner(np.array([cent[0], cent[1], 0.0]), MN)  # tilt detector cent
    dxyz1 += np.array([0.0, 0.0, dist])  # translate to distance
    dxyz1 = np.inner(dxyz1, makeMat(data["det2theta"], 1).T)  # rotate on 2-theta
    xyz = LinePlaneCollision(dxyz0, dxyz1, vect, 2.0 * dist * vect)
    if xyz is None:
        return np.zeros(2)
    #        return None
    xyz = np.inner(xyz, makeMat(-data["det2theta"], 1).T)
    xyz -= np.array([0.0, 0.0, dist])  # translate back
...
def GetDetectorXY2(dsp, azm, data):
    ...
    if radii[0] > 0.0:
        ...
        R = rsqminus * cosd(2.0 * azm - 2.0 * phi) + rsqplus
        Q = (
            np.sqrt(2.0)
            * radii[0]
            * radii[1]
            * np.sqrt(R - 2.0 * zdis**2 * sind(azm - phi) ** 2)
        )
        P = 2.0 * radii[0] ** 2 * zdis * cosd(azm - phi)
        radius = (P + Q) / R
        ...
        xy = -np.array(
            [
                xy[0] * cosd(phi) + xy[1] * sind(phi),
                xy[0] * sind(phi) - xy[1] * cosd(phi),
            ]
        )
        xy += cent
    if data["det2theta"]:
...
def GetTthAzmG2(x, y, data):
    ...
    sinB2 = np.minimum(
        np.ones_like(tth), (data["distance"] * npsind(tth)) ** 2 / (dx**2 + dy**2)
    )
    # sinB2 = (data['distance']*npsind(tth))**2/(dx**2+dy**2)
    C = 180.0 - tth - npacosd(np.sqrt(1.0 - sinB2))
...
def GetLineScan(image, data):
    ...
    Ty *= (data["distance"] / 1000.0) ** 2  # do dist correction
    return [Tx, Ty]
...
def CalcRings(G2frame, ImageZ, data, masks):  # noqa: ARG001
    ...
    for iH, H in enumerate(HKL):
        ...
        if iH not in absent and iH >= skip:
            Ring = makeRing(
                dsp, ellipse, 0, -1.0, scalex, scaley, ma.array(ImageZ, mask=tam)
            )[0]
        else:
            Ring = makeRing(
                dsp, ellipse, 0, -1.0, scalex, scaley, ma.array(ImageZ, mask=tam)
...
def ImageCalibrate(G2frame, data):
    ...
    if varyDict["dist"] and varyDict["wave"]:
        G2fil.G2Print(
            "ERROR - you can not simultaneously calibrate distance and wavelength"
        )
        return False
    if len(ring) < 5:
    ...
    if varyDict["tilt"]:
        tilt = npasind(np.sqrt(max(0.0, 1.0 - (radii[0] / radii[1]) ** 2)) * ctth)
        if not tilt:
            G2fil.G2Print("WARNING - selected ring was fitted as a circle")
            G2fil.G2Print(
                " - if detector was tilted we suggest you skip this ring - WARNING"
            )
    else:
        tilt = data["tilt"]
    ...
    if varyDict["tilt"]:
        ...
        while fail:
            ...
            if len(Ringp) > 10:
                ...
                chip = 1e6
            ellipsem = GetEllipse2(tth, 0.0, dist, centm, -tilt, phi)
            G2fil.G2Print(
                fmt
                % (
                    "minus ellipse:",
    ...
    for i, H in enumerate(HKL):
        ...
        if debug:
            print(
                fmt
                % ("predicted ellipse:", elcent[0], elcent[1], phi, radii[0], radii[1])
            )
    ...
    if 2 * radii[1] < 0.9 * fullSize:
        G2fil.G2Print(
            "Are all usable rings (>25% visible) used? Try reducing Min ring I/Ib"
        )
    N = len(data["ellipses"])
    if N > 2:
...
def Make2ThetaAzimuthMap(data, iLim, jLim):  # most expensive part of integration!
    ...
    TA[3] = G2pwd.Polarization(data["PolaVal"][0], TA[0], TA[1] - 90.0)[0]
...
def polymask(data, Poly, Spots=[]):  # noqa: B006
    ...
    scalex = pixelSize[0] / 1000.0
    scaley = pixelSize[1] / 1000.0
    figure = mplfig.Figure(figsize=(Nx / 400.0, Ny / 400.0), dpi=400, facecolor=outmask)
...
def MakeMaskMap(data, masks, iLim, jLim):
    ...
    if len(masks["Pmask"]):
        tam = ma.mask_or(
            tam,
            ma.make_mask(
                masks["Pmask"][iLim[0] : iLim[1], jLim[0] : jLim[1]].flatten()
            ),
        )
    if tam.shape:
...
def Fill2ThetaAzimuthMap(masks, TAr, tam, image, ringMask=False):
    ...
    for tth, thick in masks["Rings"]:
        ...
        mask |= (tay > max(0.01, tth - thick / 2.0)) & (tay < (tth + thick / 2.0))
...
def MakeGainMap(image, Ix, Iy, data, mask, blkSize=128):
    ...
    Iy /= np.array(
        G2pwd.Polarization(data["PolaVal"][0], Ix[:-1], 0.0)[0]
    )  # undo polarization
    if data["Oblique"][1]:
    ...
    for iBlk in range(nYBlks):
        ...
        for jBlk in range(nXBlks):
            ...
            GainMap[iBeg:iFin, jBeg:jFin] = np.array(
                image[iBeg:iFin, jBeg:jFin] / (Ipix * TA[3])
            )
    GainMap /= np.nanmedian(GainMap)
    return 1.0 / GainMap, Mask
...
def AzimuthIntegrate(image, data, masks, ringId, blkSize=1024):
    ...
    for iBlk in range(nYBlks):
        ...
        for jBlk in range(nXBlks):
            ...
            TA = Make2ThetaAzimuthMap(
                data, (iBeg, iFin), (jBeg, jFin)
            )  # 2-theta & azimuth arrays & create position mask (none here)
            TA = np.dstack(
                (
                    ma.getdata(TA[1]),
                    ma.getdata(TA[0]),
            ...
            Block = image[
                iBeg:iFin, jBeg:jFin
            ]  # image Pixel mask has been applied here
            tax, tay, taz, tad = Fill2ThetaAzimuthMap(
                AMasks, TAr, tam, Block, ringMask=True
            )  # applies Ring masks only & returns contents
...
def ImageIntegrate(
    image, data, masks, blkSize=128, returnN=False, useTA=None, useMask=None
):
    ...
    if GSASIIpath.binaryPath:
        ...
        from . import histogram2d as h2d
    G2fil.G2Print(
        "Beginning image integration; image range: %d %d"
    ...
    if "2-theta" in data.get("binType", "2-theta"):
        ...
        lutth = np.log(4.0 * np.pi * npsind(LUtth / 2.0) / data["wavelength"])
    elif data["binType"].lower() == "q":
        lutth = 4.0 * np.pi * npsind(LUtth / 2.0) / data["wavelength"]
    dtth = (lutth[1] - lutth[0]) / numChans
    muT = data.get("SampleAbs", [0.0, ""])[0]
    if data["DetDepth"] > 0.5:  # patch - redefine DetDepth
        data["DetDepth"] /= data["distance"]
    if "SASD" in data["type"]:
        muT = -np.log(muT) / 2.0  # Transmission to 1/2 thickness muT
    Masks = copy.deepcopy(masks)
    ...
    for iBlk in range(nYBlks):
        ...
        for jBlk in range(nXBlks):
            ...
            tax, tay, taz, tad = Fill2ThetaAzimuthMap(
                Masks, TAr, tam, Block
            )  # applies remaining masks
            times[0] += time.time() - t0  # time mask application
            t0 = time.time()
            tax = np.where(
                tax > LRazm[1], tax - 360.0, tax
            )  # put azm inside limits if possible
            tax = np.where(tax < LRazm[0], tax + 360.0, tax)  # are these really needed?
            if data.get("SampleAbs", [0.0, ""])[1]:
                tabs = SamAbs(data, tax, tay, muT)
            else:
                tabs = np.ones_like(taz)
            if "log(q)" in data.get("binType", ""):
                tay = np.log(4.0 * np.pi * npsind(tay / 2.0) / data["wavelength"])
            elif data.get("binType", "").lower() == "q":
                tay = 4.0 * np.pi * npsind(tay / 2.0) / data["wavelength"]
    ...
    H0msk = [
        ma.array(h0 / nst, mask=np.logical_not(nst))
        for nst, h0 in zip(NST, H0, strict=False)
    ]
    ...
    for i, (h0msk, h2msk) in enumerate(zip(H0msk, H2msk, strict=False)):
        try:
            h0int = scint.interp1d(
                h2msk.compressed(), h0msk.compressed(), bounds_error=False
            )
            # do interpolation on all points - fills in the empty bins; leaves others the same
            h0 = h0int(H2[:-1])
            H0.append(h0)
    ...
    H0 = np.nan_to_num(np.array(H0))
    if "log(q)" in data.get("binType", ""):
        H2 = 2.0 * npasind(np.exp(H2) * data["wavelength"] / (4.0 * np.pi))
    elif data.get("binType", "").lower() == "q":
    ...
    if "SASD" not in data["type"]:
        H0 *= np.array(G2pwd.Polarization(data["PolaVal"][0], H2[:-1], 0.0)[0])
        # if np.abs(data['det2theta']) < 1.0:         #small angle approx only; not appropriate for detectors at large 2-theta
        H0 /= (
            np.abs(npcosd(H2[:-1] - np.abs(data["det2theta"]))) ** 4
        )  # parallax correction (why **4?)
    if "SASD" in data["type"]:
...
def MakeStrStaRing(ring, Image, Controls):
    ...
    scalex = 1000.0 / pixSize[0]
    scaley = 1000.0 / pixSize[1]
    Ring = np.array(
        makeRing(
            ring["Dset"],
            ellipse,
    ...
    ).T  # returns x,y,dsp for each point in ring
    if len(Ring):
        ring["ImxyObs"] = copy.copy(Ring[:2])
        TA = GetTthAzm(Ring[0], Ring[1], Controls)  # convert x,y to tth,azm
        TA[0] = Controls["wavelength"] / (2.0 * npsind(TA[0] / 2.0))  # convert 2th to d
        ring["ImtaObs"] = TA
...
def FitStrSta(Image, StrSta, Controls):
    ...
    for ring in StrSta["d-zero"]:  # get observed x,y,d points for the d-zeros
        ...
        if len(Ring):
            ...
            ringxy, ringazm = makeRing(
                ring["Dcalc"], ellipse, 0, 0.0, scalex, scaley, Image
            )
            ring["ImxyCalc"] = np.array(ringxy).T[:2]
            ringint = np.array(
                [
                    float(Image[int(x * scalex), int(y * scaley)])
                    for y, x in np.array(ringxy)[:, :2]
                ]
            )
...
def IntStrSta(Image, StrSta, Controls):
    ...
    for ring in StrSta["d-zero"]:  # get observed x,y,d points for the d-zeros
        ...
        if len(Ring):
            ellipse = FitEllipse(R["ImxyObs"].T)
            ringxy, ringazm = makeRing(
                ring["Dcalc"], ellipse, 0, 0.0, scalex, scaley, Image, 5
            )
...
def CalcStrSta(StrSta, Controls):
    ...
    for ring in StrSta["d-zero"]:
        ...
        th, azm = ring["ImtaObs"]
        th0 = np.ones_like(azm) * npasind(wave / (2.0 * ring["Dset"]))
        V = -np.sum(np.sum(E * calcFij(90.0, phi, azm, th0).T / 1.0e6, axis=2), axis=1)
        if StaType == "True":
            ring["ImtaCalc"] = np.array([np.exp(V) * ring["Dset"], azm])
        else:
            ring["ImtaCalc"] = np.array([(V + 1.0) * ring["Dset"], azm])
        dmin = np.min(ring["ImtaCalc"][0])
        dmax = np.max(ring["ImtaCalc"][0])
        if ring.get("fixDset", True):
            if abs(Eij[0]) < abs(Eij[2]):  # tension
                ring["Dcalc"] = dmin + (dmax - dmin) / 4.0
            else:  # compression
                ring["Dcalc"] = dmin + 3.0 * (dmax - dmin) / 4.0
        else:
...
def calcFij(omg, phi, azm, th):
    ...
    Fij = np.array([[d**2, d * e, c * d], [d * e, e**2, c * e], [c * d, c * e, c**2]])
    return -Fij
...
def FitStrain(rings, p0, dset, wave, phi, StaType):
    ...
    def strainCalc(p, xyd, dset, wave, phi, StaType):
        ...
        V = -np.sum(np.sum(E * calcFij(90.0, phi, azm, th).T / 1.0e6, axis=2), axis=1)
        if StaType == "True":
            dspc = dset * np.exp(V)
        else:
            dspc = dset * (V + 1.0)
    ...
    result = leastsq(
        strainCalc, p0, args=(rings, dset, wave, phi, StaType), full_output=True
    )
    vals = list(result[0])
    chisq = np.sum(result[2]["fvec"] ** 2) / (
        rings.shape[1] - 3
...
def FitImageSpots(Image, ImMax, ind, pixSize, nxy, spotSize=1.0):
    ...
    def calcMean(nxy, pixSize, img):  # noqa: ARG001
        ...
        gdy = ma.array(
            (gdy - nxy // 2) * pixSize[1] / 1000.0, mask=~ma.getmaskarray(ImBox)
        )
    ...
    def calcPeak(values, nxy, pixSize, img):
        ...
        nor = 1.0 / (2.0 * np.pi * sig**2)
        gdx, gdy = np.mgrid[0:nxy, 0:nxy]
        gdx = (gdx - nxy // 2) * pixSize[0] / 1000.0
        gdy = (gdy - nxy // 2) * pixSize[1] / 1000.0
        arg = (gdx - px) ** 2 + (gdy - py) ** 2
        peak += mag * nor * np.exp(-arg / (2.0 * sig**2))
    ...
    def calc2Peak(values, nxy, pixSize, img):
        ...
        peak = np.zeros([nxy, nxy]) + back
        nor = 1.0 / (2.0 * np.pi * sigx * sigy * np.sqrt(1.0 - rho**2))
        gdx, gdy = np.mgrid[0:nxy, 0:nxy]
        gdx = (gdx - nxy // 2) * pixSize[0] / 1000.0
        gdy = (gdy - nxy // 2) * pixSize[1] / 1000.0
        argnor = -1.0 / (2.0 * (1.0 - rho**2))
        arg = (
            (gdx - px) ** 2 / sigx**2
            + (gdy - py) ** 2 / sigy**2
            - 2.0 * rho * (gdx - px) * (gdy - py) / (sigx * sigy)
        )
    ...
    vals = [back, mag, 0.0, 0.0, 0.2, 0.2, 0.0]
    ImBox = ma.array(ImBox, dtype=float, mask=ImBox > 0.75 * ImMax)
    px = (ind[0] + 0.5) * pixSize[0] / 1000.0
    py = (ind[1] + 0.5) * pixSize[1] / 1000.0
    if ma.any(ma.getmaskarray(ImBox)):
        ...
        vals = result[0]
        ratio = vals[4] / vals[5]
        if 0.5 < ratio < 2.0 and vals[2] < 2.0 and vals[3] < 2.0:
            posx, posy = [px + vals[2], py + vals[3]]
            return [posx, posy, min(6.0 * vals[4], spotSize)]
        else:
            return [px, py, spotSize]
...
def FastAutoPixelMask(Image, Masks, Controls, numChans, dlg=None):
    ...
    print(
        " Fast mask: Spots greater or less than %.1f of median abs deviation are masked"
        % esdMul
    )
...
def AutoPixelMask(Image, Masks, Controls, numChans, dlg=None):
    ...
    for it in range(len(TThs)):
        ...
        if TThs[it] < Masks["SpotMask"].get("SearchMin", 0.0) or TThs[it] > Masks[
            "SpotMask"
        ].get("SearchMax", 180.0):
            mask |= masker
            continue
...
def DoPolaCalib(ImageZ, imageData, arcTth):
    ...
    data["LRazimuth"] = [0.0, 360.0]
    data["outChannels"] = 200
    data["outAzimuths"] = 1
    Arc = [arcTth, [85.0, 95.0], 2.0]
    print(
    ...
    time0 = time.time()
    res = minimize_scalar(func, bracket=[1.0, 0.999], tol=0.0001)
    print(res)
    pola = min(1.0, max(res.x, 0.0))

    Pola = []
    for arc in [75, 80, 85, 90, 95]:
        Arc = [arcTth, [arc, arc + 10.0], 2.0]
        AMasks = {
            "Points": [],
            "Rings": [],
            "Arcs": [
