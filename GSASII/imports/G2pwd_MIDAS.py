...
class MIDAS_Zarr_Reader(G2obj.ImportPowderData):
    ...
    def __init__(self):
        ...
        super(self.__class__, self).__init__(  # fancy way to self-reference
            extensionlist=(".zarr.zip",),
            strictExtension=True,
            formatName="MIDAS zarr",
            longFormatName="MIDAS zarr intergrated scans",
        )
        ...
        async def zarrAsyncOpen(filename):
            ...
            try:
                ...
                #                except FileNotFoundError as msg:
                    ...
                    self.errors = (
                        f"Exception from zarr module (version={zarr.__version__}):"
                    )
                    self.errors += "\n\t" + str(msg)
            return None, None
    ...
    def readMidas(self, filename, fpbuffer={}):
        ...
        if doread:  # read into buffer
            ...
            try:
                ...
                fpbuffer["unmasked"] = [
                    (fpbuffer["REtaMap"][3][:, i] != 0) for i in range(Nazim)
                ]  # will be True if area is >0
                ...
                fpbuffer["2Read"] = [
                    (i % Nazim, i // Nazim) for i in sel if i % Nazim in mAzm
                ]
                # xfrom Zarr dict into a native dict
                self.MIDASinstprm = {
                    i: fp["InstrumentParameters"][i][0]
                    for i in fp["InstrumentParameters"]
                }
                # change a few keys
        ...
        nFrame = fpbuffer["attributes"][iImg].get("Number Of Frames Summed", 1.0)
        ...
        omega = 999.0  # indicates an error
        try:
            omega = 0.5 * (
        ...
        self.pwdparms["Instrument Parameters"][0]["Azimuth"] = [
            90 - eta,
            90 - eta,
            False,
        ]
        ...
        self.numbanks = len(
            fpbuffer["2Read"]
        )  # number of remaining lineouts to be read
        ...
        self.idstring = (
            f"{os.path.split(filename)[1]} img={iImg} omega={omega} eta={eta}"
        )
        if GSASIIpath.GetConfigValue("debug"):
            print(
                f"Read entry #{iAzm} img# {iImg} from file {filename}\n{self.idstring}"
            )
        #        else:
