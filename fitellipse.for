C
      SUBROUTINE FITCRC(N,A,B,X,Y,KRANK,RNORM)
Cf2py intent(in) N
Cf2py intent(in) A
Cf2py depend(N) A
...
C
C     Daniel Pfenniger, Geneva Observatory, 4/1992
C
      PARAMETER (NPAR=3, TAU=1E-7)
C
      DIMENSION A(N,NPAR), B(N), X(N), Y(N)
C
...
C     See Lawson & Hanson (1974) for detail
      CALL HFTI(A,N,N,NPAR,B,N,1,TAU,KRANK,RNORM,H,G,IP)
C
      RETURN
      END
C
      SUBROUTINE FITQDR(N,A,B,X,Y,KRANK,RNORM)
Cf2py intent(in) N
Cf2py intent(in) A
Cf2py depend(N) A
Cf2py intent(in,out) B
