...
class testDeriv(wx.Frame):
    def _init_ctrls(self, parent):
        wx.Frame.__init__(
            ...
            name="testDeriv",
            parent=parent,
            size=wx.Size(800, 250),
            style=wx.DEFAULT_FRAME_STYLE,
            title="Test Jacobian Derivatives",
        )
    ...
    def TestRead(self):
        ...
        G2mv.GenerateConstraints(
            self.varylist, self.constrDict, self.fixedList, self.parmDict
        )
        print(G2mv.VarRemapShow(self.varylist))
        print("Dependent Vary List:", self.depVarList)
        G2mv.Map2Dict(
            self.parmDict, copy.copy(self.varylist)
    ...
    def UpdateControls(self, event):
        ...
        self.timingVal = wx.CheckBox(
            self.testDerivPanel, label="Show Execution Profiling"
        )
        ...
        self.testDerivPanel.SetScrollbars(
    ...
    def OnMakePlots(self, event):
        ...
        def test2(name, delt, doProfile):
            ...
            if name in self.varylist:
                ...
                self.parmDict[name] = self.values[ind] = self.values[ind] + 2.0 * delt
                G2mv.Dict2Map(self.parmDict)
                M1 = G2stMth.errRefine(
                    self.values,
                    self.HistoPhases,
                    self.parmDict,
                    {},
