...
def GetSelectedRows(widget, lbl="edit", G2frame=None):
    ...
    try:
        ...
        choices = [
            f"{widget.GetRowLabelValue(i)}: {widget.GetCellValue(i, 0)}"
            for i in range(widget.GetNumberRows())
        ]
        try:
            dlg = G2G.G2MultiChoiceDialog(
                G2frame, "Restraints to " + lbl, "Select restraints", choices
            )
...
def UpdateRestraints(G2frame, data, phaseName):
    ...
    def getMacroFile(macName):
        ...
        dlg = wx.FileDialog(
            G2frame,
            message="Choose " + macName + " restraint macro file",
            defaultDir=defDir,
            defaultFile="",
    ...
    def getMOGULFile():
        ...
        dlg = wx.FileDialog(
            G2frame,
            message="Choose MOGUL csv file",
            defaultDir=".",
            defaultFile="",
            wildcard="MOGUL csv file (*.csv)|*.csv",
            style=wx.FD_OPEN | wx.FD_CHANGE_DIR,
        )
        try:
            mogul = ""
            if dlg.ShowModal() == wx.ID_OK:
                csvfile = dlg.GetPath()
                mogul = open(csvfile)  # noqa: SIM115
                head = mogul.readline()
                if "Type" not in head:
                    print("Note: header line is\n", head)
                    print(
                        "**** ERROR - file selected is not a MOGUL csv file, try again ****"
                    )
                    mogul = []
                else:
    ...
    def OnPlotAARestraint(event):  # noqa: ARG001
        ...
        if "Torsion" in G2frame.restrBook.GetPageText(page):
            ...
            dlg = wx.SingleChoiceDialog(
                G2frame, "Select", "Ramachandran data", ramaNames
            )
            try:
                if dlg.ShowModal() == wx.ID_OK:
    ...
    def OnAddRestraint(event):  # noqa: ARG001
        ...
        if "Bond" in G2frame.restrBook.GetPageText(page):
            ...
            dlg = G2exG.ExpressionDialog(
                ...
                fit=False,
                wildCard=G2frame.testSeqRefineMode(),
            )
    ...
    def OnAddAARestraint(event):  # noqa: ARG001
        page = G2frame.restrBook.GetSelection()
        if "Bond" in G2frame.restrBook.GetPageText(page):
    ...
    def OnUseMogul(event):  # noqa: ARG001
        page = G2frame.restrBook.GetSelection()
        if "Bond" in G2frame.restrBook.GetPageText(page):
    ...
    def AddBondRestraint(bondRestData):
        ...
        for listName in ["origin", "target"]:
            dlg = G2G.G2MultiChoiceDialog(
                G2frame,
                "Bond restraint " + listName + " for " + General["Name"],
                "Select bond restraint " + listName + " atoms",
                Names,
            )
            if dlg.ShowModal() == wx.ID_OK:
                ...
                for x in sel:
                    if "all" in Names[x]:
                        allType = Types[x]
                        for name, Type, coords, Id in zip(
                            Names, Types, Coords, Ids, strict=False
                        ):
                            if Type == allType and "all" not in name:
                                Lists[listName].append([Id, Type, coords])
                    else:
                        Lists[listName].append(
                            [
                                Ids[x],
                                Types[x],
                                Coords[x],
        ...
        if len(Lists["origin"]) and len(Lists["target"]):
            ...
            dlg = G2G.SingleFloatDialog(
                G2frame,
                "Distance",
                "Enter restraint distance for bond",
                bond,
                [0.01, 4.0],
                "%.4f",
            )
        ...
        dlg = wx.ProgressDialog(
            "Generating bond restraints",
            "Processed origin atoms",
            len(Lists["origin"]),
    ...
    def AddAABondRestraint(bondRestData):
        ...
        while macStr:
            ...
            if "F" in items[0]:
                ...
                for i, [oId, oCoord, odis] in enumerate(
                    zip(oIds, oCoords, oDis, strict=False)
                ):
                    for tId, tCoord, tdis in list(
                        zip(tIds, tCoords, tDis, strict=False)
                    )[i:]:
            ...
            macStr = macro.readline()
        macro.close()
        print(f" Found {len(bondRestData['Bonds'])} bond restraints")
        UpdateBondRestr(bondRestData)
    ...
    def AddAngleRestraint(angleRestData):
        Radii = dict(
            zip(
                General["AtomTypes"],
                zip(General["BondRadii"], General["AngleRadii"], strict=False),
                strict=False,
            )
        )
        ...
        for listName in ["A-atom", "B-atom"]:
            dlg = G2G.G2MultiChoiceDialog(
                G2frame,
                "Select " + listName + " for angle A-B-C for " + General["Name"],
                "Select angle restraint " + listName,
                Names,
            )
            if dlg.ShowModal() == wx.ID_OK:
                sel = dlg.GetSelections()
                ...
                return
            targAtoms = [
                [Ids[x + iBeg], Types[x + iBeg], Coords[x + iBeg]]
                for x in range(len(Names[iBeg:]))
            ]
        if len(Lists["B-atom"]):
            value = 109.54
            dlg = G2G.SingleFloatDialog(
                G2frame, "Angle", "Enter restraint angle ", value, [30.0, 180.0], "%.2f"
            )
            if dlg.ShowModal() == wx.ID_OK:
                value = dlg.GetValue()
            dlg.Destroy()
    ...
    def AddAAAngleRestraint(angleRestData):
        ...
        while macStr:
                    ...
                    for residue in residues:
                        for ires in [0, 1]:
                            if Res != "*":  # works with disordered res
                                ...
                                try:
                                    for res, name, Id in Chains[chain][residue][ires]:  # noqa: B007
                                        try:
                                            ipos = Atms.index(name[:3].strip())
                                            ids[ipos] = Id
                                        except ValueError:
                                            continue
                                    for res, name, Id in Chains[chain][residue + 1][  # noqa: B007
                                        ires
                                    ]:
                                        try:
            ...
            macStr = macro.readline()
        macro.close()
        print(f" Found {len(angleRestData['Angles'])} angle restraints")
        UpdateAngleRestr(angleRestData)
    ...
    def AddPlaneRestraint(restrData):
        ...
        dlg = G2G.G2MultiChoiceDialog(
            G2frame,
            "Select 4 or more atoms for plane in " + General["Name"],
            "Select 4+ atoms",
            Names[iBeg:],
        )
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if len(sel) > 3:
                ...
                print(
                    "**** ERROR - not enough atoms for a plane restraint - try again ****"
                )
    ...
    def AddAAPlaneRestraint(planeRestData):
        ...
        while macStr:
            ...
            macStr = macro.readline()
        macro.close()
        print(f" Found {len(planeRestData['Planes'])} plane restraints")
    ...
    def AddAAChiralRestraint(chiralRestData):
        ...
        while macStr:
            ...
            macStr = macro.readline()
        macro.close()
        print(f" Found {len(chiralRestData['Volumes'])} chiral volumes restraints")
    ...
    def AddAATorsionRestraint(torsionRestData):
        ...
        while macStr:
            ...
            macStr = macro.readline()
        macro.close()
        print(f" Found {len(torsionRestData['Torsions'])} torsion restraints")
    ...
    def AddAARamaRestraint(ramaRestData):
        ...
        while macStr:
            ...
            if "F" in items[0]:
                ...
                for chain in chains:
                    ...
                    if not (any(mAtms) or any(pAtms)):
                        ...
                        for ires, residue in enumerate(residues[1:-1]):  # noqa: B007
                            for jres in [0, 1]:
                                try:
                                    for res, name, Id in Chains[chain][residue - 1][  # noqa: B007
                                        jres
            ...
            macStr = macro.readline()
        macro.close()
        print(f" Found {len(ramaRestData['Ramas'])} Ramachandran restraints")
    ...
    def AddChemCompRestraint(chemcompRestData):
        ...
        dlg = G2G.G2MultiChoiceDialog(
            G2frame,
            "Select atoms for chemical restraint in " + General["Name"],
            "Select atoms",
            Names,
        )
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if len(ids) > 0:
                ...
                print(
                    "**** ERROR - not enough atoms for a composition restraint - try again ****"
                )
    ...
    def AddMomentRestraint(momentRestData):
        ...
        dlg = G2G.G2MultiChoiceDialog(
            G2frame,
            "Select 2 or more atoms for average moment restraint in " + General["Name"],
            "Select 2+ atoms",
            Names[iBeg:],
        )
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if len(sel) > 1:
                ...
                print(
                    "**** ERROR - not enough atoms for a average moment restraint - try again ****"
                )
        UpdateMomentRestr(momentRestData)

    def AddTextureRestraint(textureRestData):
        dlg = wx.TextEntryDialog(
            G2frame, "Enter h k l for pole figure restraint", "Enter HKL", ""
        )
        if dlg.ShowModal() == wx.ID_OK:
            text = dlg.GetValue()
            vals = text.split()
    ...
    def WtBox(wind, restData):
        if "Range" not in restData:
            restData["Range"] = 1.1  # patch
        ...
        wtBox.Add(
            wx.StaticText(
                wind, -1, "Phase " + phaseName + " Restraint weight factor: "
            ),
            0,
            WACV,
        )
        wtfactor = G2G.ValidatedTxtCtrl(
            wind, restData, "wtFactor", nDig=(10, 2), typeHint=float
        )
        ...
        if "Bonds" in restData or "Angles" in restData:
            ...
            sRange = G2G.ValidatedTxtCtrl(
                wind, restData, "Range", nDig=(10, 2), typeHint=float
            )
    ...
    def UpdateBondRestr(bondRestData):
        ...
        def OnChangeValue(event):  # noqa: ARG001
        ...
        def OnChangeEsd(event):  # noqa: ARG001
            rows = GetSelectedRows(Bonds)
            if not rows:
                return
            Bonds.ClearSelection()
            ...
            dlg = G2G.SingleFloatDialog(
                G2frame, "New value", "Enter new esd for bond", val, [0.0, 1.0], "%.4f"
            )
            if dlg.ShowModal() == wx.ID_OK:
                parm = dlg.GetValue()
                for r in rows:
        ...
        if bondList:
            ...
            if "macro" in General["Type"]:
                ...
                for i, [indx, ops, obs, esd] in enumerate(bondList):  # noqa: B007
                    try:
                        atoms = G2mth.GetAtomItemsById(Atoms, AtLookUp, indx, 0, 4)
                        name = ""
                        for atom in atoms:
            ...
            if bondList:
                ...
                for i in (
                    G2G.wxID_RESTDELETE,
                    G2G.wxID_RESRCHANGEVAL,
                    G2G.wxID_RESTCHANGEESD,
                ):
                ...
                mainSizer.Add(
                    wx.StaticText(
                        BondRestr, -1, "No bond distance restraints for this phase"
                    ),
                    0,
                )
        else:
    ...
    def UpdateAngleRestr(angleRestData):
        ...
        def OnChangeEsd(event):  # noqa: ARG001
            ...
            dlg = G2G.SingleFloatDialog(
                G2frame, "New value", "Enter new esd for angle", val, [0.0, 5.0], "%.2f"
            )
            if dlg.ShowModal() == wx.ID_OK:
                parm = dlg.GetValue()
                for r in rows:
        ...
        if len(angleList):
            ...
            if "macro" in General["Type"]:
                ...
                for i, [indx, ops, obs, esd] in enumerate(angleList):  # noqa: B007
                    try:
                        atoms = G2mth.GetAtomItemsById(Atoms, AtLookUp, indx, 0, 4)
                        name = ""
                        for atom in atoms:
            ...
            if len(angleList):
                ...
                mainSizer.Add(
                    wx.StaticText(
                        AngleRestr, -1, "No bond angle restraints for this phase"
                    ),
                    0,
                )
        else:
    ...
    def UpdatePlaneRestr(planeRestData):
        ...
        def OnChangeEsd(event):  # noqa: ARG001
            ...
            dlg = G2G.SingleFloatDialog(
                G2frame, "New value", "Enter new esd for plane", val, [0.0, 5.0], "%.2f"
            )
            if dlg.ShowModal() == wx.ID_OK:
                parm = dlg.GetValue()
                for r in rows:
        ...
        if len(planeList):
            ...
            if "macro" in General["Type"]:
                ...
                for i, [indx, ops, obs, esd] in enumerate(planeList):  # noqa: B007
                    try:
                        atoms = G2mth.GetAtomItemsById(Atoms, AtLookUp, indx, 0, 4)
                        name = ""
            ...
            if len(planeList):
                ...
                mainSizer.Add(
                    wx.StaticText(PlaneRestr, -1, "No plane restraints for this phase"),
                    0,
                )
        else:
            mainSizer.Add(
    ...
    def UpdateChiralRestr(chiralRestData):
        ...
        def OnChangeValue(event):  # noqa: ARG001
            ...
            dlg = G2G.SingleFloatDialog(
                G2frame,
                "New value",
                "Enter new value for chiral volume",
                val,
                [0.0, 360.0],
                "%.2f",
            )
        ...
        def OnChangeEsd(event):  # noqa: ARG001
            ...
            dlg = G2G.SingleFloatDialog(
                G2frame,
                "New value",
                "Enter new esd for chiral volume",
                val,
                [0.0, 5.0],
                "%.2f",
            )
        ...
        if len(volumeList):
            ...
            if "macro" in General["Type"]:
                colLabels = [
                    "(res) O (res) A (res) B (res) C",
                    "calc",
                    "target",
                    "esd",
                    "delt/sig",
                ]
                for i, [indx, ops, obs, esd] in enumerate(volumeList):  # noqa: B007
                    try:
                        atoms = G2mth.GetAtomItemsById(Atoms, AtLookUp, indx, 0, 4)
                        name = ""
                        for atom in atoms:
                ...
                colLabels = [
                    "O+SymOp  A+SymOp  B+SymOp  C+SymOp)",
                    "calc",
                    "target",
                    "esd",
                    "delt/sig",
            ...
            if len(volumeList):
                ...
                mainSizer.Add(
                    wx.StaticText(
                        ChiralRestr, -1, "No chiral volume restraints for this phase"
                    ),
                    0,
                )
        else:
    ...
    def UpdateTorsionRestr(torsionRestData):
        ...
        def OnChangeEsd(event):  # noqa: ARG001
            ...
            dlg = G2G.SingleFloatDialog(
                G2frame,
                "New value",
                "Enter new esd for torsion restraints",
                val,
                [0.0, 5.0],
                "%.2f",
            )
        ...
        if len(coeffDict):
            mainSizer.Add((5, 5))
            mainSizer.Add(
                wx.StaticText(TorsionRestr, -1, "Torsion function coefficients:"), 0
            )
        ...
        if len(torsionList):
            ...
            if "macro" in General["Type"]:
                ...
                for i, [indx, _ops, cofName, esd] in enumerate(torsionList):
                    try:
                        atoms = G2mth.GetAtomItemsById(Atoms, AtLookUp, indx, 0, 4)
                        name = (
                            "(" + atoms[2][1] + atoms[2][0].strip() + atoms[2][2] + ")"
                        )
                        for atom in atoms:
                            name += "  " + atom[3]
                        XYZ = np.array(
            ...
            mainSizer.Add(
                wx.StaticText(TorsionRestr, -1, "No torsion restraints for this phase"),
                0,
            )
    ...
    def UpdateRamaRestr(ramaRestData):
        ...
        if len(coeffDict):
            mainSizer.Add(
                wx.StaticText(RamaRestr, -1, "Ramachandran function coefficients:"), 0
            )
            mainSizer.Add(coeffSizer(), 1, wx.EXPAND, 1)
        ...
        if len(ramaList):
            ...
            if "macro" in General["Type"]:
                ...
                for i, [indx, _ops, cofName, esd] in enumerate(ramaList):
                    try:
                        atoms = G2mth.GetAtomItemsById(Atoms, AtLookUp, indx, 0, 4)
                        name = (
                            "(" + atoms[3][1] + atoms[3][0].strip() + atoms[3][2] + ")"
                        )
                        for atom in atoms:
                            name += "  " + atom[3]
                        XYZ = np.array(
            ...
            mainSizer.Add(
                wx.StaticText(
                    RamaRestr, -1, "No Ramachandran restraints for this phase"
                ),
                0,
            )
    ...
    def UpdateChemcompRestr(chemcompRestData):
        ...
        mainSizer.Add(
            wx.StaticText(
                ChemCompRestr,
                -1,
                "NB: The chemical restraint sum is over the unit cell contents",
            ),
            0,
        )
        mainSizer.Add((5, 5), 0)
        ...
        if len(chemcompList):
            ...
            for i, [indx, factors, obs, esd] in enumerate(chemcompList):
                try:
                    ...
                    chisq += (
                        chemcompRestData["wtFactor"]
                        * ((obs - np.sum(calcs)) / esd) ** 2
                    )
            ...
            if len(chemcompList):
                ...
                for r in range(chemcompTable.GetNumberRows()):
                    ...
                    if "Restr" in ChemComps.GetRowLabelValue(r):
                        ...
                        ChemComps.SetCellTextColour(
                            r, 1, VERY_YELLOW
                        )  # make spurious #'s disappear
                ...
                G2frame.dataWindow.RestraintEdit.Enable(
                    id=G2G.wxID_RESTDELETE, enable=True
                )
                G2frame.Bind(wx.EVT_MENU, OnDeleteRestraint, id=G2G.wxID_RESTDELETE)
                # G2frame.Bind(wx.EVT_MENU, OnChangeValue, id=G2G.wxID_RESRCHANGEVAL)
                frac = "?"
                ...
                mainSizer.Add(
                    wx.StaticText(
                        ChemCompRestr,
                        -1,
                        "No chemical composition restraints for this phase",
                    ),
                    0,
                )
        else:
    ...
    def UpdateMomentRestr(momentRestData):
        ...
        try:
            if MomentRestr.GetSizer():
                MomentRestr.GetSizer().Clear(True)
        except:  # noqa: E722
            pass
        mainSizer = wx.BoxSizer(wx.VERTICAL)
        mainSizer.Add((5, 5), 0)
        ...
        if len(momentList):
            ...
            for i, [indx, obs, esd] in enumerate(momentList):
                try:
                    ...
                    atoms = G2mth.GetAtomItemsById(Atoms, AtLookUp, indx, ct - 1)
                    Mom = np.array(
                        G2mth.GetAtomItemsById(Atoms, AtLookUp, indx, cx + 4, 3)
                    )
                    line = []
                    for a, atom in enumerate(atoms):
                    ...
                    obs = sum / len(atoms)  # noqa: PLW2901
                    line += [obs, esd]
                    for a, atom in enumerate(atoms):  # noqa: B007
            ...
            mainSizer.Add(
                wx.StaticText(
                    MomentRestr, -1, "No magnetic moment restraints for this phase"
                ),
                0,
            )
    ...
    def UpdateTextureRestr(textureRestData):
        ...
        mainSizer.Add(
            wx.StaticText(
                ...
                '    "unit esd" gives a bias toward a flatter polefigure',
            ),
            0,
        )
        mainSizer.Add((5, 5), 0)
        ...
        if len(textureList):
            ...
            for i, [hkl, grid, esd1, ifesd2, esd2] in enumerate(textureList):
                table.append(
                    ["%d %d %d" % (hkl[0], hkl[1], hkl[2]), grid, esd1, ifesd2, esd2]
                )
                rowLabels.append(str(i))
            textureTable = G2G.Table(
                table, rowLabels=rowLabels, colLabels=colLabels, types=Types
            )
            ...
            mainSizer.Add(
                wx.StaticText(TextureRestr, -1, "No texture restraints for this phase"),
                0,
            )
    ...
    def UpdateGeneralRestr(generalRestData):
        ...
        try:
            if GeneralRestr.GetSizer():
                GeneralRestr.GetSizer().Clear(True)
        except:  # noqa: E722
            pass
        mainSizer = wx.BoxSizer(wx.VERTICAL)
        mainSizer.Add((5, 5), 0)
        ...
        hSizer.Add(
            G2G.ValidatedTxtCtrl(
                GeneralRestr, generalRestData, "wtFactor", nDig=(10, 1), typeHint=float
            )
        )
        ...
        if generalRestData["General"]:
            ...
            for i, rest in enumerate(generalRestData["General"]):
                ...
                GridSiz.Add(
                    G2G.ValidatedTxtCtrl(
                        GeneralRestr, rest, 1, nDig=(10, 3, "g"), typeHint=float
                    )
                )
                ...
                GridSiz.Add(
                    G2G.ValidatedTxtCtrl(
                        GeneralRestr, rest, 2, nDig=(10, 3, "g"), typeHint=float
                    )
                )
                ...
                GridSiz.Add(btn)
                txt = ""
                for i in eq.assgnVars:  # noqa: PLW2901
                    if txt:
                        txt += "; "
    ...
    def OnPageChanged(event):
        ...
        G2frame.dataWindow.RestraintEdit.SetLabel(
            G2G.wxID_RESRCHANGEVAL, "Change target value"
        )
        ...
        if text == "Bond":
            ...
            G2frame.dataWindow.RestraintEdit.Enable(G2G.wxID_RESTRAINTADD, True)
            G2frame.dataWindow.RestraintEdit.SetLabel(
                G2G.wxID_RESRCHANGEVAL, "Change factor"
            )
            G2frame.dataWindow.RestraintEdit.Enable(G2G.wxID_RESRCHANGEVAL, True)
    ...
    Rvals = {}
    if "Rvals" in covdata:
        Nvars = len(covdata["varyList"])  # noqa: F841
        Rvals = covdata["Rvals"]
    ...
    try:
        ...
        mainSizer.Add(wx.StaticText(G2frame.dataWindow, -1, " Phase does not exist"), 0)
        return
    ...
    if "macro" in General["Type"]:
        Names = [
            atom[0] + ":" + atom[1] + atom[2] + " " + atom[3].ljust(4) for atom in Atoms
        ]
        Ids = []
        Coords = []
        Types = []
