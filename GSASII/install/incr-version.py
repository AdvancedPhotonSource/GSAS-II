...
if __name__ == "__main__":
    ...
    if tags0:
        print(
            f"Latest commit ({commit.hexsha[:7]}) is already tagged ({', '.join(tags0)})."
        )
        sys.exit()
    ...
    if len(prev) == 1:
        ...
        if tagsm1:
            print(
                f"Previous commit ({prev[0].hexsha[:7]}) is already tagged ({', '.join(tagsm1)})."
            )
            sys.exit()
    ...
    if versiontag in releases:
        print(
            f"Versioning problem, generated next version {versiontag} already defined!"
        )
        versiontag = "?"
        sys.exit()
    if versiontag != "?":
