...
[
    ...
    wxID_FPRIMEKALPHAGAKA,
    wxID_FPRIMEKALPHAINKA,
    ...
    def UpDateFPlot(self, Wave, rePlot:bool = True):
        ...
        if self.FPPS:
            for i, Fpps in enumerate(self.FPPS):
                ...
                self.ax.plot(
                    fppsP1, fppsP3, Color, linestyle="dashed", label=Fpps[0] + ' f "'
                )
        if self.ifWave:
...
if __name__ == "__main__":
    ...
    frm.plotFrame = wx.Frame(
        frm, -1, "GSASII Plots", size=size, style=wx.DEFAULT_FRAME_STYLE ^ wx.CLOSE_BOX
    )
    frm.plotFrame.Show()
