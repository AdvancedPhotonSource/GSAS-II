...
def test_refine():
    ...
    h1.set_refinements(
        {"Limits": [16.0, 110]}
    )  # decreasing range speeds this up by ~40%
    h2.set_refinements({"Limits": [19.0, 120.0]})
    ...
    gpx.refine()
    testR("add Mustrain, Shift, Displace[XY], atomic X & Uiso", 13.407161, 6.360408)
    #
    ...
    npt.assert_allclose(
        (POdict["C(2,0)"], POdict["C(2,2)"]), [0.127404, 0.09406], rtol=0.001
    )
