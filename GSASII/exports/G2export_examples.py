...
class ExportPhaseText(G2fil.ExportBaseclass):
    ...
    def Exporter(self, event=None):
        ...
        if self.ExportSelect():
            return  # set export parameters; prompt for file name
        self.OpenFile()
        ...
        for phasenam in self.phasenam:
            ...
            self.Write("\nAtoms\n" + 80 * "-")
            self.Write(
                fmt.format("label", "elem", "mult", "x", "y", "z", "frac", "Uiso")
            )
            self.Write(80 * "-")
            aniso = False
            ...
            if aniso:
                ...
                self.Write(
                    fmt.format(
                        "label", "elem", "U11", "U22", "U33", "U12", "U13", "U23"
                    )
                )
                self.Write(80 * "-")
                for lbl, typ, mult, xyz, td in AtomsList:
...
class ExportSingleText(G2fil.ExportBaseclass):
    ...
    def Writer(self, hist, filename=None):
        ...
        self.Write(
            hfmt.format(
                "h,k,l", "d-space", "F_obs", "sig(Fobs)", "F_calc", "phase", "twin"
            )
        )
        self.Write(80 * "=")
        for (
            ...
            if twin > 0:
                self.Write(
                    fmt.format(
                        hklfmt.format(h, k, l), dsp, Fobs, sigFobs, Fcalc, phase, twin
                    )
                )
        self.CloseFile()
