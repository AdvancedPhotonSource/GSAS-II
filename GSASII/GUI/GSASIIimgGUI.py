...
def GetImageZ(G2frame, data, newRange=False):
    ...
    sumImg = np.array(
        G2fil.GetImageData(
            G2frame, imagefile, True, ImageTag=imagetag, FormatName=formatName
        ),
        dtype=np.float32,
    )
    if sumImg is None:
    ...
    if "dark image" in data:
        ...
        if darkImg:
            ...
            if Did:
                Ddata = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, Did, "Image Controls")
                )
    ...
    if "background image" in data:
        ...
        if backImg:  # ignores any transmission effect in the background image
            ...
            if Bid:
                ...
                Bdata = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, Bid, "Image Controls")
                )
                ...
                backImage = np.array(
                    G2fil.GetImageData(
                        ...
                        True,
                        ImageTag=imagetag,
                        FormatName=bformatName,
                    ),
                    dtype=np.float32,
                )
                if darkImg and backImage is not None:
                    backImage += np.array(
                        darkImage * darkScale / backScale, dtype=np.float32
                    )
    ...
    if "Gain map" in data:
        ...
        if gainMap:
            ...
            if GMid:
                ...
                Gdata = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, GMid, "Image Controls")
                )
                gformat = Gdata["formatName"]
                GMimage = G2fil.GetImageData(
                    G2frame, gainfile, True, ImageTag=imagetag, FormatName=gformat
                )
    ...
    return np.array(
        np.array(np.rint(sumImg), dtype=int), dtype=np.int32
    )  # double-cast removes warning. Why?


def UpdateImageData(G2frame, data):
    def OnPixVal(invalid, value, tc):  # noqa: ARG001
        G2plt.PlotExposedImage(G2frame, newPlot=True, event=tc.event)

    def OnPolaCalib(event):  # noqa: ARG001
        if data["IOtth"][1] < 34.0:
            G2G.G2MessageBox(
                G2frame,
                "Maximum 2-theta not greater than 34 deg",
                "Polarization Calibration Error",
            )
            return
        IOtth = [32.0, data["IOtth"][1] - 2.0]
 ...
 Enter 2-theta position for arc mask (32-{IOtth[1]:.1f}) """,
    ...
    mainSizer.Add(
        wx.StaticText(
            G2frame.dataWindow,
            label=" Image size: %d by %d" % (data["size"][0], data["size"][1]),
        ),
        0,
    )
    ...
    distSizer.Add(
        wx.StaticText(G2frame.dataWindow, label=" Set detector distance: "), 0, WACV
    )
    if "setdist" not in data:
        data["setdist"] = data["distance"]
    distSizer.Add(
        G2G.ValidatedTxtCtrl(
            G2frame.dataWindow, data, "setdist", nDig=(10, 4), typeHint=float
        ),
        0,
        WACV,
    )
    ...
    tthSizer.Add(
        wx.StaticText(G2frame.dataWindow, label=" Detector 2-theta: "), 0, WACV
    )
    tthSizer.Add(
        G2G.ValidatedTxtCtrl(
            G2frame.dataWindow, data, "det2theta", xmin=-180.0, xmax=180.0, nDig=(10, 2)
        ),
        0,
        WACV,
    )
    tthSizer.Add(
        wx.StaticText(
            G2frame.dataWindow,
            label=f" Sample changer position {data['samplechangerpos']:.2f} mm ",
        ),
        0,
        WACV,
    )
...
def UpdateImageControls(
    G2frame, data, masks, useTA=None, useMask=None, IntegrateOnly=False
):
    ...
    def OnCalibrate(event):  # noqa: ARG001
        if not data["calibrant"]:
            G2G.G2MessageBox(
                G2frame,
                "No calibrant material specified.\n"
                + "Please correct this and try again.",
            )
            return
        G2frame.GetStatusBar().SetStatusText(
            "Select > 4 points on 1st used ring; LB to pick (shift key to force pick), RB on point to delete else RB to finish",
            1,
        )
        G2frame.ifGetRing = True

    def OnRecalibrate(event):  # noqa: ARG001
    ...
    def OnRecalibAll(event):  # noqa: ARG001
        """Use existing calibration values as starting point for a calibration
        ...
        dlg = G2G.G2MultiChoiceDialog(
            ...
            "Select images to recalibrate:",
            Names,
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
                Id = G2gd.GetGPXtreeItemId(
                    G2frame, G2frame.root, "Sequential image calibration results"
                )
                if Id:
                    SeqResult = G2frame.GPXtree.GetItemPyData(Id)
                else:
                    Id = G2frame.GPXtree.AppendItem(
                        parent=G2frame.root, text="Sequential image calibration results"
                    )
            ...
            dlg.Destroy()
        print(
            "All selected images recalibrated - results in Sequential image calibration results"
        )
    ...
    def OnMultiGainMap(event):  # noqa: ARG001
        Id = None
        Names = G2gd.GetGPXtreeDataNames(
            G2frame,
            [
        ...
        First = True
        dlg = G2G.G2MultiChoiceDialog(
            G2frame, "Image calibration controls", "Select images for gain map:", Names
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
                items = dlg.GetSelections()
                ...
                for item in items:
                    ...
                    G2frame.Image = G2gd.GetGPXtreeItemId(G2frame, G2frame.root, name)
                    Npix, imagefile, imagetag = G2IO.GetCheckImageFile(
                        G2frame, G2frame.Image
                    )
                    pth = os.path.split(os.path.abspath(imagefile))[0]
                    ImDat = copy.deepcopy(
                    ...
                    GainMap, Mask = G2img.MakeGainMap(
                        sumImg, Ix, Iy, ImDat, ImMsk, blkSize
                    )
                    if ImDat["invert_x"]:
                        Mask = np.flip(Mask, 1)
                    if ImDat["invert_y"]:
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            ImDat.update(
                {
                    ...
                    "calibrant": " ",
                    "center": [
                        500.0 * ImSize[0] / pixSize[0],
                        500.0 * ImSize[1] / pixSize[1],
                    ],
                }
            )
            ImMsk = {
                ...
                "Polygons": [],
                "Frames": [],
                "Thresholds": [(500.0, 2000.0), [800.0, 1200.0]],
            ...
            if not Id:
                ...
                G2frame.GPXtree.SetItemPyData(
                    G2frame.GPXtree.AppendItem(Id, text="Image Controls"), ImDat
                )
                G2frame.GPXtree.SetItemPyData(
                    G2frame.GPXtree.AppendItem(Id, text="Masks"), ImMsk
                )
            else:
                G2frame.GPXtree.SetItemPyData(Id, [Npix, newimagefile])
                G2frame.GPXtree.SetItemPyData(
                ...
                G2frame.GPXtree.SetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, Id, "Image Controls"), ImDat
                )
                G2frame.GPXtree.SetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, Id, "Masks"), ImMsk
                )
            G2frame.GPXtree.SelectItem(Id)
        else:
        ...
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                for item in items:
                    ...
                    Masks = G2frame.GPXtree.GetItemPyData(Mid)
                    result = G2img.ImageRecalibrate(
                        G2frame, G2frame.ImageZ, Data, Masks, getRingsOnly=True
                    )
                    if not len(result):
                        print("calibrant missing from local image calibrants files")
                        return
                    ...
                    obsArr = np.concatenate(
                        (
                            obsArr,
                            np.concatenate(
                                (rings[:, 0:2], distarr, rings[:, 2:3]), axis=1
                            ),
                        ),
                        axis=0,
                    )
                ...
                G2frame.GPXtree.SelectItem(
                    G2frame.root
                )  # there is probably a better way to force the reload of the current page
    ...
    def ResetThresholds():
        Imin = max(0.0, np.min(G2frame.ImageZ))
    ...
    def OnIntegrate(event, useTA=None, useMask=None):
        ...
        G2frame.Integrate = G2img.ImageIntegrate(
            sumImg, data, masks, blkSize, useTA=useTA, useMask=useMask
        )
        ...
        G2frame.GPXtree.SelectItem(Id)
        G2frame.GPXtree.Expand(Id)
        for item in G2frame.MakePDF:
            item.Enable(True)

    def OnIntegrateAll(event):  # noqa: ARG001
        Names = G2gd.GetGPXtreeDataNames(
            G2frame,
            [
                "IMG ",
        ...
        dlg = G2G.G2MultiChoiceDialog(
            G2frame, "Image integration controls", "Select images to integrate:", Names
        )
        try:
            if dlg.ShowModal() != wx.ID_OK:
                return
            ...
            try:
                ...
                for icnt, item in enumerate(items):
                    ...
                    name = Names[item]
                    G2frame.Image = G2gd.GetGPXtreeItemId(G2frame, G2frame.root, name)
                    CId = G2gd.GetGPXtreeItemId(
                        G2frame, G2frame.Image, "Image Controls"
                    )
                    Data = G2frame.GPXtree.GetItemPyData(CId)
                    same = True
                    for item in [  # noqa: PLW2901
                        "tilt",
                        "distance",
                        "rotation",
                        "DetDepth",
                        ...
                        if Data[item] != oldData[item]:
                            same = False
                    if (
                        Data["center"][0] != oldData["center"][0]
                        or Data["center"][1] != oldData["center"][1]
                    ):
                        same = False
                    if not same:
                        t0 = time.time()
                        ...
                        print(
                            " Use new image controls; new xy -> th,azm time %.3f"
                            % (time.time() - t0)
                        )
                    ...
                    if Mhash != oldMhash:
                        ...
                        print(f" Use new mask; make mask time: {time.time() - t0:.3f}")
                        oldMhash = Mhash
                    else:
                    ...
                    G2frame.Integrate = G2img.ImageIntegrate(
                        image, Data, Masks, blkSize, useTA=useTA, useMask=useMask
                    )
    ...
    def OnCopyControls(event):  # noqa: ARG001
        Names = G2gd.GetGPXtreeDataNames(
            G2frame,
            [
                "IMG ",
        ...
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                for item in items:  # preserve some values
                    ...
                    Data["GonioAngles"] = oldData.get("GonioAngles", [0.0, 0.0, 0.0])
                    ...
                    G2frame.GPXtree.SetItemPyData(
                        G2gd.GetGPXtreeItemId(G2frame, Id, "Image Controls"), Data
                    )
        finally:
            dlg.Destroy()
            if G2frame.PickId:
    ...
    def OnCopySelected(event):  # noqa: ARG001
        Names = G2gd.GetGPXtreeDataNames(
            G2frame,
            [
                "IMG ",
    ...
    def OnSaveControls(event):  # noqa: ARG001
        pth = G2G.GetExportPath(G2frame)
        dlg = wx.FileDialog(
            G2frame,
            "Choose image controls file",
            pth,
            "",
            "image control files (*.imctrl)|*.imctrl",
            wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT,
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
    ...
    def OnSaveMultiControls(event):  # noqa: ARG001
        ...
        for img in treeEntries:
            ...
            data = G2frame.GPXtree.GetItemPyData(
                G2gd.GetGPXtreeItemId(G2frame, item, "Image Controls")
            )
            Npix, imagefile, imagetag = G2frame.GPXtree.GetImageLoc(item)
            filename = os.path.join(
                outdir, os.path.splitext(os.path.split(imagefile)[1])[0] + ".imctrl"
            )
    ...
    def OnLoadMultiControls(
        event,  # noqa: ARG001
    ):  # TODO: how read in multiple image controls & match them by 'twoth' tag?
        ...
        dlg = G2G.G2MultiChoiceDialog(
            G2frame, "Select images", "Select images for updating controls:", Names
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                for image in images:
                    ...
                    imctrls = G2frame.GPXtree.GetItemPyData(
                        G2gd.GetGPXtreeItemId(G2frame, Id, "Image Controls")
                    )
    ...
    def OnTransferAngles(event):  # noqa: ARG001
        ...
        if len(Names) == 1:
            G2frame.ErrorDialog(
                "No images to transfer integration angles to", 'Need more "IMG"s'
            )
            return
        Source = G2frame.GPXtree.GetItemText(G2frame.Image)
        Names.pop(Names.index(Source))
        # select targets & do copy
        extraopts = {
            "label_1": "Xfer scaled calib d-min",
            "value_1": False,
            "label_2": "Xfer scaled 2-theta min",
            "value_2": False,
            "label_3": "Xfer scaled 2-theta max",
            "value_3": True,
            "label_4": "Xfer fixed background  ",
            "value_4": False,
        }
        dlg = G2G.G2MultiChoiceDialog(
            G2frame,
            "Xfer angles",
            "Transfer integration range from " + Source + " to:",
            Names,
            extraOpts=extraopts,
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                print(
                    f"distance = {dist0:.2f} integration range: [{ttmin0:.4f}, {ttmax0:.4f}], calib dmin {dsp0:.3f}"
                )
                for item in items:
                    name = Names[item]
                    Id = G2gd.GetGPXtreeItemId(G2frame, G2frame.root, name)
                    ...
                    if extraopts["value_1"]:
                        ang1 = xferAng(2.0 * asind(wave0 / (2.0 * dsp0)), dist0, dist1)
                        data["calibdmin"] = data["wavelength"] / (
                            2.0 * sind(ang1 / 2.0)
                        )
                        print(
                            "distance = {:.2f} integration range: [{:.4f}, {:.4f}], calib dmin {:.3f}".format(
                                dist1,
                                data["IOtth"][0],
                                data["IOtth"][1],
                                data["calibdmin"],
                    ...
                    if extraopts["value_4"]:
                        ...
                        print(
                            "distance = {:.2f} integration range: [{:.4f}, {:.4f}]".format(
                                dist1, data["IOtth"][0], data["IOtth"][1]
                            )
                        )
        finally:
            dlg.Destroy()
    ...
    def OnResetDist(event):  # noqa: ARG001
        dlg = wx.MessageDialog(
            G2frame,
            "Are you sure you want to do this?",
            caption="Reset dist to set dist",
        ...
        dlg = G2G.G2MultiChoiceDialog(
            G2frame, "Reset dist", "Reset dist to set dist for:", Names
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
                items = dlg.GetSelections()
    ...
    def ComboSizer():
        def OnDataType(event):  # noqa: ARG001
            data["type"] = typeSel.GetValue()[:4]
            if "SASD" in data["type"]:
                data["SampleAbs"][0] = np.exp(
                    -data["SampleAbs"][0]
                )  # switch from muT to trans!
                if data["binType"] == "2-theta":
                    data["binType"] = "log(q)"  # switch default bin type
            elif "PWDR" in data["type"]:
                data["SampleAbs"][0] = -np.log(
                    data["SampleAbs"][0]
                )  # switch from trans to muT!
                if data["binType"] == "log(q)":
                    data["binType"] = "2-theta"  # switch default bin type
            wx.CallLater(100, UpdateImageControls, G2frame, data, masks)
        ...
        def OnAzmthOff(invalid, value, tc):  # noqa: ARG001
            wx.CallAfter(G2plt.PlotExposedImage, G2frame, event=tc.event)

        comboSizer = wx.BoxSizer(wx.HORIZONTAL)
        comboSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" Type of image data: "),
            0,
            WACV,
        )
        typeSel = wx.ComboBox(
            parent=G2frame.dataWindow,
            value=typeDict[data["type"]],
            choices=typeList,
        ...
        comboSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" Color bar "), 0, WACV
        )
        colSel = wx.ComboBox(
            parent=G2frame.dataWindow,
            value=data["color"],
        ...
        comboSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" Azimuth offset "), 0, WACV
        )
        azmthOff = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            data,
            "azmthOff",
            nDig=(10, 2),
    ...
    def MaxSizer():
        ...
        def OnNewVal(invalid, value, tc):  # noqa: ARG001
            ...
            r11 = min(
                max(Range[1][1], Range[1][0] + 1), Range[0][1]
            ...
            sqrtDeltZero = math.sqrt(
                max(1.0, Range[0][1] - max(0.0, Range[1][0]) - 1)
            )  # sqrt(Imax0-Imin-1)
            sqrtDeltOne = math.sqrt(
                max(1.0, Range[1][1] - max(0.0, Range[1][0]) - 1)
            )  # sqrt(Imax-Imin-1)
            sv1 = min(100, max(0, int(0.5 + 100.0 * sqrtDeltOne / sqrtDeltZero)))
            maxSel.SetValue(sv1)
            DeltOne = max(1.0, Range[1][1] - max(0.0, Range[0][0]) - 1)
            sv0 = min(
                100, max(0, int(0.5 + 100.0 * (Range[1][0] - Range[0][0]) / DeltOne))
            )
            minSel.SetValue(sv0)
            new, plotNum, Page, Plot, lim = G2frame.G2plotNB.FindPlotTab(
                "2D Powder Image", "mpl", newImage=False
            )
        ...
        def OnMaxSlider(event):  # noqa: ARG001
            val = maxSel.GetValue()
            if G2frame.prevMaxValue == val:
                return  # if this val has been processed, no need to repeat
            scaleSel.SetSelection(len(scaleChoices) - 1)
            G2frame.prevMaxValue = val
            sqrtDeltZero = math.sqrt(
                max(1.0, Range[0][1] - max(0.0, Range[1][0]) - 1)
            )  # sqrt(Imax0-Imin-1)
            Range[1][1] = int(0.5 + (val * sqrtDeltZero / 100.0) ** 2 + Range[1][0] + 1)
        ...
        def OnMinSlider(event):  # noqa: ARG001
            val = minSel.GetValue()
            scaleSel.SetSelection(len(scaleChoices) - 1)
            if G2frame.prevMinValue == val:
                return  # if this val has been processed, no need to repeat
        ...
        def OnAutoSet(event):
            ...
            if margin == 0:
                ...
                Range[1][0] = int(
                    np.percentile(Page.ImgObj.get_array().compressed(), margin)
                )
                Range[1][1] = int(
                    np.percentile(Page.ImgObj.get_array().compressed(), 100 - margin)
                )
            sqrtDeltZero = math.sqrt(
                max(1.0, Range[0][1] - max(0.0, Range[1][0]) - 1)
            )  # sqrt(Imax0-Imin-1)
        ...
        def OnNewLineScan(invalid, value, tc):  # noqa: ARG001
            G2plt.PlotExposedImage(G2frame, event=None)

        def OnMoveAzm(event):
            incr = azmSpin.GetValue()
            if incr == 0:
                return  # ignore SetValue(0) event
            data["linescan"][1] += float(incr)
            data["linescan"][1] = data["linescan"][1] % 360.0
        ...
        mplOld = (
            mplv[0] == "1" and int(mplv[1]) < 4
        )  # use draw_idle for newer matplotlib versions
        ...
        G2frame.slideSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" Max intensity"), 0, WACV
        )
        ...
        sv1 = min(100, max(0, int(0.5 + 100.0 * sqrtDeltOne / sqrtDeltZero)))
        maxSel = G2G.G2Slider(
            parent=G2frame.dataWindow, style=wx.SL_HORIZONTAL, value=sv1
        )
        G2frame.slideSizer.AddGrowableCol(2)
        maxSel.Bind(wx.EVT_SLIDER, OnMaxSlider)
        maxVal = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            Range[1],
            1,
            xmin=Range[0][0] + 1,
        ...
        G2frame.slideSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" Min intensity"), 0, WACV
        )
        ...
        DeltOne = max(1.0, Range[1][1] - max(0.0, Range[0][0]) - 1)  # Imax-Imin0-1
        sv0 = min(100, max(0, int(0.5 + 100.0 * (Range[1][0] - Range[0][0]) / DeltOne)))
        minSel = G2G.G2Slider(
            parent=G2frame.dataWindow, style=wx.SL_HORIZONTAL, value=sv0
        )
        minSel.Bind(wx.EVT_SLIDER, OnMinSlider)
        minVal = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            Range[1],
            0,
            xmax=Range[0][1],
        ...
        if data["linescan"][0]:
            linescan = wx.CheckBox(
                G2frame.dataWindow, label=" Show line scan at azm = "
            )
        else:
        ...
        if data["linescan"][0]:
            G2frame.scanazm = G2G.ValidatedTxtCtrl(
                G2frame.dataWindow,
                data["linescan"],
                1,
                xmin=0.0,
                xmax=360.0,
                OnLeave=OnNewLineScan,
            )
            autoSizer.Add(G2frame.scanazm, 0, WACV)
            azmSpin = wx.SpinButton(
                G2frame.dataWindow, style=wx.SP_VERTICAL
            )  # ,size=wx.Size(20,25)) # size fails in Linux
            azmSpin.SetValue(0)
    ...
    def CalibCoeffSizer():
        ...
        calibSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" Calibration coefficients"),
            0,
            WACV,
        )
        ...
        if "PWDR" in data["type"]:
            Names.append("dep")
        Parms = {
            "dist": ["Distance", (10, 3), data, "distance"],
            "det-X": ["Beam center X", (10, 3), data["center"], 0],
            "det-Y": ["Beam center Y", (10, 3), data["center"], 1],
            "tilt": ["Tilt angle*", (10, 3), data, "tilt"],
            "phi": ["Tilt rotation*", (10, 2), data, "rotation"],
            "dep": ["Penetration*", (10, 4), data, "DetDepth"],
    ...
    def IntegrateSizer():
        def OnNewBinType(event):  # noqa: ARG001
            data["binType"] = binSel.GetValue()
            wx.CallLater(100, UpdateImageControls, G2frame, data, masks)

        def OnIOtth(invalid, value, tc):  # noqa: ARG001
            ...
            if "q" in data["binType"].lower():
                data["IOtth"] = [
                    2.0 * asind(Ltth * wave / (4.0 * math.pi)),
                    2.0 * asind(Utth * wave / (4.0 * math.pi)),
                ]
            else:
                data["IOtth"] = [Ltth, Utth]
            wx.CallAfter(G2plt.PlotExposedImage, G2frame, event=tc.event)

        def OnLRazim(invalid, value, tc):  # noqa: ARG001
            """Respond to a change in integration azimuth range"""
            Lazm = data["LRazimuth"][0] % 360.0
            Razm = data["LRazimuth"][1] % 360.0
            if Lazm > Razm:
                Razm += 360.0
        ...
        def OnNumOutAzms(invalid, value, tc):  # noqa: ARG001
            wx.CallAfter(G2plt.PlotExposedImage, G2frame, event=tc.event)

        def OnNumOutBins(invalid, value, tc):  # noqa: ARG001
            # make sure # channels is divisible by 4
            data["outChannels"] = (data["outChannels"] // 4) * 4
            outChan.ChangeValue(data["outChannels"])

        def OnOblique(event):  # noqa: ARG001
            data["Oblique"][1] = not data["Oblique"][1]

        def OnSampleShape(event):  # noqa: ARG001
        ...
        def OnSamAbs(event):  # noqa: ARG001
        ...
        def OnFullIntegrate(event):
            ...
            if data["fullIntegrate"]:
                ...
                data["LRazimuth"] = [Lazm, Lazm + 20.0]
            else:
                data["fullIntegrate"] = True
                data["LRazimuth"] = [Lazm, Lazm + 360.0]
            wx.CallLater(100, UpdateImageControls, G2frame, data, masks)
            G2plt.PlotExposedImage(G2frame, event=event)

        def OnSetDefault(event):  # noqa: ARG001
            if data["setDefault"]:
                ...
                if "formatName" in G2frame.imageDefault:
                    del G2frame.imageDefault["formatName"]
        ...
        def OnApplyPola(event):  # noqa: ARG001
            data["PolaVal"][1] = not data["PolaVal"][1]

        def OnIfPink(event):  # noqa: ARG001
            data["IfPink"] = not data["IfPink"]

        def OnOchoice(event):  # noqa: ARG001
            data["orientation"] = ochoice.GetValue()

        dataSizer = wx.FlexGridSizer(0, 2, 5, 3)
        dataSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" Integration coefficients"),
            0,
            WACV,
        )
        dataSizer.Add((5, 0), 0)
        ...
        dataSizer.Add(
            wx.StaticText(
                G2frame.dataWindow, label=" Bin style: Constant step bins in"
            ),
            0,
            WACV,
        )
        ...
        if "q" in data["binType"].lower():
            binType = "Q"
        dataSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" Inner/Outer " + binType),
            0,
            WACV,
        )
        IOtth = data["IOtth"][:]
        if "q" in data["binType"].lower():
            ...
            IOtth = [
                4.0 * math.pi * sind(IOtth[0] / 2.0) / wave,
                4.0 * math.pi * sind(IOtth[1] / 2.0) / wave,
            ]
        littleSizer = wx.BoxSizer(wx.HORIZONTAL)
        G2frame.InnerTth = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            IOtth,
            0,
            nDig=(8, 3, "f"),
        ...
        G2frame.OuterTth = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            IOtth,
            1,
            nDig=(8, 3, "f"),
        ...
        dataSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" Start/End azimuth"),
            0,
            WACV,
        )
        LRazim = data["LRazimuth"]
        littleSizer = wx.BoxSizer(wx.HORIZONTAL)
        G2frame.Lazim = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            LRazim,
            0,
            nDig=(6, 1, "f"),
        ...
        G2frame.Razim = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            LRazim,
            1,
            nDig=(6, 1, "f"),
        ...
        if data["fullIntegrate"]:
            G2frame.Razim.ChangeValue(LRazim[0] + 360.0)
        ...
        dataSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" No. 2-theta/azimuth bins"),
            0,
            WACV,
        )
        littleSizer = wx.BoxSizer(wx.HORIZONTAL)
        outChan = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            data,
            "outChannels",
            typeHint=int,
            xmin=10,
            OnLeave=OnNumOutBins,
        )
        littleSizer.Add(outChan, 0, WACV)
        outAzim = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            data,
            "outAzimuths",
            xmin=1,
            typeHint=int,
            OnLeave=OnNumOutAzms,
        )
        littleSizer.Add(outAzim, 0, WACV)
        dataSizer.Add(littleSizer)
        showLines = wx.CheckBox(
            parent=G2frame.dataWindow, label="Show integration limits?"
        )
        dataSizer.Add(showLines, 0, WACV)
        showLines.Bind(wx.EVT_CHECKBOX, OnShowLines)
        showLines.SetValue(data["showLines"])
        fullIntegrate = wx.CheckBox(
            parent=G2frame.dataWindow, label="Do full integration?"
        )
        ...
        setDefault = wx.CheckBox(
            parent=G2frame.dataWindow, label="Use for all new images?"
        )
        dataSizer.Add(setDefault, 0, WACV)
        setDefault.Bind(wx.EVT_CHECKBOX, OnSetDefault)
        setDefault.SetValue(data["setDefault"])
        centerAzm = wx.CheckBox(
            parent=G2frame.dataWindow, label="Azimuth at bin center?"
        )
        ...
        littleSizer = wx.BoxSizer(wx.HORIZONTAL)
        samabs = wx.CheckBox(
            parent=G2frame.dataWindow, label="Apply sample absorption?"
        )
        ...
        if data["SampleAbs"][1]:
            ...
            littleSizer.Add(
                wx.StaticText(G2frame.dataWindow, label="Select shape "), 0, WACV
            )
            samShape = wx.ComboBox(
                G2frame.dataWindow,
                value=data["SampleShape"],
        ...
        if data["SampleAbs"][1]:
            littleSizer = wx.BoxSizer(wx.HORIZONTAL)
            if (
                "Cylind" in data["SampleShape"]
            ):  # cylinder mu*R; flat plate transmission
                littleSizer.Add(
                    wx.StaticText(G2frame.dataWindow, label="mu*R (0.00-2.0) "), 0, WACV
                )
            elif "Fixed" in data["SampleShape"]:
                littleSizer.Add(
                    wx.StaticText(G2frame.dataWindow, label="transmission "), 0, WACV
                )  # for flat plate
                minmax = [0.05, 1.0]
            samabsVal = G2G.ValidatedTxtCtrl(
                G2frame.dataWindow,
                data["SampleAbs"],
                0,
                nDig=(10, 3),
        ...
        if "Cylind" in data["SampleShape"] and data["SampleAbs"][1]:
            littleSizer = wx.BoxSizer(wx.HORIZONTAL)
            littleSizer.Add(
                wx.StaticText(G2frame.dataWindow, label="Select orientation "), 0, WACV
            )
        ...
        if "PWDR" in data["type"]:
            littleSizer = wx.BoxSizer(wx.HORIZONTAL)
            oblique = wx.CheckBox(
                parent=G2frame.dataWindow, label="Apply detector absorption?"
            )
            dataSizer.Add(oblique, 0, WACV)
            oblique.Bind(wx.EVT_CHECKBOX, OnOblique)
            oblique.SetValue(data["Oblique"][1])
            littleSizer.Add(
                wx.StaticText(G2frame.dataWindow, label="Value (0.01-0.99)  "), 0, WACV
            )
            obliqVal = G2G.ValidatedTxtCtrl(
                G2frame.dataWindow,
                data["Oblique"],
                0,
                nDig=(10, 3),
        ...
        if "SASD" in data["type"]:
            littleSizer = wx.BoxSizer(wx.HORIZONTAL)
            setPolariz = wx.CheckBox(
                parent=G2frame.dataWindow, label="Apply polarization?"
            )
            dataSizer.Add(setPolariz, 0, WACV)
            setPolariz.Bind(wx.EVT_CHECKBOX, OnApplyPola)
            setPolariz.SetValue(data["PolaVal"][1])
            littleSizer.Add(
                wx.StaticText(G2frame.dataWindow, label="Value (0.001-0.999)  "),
                0,
                WACV,
            )
            polaVal = G2G.ValidatedTxtCtrl(
                G2frame.dataWindow,
                data["PolaVal"],
                0,
                nDig=(10, 3),
    ...
    def BackSizer():
        ...
        def OnFlatBkg(invalid, value, tc):  # noqa: ARG001
            global oldFlat  # noqa: PLW0603
        ...
        backSizer.Add(
            wx.StaticText(G2frame.dataWindow, -1, " Background image"), 0, WACV
        )
        backImage = wx.ComboBox(
            parent=G2frame.dataWindow,
            value=data["background image"][0],
            choices=Choices,
            style=wx.CB_READONLY | wx.CB_DROPDOWN,
        )
    ...
    def CalibSizer():
        def OnNewCalibrant(event):  # noqa: ARG001
            data["calibrant"] = calSel.GetValue().strip()
            if data["calibrant"]:
                G2frame.dataWindow.ImageEdit.Enable(
                    id=G2G.wxID_IMRECALIBRATE, enable=True
                )
                G2frame.dataWindow.ImageEdit.Enable(
                    id=G2G.wxID_IMCALIBRATE, enable=True
                )
                G2frame.dataWindow.ImageEdit.Enable(
                    id=G2G.wxID_IMRECALIBALL, enable=True
                )
                ...
                G2frame.dataWindow.ImageEdit.Enable(
                    id=G2G.wxID_IMRECALIBRATE, enable=False
                )
                G2frame.dataWindow.ImageEdit.Enable(
                    id=G2G.wxID_IMCALIBRATE, enable=False
                )
                G2frame.dataWindow.ImageEdit.Enable(
                    id=G2G.wxID_IMRECALIBALL, enable=False
                )

        def OnCalibSkip(event):  # noqa: ARG001
            data["calibskip"] = int(calibSkip.GetValue())

        def OnPixLimit(event):  # noqa: ARG001
        ...
        comboSizer = wx.BoxSizer(wx.HORIZONTAL)
        comboSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" Calibrant "), 0, WACV
        )
        if (
            GSASIIpath.GetConfigValue("Image_calibrant")
            and GSASIIpath.GetConfigValue("Image_calibrant") in calList
            and not data["calibrant"]
        ):
            data["calibrant"] = GSASIIpath.GetConfigValue("Image_calibrant")
        calSel = wx.ComboBox(
            parent=G2frame.dataWindow,
            value=data["calibrant"],
            choices=calList,
            style=wx.CB_READONLY | wx.CB_DROPDOWN,
        )
        ...
        comboSizer = wx.BoxSizer(wx.HORIZONTAL)
        comboSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" Calib lines to skip   "),
            0,
            WACV,
        )
        calibSkip = wx.ComboBox(
            parent=G2frame.dataWindow,
            value=str(data["calibskip"]),
        ...
        comboSizer = wx.BoxSizer(wx.HORIZONTAL)
        comboSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" Min calib d-spacing "),
            0,
            WACV,
        )
        G2frame.calibDmin = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            data,
            "calibdmin",
            nDig=(10, 2),
        ...
        comboSizer = wx.BoxSizer(wx.HORIZONTAL)
        comboSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" Min ring I/Ib "), 0, WACV
        )
        cutOff = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow, data, "cutoff", nDig=(10, 2), xmin=0.1
        )
        ...
        comboSizer = wx.BoxSizer(wx.HORIZONTAL)
        comboSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" Pixel search range "),
            0,
            WACV,
        )
        pixLimit = wx.ComboBox(
            parent=G2frame.dataWindow,
            value=str(data["pixLimit"]),
    ...
    def GonioSizer():
        def OnGlobalEdit(event):  # noqa: ARG001
            Names = []
            Items = []
            if G2frame.GPXtree.GetCount():
                ...
                while Id:
                    ...
                    if "IMG" in name:
                        ctrls = G2frame.GPXtree.GetItemPyData(
                            G2gd.GetGPXtreeItemId(G2frame, Id, "Image Controls")
                        )
                        Names.append(name)
                ...
                if len(Names) == 1:
                    G2frame.ErrorDialog(
                        "Nothing for global editing",
                        'There must be more than one "IMG" pattern',
                    )
                    return
                dlg = G2G.G2HistoDataDialog(
                    G2frame,
                    " Edit sample goniometer data:",
                    "Edit data",
                    ["Omega", "Chi", "Phi"],
                    ["%.2f", "%.2f", "%.2f"],
                    Names,
                ...
                try:
                    if dlg.ShowModal() == wx.ID_OK:
                        ...
                        while Id:
                            ...
                            Id, cookie = G2frame.GPXtree.GetNextChild(
                                G2frame.root, cookie
                            )
                finally:
                    dlg.Destroy()
        ...
        names = ["Omega", "Chi", "Phi"]
        gonioSizer.Add(
            wx.StaticText(G2frame.dataWindow, -1, "Sample goniometer angles: "), 0, WACV
        )
        for i, name in enumerate(names):
            gonioSizer.Add(wx.StaticText(G2frame.dataWindow, -1, name), 0, WACV)
            angle = G2G.ValidatedTxtCtrl(
                G2frame.dataWindow, data["GonioAngles"], i, nDig=(8, 2), typeHint=float
            )
    ...
    def OnSelectPhases(event):  # noqa: ARG001
            ...
            for H in HKL:
                if len(G2frame.PhaseRing2Th) == 250:
                    ...
                    if phaseOpts["warnOnce"]:
                        G2plt.PlotExposedImage(G2frame, event=None)
                        return
                tth = 2.0 * asind(data["wavelength"] / (2.0 * H[3]))
    ...
    def OnDestroy(event):  # noqa: ARG001
        G2frame.autoIntFrame = None

    def OnAutoInt(event):  # noqa: ARG001
        if G2frame.autoIntFrame:  # ensure only one open at a time
            ...
            return
        PollTime = GSASIIpath.GetConfigValue("Autoint_PollTime", 30.0)
        ...
        G2frame.autoIntFrame.Bind(
            wx.EVT_WINDOW_DESTROY, OnDestroy
        )  # clean up name on window close

    G2frame.Bind(wx.EVT_MENU, OnAutoInt, id=G2G.wxID_IMAUTOINTEG)

    def OnIntPDFtool(event):  # noqa: ARG001
        import subprocess

        ex = sys.executable
        if (
            sys.platform == "darwin"
        ):  # mac requires pythonw which is not always reported as sys.executable
            if os.path.exists(ex + "w"):
                ex += "w"
        if G2frame.GSASprojectfile:
            ...
            project = ""
        subprocess.Popen(
            [ex, os.path.join(GSASIIpath.path2GSAS2, "GSASIIIntPDFtool.py"), project]
        )
    ...
    mainSizer.Add(
        wx.StaticText(parent=G2frame.dataWindow, label=" Calibration controls:"), 0
    )
    mainSizer.Add((5, 5), 0)
...
def UpdateMasks(G2frame, data):
    ...
    def Replot(*args, **kwargs):  # noqa: ARG001
        wx.CallAfter(G2plt.PlotExposedImage, G2frame)

    def newReplot(*args, **kwargs):  # noqa: ARG001
    ...
    def OnCopyMask(event):  # noqa: ARG001
        Names = G2gd.GetGPXtreeDataNames(
            G2frame,
            [
                "IMG ",
        ...
        if len(Names) == 1:
            G2frame.ErrorDialog(
                "Nothing to copy masks to", 'There must be more than one "IMG" pattern'
            )
            return
        Source = G2frame.GPXtree.GetItemText(G2frame.Image)
        Names.pop(Names.index(Source))
        ...
        dlg = G2G.G2MultiChoiceDialog(
            G2frame, "Copy mask data", "Copy masks from " + Source + " to:", Names
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
                items = dlg.GetSelections()
                for item in items:
                    ...
                    G2frame.GPXtree.SetItemPyData(
                        G2gd.GetGPXtreeItemId(G2frame, Id, "Masks"), Mask
                    )
        finally:
            dlg.Destroy()
    ...
    def OnCopySelected(event):  # noqa: ARG001
        ...
        selectedKeys = []
        dlg = G2G.G2MultiChoiceDialog(
            G2frame, "Select which masks\nto copy", "Select masks", keyText
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
                selectedKeys = [keyList[i] for i in dlg.GetSelections()]
        ...
        for parm in selectedKeys:
            copyDict[parm] = data[parm]
        dlg = G2G.G2MultiChoiceDialog(
            G2frame, "Copy mask from\n" + Source + " to...", "Copy masks", Names
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
                result = dlg.GetSelections()
                for i in result:
                    ...
                    Id = G2gd.GetGPXtreeItemId(G2frame, G2frame.root, item)
                    Controls = G2frame.GPXtree.GetItemPyData(
                        G2gd.GetGPXtreeItemId(G2frame, Id, "Masks")
                    )
                    Controls.update(copy.deepcopy(copyDict))
        finally:
            dlg.Destroy()

    def OnSaveMask(event):  # noqa: ARG001
        CleanupMasks(data)
        pth = G2G.GetExportPath(G2frame)
        dlg = wx.FileDialog(
            G2frame,
            "Choose image mask file",
            pth,
            "",
            "image mask files (*.immask)|*.immask",
            wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT,
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
    ...
    def OnFindPixelMask(event):
        ...
        if G2img.TestFastPixelMask() and data["SpotMask"].get("FastSearch", True):
            ...
            dlg = wx.ProgressDialog(
                "Pixel masking search", "Setting up fast scan", parent=G2frame
            )
            dlg.Update(1)
            dlg.CenterOnParent()
            time0 = time.time()
            if (
                data["SpotMask"].get("ClearPrev", True)
                or data["SpotMask"]["spotMask"] is None
            ):
                data["SpotMask"]["spotMask"] = G2img.FastAutoPixelMask(
                    G2frame.ImageZ, data, Controls, nChans, dlg
                )
            else:
                data["SpotMask"]["spotMask"] |= G2img.FastAutoPixelMask(
                    G2frame.ImageZ, data, Controls, nChans, dlg
                )
            print(f" Pixel mask search time: {time.time() - time0:.2f} sec")
            wx.CallAfter(UpdateMasks, G2frame, data)
            wx.CallAfter(G2plt.PlotExposedImage, G2frame, event=event)
            dlg.Destroy()
            wx.EndBusyCursor()
        ...
        dlg = wx.ProgressDialog(
            "Pixel masking search for %d rings" % nChans,
            "Processed 2-theta rings = ",
            nChans + 3,
            style=wx.PD_ELAPSED_TIME | wx.PD_CAN_ABORT,
            parent=G2frame,
        )
        ...
        if (
            ...
            data["SpotMask"]["spotMask"] |= mask
        print(f" Pixel mask search time: {(time.time() - time0) / 60.0:.2f} m")
        wx.CallAfter(UpdateMasks, G2frame, data)
        wx.CallAfter(G2plt.PlotExposedImage, G2frame, event=event)

    def OnAutoFindPixelMask(event):  # noqa: ARG001
        Names = G2gd.GetGPXtreeDataNames(
            G2frame,
            [
                "IMG ",
        ...
        dlg = G2G.G2MultiChoiceDialog(
            ...
            "Select images for pixel masking:",
            Names,
        )
        if dlg.ShowModal() != wx.ID_OK:
            return
        ...
        for item in items:
            try:
                ...
                if fast and Mask["SpotMask"].get("FastSearch", True):
                    ...
                    time0 = time.time()
                    if (
                        Mask["SpotMask"].get("ClearPrev", True)
                        or Mask["SpotMask"]["spotMask"] is None
                    ):
                ...
                try:
                    dlg = wx.ProgressDialog(
                        "Pixel mask search for %d bins" % nChans,
                        "Processed 2-theta rings = ",
                        nChans + 3,
                        style=wx.PD_ELAPSED_TIME | wx.PD_CAN_ABORT,
                        parent=G2frame,
                    ...
                    time0 = time.time()
                    mask = G2img.AutoPixelMask(
                        G2frame.ImageZ, Mask, Controls, nChans, dlg
                    )
                    if mask is None:
                        return  # aborted search
                    if (
                        Mask["SpotMask"].get("ClearPrev", True)
                        or Mask["SpotMask"]["spotMask"] is None
                    ):
                        ...
                        Mask["SpotMask"]["spotMask"] |= mask
                    print(
                        f"Pixel mask search time: {(time.time() - time0) / 60.0:.2f} m"
                    )
                finally:
                    dlg.Destroy()
    ...
    def ToggleSpotMaskMode(event):  # noqa: ARG001
        G2plt.ToggleMultiSpotMask(G2frame)

    def OnNewArcMask(event):  # noqa: ARG001
        "Start a new arc mask"
        G2frame.MaskKey = "a"
        G2plt.OnStartMask(G2frame)

    def OnNewRingMask(event):  # noqa: ARG001
        "Start a new ring mask"
        G2frame.MaskKey = "r"
        G2plt.OnStartMask(G2frame)

    def OnNewXlineMask(event):  # noqa: ARG001
        "Start a new x-line mask"
        G2frame.MaskKey = "x"
        G2plt.OnStartMask(G2frame)

    def OnNewYlineMask(event):  # noqa: ARG001
        "Start a new y-line mask"
        G2frame.MaskKey = "y"
        G2plt.OnStartMask(G2frame)

    def OnNewPolyMask(event):  # noqa: ARG001
        "Start a new polygon mask"
        G2frame.MaskKey = "p"
        G2plt.OnStartMask(G2frame)

    def OnNewFrameMask(event):  # noqa: ARG001
    ...
    def OnDelPixMask(event):
        data["SpotMask"] = {"esdMul": 3.0, "spotMask": None}
        wx.CallAfter(UpdateMasks, G2frame, data)
        G2plt.PlotExposedImage(G2frame, event=event)
    ...
    def OnAzimuthPlot(event):
        ...
        G2plt.PlotXY(
            ...
            labelY="Intensity",
            newPlot=True,
            Title=f"Ring Mask Intensity: 2{GkTheta}={data['Rings'][ringId][0]:.2f}",
            lines=True,
        )
    ...
    if G2frame.dataWindow:
        startScroll = G2frame.dataWindow.GetScrollPos(
            wx.VERTICAL
    ...
    if G2frame.MaskKey == "f":
        G2frame.GetStatusBar().SetStatusText(
            "Frame mask active - LB pick next point, RB close polygon", 1
        )
    elif G2frame.MaskKey == "p":
        G2frame.GetStatusBar().SetStatusText(
            "Polygon mask active - LB pick next point, RB close polygon", 1
        )
    elif G2frame.MaskKey == "a":
        G2frame.GetStatusBar().SetStatusText(
            "Arc mask active - LB pick arc location", 1
        )
    elif G2frame.MaskKey == "r":
        G2frame.GetStatusBar().SetStatusText(
            "Ring mask active - LB pick ring location", 1
        )
    elif G2frame.MaskKey == "x":
        G2frame.GetStatusBar().SetStatusText(
            "X-line mask active - LB pick x line of pixels", 1
        )
    elif G2frame.MaskKey == "y":
        G2frame.GetStatusBar().SetStatusText(
            "Y-line mask active - LB pick y line of pixels", 1
        )
    else:
        G2frame.GetStatusBar().SetStatusText(
            "To add mask: press a,r,s,x,y,p or f on 2D image for arc/ring/spot/xline/yline/polygon/frame",
            1,
        )
    topSizer = G2frame.dataWindow.topBox
    topSizer.Clear(True)
    ...
    data["SpotMask"]["SearchMax"] = data["SpotMask"].get("SearchMax", 180.0)
    ...
    littleSizer.Add(
        wx.StaticText(parent=G2frame.dataWindow, label=" Lower/Upper limits "), 0, WACV
    )
    Text = wx.TextCtrl(
        G2frame.dataWindow, value=str(thresh[0][0]), style=wx.TE_READONLY
    )
    littleSizer.Add(Text, 0, WACV)
    Text.SetBackgroundColour(VERY_LIGHT_GREY)
    Text = wx.TextCtrl(
        G2frame.dataWindow, value=str(thresh[0][1]), style=wx.TE_READONLY
    )
    littleSizer.Add(Text, 0, WACV)
    Text.SetBackgroundColour(VERY_LIGHT_GREY)
    littleSizer.Add(
        wx.StaticText(parent=G2frame.dataWindow, label=" Lower/Upper thresholds "),
        0,
        WACV,
    )
    lowerThreshold = G2G.ValidatedTxtCtrl(
        G2frame.dataWindow,
        loc=thresh[1],
        key=0,
        xmin=thresh[0][0],
    ...
    upperThreshold = G2G.ValidatedTxtCtrl(
        G2frame.dataWindow,
        loc=thresh[1],
        key=1,
        xmax=thresh[0][1],
    ...
    spotSizer.Add(
        wx.StaticText(
            G2frame.dataWindow,
            label=" Number of masked pixels: %d  " % numPix,
        ),
        0,
        WACV,
    )
    ...
    spotSizer = wx.BoxSizer(wx.HORIZONTAL)
    spotSizer.Add(
        wx.StaticText(G2frame.dataWindow, label="Select n*sigma rejection (n=1-10): "),
        0,
        WACV,
    )
    spotSizer.Add(
    ...
    spotSizer.Add(
        G2G.G2CheckBoxFrontLbl(
            G2frame.dataWindow,
            "Clear previous pixel mask on search",
            data["SpotMask"],
            "ClearPrev",
        ),
        0,
    ...
    if G2img.TestFastPixelMask():
        spotSizer.Add(
            G2G.G2CheckBoxFrontLbl(
                ...
                data["SpotMask"],
                "FastSearch",
                OnChange=lambda x: wx.CallAfter(UpdateMasks, G2frame, data),  # noqa: ARG005
            ),
            0,
            WACV,
        )
        ...
        spotSizer.Add(
            wx.StaticText(G2frame.dataWindow, label="(Fast search not installed) "),
            0,
            WACV,
        )
    txt = wx.StaticText(
        G2frame.dataWindow, label="  Pixel mask search range, 2theta min: "
    )
    ...
    if len(Spots):
        lbl = wx.StaticText(
            parent=G2frame.dataWindow,
            label=" Spot masks (on plot, LB drag to move, shift-LB drag to resize, RB to delete)",
        )
    ...
    if Rings:
        ...
        littleSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" 2-theta,deg"), 0, WACV
        )
        littleSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" thickness, deg"), 0, WACV
        )
        ...
        for i in range(len(Rings)):
            if Rings[i]:
                ...
                ringThick = G2G.ValidatedTxtCtrl(
                    G2frame.dataWindow,
                    loc=Rings[i],
                    key=1,
                    xmin=0.001,
    ...
    if Arcs:
        ...
        littleSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" azimuth, deg"), 0, WACV
        )
        littleSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" thickness, deg"), 0, WACV
        )
        ...
        for i in range(len(Xlines)):
            if Xlines[i]:
                littleSizer.Add(
                    wx.StaticText(
                        G2frame.dataWindow,
                        label="at Y-pixel: %d" % (Xlines[i]),
                    ),
                    0,
                    WACV,
                )
                code = "%s+%d" % ("Xlines", i)
                xlineDelete = G2G.G2Button(
                    G2frame.dataWindow,
                    label="delete?",
                    locationcode=code,
                    handler=onDeleteMask,
        ...
        for i in range(len(Ylines)):
            if Ylines[i]:
                littleSizer.Add(
                    wx.StaticText(
                        G2frame.dataWindow,
                        label="at X-pixel: %d" % (Ylines[i]),
                    ),
                    0,
                    WACV,
                )
                code = "%s+%d" % ("Ylines", i)
                ylineDelete = G2G.G2Button(
                    G2frame.dataWindow,
                    label="delete?",
                    locationcode=code,
                    handler=onDeleteMask,
...
def UpdateStressStrain(G2frame, data):
    ...
    def OnAppendDzero(event):  # noqa: ARG001
        data["d-zero"].append(
            {
                "Dset": 1.0,
                "Dcalc": 0.0,
        ...
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                for item in items:
                    ...
                    G2frame.GPXtree.SetItemPyData(
                        G2gd.GetGPXtreeItemId(G2frame, Id, "Stress/Strain"), Data
                    )
        finally:
            dlg.Destroy()
            G2frame.GPXtree.SelectItem(G2frame.PickId)
    ...
    def OnLoadStrSta(event):
        ...
        dlg = wx.FileDialog(
            G2frame,
            "Choose stress/strain file",
            pth,
            "",
            "image control files (*.strsta)|*.strsta",
            wx.FD_OPEN,
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
    ...
    def OnSaveStrSta(event):  # noqa: ARG001
        pth = G2G.GetExportPath(G2frame)
        dlg = wx.FileDialog(
            G2frame,
            "Choose stress/strain file",
    ...
    def OnStrStaSample(event):  # noqa: ARG001
        filename = ""
        pth = G2G.GetImportPath(G2frame)
        if not pth:
            pth = "."
        ...
        for hist in histList:
            ...
            if name in dataDict:
                ...
                stsrData = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, Id, "Stress/Strain")
                )
    ...
    def OnPlotStrSta(event):
        ...
        Names = [f"d={ring['Dcalc']:.3f}" for ring in data["d-zero"]]
        G2plt.PlotExposedImage(G2frame, event=event)
        ...
        G2plt.PlotXY(
            ...
            labelY="MRD",
            newPlot=True,
            Title="Ring Intensities",
            names=Names,
            lines=True,
        )

    def OnSaveStrRing(event):  # noqa: ARG001
        Controls = G2frame.GPXtree.GetItemPyData(
            G2gd.GetGPXtreeItemId(G2frame, G2frame.Image, "Image Controls")
        )
        RingInt = G2img.IntStrSta(G2frame.ImageZ, data, Controls)
        ...
        dlg = wx.FileDialog(
            G2frame,
            "Choose strain ring intensity file",
            pth,
            "",
            "ring intensity file (*.txt)|*.txt",
            wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT,
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                File = open(filename, "w")  # noqa: SIM115
                for i, name in enumerate(Names):
                    File.write(f" Ring intensity for {name}\n")
                    File.write("%12s %12s\n" % ("Azimuth", "RMD"))
    ...
    def OnFitAllStrSta(event):  # noqa: ARG001
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            Id = G2gd.GetGPXtreeItemId(
                G2frame, G2frame.root, "Sequential strain fit results"
            )
            if Id:
                SeqResult = G2frame.GPXtree.GetItemPyData(Id)
            else:
                SeqResult = {}
                Id = G2frame.GPXtree.AppendItem(
                    parent=G2frame.root, text="Sequential strain fit results"
                )
        ...
        try:
            ...
            for i, name in enumerate(names):
                ...
                G2frame.Image = sId
                Controls = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, sId, "Image Controls")
                )
                StaCtrls = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, sId, "Stress/Strain")
                )
                ...
                parmDict = {
                    "Sample load": StaCtrls["Sample load"],
                }
                ...
                for j, item in enumerate(StaCtrls["d-zero"]):
                    ...
                    variables.append(1.0e6 * (item["Dcalc"] / item["Dset"] - 1.0))
    ...
    def SamSizer():
        def OnStrainType(event):  # noqa: ARG001
            data["Type"] = strType.GetValue()

        samSizer = wx.FlexGridSizer(0, 4, 0, 5)
        samSizer.Add(
            wx.StaticText(G2frame.dataWindow, -1, label=" Strain type: "), 0, WACV
        )
        strType = wx.ComboBox(
            G2frame.dataWindow,
            value=data["Type"],
            choices=["True", "Conventional"],
            style=wx.CB_READONLY | wx.CB_DROPDOWN,
        )
        ...
        samSizer.Add(
            wx.StaticText(G2frame.dataWindow, -1, label=" Sample phi: "), 0, WACV
        )
        samPhi = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            data,
            "Sample phi",
            nDig=(10, 3),
        ...
        samSizer.Add(
            wx.StaticText(G2frame.dataWindow, -1, label=" Sample delta-z(mm): "),
            0,
            WACV,
        )
        samZ = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow, data, "Sample z", nDig=(10, 3), typeHint=float
        )
        samSizer.Add(samZ, 0, WACV)
        samSizer.Add(
            wx.StaticText(G2frame.dataWindow, -1, label=" Sample load(MPa): "), 0, WACV
        )
        samLoad = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            data,
            "Sample load",
            nDig=[8, 3],
    ...
    def DzeroSizer():
        def OnStrainChange(event):
            ...
            r, c = event.GetRow(), event.GetCol()
            if c == 0:
                data["d-zero"][r]["Dset"] = min(
                    max(float(StrainGrid.GetCellValue(r, c)), 0.25), 20.0
                )
            elif c == 1:
                data["d-zero"][r]["fixDset"] = bool(StrainGrid.GetCellValue(r, c))
            elif c == 3:
                data["d-zero"][r]["cutoff"] = min(
                    max(float(StrainGrid.GetCellValue(r, c)), 0.5), 20.0
                )
            elif c == 4:
        ...
        def OnSetCol(event):
            ...
            if c == 1:
                ...
                dlg = wx.SingleChoiceDialog(
                    G2frame,
                    "Select option for " + StrainGrid.GetColLabelValue(c - 1),
                    "Strain controls",
                    choice,
                )
                dlg.CenterOnParent()
                if dlg.ShowModal() == wx.ID_OK:
                    ...
                    if sel == 0:
                        for row in range(StrainGrid.GetNumberRows()):
                            data["d-zero"][row]["fixDset"] = True
                    else:
                        for row in range(StrainGrid.GetNumberRows()):
                            data["d-zero"][row]["fixDset"] = False
                wx.CallAfter(UpdateStressStrain, G2frame, data)
        ...
        colTypes = (
            [
                ...
                wg.GRID_VALUE_CHOICE + ":1,2,5,10,15,20",
            ]
            + 3
            * [
                wg.GRID_VALUE_FLOAT + ":10,2",
        ...
        table = [
            [
                ...
                item["Emat"][2],
                False,
                1.0e6 * (item["Dcalc"] / item["Dset"] - 1.0),
                item["Ivar"],
            ]
            for item in data["d-zero"]
        ]
        StrainTable = G2G.Table(
            table, rowLabels=rowIds, colLabels=colIds, types=colTypes
        )
    ...
    if G2frame.StrainKey == "a":  # probably doesn't happen
        G2frame.GetStatusBar().SetStatusText(
            "Add strain ring active - LB pick d-zero value", 1
        )
    else:
        G2frame.GetStatusBar().SetStatusText(
            "To add strain data: On 2D Powder Image, key a:add ring", 1
        )
...
def Read_imctrl(imctrl_file):
    ...
    File = open(imctrl_file)  # noqa: SIM115
    fullIntegrate = False
    try:
        S = File.readline()
        while S:
        ...
        File.close()
        if fullIntegrate:
            save["LRazimuth_min"], save["LRazimuth_max"] = 0.0, 360.0
    return save
...
class AutoIntFrame(wx.Frame):
    ...
    def __init__(self, G2frame, PollTime=30.0):
        ...
        def OnBrowse(event):
            ...
            if btn3 == event.GetEventObject():
                ...
                if "Error" in msg:
                    ...
                    msg = f"Information from file {self.params['pdfprm']}\n\n{msg}"
                    lbl = "PDFPRM information"
                G2G.G2MessageBox(self, msg, lbl)
                return

        def OnRadioSelect(event):  # noqa: ARG001
            ...
            if self.useTable.GetValue():
                ...
                self.useActive.SetLabel(
                    "Active Image: " + G2frame.GPXtree.GetItemText(self.imageBase)
                )
                self.editTable.Enable(False)
            else:
                print("unexpected mode in OnRadioSelect")

        def OnEditTable(event):  # noqa: ARG001
        ...
        ic = G2frame.GPXtree.GetItemPyData(
            G2gd.GetGPXtreeItemId(G2frame, G2frame.Image, "Image Controls")
        )
        ...
        Comments = G2frame.GPXtree.GetItemPyData(
            G2gd.GetGPXtreeItemId(G2frame, self.imageBase, "Comments")
        )
        ...
        self.readDir = G2G.ValidatedTxtCtrl(
            mnpnl,
            self.params,
            "readdir",
        ...
        flterInp = G2G.ValidatedTxtCtrl(
            mnpnl, self.params, "filter", OnLeave=self.ShowMatchingFiles
        )
        sizer.Add(flterInp)
        mnsizer.Add(sizer, 0, wx.EXPAND, 0)
        ...
        fInp3 = G2G.ValidatedTxtCtrl(
            mnpnl, self.params, "outdir", notBlank=False, size=(300, -1)
        )
        ...
        for dfmt in sorted(self.fmtlist[0]):
            ...
            if fmt in usedfmt:  # is extension used more than once
                ...
                usedfmt.append(fmt)
                btn = G2G.G2CheckBox(
                    mnpnl, dfmt, self.params, fmt, OnChange=self.TestInput
                )
                sizer.Add(btn)
        lblsizr.Add(sizer)
        sizer = wx.BoxSizer(wx.HORIZONTAL)
        ...
        if self.AutoScales:
            ...
            scalename = wx.ComboBox(
                mnpnl,
                value=self.AutoScaleName,
                choices=list(self.AutoScales.keys()),
                style=wx.CB_READONLY | wx.CB_DROPDOWN,
            )
            scalename.Bind(wx.EVT_COMBOBOX, OnAutoScaleName)
            sizer.Add(scalename, 0, WACV)
            sizer.Add(wx.StaticText(mnpnl, label=" to "), 0, WACV)
            scaleval = G2G.ValidatedTxtCtrl(
                mnpnl, self.Scale, 0, nDig=(10, 2), xmin=1.0
            )
        ...
        sizer.Add(
            G2G.G2CheckBox(mnpnl, "", self.params, "ComputePDF", OnChange=showPDFctrls)
        )
        ...
        # buttons on bottom
        mnsizer.Add(
            wx.StaticText(mnpnl, wx.ID_ANY, "AutoIntegration controls"), 0, wx.TOP, 5
        )
        sizer = wx.BoxSizer(wx.HORIZONTAL)
        sizer.Add((20, -1))
    ...
    def TestInput(self, event):  # noqa: ARG002
        for fmt in self.multipleFmtChoices:
            if not self.params[fmt]:
                continue
            if self.multipleFmtChoices[fmt]:
                continue
            choices = []
            for f, l in zip(self.fmtlist[0], self.fmtlist[1], strict=False):  # noqa: E741
                if f[1:] == fmt:
                    choices.append(l)
            if len(choices) < 2:
    ...
    def checkPDFprm(self, ShowContents=False):
        ...
        File = None
        try:
            File = open(self.params["pdfprm"])  # noqa: SIM115
            S = File.readline()
            while S:
                if "#" in S:
                    S = File.readline()
            ...
            msg += "PDF Processing Error: error with open or read of {}".format(
                self.params["pdfprm"]
            )
            if GSASIIpath.GetConfigValue("debug"):
                print("DBG_" + msg)
        ...
        for el in self.pdfControls["ElList"]:
            if self.pdfControls["ElList"][el]["FormulaNo"] <= 0:
                continue
            if formula:
                formula += " "
            formula += f"{el}({self.pdfControls['ElList'][el]['FormulaNo']:.1f})"
        if not formula:
        ...
        for key in ["Sample Bkg.", "Container", "Container Bkg."]:
            if key not in self.pdfControls:
                ...
                continue
        if msg or not ShowContents:
            return msg  # stop on error
    ...
    def SetSourceDir(self, event):  # noqa: ARG002
        ...
        dlg = wx.DirDialog(
            self,
            "Select directory for image files",
            self.params["readdir"],
            wx.DD_DEFAULT_STYLE,
        )
        dlg.CenterOnParent()
    ...
    def ShowMatchingFiles(self, value, invalid=False, **kwargs):  # noqa: ARG002
        ...
        if invalid:
            return
        msg = ""
        if self.PreventReEntryShowMatch:
            return
        ...
        for img in G2gd.GetGPXtreeDataNames(G2frame, ["IMG "]):
            ...
            if imagefile not in imageFileList:
            ...
            if img not in G2frame.IntegratedList:
                ...
                msg += "  " + img
        if msg:
            msg = "Loaded images to integrate:\n" + msg + "\n"
        msg1 = ""
        try:
            if os.path.exists(self.params["readdir"]):
                imageList = sorted(
                    glob.glob(
                        os.path.join(self.params["readdir"], self.params["filter"])
                    )
                )
                if not imageList:
                    msg1 = "Warning: No files match search string " + os.path.join(
                        self.params["readdir"], self.params["filter"]
                    )
                else:
                    for fil in imageList:
                        if fil not in imageFileList:
                            msg1 += "\n  " + fil
                    if msg1:
                        msg += (
                            "Files to integrate from "
                            + os.path.join(
                                self.params["readdir"], self.params["filter"]
                            )
                            + msg1
            ...
            msg += "Error searching for files named " + os.path.join(
                self.params["readdir"], self.params["filter"]
            )
        self.ListBox.Clear()
    ...
    def IntegrateImage(self, img, useTA=None, useMask=None):
        ...
        if self.AutoScale:
            Comments = G2frame.GPXtree.GetItemPyData(
                G2gd.GetGPXtreeItemId(G2frame, imgId, "Comments")
            )
            for comment in Comments:
                if "=" in comment:
                    name, val = comment.split("=", 1)
        ...
        UpdateImageControls(
            G2frame, data, masks, useTA=useTA, useMask=useMask, IntegrateOnly=True
        )
        ...
        try:
            ...
            AttributeError
        ):  # not sure why, but sometimes imagefile is a list here (should not be)!
            s = re.split(
                r"(\d+)\Z", os.path.split(os.path.splitext(imagefile[0])[0])[1]
            )
        namepre = s[0]
        if len(s) > 1:
            namenum = s[1]
        ...
        for Id in G2frame.IntgOutList:  # loop over newly created PWDR entry(ies)
            ...
            Sdata = G2frame.GPXtree.GetItemPyData(
                ...
                if self.ImageType == "SASD":
                    ...
                    for obj in G2frame.exporterlist:
                        ...
                        if obj.extension == "." + fmt and "sasd" in obj.exporttype:
                            if hint and hint not in obj.formatName:
                                continue
                            try:
                    ...
                    G2IO.ExportPowder(
                        G2frame, treename, fil + ".x", "." + fmt, hint=hint
                    )  # dummy extension (.x) is replaced before write)
    ...
    def ResetFromTable(self, dist):
        ...
        interpDict, imgctrl, immask = self.Evaluator(
            dist
        )  # interpolated calibration values
        if GSASIIpath.GetConfigValue("debug"):
        ...
        if immask:
            ...
            if (
                list(self.ImageMasks["Thresholds"][0])
                == self.ImageMasks["Thresholds"][1]
            ):  # avoid copy of unchanged thresholds
    ...
    def StartLoop(self):
        ...
        if self.params["Mode"] != "table":
            ...
            if (
                list(self.Thresholds[0]) == self.Thresholds[1]
            ):  # avoid copy of unchanged thresholds
                del self.ImageMasks["Thresholds"]
        # make sure all output directories exist
        if self.params["SeparateDir"]:
            for dfmt in self.fmtlist[0]:
                if not self.params[dfmt[1:]]:
                    continue
        ...
        if self.params["ComputePDF"]:
            ...
            for key in ["Sample Bkg.", "Container", "Container Bkg."]:
                ...
                if not G2gd.GetGPXtreeItemId(G2frame, G2frame.root, name):
                    indx = G2G.ItemSelector(
                        fileList,
                        self,
                        header="Select PWDR item",
                        title="Select a PWDR tree item for "
                        + key
                        + "\n(or cancel to quit)",
                    )
                    if indx is None:
    ...
    def OnTimerLoop(self, event):  # noqa: ARG002
        ...
        def AutoComputePDF(imgId):
            ...
            for pwdr in G2frame.AutointPWDRnames[:]:
                if not pwdr.startswith("PWDR "):
                    continue
                if pwdr in G2frame.AutointPDFnames:
                    continue
                ...
                if (
                    self.params["pdfDmax"] != 0
                    and controlsDict["distance"] > self.params["pdfDmax"]
                ):
                ...
                Parms = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, PWid, "Instrument Parameters")
                )[0]
                fullLimits = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, PWid, "Limits")
                )[0]
                ...
                for item in Comments:  # grab chemical formula from Comments, if there
                    if "formula" in item[:15].lower():
                        ...
                        for [elem, num] in formula:
                            ...
                            sumnum += float(num)
                            ElList[elem] = ElData
                PDFnames = G2gd.GetGPXtreeDataNames(
                    G2frame,
                    [
                        "PDF ",
                    ],
                )
                PDFid = G2obj.CreatePDFitems(
                    G2frame, pwdr, ElList.copy(), Qlimits, sumnum, pwdrMin, PDFnames
                )
                if not PDFid:
                    continue
                PDFdata = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, PDFid, "PDF Controls")
                )
                PDFdata.update(copy.deepcopy(self.pdfControls))
                if ElList:
                    PDFdata["ElList"] = (
                        ElList  # override with formula from comments, if present
                    )
        ...
        for img in G2gd.GetGPXtreeDataNames(G2frame, ["IMG "]):
            ...
            if img in G2frame.IntegratedList:
                continue
            Data = G2frame.GPXtree.GetItemPyData(
                G2gd.GetGPXtreeItemId(G2frame, imgId, "Image Controls")
            )
            ...
            if not sameTA:
                ...
                print(
                    f" Use new image controls; xy->th,azm mtime: {time.time() - t0:.3f}"
                )
            Mask = G2frame.GPXtree.GetItemPyData(
                G2gd.GetGPXtreeItemId(G2frame, imgId, "Masks")
            )
        ...
        if self.useTA is None or self.useMask is None:
            print(
                "Integration will not be fast; there is no beginning image controls"
            )  # TODO: work this out??
        for newImage in self.currImageList:
            if newImage in imageFileList or self.Pause:
...
class IntegParmTable(wx.Dialog):
    ...
    nonInterpVars = (
        ...
        "IOtth_max",
        "outChannels",
    )  # values in this list are taken from nearest rather than interpolated
    HeaderList = (
        "Set Dist",
        "Calib Dist",
        "X cntr",
    ...
    def __init__(self, parent, parms=None, IMfileList=None, readFileList=None):
        ...
        pth = ""
        wx.Dialog.__init__(
            self, parent, style=wx.RESIZE_BORDER | wx.DEFAULT_DIALOG_STYLE
        )
        if readFileList:
            ...
            self.IMfileList = (
                IMfileList  # list of .imctrl file names for each entry in table
            )
        else:
            ...
            try:
                ...
                dlg = wx.FileDialog(
                    parent,
                    "Read previous table or build new table by selecting image control files",
                    pth,
            ...
            if not files:
                ...
                return
        mainSizer = wx.BoxSizer(wx.VERTICAL)
        self.list = ImgIntLstCtrl(
            self, wx.ID_ANY, style=wx.LC_REPORT | wx.BORDER_SUNKEN
        )
    ...
    def ReadFiles(self, files):
        ...
        if os.path.splitext(files[0])[1] == ".imtbl":
            ...
            m1 = [
                i for i, f in enumerate(tmpDict["filenames"]) if not os.path.exists(f)
            ]
            if m1:
                print("\nimctrl file not found:")
                for i in m1:
                    print("\t#" + str(i) + ": " + tmpDict["filenames"][i])
            m2 = [
                i
                for i, f in enumerate(tmpDict["maskfile"])
                if not (os.path.exists(f) or f.startswith("("))
            ]
            if m2:
                print("\nmask file not found")
                for i in m2:
                    print("\t#" + str(i) + ": " + tmpDict["maskfile"][i])
            ...
            m.reverse()
            for c in m:
                for key in tmpDict:  # noqa: PLC0206
                    del tmpDict[key][c]
            ...
            for key in self.ParmList:
                try:
                    ...
                    parms.append(
                        [
                            str(G2fil.FormatSigFigs(val1, sigfigs=5))
                            for val1 in tmpDict[key]
                        ]
                    )
                except ValueError:
    ...
    def _onClose(self, event):  # noqa: ARG002
...
class ImgIntLstCtrl(wx.ListCtrl, listmix.ListCtrlAutoWidthMixin, listmix.TextEditMixin):
    ...
    def FillList(self, parms):
        ...
        for i, lbl in enumerate(self.parent.ParmList):  # noqa: B007
            self.SetColumnWidth(i, wx.LIST_AUTOSIZE)

    def OnDouble(self, evt):  # noqa: ARG002
        ...
        try:
            dlg = wx.FileDialog(
                self,
                "Select mask or control file to add (Press cancel if none)",
                pth,
                style=wx.FD_OPEN,
        ...
        if os.path.splitext(fil)[1] != ".imctrl":
            ...
            for c, lbl in enumerate(self.parent.ParmList):
                try:
                    ...
                    vali = imgDict[lbl]
                if abs(closeX - dist) < 1.0:  # distance is within 1 mm, replace
...
def testColumnMetadata(G2frame):
    ...
    if not GSASIIpath.GetConfigValue("Column_Metadata_directory"):
        G2G.G2MessageBox(
            G2frame,
            "The configuration option for I-ID Metadata is not set.\n"
            + "Please use the File/Preferences menu to set Column_Metadata_directory",
            "Warning",
        )
        return
    parFiles = glob.glob(
        os.path.join(GSASIIpath.GetConfigValue("Column_Metadata_directory"), "*.par")
    )
    if not parFiles:
        G2G.G2MessageBox(
            G2frame,
            "No .par files found in directory {}. ".format(
    ...
    if len(parList) == 0:
        G2G.G2MessageBox(
            G2frame,
            "No .lbls or .EXT_lbls file found for .par file(s) in directory {}. ".format(
                GSASIIpath.GetConfigValue("Column_Metadata_directory")
            )
            + "\nThis is set by config variable Column_Metadata_directory "
            + "(Set in File/Preferences menu).",
        ...
        dlg = G2G.G2SingleChoiceDialog(
            ...
            + GSASIIpath.GetConfigValue("Column_Metadata_directory"),
            "Choose .par file",
            [os.path.split(i)[1] for i in parList],
        )
        if dlg.ShowModal() == wx.ID_OK:
    ...
    if len(lblList) == 1:
        ...
        dlg = G2G.G2SingleChoiceDialog(
            ...
            "Choose label file",
            [os.path.split(i)[1] for i in lblList],
        )
        if dlg.ShowModal() == wx.ID_OK:
    ...
    while errors:
        ...
        if errors:
            ...
            dlg = wx.MessageDialog(
                ...
                message=t,
                caption="Read Error",
                style=wx.ICON_ERROR | wx.OK | wx.STAY_ON_TOP | wx.CANCEL,
            )
            if dlg.ShowModal() != wx.ID_OK:
                return
    ...
    n = ["Named parameters:"]
    l = ["", "Labeled columns:"]  # noqa: E741
    for key in sorted(metadata):
        if key == "filename" or key.startswith("label_prm"):
            continue
        if key in keyCols:
            n += [f"  {key} = {metadata[key]}"]
        elif key in lbldict.values():
            l += [f"  {key} = {metadata[key]}"]  # noqa: E741
        else:
            t += [f"** Unexpected: {key}:{metadata[key]}"]
    if type(metadata["filename"]) is str:
        l += ["", "Filename: " + metadata["filename"]]  # noqa: E741
    else:
        l += ["", "Filename(s): "]  # noqa: E741
...
def selectPhase(G2frame, calList, RefreshPlot):
    ...
    if "phaseColors" not in phaseOpts:
        ...
        for color in GSASIIpath.GetConfigValue("Ref_Colors", getDefault=True).split():
            ...
            if invalid_colors:  # show error once
                print(
                    f'**** bad color code(s): "{", ".join(invalid_colors)}" - redo Preferences/Ref_Colors ****'
                )
    ...
    mainSizer.Add(
        wx.StaticText(
            dlg,
            wx.ID_ANY,
            "Set options for superimposing phases on image",
    ...
    txt = wx.StaticText(
        dlg,
        wx.ID_ANY,
        "Phases listed below are those imported into the current project followed by defined calibrants.",
    )
    txt.Wrap(510)
    mainSizer.Add(txt)
    mainSizer.Add((-1, 10))
...
def initPhaseOpts(phases):
    ...
    for p in phases:
        ...
        phaseOpts[p]["style"] = phaseOpts[p].get(
            "style", 2
        )  # index in ltypeChoices/ltypeMPLname (see below)
...
def displayPhase(G2frame, phList, panel, gsizer, headers, RefreshPlot):
    ...
    for p in phList:
        ...
        if phaseOpts[p]["Show"]:
            ...
            gsizer.Add(ch, 0, wx.ALIGN_CENTER)
            ch = G2G.G2ChoiceButton(
                panel, ltypeChoices, phaseOpts[p], "style", None, None, StyleChange
            )
            gsizer.Add(ch, 0, wx.ALIGN_CENTER)
        else:
            b = (-1, -1)
...
if __name__ == "__main__":
    ...
    def RefreshPlot(*args):  # noqa: ARG001
        print("plot refresh")
