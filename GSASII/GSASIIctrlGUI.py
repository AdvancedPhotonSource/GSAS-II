...
class MultiIntegerDialog(wx.Dialog):
    ...
    def Draw(self):
        ...
        for ival, [prompt, value] in enumerate(
            ...
            valItem = wx.TextCtrl(
                self.panel, -1, value="%d" % (value), style=wx.TE_PROCESS_ENTER
            )
            mainSizer.Add(valItem, 0, wx.ALIGN_CENTER)
...
class SGMessageBox(wx.Dialog):
    ...
    def __init__(
        ...
        for item in self.table:
            ...
            num, flds = item.split(")")
            tableSizer.Add(
                wx.StaticText(self.panel, label="     %s  " % (num + ")")),
                0,
                WACV | wx.ALIGN_LEFT,
            )
        ...
        for ng in range(nG):
            ...
            for ic, cent in enumerate(cents):
                ...
                for item in self.table:
                    ...
                    flds = item.split(")")[1]
                    tableSizer.Add(
                        wx.StaticText(self.panel, label="  (%2d)  " % (j + 1)), 0, WACV
                    )
                    flds = flds.replace(" ", "").split(",")
                    for i, fld in enumerate(flds):
                        if i < ncol - 1:
...
def viewWebPage(parent, URL, size=(750, 450), newFrame=False, HTML=""):
    ...
    dlg.wv.Bind(wx.EVT_UPDATE_UI, lambda event: dlg.SetTitle(dlg.wv.GetCurrentTitle()))
    # lastWebView = dlg.wv
