    ...
    def ReadINSPhase(self, filename, parent=None):
        ...
        while S:
            ...
            if "TITL" in S[:4].upper():
                ...
                S[:4].strip().upper() not in Shelx
            ):  # this will find an atom record or an extended title!
                if ifTITL:
                    Title += S.strip()
                else:
                    AtRec = S.split()
                    ...
                    if "=" not in S:
                        IA = "I"
                        Uiso = float(AtRec[6])
                        if Uiso < 0.0 or Uiso > 1.0:
                            Uiso = 0.025
                        Uij = [0.0 for i in range(6)]
                    else:
                        IA = "A"
                        Uiso = 0.0
                        Ustr = AtRec[6:8]
