...
def UpdateSeqResults(G2frame, data, prevSize=None):
    ...
    def PlotSelectedColRow(calltyp="", event=None):
        ...
        if cols and calltyp == "left":
            ...
            if name.startswith("PWDR"):
                ...
                G2frame.PickId = G2frame.PatternId = G2gd.GetGPXtreeItemId(
                    G2frame, G2frame.root, name
                )
                ...
                PFdata = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, G2frame.PickId, "PDF Controls")
                )
    ...
    def SetLabelString(col):
        ...
        dlg = G2G.SingleStringDialog(
            G2frame, "Set variable label", head, lbl, size=(400, -1)
        )
        if dlg.Show():
    ...
    def OnRenameSelSeq(event):
        ...
        if not cols:
            ...
            return
        dlg = G2G.MultiStringDialog(
            G2frame.dataDisplay, "Set column names", colNames, newNames
        )
        if dlg.Show():
    ...
    def OnSaveSelSeq(event, csv=False, allcols=False):
            ...
            for row, name in enumerate(saveNames):
                ...
                for col in cols:
                    if saveData[col][row] is None:
                        ...
                        line += (
                            ","
                            + str(saveData[col][row])
                            + ","
                            + str(saveSigs[col][row])
                        )
                    else:
        ...
        if allcols:
            ...
            cols = sorted(
                G2frame.dataDisplay.GetSelectedCols()
            )  # ignore selection order
        nrows = G2frame.SeqTable.GetNumberRows()
        if not cols:
            choices = [
                G2frame.SeqTable.GetColLabelValue(r)
                for r in range(G2frame.SeqTable.GetNumberCols())
            ]
            dlg = G2G.G2MultiChoiceDialog(
                G2frame, "Select columns to write", "select columns", choices
            )
            # dlg.SetSelections()
            if dlg.ShowModal() == wx.ID_OK:
    ...
    def UpdateParmDict(parmDict):
        ...
        for item in parmDict:
            ...
            if key[0] and key[0] not in phaselist:
                phaselist.append(key[0])
        ...
        for i in phaselist:
            ...
            try:
                A, zeros = G2stIO.cellFill(
                    str(pId) + "::", SGdata[pId], parmDict, zeroDict[pId]
                )
    ...
    def EvalPSvarDeriv(calcobj, parmDict, sampleDict, var, ESD):
        ...
        if None in results:
            return None
        return (results[0] - results[1]) / (2.0 * step)
    ...
    def ParEqEval(Values, calcObjList, varyList):
        ...
        for calcobj in calcObjList:
            ...
            if calcobj.depSig:
                result.append(
                    (calcobj.depVal - calcobj.EvalExpression()) / calcobj.depSig
                )
            else:
    ...
    def DoParEqFit(event, eqObj=None):
        ...
        for fitnum, obj in enumerate(eqObjList):
            ...
            G2plt.PlotSelectedSequence(
                G2frame, [indx], GetColumnInfo, SelectXaxis, fitnum, fitvals
            )
    ...
    def DelParFitEq(event):
        ...
        txtlst = [
            obj.GetDepVar() + " = " + obj.expression for obj in data["SeqParFitEqList"]
        ]
        selected = G2G.ItemSelector(
            txtlst,
            G2frame,
        ...
        data["SeqParFitEqList"] = [
            obj for i, obj in enumerate(data["SeqParFitEqList"]) if i not in selected
        ]
        EnableParFitEqMenus()
        if data["SeqParFitEqList"]:
            DoParEqFit(event)
    ...
    def AddNewParFitEq(event):
        ...
        for obj in data["SeqParFitEqList"]:
            for var in obj.freeVars:
                if obj.freeVars[var][0] not in usedvarlist:
                    usedvarlist.append(obj.freeVars[var][0])
    ...
    def CopyParFitEq(event):
        ...
        if selected is not None:
            ...
            for var in newEqn.freeVars:
                newEqn.freeVars[var][0] = G2obj.MakeUniqueLabel(
                    newEqn.freeVars[var][0], usedvarlist
                )
            dlg = G2exG.ExpressionDialog(
                G2frame.dataDisplay,
                VarDict,
    ...
    def GridSetToolTip(row, col):
        ...
        if G2frame.colSigs[col]:
            if G2frame.colSigs[col][row] == -0.1:
                return "frozen"
    ...
    def onSelectSeqVars(event):
        ...
        hides = [
            ...
            for item in G2frame.SeqTblHideList
            if item in saveColLabels[2:]
        ]
        dlg = G2G.G2MultiChoiceDialog(
            G2frame, "Select columns to hide", "Hide columns", saveColLabels[2:]
        )
        dlg.SetSelections(hides)
        if dlg.ShowModal() == wx.ID_OK:
            G2frame.SeqTblHideList = [
                saveColLabels[2:][sel] for sel in dlg.GetSelections()
            ]
            dlg.Destroy()
            UpdateSeqResults(
                G2frame, data, G2frame.dataDisplay.GetSize()
        ...
        for phase in Phases:
            ...
            for i, at in enumerate(Atoms):
                names = {
                    ...
                    cx + 1: pfx + "Ay:" + str(i),
                    cx + 2: pfx + "Az:" + str(i),
                    cx + 3: pfx + "Afrac:" + str(i),
                    cia + 1: pfx + "AUiso:" + str(i),
                    cia + 2: pfx + "AU11:" + str(i),
                    cia + 3: pfx + "AU22:" + str(i),
                    cia + 4: pfx + "AU33:" + str(i),
                    cia + 5: pfx + "AU12:" + str(i),
                    cia + 6: pfx + "AU13:" + str(i),
                    cia + 7: pfx + "AU23:" + str(i),
                    cx + 4: pfx + "AMx:" + str(i),
                    cx + 5: pfx + "AMy:" + str(i),
                    cx + 6: pfx + "AMz:" + str(i),
                }
    ...
    def OnEditSelectPhaseVars(event):
        ...
        dlg = G2G.G2MultiChoiceDialog(
            G2frame,
            "Choose phase parmDict item(s) to set" + lbl,
    ...
    if not len(Histograms) and not len(
        ...
        if "SeqPseudoVars" not in data:
            data["SeqPseudoVars"] = {}
        if "SeqParFitEqList" not in data:
            data["SeqParFitEqList"] = []
        histNames = [name for name in data["histNames"] if name in data]
    if G2frame.dataDisplay:
        G2frame.dataDisplay.Destroy()
    G2frame.GetStatusBar().SetStatusText(
        "Select column to export; LMB/RMB column to plot data/change label; LMB/RMB on row for PWDR/Covariance plot",
        1,
    )
    ...
    for i, name in enumerate(histNames):
        ...
        tmp = {
            striphist(var, "*"): tmp[var] for var in tmp
        }  # replace histogram #s with "*"
    ...
    if histNames[0][:4] not in ["SASD", "IMG ", "REFD"] and Controls.get("ShowCell"):
        G2frame.colList += [
            [100.0 * data[name]["Rvals"].get("DelChi2", -1) for name in histNames]
        ]
        G2frame.colSigs += [None]
        colLabels += ["\u0394\u03c7\u00b2 (%)"]
        Types += [
    ...
    if Controls.get("ShowCell", False) and len(newCellDict):
        ...
        for pId in sorted(RecpCellTerms):
            ...
            for name in histNames:
                ...
                covData = {
                    "varyList": [
                        dLookup.get(striphist(v), v) for v in data[name]["varyList"]
                    ],
                ...
                for i, j in enumerate(("D11", "D22", "D33", "D12", "D13", "D23")):
                    ...
                    if var in cellAlist:
                        try:
                            A[i] = data[name]["newCellDict"][esdLookUp[var]][
                                1
                            ]  # get refined value
                        except KeyError:
                            pass
                ...
                if name in Phases[phaseLookup[pId]]["Histograms"]:
                    ...
                    tmp = dict(
                        zip(
                            [pfx + G2lat.cellUlbl[i] for i in uniqCellIndx[pId]]
                            + [pfx + "Vol"],
    ...
    for phase in Phases:
        ISOlist += [
            i.varname()
            for i in Phases[phase].get("ISODISTORT", {}).get("G2ModeList", [])
    ...
    for expr in data["SeqPseudoVars"]:
        ...
        for seqnum, name in enumerate(histNames):
            ...
            if "Dist" in expr:
                derivs = G2mth.CalcDistDeriv(
                    obj.distance_dict, obj.distance_atoms, parmDict
                )
                ...
                varyNames = ["%d::dA%s:%d" % (pId, ip, aId) for ip in ["x", "y", "z"]]
                varyNames += [
                    "%d::dA%s:%d" % (pId, ip, bId[0]) for ip in ["x", "y", "z"]
                ]
                varyNames += [
                    "%d::dA%s:%d" % (pId, ip, bId[1]) for ip in ["x", "y", "z"]
                ]
                VCoV = G2mth.getVCov(varyNames, varyList, data[name]["covMatrix"])
                esdList.append(np.sqrt(np.inner(derivs, np.inner(VCoV, derivs.T))))
            else:
                derivs = np.array(  # TODO: this needs to be reworked
                    [
                        EvalPSvarDeriv(
                            calcobj, parmDict.copy(), sampleDict[name], var, ESD
                        )
                ...
                if None in list(derivs):
                    ...
                    esdList.append(
                        np.sqrt(
                            np.inner(
                                derivs, np.inner(data[name]["covMatrix"], derivs.T)
                            )
                        )
                    )
    ...
    PSvarDict = {
        i: PSvarDict[i] for i in PSvarDict if not (remDij.match(i) or remdAxyz.match(i))
    }
    ...
    if G2frame.SeqTblHideList is None:  # set default hides
        G2frame.SeqTblHideList = [
            item for item in saveColLabels if "Back" in item and ifPWDR
        ]
    ...
    G2frame.SeqTable = G2G.Table(
        [
            list(cl) for cl in zip(*G2frame.colList, strict=False)
        ],  # convert from columns to rows
        colLabels=displayLabels,
        rowLabels=rowLabels,
        types=Types,
    )
            ...
            try:
                if deltaChi > 10.0:
                    G2frame.dataDisplay.SetCellStyle(
                        row, deltaChiCol, color=wx.Colour(255, 0, 0)
                    )
                elif deltaChi > 1.0:
                    G2frame.dataDisplay.SetCellStyle(
...
def UpdateClusterAnalysis(G2frame, ClusData, shoNum=-1):
    ...
    def FileSizer():
        def OnSelectData(event):
                ...
                for name in names:
                    ...
                    if "PWDR" in name:
                        ...
                        PDFControls = G2frame.GPXtree.GetItemPyData(
                            G2gd.GetGPXtreeItemId(G2frame, item, "PDF Controls")
                        )
            ...
            if len(choices) == 0:
                G2G.G2MessageBox(
                    G2frame,
                    "No PWDR or PDF histograms found for cluster analysis.",
                    "No Histograms",
                )
                return
            sel = []
            try:
                if "Cluster Data" in ClusData:
                    sel = [
                        choices.index(item)
                        for item in ClusData["Cluster Data"]["Files"]
                    ]
            ...
            dlg = G2G.G2MultiChoiceDialog(
                ...
                "Cluster analysis data selection",
                choices,
            )
            ...
            if dlg.ShowModal() == wx.ID_OK:
                for sel in dlg.GetSelections():
                    ...
                    if Type != choices[sel].split()[0]:
                        G2G.G2MessageBox(
                            G2frame,
                            "Histogram types not all the same; revise selection",
    ...
    def LimitSizer():
        ...
        limitSizer.Add(
            wx.StaticText(
                G2frame.dataWindow, label="Enter cluster analysis data limits: "
            ),
            0,
            WACV,
        )
        limitSizer.Add(
            G2G.ValidatedTxtCtrl(
                ...
                0,
                nDig=(10, 3),
                xmin=ClusData["Limits"][0][0],
                xmax=ClusData["Limits"][0][1],
                OnLeave=CheckLimits,
            ),
            0,
            WACV,
    ...
    def MethodSizer():
        ...
        def OnCompute(event):
            if "minkowski" in ClusData["Method"]:
                ...
                ClusData["ConDistMat"] = SSD.pdist(
                    ClusData["DataMatrix"], ClusData["Method"]
                )
            wx.CallAfter(UpdateClusterAnalysis, G2frame, ClusData)
        ...
        methsizer.Add(
            wx.StaticText(
                G2frame.dataWindow, label="Select cluster analysis distance method: "
            ),
            0,
            WACV,
        )
        method = wx.ComboBox(
            G2frame.dataWindow, choices=choice, style=wx.CB_READONLY | wx.CB_DROPDOWN
        )
        ...
        if "minkowski" in ClusData["Method"]:
            methsizer.Add(
                wx.StaticText(G2frame.dataWindow, label=" exponent: "), 0, WACV
            )
            choicep = ["1", "2", "3", "4", "10"]
            minp = wx.ComboBox(
                G2frame.dataWindow,
    ...
    def HierSizer():
        ...
        def OnCompute(event):
            ClusData["CLuZ"] = SCH.linkage(
                ...
                optimal_ordering=ClusData["Opt Order"],
            )
            wx.CallAfter(UpdateClusterAnalysis, G2frame, ClusData)
        ...
        hierSizer.Add(
            wx.StaticText(
                G2frame.dataWindow,
                label="Hierarchical clustering: Select linkage method: ",
            ),
            0,
            WACV,
        )
    ...
    def kmeanSizer():
        ...
        kmeanssizer.Add(
            wx.StaticText(
                G2frame.dataWindow,
                label="K-means clustering: select number of clusters (2-15): ",
            ),
            0,
            WACV,
        )
        numclust = wx.ComboBox(
            G2frame.dataWindow, choices=choice, style=wx.CB_READONLY | wx.CB_DROPDOWN
        )
    ...
    def OnPlotSel(event):
        ...
        if ClusData["plots"] == "Suprise":
            ...
            G2plt.PlotClusterXYZ(
                G2frame,
                YM,
                XYZ,
                ClusData,
                PlotName=ClusData["Method"],
                Title=ClusData["Method"],
            )
    ...
    def ScikitSizer():
        ...
        def OnCompute(event):
            ...
            if ClusData["Scikit"] == "K-Means":
                ...
                print(
                    "Number of Mean-shift clusters found: %d"
                    % (np.max(result.labels_) + 1)
                )
            elif ClusData["Scikit"] == "Affinity propagation":
                result = SKC.AffinityPropagation(
                    affinity="precomputed", damping=0.5
                ).fit(SSD.squareform(ClusData["ConDistMat"]))
                print(
                    "Number of Affinity propagation clusters found: %d"
                    % (np.max(result.labels_) + 1)
                )
            elif ClusData["Scikit"] == "Agglomerative clustering":
                result = SKC.AgglomerativeClustering(
                    n_clusters=ClusData["NumClust"],
        ...
        def Metrics(whitMat, result):
            if np.max(result.labels_) >= 1:
                ...
                print(
                    "number of clusters found must be > 1 for metrics to be determined"
                )
                return None
        ...
        txt = wx.StaticText(
            G2frame.dataWindow,
            label="If you use Scikit-Learn Cluster Analysis, please cite: "
        ...
        if ClusData["Scikit"] in [
            ...
            clusSizer.Add(
                wx.StaticText(
                    G2frame.dataWindow, label=" Select number of clusters (2-15): "
                ),
                0,
                WACV,
            )
    ...
    def memberSizer():
        ...
        Colors = [
            ...
            ["xkcd:green", 0x00A000],
            ["xkcd:cyan", 0xD0D000],
            ["xkcd:magenta", 0xA000A0],
            ["xkcd:black", 0x000000],
            ["xkcd:pink", 0xB469FF],
            ["xkcd:brown", 0x13458B],
            ["xkcd:teal", 0x808000],
            ["xkcd:orange", 0x008CFF],
            ["xkcd:grey", 0x808080],
            ["xkcd:violet", 0xE22B8A],
            ["xkcd:aqua", 0xAAAA00],
            ["xkcd:blueberry", 0xCD5A6A],
            ["xkcd:bordeaux", 0x00008B],
        ]
        ...
        memSizer.Add(
            wx.StaticText(
                G2frame.dataWindow,
                label="Cluster populations (colors refer to cluster colors in PCA plot):",
            )
        )
        ...
        headSizer.Add(
            wx.StaticText(G2frame.dataWindow, label="Select cluster to list members: "),
            0,
            WACV,
        )
        ...
        if shoNum >= 0:
            memSizer.Add(
                wx.StaticText(
                    G2frame.dataWindow,
                    label="Members of cluster %d (select to show data plot):" % shoNum,
                )
            )
            ClusList = []
    ...
    def outlierSizer():
        ...
        def OnCompute(event):
            if ClusData["OutMethod"] == "One-Class SVM":
                ...
                ClusData["codes"] = SKE.IsolationForest().fit_predict(
                    ClusData["DataMatrix"]
                )
            elif ClusData["OutMethod"] == "Local Outlier Factor":
                ClusData["codes"] = SKN.LocalOutlierFactor().fit_predict(
                    ClusData["DataMatrix"]
                )
            wx.CallAfter(UpdateClusterAnalysis, G2frame, ClusData, shoNum)
        ...
        outSizer.Add(
            wx.StaticText(
                G2frame.dataWindow,
                label="Outlier (bad data) analysis with Scikit-learn:",
            )
        )
    ...
    if len(ClusData["Files"]):
        ...
        mainSizer.Add(
            wx.StaticText(
                G2frame.dataWindow,
                label="Cluster Analysis input data size: %d" % (GetYMatSize()),
            )
        )
        mainSizer.Add(
            wx.StaticText(
        ...
        makeArray = wx.Button(
            G2frame.dataWindow, label="Make Cluster Analysis data array"
        )
        ...
        if len(ClusData["DataMatrix"]):
            ...
            mainSizer.Add(
                wx.StaticText(G2frame.dataWindow, label="Distance Cluster Analysis:")
            )
            ...
            if len(ClusData["ConDistMat"]):
                ...
                mainSizer.Add(
                    wx.StaticText(
                        G2frame.dataWindow, label="Hierarchical Cluster Analysis:"
                    )
                )
                ...
                G2G.HorizontalLine(mainSizer, G2frame.dataWindow)
                mainSizer.Add(
                    wx.StaticText(G2frame.dataWindow, label="K-means Cluster Analysis:")
                )
            ...
            plotSizer.Add(
                wx.StaticText(G2frame.dataWindow, label="Plot selection: "), 0, WACV
            )
            ...
            if ClusData["SKLearn"] and len(ClusData["ConDistMat"]):
                ...
                subSizer.Add(
                    wx.StaticText(
                        G2frame.dataWindow, label="Scikit-Learn Cluster Analysis: "
                    ),
                    0,
                    WACV,
                )
        ...
        if ClusData["SKLearn"] and len(ClusData["DataMatrix"]) > 15:
            ...
            if Nout > 0:
                mainSizer.Add(
                    wx.StaticText(
                        G2frame.dataWindow,
                        label="%d Probable outlier data found by %s (select to show data plot):"
                        % (Nout, ClusData["OutMethod"]),
                    )
                )
                OutList = []
                for i, item in enumerate(ClusData["Files"]):
                ...
                mainSizer.Add(
                    wx.StaticText(G2frame.dataWindow, label="No outlier data found")
                )
