...
except:  # Don't depend on GUI  # noqa: E722
...
class G2TreeCtrl(wx.TreeCtrl):
    ...
    def __init__(self, parent=None, *args, **kwargs):
        super(self.__class__, self).__init__(parent=parent, *args, **kwargs)  # noqa: B026
    ...
    def AppendItem(self, parent, text, image=-1, selImage=-1, data=None):
        ...
        item = super(self.__class__, self).AppendItem(
    ...
    def ConvertRelativeHistNum(self, histtype, histnum):
        ...
        while item:
            ...
            item, cookie = self.GetNextChild(self.root, cookie)
        raise Exception(
            "Histogram #'+str(histnum)+' of type " + histtype + " not found"
        )
...
class ValidatedTxtCtrl(wx.TextCtrl):
    ...
    def __init__(
        ...
        **kw,
        ...
        self.evaluated = (
            False  # set to True when the validator recognizes an expression
        )
        ...
        if typeHint is not None:
            ...
            raise Exception(
                "ValidatedTxtCtrl error: value of "
                + str(key)
                + " element is None and typeHint not defined as int or float"
            )
        else:
            raise Exception(
                "ValidatedTxtCtrl error: Unknown element ("
                + str(key)
                + ") type: "
                + str(type(val))
            )
    ...
    def _setValue(self, val, show=True):
        ...
        if self.type is int:
            ...
            if self.nDig and show and not self.invalid:
                ...
                self.evaluated = (
                    False  # expression has been recast as value, reset flag
                )
    ...
    def _SaveStringValue(self):
        ...
        if (
            self.CIFinput and "2" in platform.python_version_tuple()[0]
        ):  # Py2/CIF make results ASCII
    ...
    def _onLeaveWindow(self, event):
        ...
        ):  # .IsModified() seems unreliable for str
            return
        if self.evaluated and not self.invalid:  # deal with computed expressions
            if self.timer:
                ...
                self.timer = wx.CallLater(
                    self.delay, delayedUpdate
                )  # this includes a try in case the widget is deleted
...
class NumberValidator(wxValidator):
    ...
    def __init__(
        ...
        if self.typ == int and self.positiveonly:  # noqa: E721
            self.validchars = "0123456789"
        elif self.typ == int:  # noqa: E721
            self.validchars = "0123456789+-%"
        elif self.typ == float:  # noqa: E721
    ...
    def TestValid(self, tc):
        ...
        try:
            ...
            if (
                self.typ is float
            ):  # for float values, see if an expression can be evaluated
    ...
    def OnChar(self, event):
        ...
        ):  # control characters get processed
            if event:
                event.Skip()
            if tc.invalid:
...
class G2Slider(wx.Slider):
    ...
    global ci  # noqa: PLW0603
    ci = lambda x: int(x + 0.5)  # closest integer  # noqa: E731

    def __init__(
        self, parent, id=wx.ID_ANY, value=0, minValue=0, maxValue=100, *arg, **kwarg
    ):
        wx.Slider.__init__(
            self, parent, id, ci(value), ci(minValue), ci(maxValue), *arg, **kwarg
        )
        self.iscale = 1
...
def G2SliderWidget(
    ...
    if sizer is None:
        ...
        hSizer = sizer
    hSizer.Add(
        wx.StaticText(parent, wx.ID_ANY, label), 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL
    )
...
def G2SpinWidget(
    ...
    def _onValSet(*args, **kwargs):  # noqa: ARG001
        if onChange:
            onChange(*onChangeArgs)
    ...
    if label:
        hSizer.Add(
            wx.StaticText(parent, wx.ID_ANY, label),
            0,
...
class popupSelectorButton(wx.Button):
    ...
    def popupSelector(self, event):  # noqa: ARG002
        ...
        for i in range(len(self.choices)):
            menuList.append(menu.Append(wx.ID_ANY, self.choices[i], "", wx.ITEM_CHECK))
            if self.selected[i]:
                menuList[-1].Check()
        self.parent.PopupMenu(menu)
        new = -1
        for i, m in enumerate(menuList):
            if self.selected[i] != m.IsChecked():
                new = i
            self.selected[i] = m.IsChecked()
        if self.choiceDict is not None:
            self.selected = len(self.selected) * [False]
            for i in self.choiceDict:
                self.choiceDict[i] = False
        ...
        menu.Destroy()
        if self.OnChange:
            wx.CallAfter(self.OnChange, **self.kw)
...
class G2ChoiceButton(wx.Choice):
    ...
    def _OnChoice(self, event):  # noqa: ARG002
        if self.indLoc is not None:
            self.indLoc[self.indKey] = self.GetSelection()
...
def G2RadioButtons(parent, loc, key, choices, values=None, OnChange=None):
    ...
    for i, c in enumerate(choices):
        ...
        if loc[key] == values[i]:
            buttons[-1].SetValue(True)
        buttons[-1].Bind(wx.EVT_RADIOBUTTON, _OnEvent)
    return buttons


#### Commonly used dialogs ################################################################################
def CallScrolledMultiEditor(
    parent,
    dictlst,
    elemlst,
    prelbl=None,
    ...
    *Example definitions for dictlst and elemlst:*
    ...
    def __init__(
        ...
        if len(checkdictlst) != len(checkelemlst):
            raise Exception(
                "ScrolledMultiEditor error: len(checkdictlst) != len(checkelemlst) "
                + str(len(checkdictlst))
                + " != "
                + str(len(checkelemlst))
            )
        ...
        self.StartCheckValues = [
            checkdictlst[i][checkelemlst[i]] for i in range(len(checkdictlst))
        ]
        self.ButtonIndex = {}
        ...
        for i, (d, k) in enumerate(zip(dictlst, elemlst, strict=False)):
            ...
            kargs = {}
            if i < len(minvals):
                if minvals[i] is not None:
                    kargs["xmin"] = minvals[i]
            if i < len(maxvals):
                if maxvals[i] is not None:
                    kargs["xmax"] = maxvals[i]
            if i < len(sizevals):
                if sizevals[i]:
                    kargs["size"] = sizevals[i]
            if CopyButton:
                if i + 1 == len(dictlst):
                    but = (-1, -1)
                else:
                    ...
                    but = wscs.ColourSelect(
                        label="v",  # would like to use u'\u2193' or u'\u25BC' but not in WinXP
                        parent=panel,
                        colour=(255, 255, 200),
                        size=wx.Size(30, 23),
                    ...
                    if "phoenix" in wx.version():
                        ...
                        but.SetToolTipString(
                            "Press to copy adjacent value to all rows below"
                        )
            ...
            if (
                ...
                print(
                    f"ScrolledMultiEditor Warning: initial space in {d[k]!r} may cause problems"
                )
        ...
        if int(i) + int(j) / 10.0 > 2.8 and "wxOSX" in wx.PlatformInfo:
    ...
    def _OnCopyButton(self, event):
        ...
        if n is None:
            return
        for i, (d, k, ctrl) in enumerate(
            zip(self.dictlst, self.elemlst, self.ValidatedControlsList, strict=False)
        ):
            if i < n:
                continue
            if i == n:
        ...
        for i in range(len(self.checkdictlst)):
            if i < n:
                continue
            self.checkdictlst[i][self.checkelemlst[i]] = self.checkdictlst[n][
                self.checkelemlst[n]
            ]
            self.CheckControlsList[i].SetValue(
                self.checkdictlst[i][self.checkelemlst[i]]
            )
...
class G2MultiChoiceDialog(wx.Dialog):
    ...
    def __init__(
        ...
        **kw,
        ...
        ]  # numbered list of choices (list of str values)
        self.Selections = len(self.ChoiceList) * [
            False,
        ]  # selection status for each choice (list of bools)
        for i in selected:
        ...
        if filterBox:
            ...
            topSizer.Add((10, -1))
            topSizer.Add(
                wx.StaticText(self, wx.ID_ANY, "Name \nFilter: "), 0, wx.ALL | WACV, 1
            )
            self.filterBox = wx.TextCtrl(
                self, wx.ID_ANY, size=(80, -1), style=wx.TE_PROCESS_ENTER
            )
        ...
        self.clb = wx.CheckListBox(
            self, wx.ID_ANY, (30, 30), wx.DefaultSize, self.ChoiceList
        )
        self._ShowSelections()
        self.clb.Bind(wx.EVT_CHECKLISTBOX, self.OnCheck)
        if monoFont:
            font1 = wx.Font(
                self.clb.GetFont().GetPointSize(),
                wx.MODERN,
                wx.NORMAL,
                wx.NORMAL,
                False,
        ...
        if toggle:
            ...
            self.stride = wx.ComboBox(
                self, value="1", choices=numbs, style=wx.CB_READONLY | wx.CB_DROPDOWN
            )
    ...
    def OnStride(self, event):  # noqa: ARG002
        self.Stride = int(self.stride.GetValue())

    def SetRange(self, event):  # noqa: ARG002
        ...
        if "phoenix" in wx.version():
            self.clb.SetCheckedItems(
                [
                    i
                    for i in range(len(self.filterlist))
                    if self.Selections[self.filterlist[i]]
                ]
    ...
    def _SetAll(self, event):  # noqa: ARG002
    ...
    def _ToggleAll(self, event):  # noqa: ARG002
    ...
    def OnCheck(self, event):
        ...
        if self.settingRange:
            ...
            if self.rangeFirst is None:
                ...
                for i in range(
                    min(self.rangeFirst, id), max(self.rangeFirst, id) + 1, self.Stride
                ):
...
class G2MultiChoiceWindow(wx.BoxSizer):
    ...
    def __init__(
        ...
        if filterBox:
            ...
            self.timer.Bind(wx.EVT_TIMER, self.Filter)
            topSizer.Add(
                wx.StaticText(self.frm, wx.ID_ANY, "Name \nFilter: "),
                0,
                wx.ALL | WACV,
                1,
            )
            self.filterBox = wx.TextCtrl(
                self.frm, wx.ID_ANY, size=(80, -1), style=wx.TE_PROCESS_ENTER
            )
            self.filterBox.Bind(wx.EVT_TEXT, self.onChar)
            self.filterBox.Bind(wx.EVT_TEXT_ENTER, self.Filter)
            topSizer.Add(self.filterBox, 0, wx.ALL | WACV, 0)
        ...
        self.rangeFirst = None
        self.clb = wx.CheckListBox(
            self.frm, wx.ID_ANY, (30, 30), wx.DefaultSize, self.ChoiceList
        )
        self.clb.Bind(wx.EVT_CHECKLISTBOX, self.OnCheck)
        if monoFont:
            font1 = wx.Font(
...
def SelectEdit1Var(G2frame, array, labelLst, elemKeysLst, dspLst, refFlgElem):
    ...
    def OnChoice(event):
        ...
        Val = ValidatedTxtCtrl(
            dlg, unkey(array, elemKeysLst[i][:-1]), elemKeysLst[i][-1], **args
        )
    ...
    subSizer.Add((-1, -1), 1, wx.EXPAND)
    subSizer.Add(
        wx.StaticText(dlg, wx.ID_ANY, "Select a parameter and set a new value")
    )
    subSizer.Add((-1, -1), 1, wx.EXPAND)
    mainSizer.Add(subSizer, 0, wx.EXPAND, 0)
    mainSizer.Add((0, 10))
    ...
    dlg = G2MultiChoiceDialog(
        G2frame, "Copy parameter " + lbl + " from\n" + hst, "Copy parameters", histList
    )
    dlg.CenterOnParent()
    try:
        if dlg.ShowModal() == wx.ID_OK:
    ...
    hstData = G2frame.GPXtree.GetItemPyData(
    ...
    for h in copyList:
        ...
        if (
            len(hstData) != len(instData) or hstData["Type"][0] != instData["Type"][0]
        ):  # don't mix data types or lam & lam1/lam2 parms!
            print(h + " not copied - instrument parameters not commensurate")
            continue
        hData = G2frame.GPXtree.GetItemPyData(
            G2gd.GetGPXtreeItemId(G2frame, Id, TreeItemType)
        )
...
class G2SingleChoiceDialog(wx.Dialog):
    ...
    def __init__(
        self, parent, title, header, ChoiceList, monoFont=False, filterBox=True, **kw
    ):
        # process keyword parameters, notably style
        options = {
            "size": (320, 310),  # default Frame keywords
        ...
        topSizer = wx.BoxSizer(wx.HORIZONTAL)
        h = max(
            35, 17 * int(len(title) / 26.0 + 1)
        )  # adjust height of title box with guessed # of lines
        topSizer.Add(
    ...
    def onDoubleClick(self, event):  # noqa: ARG002
        self.EndModal(wx.ID_OK)
...
def ShowScrolledInfo(
    parent, txt, width=600, height=400, header="Warning info", buttonlist=None
):
...
def ShowScrolledColText(
    parent, txt, width=600, height=400, header="Warning info", col1len=999
):
...
def G2ScrolledGrid(
    G2frame, lbl, title, tbl, colLbls, colTypes, maxSize=(600, 300), comment=""
):
    ...
    dlg = wx.Dialog(
        G2frame, title=title, style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER
    )
    sizer = wx.BoxSizer(wx.VERTICAL)
    sizer.Add(wx.StaticText(dlg, label=lbl), 0, wx.ALIGN_CENTER_HORIZONTAL, 0)
    sizer.Add((-1, 15))
...
class PickTwoDialog(wx.Dialog):
    ...
    def Draw(self):
        ...
        for isel, name in enumerate(self.choices):
            ...
            lineSizer.Add(
                wx.StaticText(self.panel, -1, "Reference atom " + str(isel + 1)),
                0,
                wx.ALIGN_CENTER,
            )
            nameList = self.names[:]
...
class MultiDataDialog(wx.Dialog):
    ...
    def Draw(self):
        ...
        for tid, [prompt, value, limits, testfxn, fmt] in enumerate(
            zip(
                self.prompts,
                self.values,
                self.limits,
                ...
                for idl, item in enumerate(fmt):
                    ...
                    if "edit" in item:
                        ...
                        listItem = wx.ComboBox(
                            self.panel, value=initVal, choices=limits[idl], style=style
                        )
...
class SingleStringDialog(wx.Dialog):
    ...
    def __init__(
        self, parent, title, prompt, value="", size=(200, -1), help="", choices=None
    ):
        wx.Dialog.__init__(
            self,
            parent,
...
class MultiStringDialog(wx.Dialog):
    ...
    def Paint(self):
        ...
        for prompt, value in zip(self.prompts, self.values, strict=False):
            ...
            valItem = wx.TextCtrl(
                self, -1, value=value, style=wx.TE_PROCESS_ENTER, size=(self.size, -1)
            )
    ...
    def onExpand(self, event):  # noqa: ARG002
        self.values.append("")
...
class G2ColumnIDDialog(wx.Dialog):
    ...
    def __init__(
        ...
        ChoiceList,
        ColumnData,
        monoFont=False,  # noqa: ARG002
        **kw,
    ):
        def OnOk(sevent):  # noqa: ARG001
            OK = True
            selCols = []
            for col in self.sel:
                item = col.GetValue()
        ...
        # process keyword parameters, notably style
        options = {
            "size": (600, 310),  # default Frame keywords
            "style": wx.DEFAULT_DIALOG_STYLE
            | wx.RESIZE_BORDER
            | wx.CENTRE
            | wx.OK
        ...
        if self.Comments:
            ...
            txt.SetBackgroundColour(wx.SystemSettings.GetColour(wx.SYS_COLOUR_WINDOW))
            font1 = wx.Font(
                txt.GetFont().GetPointSize(), wx.MODERN, wx.NORMAL, wx.NORMAL, False
            )
            txt.SetFont(font1)
        ...
        for icol, _col in enumerate(self.ColumnData):
            ...
            mod = wx.TextCtrl(
                panel, size=(120, -1), value="", style=wx.TE_PROCESS_ENTER
            )
        ...
        Sizer.Add(
            wx.StaticText(
                panel,
                label=' For modify by, enter arithmetic string eg. "-12345.67". "+", "-", "*", "/", "**" all allowed',
            ),
            0,
        )
        Sizer.Add((-1, 10))
    ...
    def __init__(
        ...
        **kw,
        ...
        for irow, name in enumerate(self.HistoList):
            dataSizer.Add(
                wx.StaticText(panel, label=name), 0, WACV | wx.LEFT | wx.RIGHT, 10
            )
            for it, item in enumerate(self.ParmData[irow]):
                dat = wx.TextCtrl(
                    panel,
                    -1,
                    value=self.ParmFmt[it] % (item),
...
def ItemSelector(
    ...
    if multiple:
        if useCancel:
            dlg = G2MultiChoiceDialog(ParentFrame, title, header, ChoiceList)
        else:
            dlg = G2MultiChoiceDialog(
                ParentFrame,
                title,
...
class MultiIntegerDialog(wx.Dialog):
    ...
    def Draw(self):
        ...
        mainSizer = wx.BoxSizer(wx.VERTICAL)
        Indx = {}
        for ival, [prompt, value] in enumerate(
            zip(self.prompts, self.values, strict=False)
        ):
...
class MultiColumnSelection(wx.Dialog):
        ...
        **kw,
        ...
        if checkLbl:
            self.list.InsertColumn(
                0, checkLbl, width=8 * len(checkLbl), format=colPosition
            )
            inc = 1
        else:
            inc = 0
        ...
        for i, item in enumerate(choices):
            ...
            for j, item in enumerate(item[1 : len(colLabels)]):  # noqa: B020, PLW2901
                item = wordwrap(  # noqa: PLW2901
                    StripIndents(item, True), colWidths[j + 1], wx.ClientDC(self)
                )
                item += "\n==========================================="  # noqa: PLW2901
    ...
    def _onSelect(self, event):  # noqa: ARG002
        if self.list.GetNextSelected(-1) == -1:
            return
...
def MultiColMultiSelDlg(parent, title, header, colInfo, choices):
    ...
    """
    dlg = wx.Dialog(
        parent, wx.ID_ANY, title, style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER
    )
    mainSizer = wx.BoxSizer(wx.VERTICAL)
...
class OrderBox(wxscroll.ScrolledPanel):
    ...
    def __init__(self, parent, keylist, vallookup, posdict, *arg, **kw):
        ...
        self.SetupScrolling()
        self.SetMinSize(
            (min(700, self.GBsizer.GetSize()[0]), self.GBsizer.GetSize()[1] + 20)
        )
    ...
    def OnChoice(self, event):
        ...
        for i in range(
            repeat
        ):  # process the posXXX and then the intXXX (or a single variable)
            col += i
            newcol += i
            if newcol in self.posdict[key]:
                ...
                l = [*range(mtcol - 1, newcol - 1, -1), col]  # noqa: E741
            else:
                l1 = [newcol]
                l = [col]  # noqa: E741
...
def GetImportFile(
    ...
    **kwargs,
    ...
    if sys.platform.startswith("linux") and browseOpt is None:
        browseOpt = True
    if browseOpt:
        filelist = G2FileBrowser(parent, message, defaultDir, style=style, **kwargs)
        if len(filelist) == 0:
            return []
        if (
            style & wx.FD_CHANGE_DIR
        ):  # to get Mac/Linux to change directory like windows!
            os.chdir(os.path.dirname(filelist[0]))
    else:
        dlg = wx.FileDialog(
            parent, message, defaultDir, defaultFile, *args, style=style, **kwargs
        )
        #    dlg.CenterOnParent()
        if not defaultDir and pth:
            dlg.SetDirectory(pth)
        try:
            if dlg.ShowModal() == wx.ID_OK:
...
def G2FileBrowser(
    ...
    **kwargs,  # noqa: ARG001
    ...
    def OnDirSelect(event, **kwargs):  # noqa: ARG001
        ...
        if filterTxt[0] and filterTxt[0].strip() != "*":
            dlg.filelist = [
                i
                for i in dlg.filelist
                if fnmatch.fnmatch(os.path.split(i)[1], filterTxt[0])
            ]
        dlg.filelist.sort()
    ...
    def OnNumSelect(event):  # noqa: ARG001
    ...
    # make the GUI
    dlg = wx.Dialog(
        parent, wx.ID_ANY, lbl, style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER
    )
    dlg.choices = []
    dlg.filelist = []
    mainSizer = wx.BoxSizer(wx.VERTICAL)
    ...
    mainSizer.Add((0, 10))
    # directory selector
    fb = filebrowse.DirBrowseButton(
        dlg, wx.ID_ANY, labelText="Directory:", changeCallback=OnDirSelect
    )
    ...
    typeSel = wx.ComboBox(
        dlg, choices=choices, value=choices[0], style=wx.CB_READONLY | wx.CB_DROPDOWN
    )
...
def GetImportPath(G2frame):
    ...
    if G2frame.TutorialImportDir:
        if os.path.exists(G2frame.TutorialImportDir):
            ...
            print(
                "DBG_Tutorial location (TutorialImportDir) not found: "
                + G2frame.TutorialImportDir
            )
    ...
    if pth:
        ...
        if os.path.exists(pth):
            ...
            print(
                "Ignoring Config Import_directory value: "
    ...
    if G2frame.LastImportDir:
        if os.path.exists(G2frame.LastImportDir):
            ...
            print(
                "DBG_Warning: G2frame.LastImportDir not found = "
...
class SGMessageBox(wx.Dialog):
    ...
    def __init__(
        ...
        for item in self.table:
            ...
            num, flds = item.split(")")
            tableSizer.Add(
                wx.StaticText(self.panel, label=f"     {num + ')'}  "),
                0,
                WACV | wx.ALIGN_LEFT,
            )
        ...
        def OnPrintOps(event):  # noqa: ARG001
            print(f" Symmetry operations for {self.text[0].split(':')[1]}:")
...
class SGMagSpinBox(wx.Dialog):
    ...
    def __init__(self, parent, title, text, table, Cents, names, spins, ifGray):
        ...
        self.ifGray = ifGray
        self.PrintTable = [
            f" Magnetic symmetry operations for {self.text[0].split(':')[1]}:",
        ]
        ...
        for ng in range(nG):
            if ng:
                mainSizer.Add(
                    wx.StaticText(self.panel, label="      for (0,0,0)+1'"), 0
                )
                j = 0
            for ic, cent in enumerate(cents):
                ...
                for item in self.table:
                    ...
                    flds = item.split(")")[1]
                    tableSizer.Add(
                        wx.StaticText(self.panel, label="  (%2d)  " % (j + 1)),
                        0,
                        WACV,
                    )
                    flds = flds.replace(" ", "").split(",")
                    ...
                    try:
                        ...
                        item += ",+1"  # noqa: PLW2901
                    M, T, S = G2spc.MagText2MTS(
                        item.split(")")[1].replace(" ", ""), CIF=False
                    )
                    T = (T + Cent) % 1.0
                    item = G2spc.MT2text([M, T], reverse=True)  # noqa: PLW2901
                    if S > 0:
                        item += ",+1"  # noqa: PLW2901
                    else:
                        item += ",-1"  # noqa: PLW2901
...
class DisAglDialog(wx.Dialog):
    ...
    def Draw(self, data):
        ...
        mainSizer = wx.BoxSizer(wx.VERTICAL)
        mainSizer.Add(
            wx.StaticText(self.panel, -1, "Controls for phase " + data["Name"]),
            0,
            wx.LEFT,
            10,
        )
        ...
        for id, item in enumerate(self.data["AtomTypes"]):
            ...
            if self.Angle:
                aRadii = ValidatedTxtCtrl(
                    self.panel, data["AngleRadii"], id, nDig=(10, 3)
                )
        ...
        for i, name in enumerate(Names):
            factorSizer.Add(
                wx.StaticText(self.panel, -1, name + " search factor"), 0, WACV
            )
    ...
    def OnReset(self, event):  # noqa: ARG002
...
class ShowLSParms(wx.Dialog):
    ...
    def __init__(
        ...
        splitNames = [
            item.split(":")
            for item in self.parmNames
            if len(item) > 3 and not isinstance(self.parmDict[item], basestr)
        ]
        globNames = [
    ...
    def DrawPanel(self):
        ...
        def _OnParmSel(event):  # noqa: ARG001
        ...
        def OnListSel(event):  # noqa: ARG001
        ...
        mainSizer.Add(
            wx.StaticText(self, label="View Parameters in Project"), 0, wx.ALIGN_CENTER
        )
        parmSizer = wx.BoxSizer(wx.HORIZONTAL)
        parmSizer.Add(
            wx.StaticText(self, label=f" Number of refined variables: {num}"),
            0,
            wx.ALIGN_LEFT,
        )
        if len(self.varyList) != len(self.fullVaryList):
            ...
            parmSizer.Add(
                wx.StaticText(
                    self,
                    label=f" + {len(self.fullVaryList) - len(self.varyList)} varied via constraints",
                )
            )
        ...
        if fcount:
            parmSizer.Add(wx.StaticText(self, label=f" - {fcount} frozen variables"))
        ...
        if self.parmChoice != "Global":
            if self.parmChoice in [
                "Phase",
            ]:
                varSel = wx.ComboBox(
                    self,
                    choices=self.phasVars,
                    value=self.varName,
                    style=wx.CB_READONLY | wx.CB_DROPDOWN,
                )
            elif self.parmChoice in [
                "Histogram",
            ]:
                varSel = wx.ComboBox(
                    self,
                    choices=self.hisVars,
                    value=self.varName,
                    style=wx.CB_READONLY | wx.CB_DROPDOWN,
                )
            elif self.parmChoice in [
        ...
        mainSizer.Add(wx.StaticText(self, label=txt), 0, wx.ALL, 0)
...
class VirtualVarBox(wx.ListCtrl):
    ...
    def SetContents(self, parent):
        ...
        for name in parent.choiceDict[parent.parmChoice]:
            if "2" in platform.python_version_tuple()[0]:
                ...
                basestr = str
            if isinstance(parent.parmDict[name], basestr):
                continue
            if (
                "Refined" in parent.listSel
                and (name not in parent.fullVaryList)
                and (name not in parent.varyList)
            ):
                continue
            if "Frozen" in parent.listSel and not (
                name in self.parmWin.fullVaryList and name in self.parmWin.frozenList
            ):
                continue
            if "Phase" in parent.parmChoice:
                if parent.phasNum != "*" and name.split(":")[0] != parent.phasNum:
                    continue
            if "Histo" in parent.parmChoice:
                if parent.hisNum != "*" and name.split(":")[1] != parent.hisNum:
                    continue
            if (parent.varName != " ") and (parent.varName not in name):
                continue
    ...
    def OnRowSelected(self, event, row=None):
        ...
        def ResetFrozen(event):  # noqa: ARG001
        ...
        if v is not None and v[-1] is not None:
            ...
            if txt:
                ...
                txtwid = wx.StaticText(
                    dlg, wx.ID_ANY, 'Parameter meaning is "' + txt + '"'
                )
                txtwid.Wrap(580)
        ...
        if name not in self.parmWin.varyList and name in self.parmWin.fullVaryList:
            mainSizer.Add(
                wx.StaticText(
                    dlg, wx.ID_ANY, "Limits not allowed on constrained variables"
                ),
                0,
            )
            ...
            n, val = G2obj.prmLookup(
                name, self.parmWin.Controls["parmMinDict"]
            )  # is this a wild-card?
            if val is None:
                ...
                subSizer = wx.BoxSizer(wx.HORIZONTAL)
                subSizer.Add(
                    wx.StaticText(dlg, wx.ID_ANY, "Minimum limit"), 0, wx.CENTER
                )
                subSizer.Add(
                    ValidatedTxtCtrl(
                        dlg, self.parmWin.Controls["parmMinDict"], n, nDig=(10, 2, "g")
                    ),
                    0,
                    WACV,
                )
            ...
            mainSizer.Add((-1, 10), 0)
            n, val = G2obj.prmLookup(
                name, self.parmWin.Controls["parmMaxDict"]
            )  # is this a wild-card?
            if val is None:
                ...
                subSizer = wx.BoxSizer(wx.HORIZONTAL)
                subSizer.Add(
                    wx.StaticText(dlg, wx.ID_ANY, "Maximum limit"), 0, wx.CENTER
                )
                subSizer.Add(
                    ValidatedTxtCtrl(
                        dlg, self.parmWin.Controls["parmMaxDict"], n, nDig=(10, 2, "g")
                    ),
                    0,
                    WACV,
                )
...
class GSGrid(wg.Grid):
    ...
    def __init__(self, parent, name=""):
        ...
        if hasattr(parent.TopLevelParent, "currentGrids"):
            parent.TopLevelParent.currentGrids.append(
                self
            )  # save a reference to the grid in the Frame
    ...
    def InstallGridToolTip(
        self, rowcolhintcallback, colLblCallback=None, rowLblCallback=None
    ):
        ...
        def OnMouseMotion(event):
            ...
            if win == self.GetGridWindow() and row >= 0 and col >= 0:
                ...
                if colLblCallback:
                    hinttext = colLblCallback(col)
            elif win == self.GetGridRowLabelWindow() and row >= 0:
                if rowLblCallback:
                    hinttext = rowLblCallback(row)
        ...
        if "phoenix" in wx.version():
            ...
            if colLblCallback:
                self.GetGridColLabelWindow().Bind(wx.EVT_MOTION, OnMouseMotion)
            if rowLblCallback:
                self.GetGridRowLabelWindow().Bind(wx.EVT_MOTION, OnMouseMotion)
        else:
            wx.EVT_MOTION(self.GetGridWindow(), OnMouseMotion)
            if colLblCallback:
                wx.EVT_MOTION(self.GetGridColLabelWindow(), OnMouseMotion)
            if rowLblCallback:
                wx.EVT_MOTION(self.GetGridRowLabelWindow(), OnMouseMotion)
    ...
    def setupPopup(self, lblList, callList):
        ...
        def createPopup(event):
            ...
            for top, bottom in zip(
                [r for r, c in self.GetSelectionBlockTopLeft()],
...
class Table(wg.GridTableBase):
    ...
    def AppendRows(self, numRows=1):  # noqa: ARG002
        self.data.append([])
        return True

    def CanGetValueAs(self, row, col, typeName):  # noqa: ARG002
        if self.dataTypes:
...
class GridFractionEditor(wg.PyGridCellEditor):
    ...
    def __init__(self, grid):  # noqa: ARG002
        if "phoenix" in wx.version():
    ...
    def EndEdit(self, row, col, grid, oldVal=None):
        ...
        if val != str(self.startValue):
            ...
            if (
                oldVal is None
            ):  # this arg appears in 2.9+; before, we should go ahead & change the table
    ...
    def OnChar(self, evt):
        key = evt.GetKeyCode()
        if (
            key < 32 or key >= 127 or chr(key).lower() in ".+-*/0123456789cosind(),"
        ):  # outside printable ascii range; needed for backspace etc.
            evt.Skip()
        else:
            evt.StopPropagation()
...
def askSaveDirectory(G2frame):
    ...
    dlg = wx.DirDialog(
        G2frame,
        "Input directory where file(s) will be written",
        pth,
        wx.DD_DEFAULT_STYLE,
    )
    dlg.CenterOnParent()
...
class GSNoteBook(wx.aui.AuiNotebook):
    ...
    def __init__(self, parent, name="", size=None, style=wxaui_NB_TOPSCROLL):  # noqa: ARG002
...
class MyHelp(wx.Menu):
    ...
    * About: opens an About dialog using OnHelpAbout. N.B. on the Mac this
    ...
    def __init__(self, frame, includeTree=False, morehelpitems=None):
        ...
        self.HelpById = {}
        self.frame = frame
        self.Append(wx.ID_ABOUT, "&About GSAS-II", "Shows version and citation info")
        ...
        if GSASIIpath.HowIsG2Installed().startswith("git"):
            ...
            helpobj = self.Append(
                wx.ID_ANY,
                "&Regress to old GSAS-II version",
            ...
            if os.access(GSASIIpath.path2GSAS2, os.W_OK):
                ...
                helpobj.Enable(False)
        if GSASIIpath.HowIsG2Installed().startswith(
            "git"
        ) and GSASIIpath.GetConfigValue("debug"):
            helpobj = self.Append(
                wx.ID_ANY,
        ...
        helpobj = self.Append(
            wx.ID_ANY,
            "Help on current data tree item\tF1",
    ...
    def OnHelpById(self, event):
        ...
        try:
            helpKey = (
                self.frame.dataWindow.helpKey
            )  # look up help from helpKey in data window
            # if GSASIIpath.GetConfigValue('debug'): print 'DBG_dataWindow help: key=',helpKey
        except AttributeError:
            helpKey = ""
            if GSASIIpath.GetConfigValue("debug"):
                print("DBG_No helpKey for current dataWindow!")
        helpType = self.HelpById.get(
            event.GetId(), helpKey
        )  # see if there is a special help topic
    ...
    def OnHelpAbout(self, event):  # noqa: ARG002
...
Also see Help/"Citation information" for other works used in GSAS-II. Citations encourage scientists to make their software available."""
    ...
    def OnCheckUpdates(self, event):  # noqa: ARG002
    ...
    def OnSelectVersion(self, event):  # noqa: ARG002
...
def ShowCitations(event):  # noqa: ARG001
    ...
    def copy2clip(event):
        ...
        if wx.TheClipboard.Open():
            ...
            return
        G2MessageBox(
            parent, "Citation information placed in clipboard. ", "Citations copied"
        )
...
def updateNotifier(G2frame, fileVersion):
    ...
    for key in updateNoticeDict:  # noqa: PLC0206
...
def viewWebPage(parent, URL, size=(750, 450), newFrame=False, HTML=""):
    ...
    global lastWebFrame  # noqa: PLW0603
    if not newFrame:
        try:
    ...
    close.Bind(wx.EVT_BUTTON, lambda event: dlg.Destroy())  # noqa: ARG005
...
def MultipleChoicesSelector(choicelist, headinglist, ParentFrame=None, **kwargs):
    ...
    """
    result = None
    dlg = MultipleChoicesDialog(choicelist, headinglist, parent=ParentFrame, **kwargs)
    dlg.CenterOnParent()
    if dlg.ShowModal() == wx.ID_OK:
        result = dlg.chosen
    dlg.Destroy()
...
def showUniqueCell(
    frame, cellSizer, row, cell, SGData=None, editAllowed=False, OnCellChange=None
):
    ...
    for txt, fmt, indx in zip(*useGUI[1:], strict=False):
        ...
        if editAllowed and indx >= 0:
            ...
            if ".3" in fmt:
                Fmt = (10, 3)
            cellVal = ValidatedTxtCtrl(
                frame, cell, indx, xmin=0.1, xmax=500.0, nDig=Fmt, OnLeave=OnCellChange
            )
            cellSizer.Add(cellVal, (cellrow, col + 1))
            cellList.append(cellVal.GetId())
        else:
            cellSizer.Add(
                wx.StaticText(frame, label=fmt.format(cell[abs(indx)])),
                (cellrow, col + 1),
            )
    # volume
    volCol = 13
...
def SaveGPXdirectory(path, write=True):
    if GSASIIpath.GetConfigValue("Starting_directory") == path:
        return
    vars = GetConfigValsDocs()
    try:
        vars["Starting_directory"][1] = path
        if GSASIIpath.GetConfigValue("debug"):
            print("DBG_Saving GPX path: " + path)
        if write:
            SaveConfigVars(vars)
    except KeyError:
        pass
...
def SaveImportDirectory(path):
    if GSASIIpath.GetConfigValue("Import_directory") == path:
        return
    vars = GetConfigValsDocs()
    try:
        vars["Import_directory"][1] = path
        if GSASIIpath.GetConfigValue("debug"):
            print("DBG_Saving Import path: " + path)
        SaveConfigVars(vars)
    except KeyError:
        pass
...
def SaveConfigVars(vars, parent=None):  # noqa: ARG001
...
class SelectConfigSetting(wx.Dialog):
    ...
    def __init__(self, parent):
        ...
        label = wx.StaticText(
            self, wx.ID_ANY, "Select a GSAS-II configuration variable to change"
        )
        self.sizer.Add(label, 0, wx.ALIGN_CENTRE | wx.ALL, 5)
        self.choice = {}
        choices = sorted(
            [k for k in self.vars if not k.startswith("enum_")], key=lambda s: s.lower()
        )
        btn = G2ChoiceButton(
            self, choices, strLoc=self.choice, strKey=0, onChoice=self.OnSelection
        )
        btn.SetLabel("")
        self.sizer.Add(btn)
    ...
    def OnChange(self, event=None):  # noqa: ARG002
    ...
    def OnSave(self, event):  # noqa: ARG002
    ...
    def onSelDir(self, event):  # noqa: ARG002
    ...
    def onSelColor(self, event):  # noqa: ARG002
    ...
    def onSelExec(self, event):  # noqa: ARG002
        ...
        while repeat:
            ...
            if dlg.ShowModal() == wx.ID_OK:
                ...
                if os.path.exists(val) and is_exe(val):
                    ...
                    dlg.Destroy()
                    G2MessageBox(
                        self, "File not found or not executable", "Invalid file"
                    )
                    repeat = True
                    continue
        dlg.Destroy()
    ...
    def OnSelection(self):
        ...
        if "enum_" + var in self.vars:
            ...
            if var == "Contour_color":
                ...
                self.strEd = ValidatedTxtCtrl(
                    self,
                    self.vars[var],
                    1,
                    typeHint=str,
                    OKcontrol=self.OnChange,
                    size=sz,
                    notBlank=False,
                )
        ...
        if (
            self.vars[var][1] is not None and self.vars[var][1] != ""
        ):  # show current value, if one
    ...
    def OnClear(self, event):  # noqa: ARG002
        var = self.choice[0]
...
class RefinementProgress(wx.ProgressDialog):
    ...
    def __init__(
        ...
        maximum=101,
        parent=None,
        trialMode=False,  # noqa: ARG002
        seqLen=0,  # noqa: ARG002
        style=None,
    ):
        if style is None:
            style = wx.PD_ELAPSED_TIME | wx.PD_AUTO_HIDE | wx.PD_CAN_ABORT
        super(self.__class__, self).__init__(
            title, message, int(maximum), parent, style
        )
        Size = self.GetSize()
...
class G2RefinementProgress(wx.Dialog):
    ...
    def __init__(
        ...
        super(self.__class__, self).__init__(
            parent, wx.ID_ANY, title, style=style, size=(-1, -1)
        )
        self.Bind(wx.EVT_CLOSE, self._onClose)
        mainSizer = wx.BoxSizer(wx.VERTICAL)
        ...
        lblSizer = (
            self._makeLabeledTable()
        )  # make the Table for Rfactors in the RwPanel
        ...
        Canvas(
            pltPanel, wx.ID_ANY, self.figure
        )  # no need to save, get this from self.figure.canvas
    ...
    def _makeLabeledTable(self):
        ...
        self.RwPanel = wx.lib.scrolledpanel.ScrolledPanel(
            self, wx.ID_ANY, size=(180, 130), style=wx.SUNKEN_BORDER
        )
        lblSizer.Add(self.RwPanel, 1, wx.EXPAND, 1)

        self.Labels.label = {}
        for i in range(self.rows):
            self.Labels.label[i] = wx.StaticText(
                self, wx.ID_ANY, "", style=wx.ALIGN_CENTER_VERTICAL
            )
    ...
    def _AddTableColumn(self, label="", col=None):  # noqa: ARG002
        ...
        for i in range(self.rows - 1):
            lbls.append(
                wx.StaticText(self.RwPanel, wx.ID_ANY, "", style=wx.ALIGN_CENTER)
            )
    ...
    def _SetSeqHistogram(self, nextHist, histLbl):
        ...
        if nextHist >= 0 and self.seqHist != nextHist:
            ...
            self.SeqCount += 1
            self.gauge.SetValue(
                int(min(self.gaugemaximum, 100.0 * self.SeqCount / self.SeqLen))
            )

    def _plotBar(self, h):
        "plot a vertical bar for a histogram"
        sym, lbl = self._getPlotSetting(h)
        l = len(self.fitVals[h])  # noqa: E741
        wid = 10.0
        if h < 0:
        ...
        self.plotaxis.bar(
            np.array(range(l)) + self.histOff[h],
            self.fitVals[h],
            width=1 / wid,
            label=lbl,
            color=sym,
    ...
    def _SetCycleRw(self, value):
        ...
        if self.maxCycle and not self.SeqLen:
            self.gauge.SetValue(
                int(min(self.gaugemaximum, 100.0 * cycle / self.maxCycle))
            )
        ...
        if not self.trialRw:  # show & plot current status here
            ...
            if self.SeqLen:
                for (
                    h
                ) in self.fitVals:  # loop over all histograms but not all get plotted
                    if h == -1:
                        continue
                ...
                for (
                    ...
                    if (
                        h < 0
                        or h == self.curHist
                        or len(self.fitVals) < 6
                        or (self.curHist < 0 and h == 0)
                    ):  # plot overall & restraints, or p to 5 histograms
                        self._plotBar(h)
            if self.plotted:
                self.plotaxis.legend(loc=3)
    ...
    def AdvanceCycle(self, cycle=None):  # noqa: ARG002
        ...
        for h, value in self.trialVals.items():
            if h not in self.fitVals or h not in self.tblLbls:
                continue
            self.fitVals[h].append(value)
            self.tblLbls[h][2].SetLabel(f"{value:8.3g}")
            self.tblLbls[h][3].SetLabel("")
            if (
                self.SeqLen or h < 3 or len(self.trialVals) < 6
            ):  # plot overall & restraints, or p to 5 histograms
                self._plotBar(h)
        if self.plotted:
            self.plotaxis.legend(loc=3)
    ...
    def Update(self, value=None, newmsg=""):
        ...
        """
        if self.curHist is not None and value != 101.0:
...
class gitVersionSelector(wx.Dialog):
    ...
    def __init__(self, parent=None):
        ...
        self.githistory = [
            h
            for h in self.githistory
            if self.g2repo.commit(h).committed_datetime > cutoff
        ]
        # end patch
        ...
        sizer.Add(wx.StaticText(self, wx.ID_ANY, "      Currently installed version:"))
        sizer1 = wx.BoxSizer(wx.HORIZONTAL)
        sizer1.Add((50, -1))

        initpnl = wxscroll.ScrolledPanel(
            self, wx.ID_ANY, size=(450, 90), style=wx.SUNKEN_BORDER
        )
        ...
        sizer1 = wx.BoxSizer(wx.HORIZONTAL)
        sizer1.Add(
            wx.StaticText(self, wx.ID_ANY, "Select how many versions to regress: "),
            0,
        ...
        sizer.Add(wx.StaticText(self, wx.ID_ANY, "      Selected version to install:"))
        sizer1 = wx.BoxSizer(wx.HORIZONTAL)
        sizer1.Add((50, -1))

        self.spanel = wxscroll.ScrolledPanel(
            self, wx.ID_ANY, size=(450, 90), style=wx.SUNKEN_BORDER
        )
        ssizer = wx.BoxSizer(wx.HORIZONTAL)
...
try:
    ...
    class G2LstCtrl(
        wx.ListCtrl, listmix.ListCtrlAutoWidthMixin, listmix.ColumnSorterMixin
    ):
...
def ShowHelp(helpType, frame, helpMode=None):
    ...
    global htmlFirstUse, htmlPanel, htmlFrame  # noqa: PLW0602, PLW0603
    ...
    if helpType:
        ...
        if anchor not in helpLocDict:
            print(
                f"Help lookup problem, anchor {anchor} not found"
    ...
    if helpMode == "internal":
        helplink = "file://" + os.path.abspath(
            os.path.join(path2GSAS2, "help", helplink)
        )
        ...
        if (
            sys.platform == "darwin"
        ):  # on Mac, use a OSXscript so that file anchors work
    ...
    return appname as text
            ...
            try:
                browser = subprocess.check_output(
                    ["osascript", "-e", appleScript], encoding="UTF-8"
                ).strip()
...
tutorialCatalog = [l for l in tutorialIndex if len(l) >= 3]  # noqa: E741
...
class OpenGitTutorial(wx.Dialog):
    ...
    def __init__(self, parent):
        ...
        sizer1 = wx.BoxSizer(wx.HORIZONTAL)
        msg = (
            "To open a tutorial, select below the mode to be used. "
        ...
        sizer1.Add(btn, 0)
        btn = wx.Button(
            pnl, wx.ID_ANY, "Select a tutorial to view and download its data files"
        )
    ...
    def SetTutorialPath(self):
        ...
        if GSASIIpath.GetConfigValue("Tutorial_location"):
            tutorialPath = os.path.abspath(
                GSASIIpath.GetConfigValue("Tutorial_location")
            )
            ...
            try:
                ...
                print("Unable to use Tutorial_location config setting", tutorialPath)
        # try a system-specific location
        if sys.platform.lower().startswith("win"):
            for p in ("Documents", "My Documents", ""):
                if os.path.exists(
                    os.path.abspath(os.path.expanduser(os.path.join("~", p)))
                ):
                    tutorialPath = os.path.abspath(
                        os.path.expanduser(os.path.join("~", p, "G2tutorials"))
                    )
                    if os.path.exists(tutorialPath):
                        self.tutorialPath = tutorialPath
                        return
            ...
            tutorialPath = os.path.abspath(
                os.path.expanduser(os.path.join("~", "G2tutorials"))
            )
            if os.path.exists(tutorialPath):
                self.tutorialPath = tutorialPath
                return
    ...
    def onWebBrowse(self, event):  # noqa: ARG002
        """Shows a list of all tutorials so user can select one to view.
    ...
    def SelectDownloadLoc(self, event):  # noqa: ARG002
        """Select a download location,
        Cancel resets to the default
        """
        dlg = wx.DirDialog(
            self,
            "Choose a directory for tutorial downloads:",
            defaultPath=self.tutorialPath,
        ...
        if not os.path.exists(pth):
            try:
                ...
                return
        if os.path.exists(os.path.join(pth, "help")) and os.path.exists(
            os.path.join(pth, "Exercises")
        ):
            print(
                "Note that you may have old tutorial files in the following directories"
            )
        ...
        if GSASIIpath.GetConfigValue("Tutorial_location") == pth:
            return
        vars = GetConfigValsDocs()
        try:
            vars["Tutorial_location"][1] = pth
            if GSASIIpath.GetConfigValue("debug"):
                print("DBG_Saving Tutorial_location: " + pth)
            GSASIIpath.SetConfigValue(vars)
            SaveConfigVars(vars)
        except KeyError:
            pass
...
def AutoLoadFiles(G2frame, FileTyp="pwd"):
    ...
    def OnBrowse(event):
        ...
        if btn3 == event.GetEventObject():
            ...
            d = wx.FileDialog(
                dlg, "Choose instrument parameter file", "", "", extList, wx.FD_OPEN
            )
    ...
    def OnFileOfFiles(event):  # noqa: ARG001
        ...
        d = wx.FileDialog(
            dlg, "Choose a text file with file names", "", "", extList, wx.FD_OPEN
        )
        filelist = []
        try:
            if d.ShowModal() == wx.ID_OK:
                if not os.path.exists(Settings["instfile"]):
                    return
                if not os.path.exists(d.GetPath()):
                    return
    ...
    def onSetExtSelection():
        Settings["filter"] = (
            os.path.splitext(Settings["filter"])[0] + Settings["extStr"]
        )
    ...
    def RunTimerPWDR(event, filelist=None):  # noqa: ARG001
        if filelist is None:
            if GSASIIpath.GetConfigValue("debug"):
                import datetime

                print(
                    f"DBG_Timer tick at {datetime.datetime.now():%d %b %Y %H:%M:%S}\n"
                )
            ...
            while repeat:
                ...
                if flag:
                    ...
                    if rd.warnings:
                        ...
                        G2fil.G2Print(
                            f"{f} block # {block} read by Reader {rd.formatName}"
                        )
                    block += 1
                    repeat = rd.repeat
                else:
                ...
                if "T" in Iparm1["Type"][0]:
                    ...
                    if (
                        rd.GSAS
                    ):  # NB: old GSAS wanted intensities*CW even if normalized!
                        npts = min(
                            len(rd.powderdata[0]), len(rd.powderdata[1]), len(cw)
                        )
                ...
                G2frame.GPXtree.SetItemPyData(
                    G2frame.GPXtree.AppendItem(Id, text="Sample Parameters"), rd.Sample
                )
                G2frame.GPXtree.SetItemPyData(
    ...
    def RunTimerGR(event):  # noqa: ARG001
        if GSASIIpath.GetConfigValue("debug"):
            import datetime
        ...
        for f in filelist:
            ...
            while repeat:
                ...
                G2frame.GPXtree.SetItemPyData(
                    G2frame.GPXtree.AppendItem(Id, text="PDF Peaks"),
                    {
                        "Limits": [1.0, 5.0],
    ...
    global AutoLoadWindow  # noqa: PLW0603
    Settings = {}
    ...
    fmtSel = G2ChoiceButton(
        mnpnl, fmtchoices, Settings, "fmt", onChoice=onSetFmtSelection
    )
    sizer.Add(fmtSel)
...
"""
    ...
    extSel = G2ChoiceButton(
        mnpnl, [], Settings, "ext", Settings, "extStr", onChoice=onSetExtSelection
    )
    sizer.Add(extSel, 0)
    sizer.Add((-1, -1), 1, wx.EXPAND, 1)
    ...
    sizer = wx.BoxSizer(wx.HORIZONTAL)
    sizer.Add(
        wx.StaticText(mnpnl, wx.ID_ANY, "Read from: "), 0, wx.ALIGN_CENTER_VERTICAL
    )
    fInp3 = ValidatedTxtCtrl(
        mnpnl, Settings, "indir", size=(300, -1), OnLeave=TestInput
    )
    ...
    if FileTyp == "pwd":
        sizer = wx.BoxSizer(wx.HORIZONTAL)
        sizer.Add(
            wx.StaticText(mnpnl, wx.ID_ANY, "Instrument parameter file from: "),
            0,
            wx.ALIGN_CENTER_VERTICAL,
        )
        fInp4 = ValidatedTxtCtrl(
            mnpnl, Settings, "instfile", size=(300, -1), OnLeave=TestInput
        )
...
def ChooseOrigin(G2frame, rd):
    ...
    for atom in O2atoms:
        for i in [0, 1, 2]:
            atom[cx + i] += T[i]
            atom[cs : cs + 2] = G2spc.SytSym(atom[3:6], SGData)[
                0:2
    ...
    for i, phObj in enumerate([rd.Phase, O2Phase]):
        ...
        for i, k in enumerate(cellContents):  # noqa: PLW2901
            if i:
                txt += ", "
    ...
    O1Btn.Bind(wx.EVT_BUTTON, lambda x: dlg.EndModal(wx.ID_OK))  # noqa: ARG005
    O2Btn = wx.Button(dlg, wx.ID_ANY, "Keep current coordinates")
    O2Btn.Bind(wx.EVT_BUTTON, lambda x: dlg.EndModal(wx.ID_YES))  # noqa: ARG005
    if centro:
        O2Btn.SetDefault()
...
def makeContourSliders(G2frame, Ymax, PlotPatterns, newPlot, plottype):
    ...
    def OnNewVal(*args, **kwargs):  # noqa: ARG001
    ...
    hbox.Add(wx.StaticText(dlg, wx.ID_ANY, "Set Contour Intensity Limits"), 1, wx.ALL)
    hbox.Add((-1, -1), 1, wx.EXPAND, 1)
    vbox.Add(hbox)
    vbox.Add((-1, 10))
    ...
    minSel = wx.Slider(
        parent=dlg, style=wx.SL_HORIZONTAL, value=int(100 * G2frame.Cmin + 0.5)
    )
    minSel.Bind(wx.EVT_SLIDER, OnSlider)
    minVal = ValidatedTxtCtrl(dlg, Range, 0, xmin=0, xmax=Ymax - 1, OnLeave=OnNewVal)
    ...
    maxSel = wx.Slider(
        parent=dlg, style=wx.SL_HORIZONTAL, value=int(100 * G2frame.Cmax + 0.5)
    )
    maxSel.Bind(wx.EVT_SLIDER, OnSlider)
    maxVal = ValidatedTxtCtrl(dlg, Range, 1, xmin=1, xmax=Ymax, OnLeave=OnNewVal)
    ...
    btn.Bind(wx.EVT_BUTTON, lambda x: dlg.Destroy())  # noqa: ARG005
    hbox.Add((-1, -1), 1, wx.EXPAND, 1)
    ...
    try:
        while True:
            ...
            if datum[0] == "Notebook":
                ...
                if "Residuals" not in datum[1][0]:
                    continue
                if "PWDR" not in result:
                    result["PWDR"] = []
                result["PWDR"].append(
                    "hist #{}: wR={:.2f} ({:})".format(
...
class gpxFileSelector(wx.Dialog):
    ...
    def __init__(self, parent, startdir=".", multiple=False, *args, **kwargs):  # noqa: ARG002
        self.timer = None
        ...
        wx.Dialog.__init__(
            self, parent=parent, style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER
        )
        self.CenterOnParent()

        topSizer = wx.BoxSizer(wx.VERTICAL)
        self.dirBtn = wxfilebrowse.DirBrowseButton(
            self,
            wx.ID_ANY,
            size=(650, -1),
        ...
        chk = G2CheckBoxFrontLbl(
            self, " Include .bakXX?", self.opt, "useBak", OnChange=self.DirSelected
        )
        subSiz.Add(chk, 0, wx.ALIGN_CENTER_VERTICAL, 0)
        subSiz.Add((10, -1), 1, wx.EXPAND, 1)
        subSiz.Add(
            wx.StaticText(self, wx.ID_ANY, "   Sort by: "),
            0,
            wx.ALIGN_CENTER_VERTICAL,
            1,
        )
        choices = ["age", "name (alpha+case)", "name (alpha)"]
        for w in G2RadioButtons(
            self, self.opt, "sort", choices, OnChange=self.DirSelected
        ):
        ...
        self.filterBox = ValidatedTxtCtrl(
            self,
            self.opt,
            "filter",
            size=(80, -1),
            style=wx.TE_PROCESS_ENTER,
            OnLeave=self.DirSelected,
            notBlank=False,
        )
    ...
    def _startUpdateTimer(self, event):  # noqa: ARG002
        if self.timer:
            self.timer.Restart(self.delay)
        else:
            self.timer = wx.CallLater(self.delay, self.DirSelected)

    def DirSelected(self, event=None, *args, **kwargs):  # noqa: ARG002
        ...
        if self.opt["useBak"]:
            ...
            self.sl = [
                (os.path.split(i)[1], os.stat(i).st_mtime, i)
                for i in self.fl
    ...
    def FileSelected(self, event):  # noqa: ARG002
...
def setColorButton(parent, array, key, callback=None, callbackArgs=None):
    ...
    def onSetColour(event):  # noqa: ARG001
...
def SaveCite(prog, text):
    ...
    """
    global CitationDict  # noqa: PLW0602
...
SaveCite("small angle scattering", "R.B. Von Dreele, J. Appl. Cryst. 47, 1748-9 (2014)")
SaveCite(
    "DIFFaX",
...
def Load2Cells(G2frame, phase):
    ...
    def setRatioMax(*arg, **kwarg):  # noqa: ARG001
    ...
    def computeNISTlatCompare(event):  # noqa: ARG001
        "run NIST*LATTICE after the compute button is pressed"
        from . import nistlat

        out = nistlat.CompareCell(
            cellLen[0],
            cellCntr[0],
            cellLen[1],
            cellCntr[1],
        ...
        if len(out):
            msg = (
                str(len(out)) + " Transformations were found. See console for matrices."
            )
    ...
    def setCellFromPhase(event):
        ...
        if len(Phases) == 0:
            ...
            dlg = G2SingleChoiceDialog(
                G2frame, "Select a phase from list", "Select phase", phaseList
            )
            dlg.CenterOnParent()
            try:
                if dlg.ShowModal() == wx.ID_OK:
    ...
    sizer = wx.BoxSizer(wx.VERTICAL)
    sizer.Add(
        wx.StaticText(dlg, label="NIST*LATTICE: Relate Two Unit Cells"),
        0,
    ...
    for cell in range(2):
        tableSizer.Add(
            wx.StaticText(dlg, label="Cell " + str(cell + 1)),
            0,
    ...
    tableSizer.Add(
        wx.StaticText(dlg, label="Cell length tolerance: "), 0, WACV | wx.ALIGN_LEFT
    )
    w = ValidatedTxtCtrl(dlg, nistInput, 0, nDig=(6, 2))
    tableSizer.Add(w)
    tableSizer.Add(wx.StaticText(dlg, label=" (A) "))
    tableSizer.Add(
        wx.StaticText(dlg, label="Cell angle tolerance: "), 0, WACV | wx.ALIGN_LEFT
    )
    w = ValidatedTxtCtrl(dlg, nistInput, 1, nDig=(6, 1))
    tableSizer.Add(w)
    tableSizer.Add(wx.StaticText(dlg, label=" (degrees) "))
    tableSizer.Add(
        wx.StaticText(dlg, label="Cell volume range: "), 0, WACV | wx.ALIGN_LEFT
    )
...
class ScrolledStaticText(wx.StaticText):
    ...
    def __init__(self, parent, label="", delay=100, lbllen=15, dots=True, **kwargs):
    ...
    def onTimer(self, event):  # noqa: ARG002
        ...
        try:
            ...
            self.timer.Stop()
        self.msgpos += 1
        if self.msgpos >= len(self.fullmsg):
            self.msgpos = 0


# ===========================================================================
def ExtractFileFromZip(
    filename,
    selection=None,
    confirmread=True,
    ...
    for zindex in zlist:
        ...
        if zindex >= 0:
            ...
            fpout = open(outfile, "wb")  # noqa: SIM115
...
def gitCheckUpdates(G2frame):
    ...
    if status & 8:  # not on local branch
        if localChanges:
            ...
            msg = (
                "You have made a local branch and have switched to that."
    ...
    if status & 2:  # detached head
        ...
        if localupdates:
            msg = (
                "Your copy of GSAS-II has been regressed. You are "
        ...
        if len(lc) != 0:
            msg = (
                "You have made local changes and committed them "
    ...
    cmds = ["--git-update"]
    if localChanges:
        if gitAskLocalChanges(G2frame, cmds):
            return
    if gitAskSave(G2frame, regressmsg, cmds):
        return
...
def gitAskLocalChanges(G2frame, cmds):
    msg = (
        "You have locally-made changes to the GSAS-II source "
...
def gitAskSave(G2frame, regressmsg, cmds):
    ...
    msg = (
        "Before continuing, do you want to save your project? "
...
def gitSelectVersion(G2frame):
    ...
    if status & 8:  # not on local branch
        if localChanges:
            msg = (
                "You have switched to a local branch and have "
            ...
            msg = (
                "You have made and switched to a local branch. "
...
def gitSelectBranch(event):  # noqa: ARG001
    ...
    if not gitInst.startswith("github-rev"):
        G2MessageBox(
            G2frame,
            "Unable to switch branches unless GSAS-II has been installed from GitHub; installed as: "
            + gitInst,
            "Not a git install",
        )
        return
    ...
    print(".done")
    branchlist = [
        i.strip() for i in g2repo.git.branch("-r").split("\n") if "->" not in i
    ]
    choices = [
        i
        for i in [os.path.split(i)[1] for i in branchlist]
        if i != g2repo.active_branch.name
    ]
    if len(choices) == 0:
        G2MessageBox(
            G2frame, "No branches were found to select. Unexpected!", "No branches"
        )
        return
    if len(choices) == 1:
        b = choices[0]
        ...
        dlg = G2SingleChoiceDialog(
            G2frame, "Select branch to use", "Select Branch", choices
        )
        dlg.CenterOnParent()
        try:
            if dlg.ShowModal() == wx.ID_OK:
...
def gitSwitchMaster2Main():
    ...
    if not gitInst.startswith("github-rev"):
        G2MessageBox(
            G2frame,
            "Unable to update to new branch because GSAS-II was not installed from GitHub; installed as: "
            + gitInst,
            "Not a git install",
        )
        return
...
The update will be made unless Cancel is pressed."""
    ...
    install = [
        "PyCifRW",
        "pybaselines",
    ]  # these need to be reinstalled as they are no longer "vendored"
    ...
    if install:
        ...
        dlg = wx.ProgressDialog(
            ...
            f"Please wait while conda downloads and installs {len(install)} package(s). This can take a while.",
            10,
...
def ImportMsg(parent, msgs):
    ...
    """
    text = (
        "Warnings message(s) from load of importers:\n\n * "
        + "\n\n * ".join(msgs)
...
def SelectPkgInstall(event):
    ...
    wx.BeginBusyCursor()
    pdlg = wx.ProgressDialog(
        "Updating", "Installing Python packages; this can take a while", parent=G2frame
    )
    if GSASIIpath.condaTest():
        patch_condarc()
