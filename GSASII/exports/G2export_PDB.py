...
class ExportPhasePDB(G2fil.ExportBaseclass):
    ...
    def Exporter(self, event=None):
        ...
        def PDBremark250():
            ...
            self.Write(
                "REMARK 250   BOND ANGLES                      (DEG) : x.xx   xxx"
            )
            ...
            self.Write(
                "REMARK 250   INTERATOMIC DISTANCES              (A) :x.xxx   xxx"
            )
            self.Write(
                "REMARK 250   DISTANCES FROM RESTRAINT PLANES    (A) :x.xxx   xxx"
            )
            self.Write(
                "REMARK 250   TORSION PSEUDOPOTENTIAL RESTRAINTS (E) : x.xx   xxx"
            )
            self.Write(
                "REMARK 250   TORSION ANGLE RESTRAINTS           (E) : x.xx   xxx"
            )
            self.Write("REMARK 250")
        ...
        def PDBseqres(seqList):
            ...
            for chain in chains:
                ...
                for irec in range(nrec):
                    ...
                    self.Write(
                        text.format(irec + 1, chain, nres, *seqList[chain][iB:iF])
                    )
                    nSeq += 1
                    iB += 13
            return nSeq
        ...
        for phasenam in self.phasenam:
            ...
            line = "CRYST1 {:8.3f} {:8.3f} {:8.3f} {:6.2f} {:6.2f} {:6.2f} ".format(
                *Cell
            )
            line += General["SGData"]["SpGrp"].ljust(13)
            line += "%2d" % (
                len(General["SGData"]["SGOps"]) * len(General["SGData"]["SGCen"])
            )
            self.Write(line)
            ...
            self.Write(
                "SCALE1     {:9.6f} {:9.6f} {:9.6f}        0.00000".format(*B[0])
            )
            self.Write(
                "SCALE2     {:9.6f} {:9.6f} {:9.6f}        0.00000".format(*B[1])
            )
            self.Write(
                "SCALE3     {:9.6f} {:9.6f} {:9.6f}        0.00000".format(*B[2])
            )
            iatom = 1
            nHet = 0
            nTer = 0
            ...
            for atom in Atoms:
                if (
                    atom[cia] == "I"
                ):  # need to deal with aniso thermals for proteins = "ANISOU" records
                    Biso = atom[cia + 1] * 8.0 * np.pi**2
                ...
                if atom[ct - 3] in AA3letter or notProt:
                    ...
                    if atom[ct - 1] == "OXT":
                        ...
                        self.Write(
                            "{:6s}{:5d}  {:4s}{:3s}".format(
                                "TER   ", iatom, atom[ct - 1], atom[ct - 3].strip()
                            )
                        )
                        nTer += 1
                else:
...
class ExportDrawPhaseCartXYZ(G2fil.ExportBaseclass):
    ...
    def Exporter(self, event=None):
        ...
        for phasenam in self.phasenam:
            ...
            self.CloseFile()
            print(
                "Phase Draw Atoms " + phasenam + " written to XYZ file " + self.fullpath
            )
