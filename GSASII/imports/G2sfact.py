        ...
        for v in S[:nCol]:
            try:
                ...
                parent.errors = (
                    "line "
                    + str(line)
                    + ": string found where a number is expected\n"
                    + l
                )
                return False
        l = filepointer.readline()
        ...
        try:
            ...
            for line, S in enumerate(fp):
                ...
                self.RefDict["RefList"].append(
                    [h, k, l, 1, 0, Fo**2, 2.0 * Fo * sigFo, 0, Fo**2, 0, 0, 0]
                )
            fp.close()
    ...
    def Reader(self, filename, ParentFrame=None, **unused):
        ...
        try:
            ...
            for line, S in enumerate(fp):
                ...
                self.RefDict["RefList"].append(
                    [h, k, l, m, 1, 0, Fo**2, sigFo2, 0, Fo**2, 0, 0, 0]
                )
            fp.close()
            self.errors = "Error after reading reflections (unexpected!)"
            self.RefDict["RefList"] = np.array(self.RefDict["RefList"])
...
class SHELX4_ReaderClass(G2obj.ImportStructFactor):
    ...
    def __init__(self):
        if "linux" in sys.platform:  # wx 3.0.0.0 on gtk does not like Unicode in menus
            formatName = "HKLF 4"
            longFormatName = (
                "Shelx HKLF 4 [hkl, Fo2, sig(Fo2)] Structure factor text file"
            )
        else:
            formatName = "Shelx HKLF 4 F\u00b2"
            longFormatName = (
                "Shelx HKLF 4 [hkl, Fo\u00b2, sig(Fo\u00b2)] Structure factor text file"
            )
        super(self.__class__, self).__init__(  # fancy way to self-reference
            extensionlist=(".hkl", ".HKL"),
            strictExtension=False,
        ...
        try:
            ...
            for line, S in enumerate(fp):
                ...
                if "-" in Tw:
                    ...
                    if self.Super == 0:
                        self.RefDict["RefList"].append(
                            [h, k, l, int(Tw), 0, Fo, sigFo, 0, Fo, 0, 0, 0]
                        )
                    elif self.Super == 1:
                        self.RefDict["RefList"].append(
                            [h, k, l, m1, int(Tw), 0, Fo, sigFo, 0, Fo, 0, 0, 0]
                        )
...
class M90_ReaderClass(G2obj.ImportStructFactor):
    ...
    def __init__(self):
        if "linux" in sys.platform:  # wx 3.0.0.0 on gtk does not like Unicode in menus
            ...
            longFormatName = (
                "JANA [hkl, Fo\u00b2, sig(Fo\u00b2)] Structure factor text file"
            )
        super(self.__class__, self).__init__(  # fancy way to self-reference
        ...
        try:
            ...
            for line, S in enumerate(fp):
                ...
                if self.Super == 0:
                    ...
                    self.RefDict["RefList"].append(
                        [h, k, l, m1, 1, 0, Fo, sigFo, 0, Fo, 0, 0, 0]
                    )
            fp.close()
            self.errors = "Error after reading reflections (unexpected!)"
            self.RefDict["RefList"] = np.array(self.RefDict["RefList"])
...
class NT_HKLF2_ReaderClass(G2obj.ImportStructFactor):
    ...
    def __init__(self):
        if "linux" in sys.platform:  # wx 3.0.0.0 on gtk does not like Unicode in menus
            ...
            longFormatName = (
                "Neutron SNS TOF [hkl, Fo2, sig(Fo2),...] Structure factor text file"
            )
        else:
            ...
            for line, S in enumerate(fp):
                ...
                if len(self.Banks):
                    self.Banks[int(bN) - 1]["RefDict"]["RefList"].append(
                        [h, k, l, 1, 0, Fo, sigFo, 0, Fo, 0, 0, 0, wave, tbar]
                    )
                else:
                    # h,k,l,m,dsp,Fo2,sig,Fc2,Fot2,Fct2,phase,...
                    self.RefDict["RefList"].append(
...
class NT_JANA2K_ReaderClass(G2obj.ImportStructFactor):
    ...
    def __init__(self):
        if "linux" in sys.platform:  # wx 3.0.0.0 on gtk does not like Unicode in menus
            ...
            longFormatName = (
                "Neutron TOF [hkl, Fo2, sig(Fo2),...] Structure factor text file"
            )
        else:
