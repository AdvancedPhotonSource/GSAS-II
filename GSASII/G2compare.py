...
class MakeTopWindow(wx.Frame):
    ...
    def __init__(self, parent: wx.Window | None):
        ...
        for i, m in enumerate(("Histogram", "Phase", "Project")):
            ...
            item = self.Mode.AppendRadioItem(
                i + 1, m + f"\tCtrl+{i + 1}", f"Display {m}s"
            )
            self.Bind(wx.EVT_MENU, self.onRefresh, id=item.GetId())
        self.hFilter = self.Mode.Append(
            wx.ID_ANY, "Filter by histogram\tCtrl+F", "Only show selected histograms"
        )
    ...
    def doneLoad(self) -> None:
        ...
        if self.getMode() == "Project":
            overId = self.GPXtree.InsertItem(
                pos=0, parent=self.root, text="Project Overview"
            )
...
'''
        ...
        for w in wl:
            ...
            for fil in glob.glob(w):
                if not os.path.exists(fil):
                    continue
                if ".bak" in fil and "bak" not in w:
                    continue
                if fil in [i for i, j in self.fileList]:
                    continue
                self.fileList.append([fil, "GPX"])
                self.loadFile(fil)
        self.doneLoad()
        ...
        try:
            while True:
                try:
                    ...
                    break
                if not data[0][0].startswith("Phase"):
                    continue
                Phases = data
                # if self.PWDRfilter is not None: # implement filter
        ...
        try:
            while True:
                ...
                if data[0][0].startswith("PWDR"):
                    self.histListOrg.append(data[0][0])
                    if self.PWDRfilter is not None:  # implement filter
                        if self.PWDRfilter in data[0][0]:
                            match = True
                if not data[0][0].startswith("Covariance"):
                    continue
                Covar = data[0]
                f = "{:d}"
    ...
    def onProjFtest(self, event):
        ...
        if len(items) < 2:
            ...
            dlg = G2G.G2SingleChoiceDialog(
                self, "Select constrained refinement", "Choose refinement", sel
            )
            if dlg.ShowModal() == wx.ID_OK:
                s0 = dlg.GetSelection()
                dlg.Destroy()
            ...
            del inds[s0]
            dlg = G2G.G2SingleChoiceDialog(
                self, "Select relaxed refinement", "Choose refinement", sel
            )
            if dlg.ShowModal() == wx.ID_OK:
                s1 = dlg.GetSelection()
                s1 = inds[s1]
            ...
            if len(baseDict["varyList"]) > len(relxDict["varyList"]):
                G2G.G2MessageBox(
                    ...
                    + "variables ({}) than relaxed refinement ({}). Swapping".format(
                        len(baseDict["varyList"]), len(relxDict["varyList"])
                    ),
                    "Fits reversed?",
                )
                s0, s1, baseDict, relxDict = s1, s0, relxDict, baseDict
        ...
        if len(baseDict["varyList"]) == len(relxDict["varyList"]):
            G2G.G2MessageBox(
                self,
                "F-test requires differing numbers of variables",
                "F-test not valid",
            )
            return
            ...
            G2G.G2MessageBox(
                self,
                "F-test requires same number of observations in each refinement",
                "F-test not valid",
            )
            return
        missingVars = []
        ...
        if missingVars:
            txt = (
                "*** Possible invalid use of F-test: "
        ...
        msg += fmt.format(
            "Constrained",
            self.GPXtree.GetItemText(s0)[:-11],
            len(baseDict["varyList"]),
        ...
        msg += fmt.format(
            "Relaxed",
            self.GPXtree.GetItemText(s1)[:-11],
            len(relxDict["varyList"]),
        ...
        if prob > 0.95:
            msg += (
                "The relaxed model is statistically preferred to the constrained model."
            )
        elif prob > 0.75:
            msg += "There is little ability to differentiate between the two models on a statistical basis."
        else:
            msg += (
                "The constrained model is statistically preferred to the relaxed model."
            )
        msg += postmsg
    ...
    def onHistPrinceTest(self, event):
        ...
        if len(items) < 2:
            ...
            for i in items:
                sel.append(self.GPXtree.GetItemText(i))
            dlg = G2G.G2SingleChoiceDialog(
                self, "Select one refinement", "Choose refinement", sel
            )
            if dlg.ShowModal() == wx.ID_OK:
                s0 = dlg.GetSelection()
                dlg.Destroy()
            ...
            del inds[s0]
            dlg = G2G.G2SingleChoiceDialog(
                self, "Select comparison refinement", "Choose refinement", sel
            )
            if dlg.ShowModal() == wx.ID_OK:
                s1 = dlg.GetSelection()
                s1 = inds[s1]
        ...
        if len(data0[0]) != len(data1[0]):
            G2G.G2MessageBox(
                self,
                "Unable to test: differing numbers of data points",
                "Comparison not valid",
            )
            return
        if max(abs((data0[0] - data1[0]) / data0[0])) > 0.01:
            G2G.G2MessageBox(
                self, 'Unable to use test: "X" values differ', "Comparison not valid"
            )
            return
        # X = data0[3] - data1[3]
