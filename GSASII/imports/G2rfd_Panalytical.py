    ...
    def Reader(self, filename, ParentFrame=None, **kwarg):
        ...
        try:
            self.comments.append(
                "Date/TimeStart=" + header.find(tag + "startTimeStamp").text
            )
            self.comments.append(
        ...
        x = 4.0 * np.pi * sind(x / 2.0) / self.instdict["wave"]
        w = np.where(y > 0, 1.0 / y, 1.0)
        ...
        if conditions is not None:
            ...
            if kind == "Temperature":
                Temperature = float(
                    conditions.find(tag + "nonAmbientValues").text.split()[-1]
                )
