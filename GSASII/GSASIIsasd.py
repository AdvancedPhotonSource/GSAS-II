...
def SphereFF(Q, R, args=()):
    ...
    """
    QR = Q[:, np.newaxis] * R
    return (3.0 / (QR**3)) * (np.sin(QR) - (QR * np.cos(QR)))
...
def CylinderFF(Q, R, args):
    ...
    L = args[0]
    NP = 200
    alp = np.linspace(0, np.pi / 2.0, NP)
    QL = Q[:, np.newaxis] * L
    LBessArg = 0.5 * QL[:, :, np.newaxis] ** np.cos(alp)
    LBess = np.where(LBessArg < 1.0e-6, 1.0, np.sin(LBessArg) / LBessArg)
    QR = Q[:, np.newaxis] * R
    SBessArg = QR[:, :, np.newaxis] * np.sin(alp)
    SBess = np.where(SBessArg < 1.0e-6, 0.5, scsp.jv(1, SBessArg) / SBessArg)
    LSBess = LBess * SBess
    return np.sqrt(2.0 * np.pi * np.sum(np.sin(alp) * LSBess**2, axis=2) / NP)
...
def CylinderDFF(Q, L, args):
    ...
    """
    R = args[0]
    return CylinderFF(Q, R, args=[2.0 * L])
...
def UniRodFF(Q, R, args):
    ...
    B2 = np.pi / L
    Rg1 = np.sqrt(3.0) * R / 2.0
    G1 = (2.0 / 3.0) * R / L
    B1 = 4.0 * (L + R) / (R**3 * L**2)
    QstV = Q[:, np.newaxis] / (scsp.erf(Q[:, np.newaxis] * Rg2 / np.sqrt(6))) ** 3
    FF = np.exp(-(Q[:, np.newaxis] ** 2) * Rg2**2 / 3.0) + (B2 / QstV) * np.exp(
        -(Rg1**2) * Q[:, np.newaxis] ** 2 / 3.0
    )
    QstV = Q[:, np.newaxis] / (scsp.erf(Q[:, np.newaxis] * Rg1 / np.sqrt(6))) ** 3
    FF += G1 * np.exp(-(Q[:, np.newaxis] ** 2) * Rg1**2 / 3.0) + (B1 / QstV**4)
    return np.sqrt(FF)
...
def UniDiskFF(Q, R, args):
    ...
    """
    T = args[0]
    Rg2 = np.sqrt(R**2 / 2.0 + T**2 / 12.0)
    B2 = 2.0 / R**2
    Rg1 = np.sqrt(3.0) * T / 2.0
    RgC2 = 1.1 * Rg1
    G1 = (2.0 / 3.0) * (T / R) ** 2
    B1 = 4.0 * (T + R) / (R**3 * T**2)
    QstV = Q[:, np.newaxis] / (scsp.erf(Q[:, np.newaxis] * Rg2 / np.sqrt(6))) ** 3
    FF = np.exp(-(Q[:, np.newaxis] ** 2) * Rg2**2 / 3.0) + (B2 / QstV**2) * np.exp(
        -(RgC2**2) * Q[:, np.newaxis] ** 2 / 3.0
    )
    QstV = Q[:, np.newaxis] / (scsp.erf(Q[:, np.newaxis] * Rg1 / np.sqrt(6))) ** 3
    FF += G1 * np.exp(-(Q[:, np.newaxis] ** 2) * Rg1**2 / 3.0) + (B1 / QstV**4)
    return np.sqrt(FF)
...
def UniTubeFF(Q, R, args):
    ...
    DR2 = R**2 - Ri**2
    Vt = np.pi * DR2 * L
    Rg3 = np.sqrt(DR2 / 2.0 + L**2 / 12.0)
    B1 = 4.0 * np.pi**2 * (DR2 + L * (R + Ri)) / Vt**2
    B2 = np.pi**2 * T / Vt
    B3 = np.pi / L
    QstV = Q[:, np.newaxis] / (scsp.erf(Q[:, np.newaxis] * Rg3 / np.sqrt(6))) ** 3
    FF = np.exp(-(Q[:, np.newaxis] ** 2) * Rg3**2 / 3.0) + (B3 / QstV) * np.exp(
        -(Q[:, np.newaxis] ** 2) * R**2 / 3.0
    )
    QstV = Q[:, np.newaxis] / (scsp.erf(Q[:, np.newaxis] * R / np.sqrt(6))) ** 3
    FF += (B2 / QstV**2) * np.exp(-(Q[:, np.newaxis] ** 2) * T**2 / 3.0)
...
def LogNormalDist(x, pos, args):
    ...
    """
    scale, shape = args
    return np.exp(-(np.log((x - pos) / scale) ** 2) / (2.0 * shape**2)) / (
        np.sqrt(2.0 * np.pi) * (x - pos) * shape
    )
...
def LSWDist(x, pos, args=None):
    ...
    result = (
        (81.0 * 2 ** (-5 / 3.0))
        * (redX**2 * np.exp(-redX / (1.5 - redX)))
        / ((1.5 - redX) ** (11 / 3.0) * (3.0 - redX) ** (7 / 3.0))
    )
...
def SchulzZimmDist(x, pos, args):
    ...
    a = b / pos
    if b < 70:  # why bother?
        return (a ** (b + 1.0)) * x**b * np.exp(-a * x) / scsp.gamma(b + 1.0)
    else:
        return np.exp(
            (b + 1.0) * np.log(a) - scsp.gammaln(b + 1.0) + b * np.log(x) - (a * x)
        )
...
def LogNormalCume(x, pos, args):
    ...
    return (scsp.erf((lredX / shape) / np.sqrt(2.0)) + 1.0) / 2.0
...
def SchulzZimmCume(x, pos, args):
    ...
    scale = args[0]
    nP = 500
    xMin = np.fmax(0.0, pos - 4.0 * scale)
    xMax = np.fmin(pos + 4.0 * scale, 1.0e5)
    X = np.linspace(xMin, xMax, nP, True)
    fxn = LSWDist(X, pos)
    mat = np.outer(np.ones(nP), fxn)
    cume = np.sum(np.tril(mat), axis=1) / np.sum(fxn)
...
def HardSpheresSF(Q, args):
    ...
    num = (1.0 + 2.0 * VolFr) ** 2
    alp = num / denom
    bet = -6.0 * VolFr * (1.0 + VolFr / 2.0) ** 2 / denom
    ...
    cbet = bet * (2.0 * Rsa / A2 - (A2 - 2.0) * Rca / A3 - 2.0 / A3)
    cgam = gamm * (
        -Rca / A
        + (4.0 / A) * ((3.0 * A2 - 6.0) * Rca / A4 + (A2 - 6.0) * Rsa / A3 + 6.0 / A4)
    )
    pfac = -24.0 * VolFr / A
...
def SquareWellSF(Q, args):
    ...
    etam1 = 1.0 - eta
    etam14 = etam1**4
    alp = (((1.0 + 2.0 * eta) ** 2) + eta3 * (eta - 4.0)) / etam14
    bet = -(eta / 3.0) * (18.0 + 20.0 * eta - 12.0 * eta2 + eta4) / etam14
    gam = 0.5 * eta * ((1.0 + 2.0 * eta) ** 2 + eta3 * (eta - 4.0)) / etam14
    ...
    T2 = bet * SK2 * (2.0 * SK * np.sin(SK) - (SK2 - 2.0) * np.cos(SK) - 2.0)
    T3 = (4.0 * SK3 - 24.0 * SK) * np.sin(SK)
...
def InterPrecipitateSF(Q, args):
    ...
    R, VolFr = args
    QV2 = Q**2 * VolFr**2
    top = 1 - np.exp(-QV2 / 4) * np.cos(2.0 * Q * R)
    bot = 1 - 2 * np.exp(-QV2 / 4) * np.cos(2.0 * Q * R) + np.exp(-QV2 / 2)
    return 2 * (top / bot) - 1
...
def G_matrix(q, r, contrast, FFfxn, Volfxn, args=()):
    ...
    FF = FFfxn(q, r, args)
    Vol = Volfxn(r, args)
    return 1.0e-4 * (contrast * Vol * FF**2).T  # 10^-20 vs 10^-24
...
def MaxEnt_SB(
    datum, sigma, G, base, IterMax, image_to_data=None, data_to_image=None, report=False
):
    """
    ...
    def MaxEntMove(fSum, blank, chisq, chizer, c1, c2, s1, s2):
        ...
        """
        a_lower, a_upper = 0.0, 1.0  # bracket  "a"
        ...
        m = SEARCH_DIRECTIONS
        if (
            w > DISTANCE_LIMIT_FACTOR * fSum / blank
        ):  # invoke the distance penalty, SB eq. 17
            for k in range(m):
    ...
    for iter in range(IterMax):
        ...
        beta[2] = 0.0
        if iter > 0:
            w, chtarg, loop, a_new, fx, beta = MaxEntMove(
                fSum, blank, chisq, chizer, c1, c2, s1, s2
            )
...
def IPG(
    ...
    Bvec = IntE * QvecP
    Xw = np.ones_like(Bins) * 1.0e-32
    calc = np.dot(G.T, Xw)
    nIter = 0
    err = 10.0
    while (nIter < IterMax) and (err > 1.0):
        # Step 1 in M&Z paper:
        ...
        alpWv = -Xw / Pk
        AkSt = [
            np.where(Pk[i] < 0, np.min((approach * alpWv[i], alpSt)), alpSt)
        ...
        if report:
            print(
                " Iteration: %d, chisq: %.3g, sum(shift^2): %.3g"
                % (nIter, chisq, np.sum(shift**2))
            )
...
def SmearData(Ic, Q, slitLen, Back):
    ...
    Ictemp = np.concatenate(
        [Ic, Ic[-1] * (1 - (Qtemp[Np:] - Qtemp[Np]) / (20 * Qtemp[Np - 1]))]
    )
    Icsm = np.zeros_like(Q)
    Qsm = (
        2
        * slitLen
        * (np.interp(np.arange(2 * Np) / 2.0, np.arange(Np), Q) - Q[0])
        / (Q[-1] - Q[0])
    )
...
def SizeDistribution(Profile, ProfDict, Limits, Sample, data):
    shapes = {
        ...
        "Cylinder AR": [CylinderARFF, CylinderARVol],
        "Unified sphere": [UniSphereFF, UniSphereVol],
        "Unified rod": [UniRodFF, UniRodVol],
        "Unified rod AR": [UniRodARFF, UniRodARVol],
        "Unified disk": [UniDiskFF, UniDiskVol],
        "Sphere": [SphereFF, SphereVol],
    ...
    if data["Size"]["logBins"]:
        Bins = (
            np.logspace(
                np.log10(data["Size"]["MinDiam"]),
                np.log10(data["Size"]["MaxDiam"]),
                data["Size"]["Nbins"] + 1,
                True,
            )
            / 2.0
    ...
    Ic[:] = 0.0
    Gmat = G_matrix(
        Q[Ibeg:Ifin], Bins, Contrast, shapes[Shape][0], shapes[Shape][1], args=Parms
    )
    if data["Size"]["Method"] == "MaxEnt":
        chisq, BinMag, Ic[Ibeg:Ifin] = MaxEnt_SB(
            Scale * Io[Ibeg:Ifin] - Back[0],
            Scale / np.sqrt(wtFactor * wt[Ibeg:Ifin]),
            Gmat,
            BinsBack,
            data["Size"]["MaxEnt"]["Niter"],
            report=True,
        )
    elif data["Size"]["Method"] == "IPG":
        chisq, BinMag, Ic[Ibeg:Ifin] = IPG(
            Scale * Io[Ibeg:Ifin] - Back[0],
            Scale / np.sqrt(wtFactor * wt[Ibeg:Ifin]),
            Gmat,
            Bins,
...
def PairDistFxn(Profile, ProfDict, Limits, Sample, data):
    def CalcMoore():
        def MoorePOR(cw, r, dmax):  # lines 1417-1428
            ...
            POR = np.zeros(len(r))
            while n < nmax:
                POR += 4.0 * r * cw[n] * np.sin((n + 1.0) * np.pi * r / dmax)
                n += 1
            return POR
        ...
        def MooreIOREFF(cw, q, dmax):  # lines 1437-1448
            ...
            POR = np.zeros(len(q))
            dq = dmax * q
            fpd = 8.0 * (np.pi**2) * dmax * np.sin(dq) / q
            while n < nmax:
                POR += (
                    cw[n]
                    * (n + 1.0)
        ...
        def calcSASD(values, Q, Io, wt, Ifb, dmax, ifBack):
            if ifBack:
                M = np.sqrt(wt) * (
                    MooreIOREFF(values[:-1], Q, dmax) + Ifb + values[-1] - Io
                )
            else:
        ...
        Ifin = np.searchsorted(Q, Qmax) + 1  # include last point
        Ic[Ibeg:Ifin] = 0
        Bins = np.linspace(0.0, pairData["MaxRadius"], pairData["NBins"] + 1, True)
        Dbins = np.diff(Bins)
        Bins = Bins[:-1] + Dbins / 2.0
        N = pairData["Moore"]
        if ifBack:
            N += 1
        MPV = np.ones(N) * 1.0e-5
        ...
        if "User" in pairData["Errors"]:
            ...
            Wt = 1.0 / (pairData["Percent error"] * Io[Ibeg:Ifin])
        result = so.leastsq(
            calcSASD,
            MPV,
            full_output=True,
            epsfcn=1.0e-8,  # ftol=Ftol,
            args=(
                Q[Ibeg:Ifin],
                Io[Ibeg:Ifin],
                wtFactor * Wt,
...
def ModelFit(Profile, ProfDict, Limits, Sample, Model):
    shapes = {
        ...
        "Unified disk": [UniDiskFF, UniDiskVol],
        "Sphere": [SphereFF, SphereVol],
        "Unified tube": [UniTubeFF, UniTubeVol],
        "Cylinder diam": [CylinderDFF, CylinderDVol],
        "Spherical shell": [SphericalShellFF, SphericalShellVol],
    }
    ...
    sfxns = {
        "Dilute": DiluteSF,
        "Hard sphere": HardSpheresSF,
        "Square well": SquareWellSF,
        "Sticky hard sphere": StickyHardSpheresSF,
        "InterPrecipitate": InterPrecipitateSF,
    }
    ...
    def SetModelParms():
        ...
        if "Back" in varyList:
            ...
            print(
                "  %15s %15.4f esd: %15.4g"
                % ("Background:", parmDict["Back"], sigDict["Back"])
            )
        partData = Model["Particle"]
        for i, level in enumerate(partData["Levels"]):
            controls = level["Controls"]
            ...
            if Type in ["LogNormal", "Gaussian", "LSW", "Schulz-Zimm", "Monodisperse"]:
                print(
                    " Component %d: Type: %s: Structure Factor: %s Contrast: %12.3f"
            ...
            if Type in ["LogNormal", "Gaussian", "LSW", "Schulz-Zimm", "Monodisperse"]:
                for item in FFparmOrder:
                    if cid + item in varyList:
                        ...
                        print(
                            " %15s: %15.4g esd: %15.4g"
                            % (cid + item, parmDict[cid + item], sigDict[cid + item])
                        )
                for item in SFparmOrder:
                    if cid + item in varyList:
    ...
    def calcSASD(values, Q, Io, wt, Ifb, levelTypes, parmDict, varyList):
        ...
        M = np.sqrt(wt) * (
            getSASD(Q, levelTypes, parmDict) + Ifb - parmDict["Scale"] * Io
        )
        return M
    ...
    def getSASD(Q, levelTypes, parmDict):
        ...
        for i, Type in enumerate(levelTypes):
            ...
            if Type in ["LogNormal", "Gaussian", "LSW", "Schulz-Zimm"]:
                ...
                rBins, dBins, dist = MakeDiamDist(
                    Type,
                    parmDict[cid + "NumPoints"],
                    parmDict[cid + "Cutoff"],
                    distDict,
                )
    ...
    try:
        ...
        if np.any(Nans):
            ...
            raise ValueError
        Negs = np.less_equal(result[0], 0.0)
        if np.any(Negs):
...
def ModelFxn(Profile, ProfDict, Limits, Sample, sasdData):
    ...
    for level in partData["Levels"]:
        ...
        if distFxn in ["LogNormal", "Gaussian", "LSW", "Schulz-Zimm"]:
            ...
            Ic[Ibeg:Ifin] += (
                Gmat[0] * level[distFxn]["Volume"][0] * SFfxn(Q[Ibeg:Ifin], args=SFargs)
            )
            Rbins.append([])
            Dist.append([])
...
def MakeDiamDist(DistName, nPoints, cutoff, distDict):
    if "LogNormal" in DistName:
        ...
        minX = 1.0  # pos
        maxX = 1.0e4  # np.min([mode+nPoints*step*40,1.e5])
        ...
        mode = distDict["Mean"]
        minX = np.max([mode - 4.0 * distDict["StdDev"], 1.0])
        maxX = np.min([mode + 4.0 * distDict["StdDev"], 1.0e5])
    elif "LSW" in DistName:
        distFxn = "LSWDist"
        cumeFxn = "LSWCume"
        pos = distDict["Mean"]
        args = []
        minX, maxX = [0.0, 2.0 * pos]
    ...
    CumeTgts = np.linspace(cutoff, (1.0 - cutoff), nPoints + 1, True)
    rBins = np.interp(CumeTgts, TCW, Diam, 0, 0)
    dBins = np.diff(rBins)
    rBins = rBins[:-1] + dBins / 2.0
