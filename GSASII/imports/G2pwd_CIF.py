    ...
    def Reader(self, filename, ParentFrame=None, **kwarg):
        ...
        if cf is None:
            if GSASIIpath.GetConfigValue("debug"):
                print(f"Starting parse of {filename} as CIF")
        ...
        if not choicelist:
            ...
            self.repeatcount += 1
            if self.repeatcount >= len(selections):
                self.repeat = False
            ...
            if len(selections) > 1:  # prepare to loop through again
                ...
                if rdbuffer is not None:
                    ...
                    rdbuffer["choicelist"] = (
                        choicelist  # save the parsed choices for the future
                    )
