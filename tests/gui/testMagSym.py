...
class testMagSym(wx.Frame):
    def _init_ctrls(self, parent):
        wx.Frame.__init__(
            ...
            title="Test magnetic space groups",
        )
    ...
    def UpdateData(self, Data):
        ...
        mainSizer.Add(
            wx.StaticText(self.testSSPanel, -1, "  Propagation vector: "), 0, WACV
        )
        propVec = wx.TextCtrl(
            self.testSSPanel, value=self.propVec, style=wx.TE_PROCESS_ENTER
        )
        propVec.Bind(wx.EVT_TEXT_ENTER, OnPropVec)
        mainSizer.Add(propVec, 0, WACV)
        magSizer = wx.BoxSizer(wx.VERTICAL)
        magSizer.Add(
            wx.StaticText(self.testSSPanel, label=" Magnetic spin operator selection:")
        )
        ...
        if "(" in SGData["BNSlattsym"][0]:
            spinSizer.Add(
                wx.StaticText(
                    self.testSSPanel, label="Choose new space group to change BNS"
                ),
                0,
                WACV,
            )
            ...
            spinSizer.Add(
                wx.StaticText(self.testSSPanel, label=" BNS lattice:"), 0, WACV
            )
            BNS = wx.ComboBox(
                self.testSSPanel,
        ...
        magStr = [str(smag) for smag in CSI[0]]
        magVal = [str(val) for val in CSI[1]]
        CSItxt = wx.StaticText(
            self.testSSPanel,
            label=" site sym: %6s, mult: %3d, MCSI: %s %s"
            % (SytSym, Mul, magStr, magVal),
        )
