...
def GetTifData(filename):
    ...
    if 34710 in IFD:
        ...
        if (
            marFrame.viewDirection
        ):  # view through sample to detector instead of TOWARD_SOURCE
        ...
        wavelength = (
            (marFrame.opticsWavelength > 0) and marFrame.opticsWavelength * 1e-5
        ) or wavelength
        ...
        distance = (
            (distance <= marFrame.startXtalToDetector * 5e-4)
            and marFrame.xtalToDetector * 1e-3
        ) or distance
    ...
    # print head,tifType,pixy
        ...
        tifType = "CheMin"
        pixy = [40.0, 40.0]
        image = np.flipud(np.array(Im.open(filename))) * 10.0
        distance = 18.0
        ...
        if b"Pilatus3" in S or sizexy == [1475, 1679]:
            ...
            image = np.array(
                np.frombuffer(File.read(4 * Npix), dtype=byteOrd + "f4"), dtype=np.int32
            )
        elif b"ImageJ" in S:
            tifType = "ImageJ"
            dataType = 0
            pixy = [200.0, 200.0] * IFD[277][2][0]
            File.seek(IFD[273][2][0])
            G2fil.G2Print("Read ImageJ tiff file: " + filename)
            if IFD[258][2][0] == 32:
                image = File.read(4 * Npix)
                image = np.array(
                    np.frombuffer(image, dtype=byteOrd + "i4"), dtype=np.int32
                )
            elif IFD[258][2][0] == 16:
                ...
                image = np.array(
                    np.frombuffer(image, dtype=byteOrd + "u2"), dtype=np.int32
                )
            ...
            image = File.read(4 * Npix)
            image = np.array(
                np.frombuffer(image, dtype=byteOrd + "f4") * 1000, dtype=np.int32
            )
        ...
        if IFD[273][2][0] == 8:
            if IFD[258][2][0] == 32:
                ...
                if dataType == 5:
                    ...
                    image = np.array(
                        np.frombuffer(File.read(4 * Npix), dtype=np.int32),
                        dtype=np.int32,
                    )
            elif IFD[258][2][0] == 16:
        ...
        if IFD[273][2][0] == 8:
            if IFD[258][2][0] == 16:
                ...
                image = np.array(
                    np.frombuffer(File.read(4 * Npix), dtype=np.float32) / 2.0**4,
                    dtype=np.int32,
                )
        elif IFD[273][2][0] == 4096:
            tifType = "Rayonix"
        ...
        File.seek(8)
        image = np.array(
            np.frombuffer(File.read(2 * Npix), dtype=np.int16), dtype=np.int32
        )
    elif sizexy == [380, 391]:
        File.seek(110)
        pixy = [109.92, 109.92]
        ...
        dt = dt.newbyteorder(byteOrd)
        image = np.array(
            np.frombuffer(File.read(Npix * 4), dtype=np.uint32), dtype=np.int32
        )
        ...
        dt = dt.newbyteorder(byteOrd)
        image = np.array(
            np.frombuffer(File.read(Npix * 2), dtype=np.uint16), dtype=np.int32
        )
...
def TIFValidator(filename):
    ...
    if "bytes" in str(type(tag)):
        tag = tag.decode("latin-1")
    if (tag == "II" and int(st.unpack("<h", fp.read(2))[0]) == 42) or (
        tag == "MM" and int(st.unpack(">h", fp.read(2))[0]) == 42
    ):  # little endian
        pass
    else:
