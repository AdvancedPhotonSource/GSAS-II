...
def scaleAbyV(A, V):
    "needs a doc string"
    v = G2lat.calc_V(A)
    scale = math.exp(math.log(v / V) / 3.0) ** 2
    for i in range(6):
        A[i] *= scale
...
def findMV(peaks, controls, ssopt, Inst, dlg):
    def Val2Vec(vec, Vref, values):
        ...
        for j, r in enumerate(Vref):
            if r:
                if values.size > 1:
                    Vec.append(max(-1.0, min(1.0, values[i])))
                else:
...
def IndexPeaks(peaks, HKL):
    ...
    for ipk, peak in enumerate(peaks):
        ...
        if peak[2]:
            ...
            if dp > dm:
                pos += 1  # closer to upper than lower
            if pos >= N:
                break
...
def FitHKL(ibrav, peaks, A, Pwr):
    ...
    return (
        True,
        np.sum(errFit(result[0], ibrav, Peaks[7], Peaks[4:7], Pwr) ** 2),
        A,
        result,
    )
...
def FitHKLZ(wave, ibrav, peaks, A, Z, Zref):
    ...
    if Zref:
        Z = result[0][-1]
    chisq = np.sum(
        errFitZ(result[0], ibrav, Peaks[7], Peaks[4:7], Peaks[0], wave, Z, Zref) ** 2
    )
...
def FitHKLZSS(wave, ibrav, peaks, A, V, Vref, Z, Zref):
    ...
    def dervFitZSS(values, ibrav, d, H, tth, wave, vec, Vref, Z, Zref):
        ...
        if Vref[0]:
            derv.append(
                2.0 * A[0] * HM[0] * H[3] + A[3] * HM[1] * H[3] + A[4] * HM[2] * H[3]
            )
        if Vref[1]:
            derv.append(
                2.0 * A[1] * HM[1] * H[3] + A[3] * HM[0] * H[3] + A[5] * HM[2] * H[3]
            )
        if Vref[2]:
            derv.append(
                2.0 * A[2] * HM[2] * H[3] + A[4] * HM[1] * H[3] + A[5] * HM[0] * H[3]
            )
        if Zref:
            derv.append(npsind(tth) * 2.0 * rpd / wave**2)
        derv = -np.array(derv)
    ...
    chisq = np.sum(
        errFitZSS(
            result[0], ibrav, Peaks[8], Peaks[4:8], Peaks[0], wave, Vec, Vref, Z, Zref
        )
        ** 2
    )
...
def FitHKLT(difC, ibrav, peaks, A, Z, Zref):
    ...
    if Zref:
        Z = result[0][-1]
    chisq = np.sum(
        errFitT(result[0], ibrav, Peaks[7], Peaks[4:7], Peaks[0], difC, Z, Zref) ** 2
    )
    return True, chisq, A, Z, result
...
def FitHKLTSS(difC, ibrav, peaks, A, V, Vref, Z, Zref):
    ...
    chisq = np.sum(
        errFitTSS(
            result[0], ibrav, Peaks[8], Peaks[4:8], Peaks[0], difC, V, Vref, Z, Zref
        )
        ** 2
    )
    return True, chisq, A, Vec, Z, result
...
def refinePeaksZ(peaks, wave, ibrav, A, Zero, ZeroRef):
    ...
    Peaks = np.array(peaks).T
    H = Peaks[4:7]
    Peaks[8] = 1.0 / np.sqrt(G2lat.calc_rDsqZ(H, Aref, Z, Peaks[0], wave))
    peaks = Peaks.T
...
def refinePeaksT(peaks, difC, ibrav, A, Zero, ZeroRef):
    ...
    Peaks = np.array(peaks).T
    H = Peaks[4:7]
    Peaks[8] = 1.0 / np.sqrt(G2lat.calc_rDsqT(H, Aref, Z, Peaks[0], difC))
    peaks = Peaks.T
    HKL = G2lat.GenHBravais(dmin, ibrav, A)
    M20, X20 = calc_M20(peaks, HKL)
    return len(HKL), M20, X20, Aref, Z
...
def refinePeaksE(peaks, tth, ibrav, A):
    ...
    Peaks = np.array(peaks).T
    H = Peaks[4:7]
    Peaks[8] = 1.0 / np.sqrt(G2lat.calc_rDsq(H, Aref))
    peaks = Peaks.T
    HKL = G2lat.GenHBravais(dmin, ibrav, A)
    M20, X20 = calc_M20(peaks, HKL)
...
def refinePeaksZSS(
    peaks, wave, Inst, SGData, SSGData, maxH, ibrav, A, vec, vecRef, Zero, ZeroRef
):
    "needs a doc string"
    dmin = getDmin(peaks)
    OK, smin, Aref, Vref, Z, result = FitHKLZSS(
        wave, ibrav, peaks, A, vec, vecRef, Zero, ZeroRef
    )
    Peaks = np.array(peaks).T
    H = Peaks[4:8]
    Peaks[9] = 1.0 / np.sqrt(
        G2lat.calc_rDsqZSS(H, Aref, Vref, Z, Peaks[0], wave)
    )  # H,A,vec,Z,tth,lam
    peaks = Peaks.T
...
def refinePeaksTSS(
    peaks, difC, Inst, SGData, SSGData, maxH, ibrav, A, vec, vecRef, Zero, ZeroRef
):
    "needs a doc string"
    dmin = getDmin(peaks)
    OK, smin, Aref, Vref, Z, result = FitHKLTSS(
        difC, ibrav, peaks, A, vec, vecRef, Zero, ZeroRef
    )
    Peaks = np.array(peaks).T
    H = Peaks[4:8]
    Peaks[9] = 1.0 / np.sqrt(G2lat.calc_rDsqTSS(H, Aref, Vref, Z, Peaks[0], difC))
    peaks = Peaks.T
    HKL = G2pwd.getHKLMpeak(dmin, Inst, SGData, SSGData, Vref, maxH, Aref)
    HKL = G2lat.GenHBravais(dmin, ibrav, A)
    M20, X20 = calc_M20SS(peaks, HKL)
...
def refinePeaks(peaks, ibrav, A, ifX20=True, cctbx_args=None):
    ...
    while len(HKL) > 2 and IndexPeaks(peaks, HKL)[0]:
        ...
        try:
            ...
            break
        if len(HKL) == 0:
            break  # absurd cell obtained!
        rat = (osmin - smin) / smin
        if abs(rat) < 1.0e-5 or not OK:
            break
        if tries > maxTries:
            break
    if OK:
        ...
        H = Peaks[4:7]
        try:
            Peaks[8] = 1.0 / np.sqrt(G2lat.calc_rDsqA(H, A))
            peaks = Peaks.T
        except FloatingPointError:
            A = oldA
...
def findBestCell(dlg, ncMax, A, Ntries, ibrav, peaks, V1, ifX20=True, cctbx_args=None):
    ...
    while tries < Ntries and GoOn:
        ...
        if IndexPeaks(peaks, HKL)[0] and len(HKL) > mHKL[ibrav]:
            Lhkl, M20, X20, Aref = refinePeaks(
                peaks, ibrav, Abeg, ifX20, cctbx_args=cctbx_args
            )
    ...
    X = sortM20(Asave)
    if X:
        Lhkl, M20, X20, A = refinePeaks(
            peaks, ibrav, X[0][1], ifX20, cctbx_args=cctbx_args
        )
...
def monoCellReduce(ibrav, A):
    ...
    if bet < 90.0:
        bet = 180.0 - bet  # always want obtuse beta
        A = G2lat.cell2A([a, b, c, 90.0, bet, 90.0])
    G, g = G2lat.A2Gmat(A)
    if ibrav in [
        14,
    ]:  # A-monoclinic - OK?
        ...
        if anew < a:
            ...
            beta = acosd(-abs(cang))
            if beta < 90.0:
                beta = 180.0 - beta  # always want obtuse beta
...
def DoIndexPeaks(
    ...
    for ibrav in range(len(bravaisNames)):
        ...
        if bravais[ibrav]:
            ...
            print(
                "      M20  X20  Nc       a          b          c        alpha       beta      gamma     volume      V-test"
            )
            V1 = controls[3]
            bestM20 = 0
            topM20 = 0
            ...
            while cycle < 5:
                if dlg:
                    ...
                    dlg.Update(
                        0,
                        newmsg=tries[cycle] + " cell search for " + bravaisNames[ibrav],
                    )
                try:
                    GoOn = True
                    while GoOn:  # Loop over increment of volume
                        ...
                        while N2s[ibrav] > N2:  # Table 2 step (iii)
                            ...
                            if ibrav > 2:
                                if not N2:
                                    A = []
                                    GoOn, Skip, Nc, M20, X20, A = findBestCell(
                                        dlg,
                                        ncMax,
                                        A,
                                        Nm[ibrav] * N1s[ibrav],
                            ...
                            if not GoOn:
                                break
                            if 1.0e6 > M20 > 1.0:  # exclude nonsense
                                ...
                                V = G2lat.calc_V(A)
                                if (M20_min <= M20) and (
                                    X20_max is None or X20_max >= X20
                                ):
                                    cell = [
                                        M20,
                                        X20,
                                    ...
                                    if not np.allclose(newcell, lastcell):
                                        print(
                                            "%10.3f %3d %3d %10.5f %10.5f %10.5f %10.3f %10.3f %10.3f %10.2f %10.2f %s"
                                            % (
                                                M20,
                                                X20,
                                                Nc,
...
def TestData():
    ...
    TestData = [
        ...
        [7.76006, 8.706215, 10.865679, 90.0, 102.947, 90.0],
        3,
        [
            [
                2.176562137832974,
    ...
    TestData2 = [
        ...
        [
            0.15336547830008007,
            0.017345499139401827,
            0.008122368657493792,
            0,
            0.02893538955687591,
            0,
        ],
