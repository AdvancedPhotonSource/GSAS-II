...
def AllPrmDerivs(
    ...
    for i, prm in enumerate(origParms):
        ...
        if (
            ...
            and "PWDR" not in Histograms.keys()
        ):
            continue
        dprm = prm
    ...
    for ihst, histogram in enumerate(histNames):
            ...
            for phase in Phases:
                if (
                    ...
                    if len(newRefs) == len(oldFsqs.T):
                        ...
                        print(
                            "ERROR - mismatch in reflection list length bewteen %s and %s; no copy done"
                            % (lasthist, histogram)
                        )
        ...
        try:
            ...
            try:
                ...
                shft = "?"
            G2fil.G2Print(
                "  wR = %7.2f%%, chi**2 = %12.6g, reduced chi**2 = %6.2f, last delta chi = %.4f, last shft/sig = %s"
                % (
                    Rvals["Rwp"],
                    Rvals["chisq"],
                    Rvals["GOF"] ** 2,
...
def RetDistAngle(DisAglCtls, DisAglData, dlg=None):
    ...
    for iO, Oatom in enumerate(origAtoms):
        ...
        for Tatom in targAtoms:
            ...
            for [Txyz, Top, Tunit, Spn] in G2spc.GenAtom(
...
def DisAglTor(DATData):
    ...
    if DATData["Natoms"] == 4:  # torsion
        ...
        G2fil.G2Print(
            " Torsion angle for %s atom sequence: %s = %s"
            % (
                DATData["Name"],
        ...
        G2fil.G2Print(
            " Angle in %s for atom sequence: %s = %s"
            % (
                DATData["Name"],
                str(atmSeq).replace("'", "")[1:-1],
        ...
        G2fil.G2Print(
            " Distance in %s for atom sequence: %s = %s"
            % (
                DATData["Name"],
                str(atmSeq).replace("'", "")[1:-1],
...
def BestPlane(PlaneData):
    ...
    G2fil.G2Print(
        " Selected atoms centered at %10.5f %10.5f %10.5f"
