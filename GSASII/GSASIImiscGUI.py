...
def GetPowderPeaks(fileName):
    ...
    limits[0] = max(1.0, (int(limits[0] - 1.0) / 5) * 5.0)
    limits[1] = min(170.0, (int(limits[1] + 1.0) / 5) * 5.0)
...
def GetCheckImageFile(G2frame, treeId):
    ...
    if not os.path.exists(imagefile):
        ...
        gpxPath = (
            Controls.get("LastSavedAs", "").replace("\\", "/").split("/")
        )  # blank in older .GPX files
    ...
    if not os.path.exists(imagefile):
        ...
        dlg = wx.FileDialog(
            G2frame,
            "Previous image file (" + prevnam + ") not found; open here",
            ".",
            prevnam,
            wildcard,
            wx.FD_OPEN,
...
def EditImageParms(parent, Data, Comments, Image, filename):
    ...
    vsizer.Add(
        wx.StaticText(dlg, wx.ID_ANY, "Wavelength (\xc5) "),
        0,
        wx.ALIGN_LEFT | wx.ALL,
        2,
    )
    ...
    vsizer = wx.BoxSizer(wx.HORIZONTAL)
    vsizer.Add(
        wx.StaticText(dlg, wx.ID_ANY, "Pixel size (\xb5m). Width "),
        0,
        wx.ALIGN_LEFT | wx.ALL,
        2,
    )
    ...
    vsizer = wx.BoxSizer(wx.HORIZONTAL)
    vsizer.Add(
        wx.StaticText(dlg, wx.ID_ANY, "Sample to detector (mm) "),
        0,
        wx.ALIGN_LEFT | wx.ALL,
        2,
    )
    ...
    vsizer = wx.BoxSizer(wx.HORIZONTAL)
    vsizer.Add(
        wx.StaticText(dlg, wx.ID_ANY, "Beam center (pixels). X = "),
        0,
        wx.ALIGN_LEFT | wx.ALL,
        2,
    )
    ...
    vsizer = wx.BoxSizer(wx.HORIZONTAL)
    vsizer.Add(wx.StaticText(dlg, wx.ID_ANY, "Comments "), 0, wx.ALIGN_LEFT | wx.ALL, 2)
...
def LoadImage2Tree(imagefile, G2frame, Comments, Data, Npix, Image):
    ...
    """
    ImgNames = G2gd.GetGPXtreeDataNames(
        G2frame,
        [
            "IMG ",
        ],
    ...
    G2frame.GPXtree.SetItemPyData(
        G2frame.GPXtree.AppendItem(Id, text="Image Controls"), Data
    )
    Masks = {
        "Points": [],
        "Rings": [],
    ...
    G2frame.GPXtree.SetItemPyData(
        G2frame.GPXtree.AppendItem(Id, text="Stress/Strain"),
        {
            "Type": "True",
            "d-zero": [],
            "Sample phi": 0.0,
    ...
    for rd in primaryReaders + secondaryReaders:
        ...
        while repeat:  # loop if the reader asks for another pass on the file
            ...
            if GSASIIpath.GetConfigValue("debug"):
                flag = rd.Reader(
                    imagefile, ParentFrame, blocknum=block, Buffer=rdbuffer
                )
            else:
                flag = False
                try:
            ...
            if flag:  # this read succeeded
                ...
                LoadImage2Tree(
                    imagefile, G2frame, rd.Comments, rd.Data, rd.Npix, rd.Image
                )
                repeat = rd.repeat
            CreatedIMGitems.append(G2frame.Image)
        if CreatedIMGitems:
            return CreatedIMGitems
...
def SaveMultipleImg(G2frame):
    ...
    if len(choices) == 1:
        ...
        dlg = G2G.G2MultiChoiceDialog(
            G2frame, "Stress/Strain fitting", "Select images to fit:", choices
        )
        dlg.SetSelections([])
        names = []
        if dlg.ShowModal() == wx.ID_OK:
...
def objectScan(data, tag, indexStack=[]):
    ...
    if type(data) is list or type(data) is tuple:
        ...
        or "GSASIIobj.G2VarObj" in str(type(data))
        or "GSASIIobj.ExpressionObj" in str(type(data))
    ):
        return None
    else:
...
def ProjFileOpen(G2frame, showProvenance=True):
    ...
    if os.path.exists(GPXphase) and os.path.exists(GPXhist):
        dlg = wx.MessageDialog(
            G2frame,
            "Load results from crashed sequential fit?\nNo deletes the files!",
            "Recover partial sequential fit?",
        ...
        try:
            ...
            if result == wx.ID_YES:
                ...
                for name, vals in data[1:]:
                    Phases[name] = vals
                name, CovData = pickleLoad(fp)[
                    0
                ]  # 2nd block in file should be Covariance
                name, RigidBodies = pickleLoad(fp)[
                    0
    ...
    try:
        ...
        while True:
            ...
            if datum[0] == "Phases" and GSASIIpath.GetConfigValue(
            ...
            if datum[0].startswith("PWDR"):
                ...
                if (
                    ...
                    if "dict" in str(type(datum[1]["PythonVersions"])):  # patch
                        for p in sorted(
                            datum[1]["PythonVersions"], key=lambda s: s.lower()
                        ):
                    ...
                    if datum[0].startswith("PWDR"):
                        ...
                        datus[1] = [
                            dict(
                                zip(
                                    datus[1][2],
                                    zip(datus[1][0], datus[1][1], strict=False),
                                    strict=False,
                                )
                            ),
                            {},
        ...
        if LastSavedUsing:
            print(
                "GPX load successful. Last saved with GSAS-II revision "
                + LastSavedUsing
            )
        else:
    ...
    if not G2frame.GPXtree.IsEmpty():
        try:
            ...
            G2G.G2MessageBox(
                G2frame,
                "Read only file",
                "Project cannot be saved; change permission & try again",
            )
            return
        ...
        try:
            ...
            Controls["LastSavedUsing"] = (
                f"#{GSASIIpath.GetVersionNumber()}, {GSASIIpath.GetVersionTag()}"
            )
...
def SaveIntegration(G2frame, PickId, data, Overwrite=False):
    ...
    for i, azm in enumerate(azms[:-1]):
        ...
        G2frame.GPXtree.SetItemPyData(
            ...
            copy.deepcopy([tuple(Xminmax), Xminmax]),
        )
        if "PWDR" in Aname:
            G2frame.GPXtree.SetItemPyData(
        ...
        if "PWDR" in Aname:
            G2frame.GPXtree.SetItemPyData(
                G2frame.GPXtree.AppendItem(Id, text="Sample Parameters"), Sample
            )
            G2frame.GPXtree.SetItemPyData(
            ...
            G2frame.GPXtree.SetItemPyData(
                G2frame.GPXtree.AppendItem(Id, text="Substances"),
            ...
            G2frame.GPXtree.SetItemPyData(
                G2frame.GPXtree.AppendItem(Id, text="Models"),
                G2pwd.SetDefaultSASDModel(),
            )
        valuesdict = {
            "wtFactor": 1.0,
            "Dummy": False,
...
def XYsave(G2frame, XY, labelX="X", labelY="Y", names=[]):
    ...
    dlg = wx.FileDialog(
        G2frame,
        "Enter csv filename for XY table",
        pth,
        "",
        "XY table file (*.csv)|*.csv",
...
def ExportSequential(G2frame, data, obj, exporttype):
    ...
    if len(data["histNames"]) == 1:
        ...
        dlg = G2G.G2MultiChoiceDialog(
            G2frame,
            "Select histograms to export from list",
            "Select histograms",
            data["histNames"],
        )
        if dlg.ShowModal() == wx.ID_OK:
    ...
    if exporttype == "Phase":
        ...
        if len(obj.Phases) == 0:
            G2G.G2MessageBox(
                G2frame, "There are no phases in sequential ref.", "Warning"
            )
            return None
    ...
    for nL in range(nLayers):
        if "=" in Struct[N]:
            ...
            Layer["Layers"].append(
                {
                    "Name": name[0],
                    "SameAs": Layer["Layers"][sameas]["Name"],
                    "Symm": "None",
                    "Atoms": [],
                }
            )
            ...
            continue
        Symm = "None"
        if "centro" in Struct[N + 1]:
            Symm = "-1"
        Layer["Layers"].append(
            {"Name": Struct[N], "SameAs": "", "Symm": Symm, "Atoms": []}
        )
        N += 2
        while "layer" not in Struct[N]:
            atom = Struct[N][4:].split()
            atomType = G2el.FixValence(
                Struct[N][:4].replace(" ", "").strip().capitalize()
            )
...
def saveNewPhase(G2frame, phData, newData, phlbl, msgs, orgFilName):
    ...
    msgs[phlbl] = (
        f"With space group {sgsym} and cell={fmtCell(generalData['Cell'][1:7])}"
    )
    ...
    msgs[phlbl] += (
        f". Asymmetric unit {G2mth.fmtPhaseContents(nacomp)} ({len(phData['Atoms'])} atoms)."
    )
...
def mkParmDictfromTree(G2frame, sigDict=None):
    ...
    if sigDict is not None:
        sigDict.update(
            dict(zip(covDict.get("varyList", []), covDict.get("sig", []), strict=False))
        )
...
def LogCellChanges(G2frame):
    ...
    if Controls["max cyc"] == 0:
        return  # no fit so no change
    ...
    for phase in Phases:
        ...
        for hist in phasedict["Histograms"]:
            if not phasedict["Histograms"][hist]["Use"]:
                continue
            if "HKLF" in hist:
                continue  # skip single crystal histograms!
            hId = Histograms[hist]["hId"]
            if (
                any(phasedict["Histograms"][hist]["HStrain"][1])
                or phasedict["General"]["Cell"][0]
            ):
                cellList, cellSig = G2lat.getCellSU(pId, hId, SGData, parmDict, covData)
                txt = G2lat.showCellSU(cellList, cellSig, SGData)
                G2frame.AddToNotebook(f"Phase {pId} Hist {hId}: {txt}", "CEL", False)
