...
def GetTifData(filename):
    ...
    if (
        ...
        if sizexy in ([1024, 402], [402, 1024]):  # confirms that it has the proper size
            ...
            )  # result must be 32 bt like all the others

    if image is None:
        print(
            "Image is improperly formatted in some way, confirm that this tiff file is being uses properly with the 1ID workflow"
        )
        lines = [
            "Image is improperly formatted in some way",
        ]
        return lines, 0, 0, 0
