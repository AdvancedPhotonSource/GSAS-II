...
spos = [5.566667, 4.916235, 4.379751]
...
def test_LatConstruct():
    ...
    _ReportTest()
    cell_params = [5.0, 5.0, 5.0, 90, 90.0, 90.0]
    latt_vec = kvs.lat_params_to_vec(cell_params)
    msg = "Lattice vectors construction failed"
    assert np.allclose(latt_vec[0], [5.0, 0.0, 0.0]), msg
    assert np.allclose(latt_vec[1], [0.0, 5.0, 0.0]), msg
    assert np.allclose(latt_vec[2], [0.0, 0.0, 5.0]), msg

    cell_params = [5.0, 5.0, 10.0, 90.0, 90.0, 120.0]
    latt_vec = kvs.lat_params_to_vec(cell_params)
    assert np.allclose(latt_vec[0], [4.33013, -2.5, 0.0], atol=1e-5), msg
    assert np.allclose(latt_vec[1], [0.0, 5.0, 0.0], atol=1e-5), msg
    assert np.allclose(latt_vec[2], [0.0, 0.0, 10.0], atol=1e-5), msg

    cell_params = [5.0, 6.0, 7.0, 91.0, 95.0, 112.0]
    latt_vec = kvs.lat_params_to_vec(cell_params)
    assert np.allclose(latt_vec[0], [4.61218, -1.88092, -0.43578], atol=1e-5), msg
    assert np.allclose(latt_vec[1], [0.0, 5.99909, -0.10471], atol=1e-5), msg
    assert np.allclose(latt_vec[2], [0.0, 0.0, 7.0], atol=1e-5), msg
...
def test_CriticalRoutines():
    ...
    assert np.allclose(hkl_p, [0.66667, 0.66667, 0.66667], atol=1e-5), msg
    msg = "Test for primitive to conventional setting failed"
    k_conv = k_search.kVecPrimToConv([0.5, 0.5, 0])
    assert np.allclose(k_conv, [0.0, 0.5, 1.0], atol=1e-5), msg
...
def test_KVecCandidateUpdate():
    ...
    k_trial = [0.5, 0.5, 0.5]  # T point
    k_opt_list = list()
    k_opt_dist = list()
    k_opt_ad = list()
    k_opt_md = list()
    k_opt_out = k_search.updateCandidateList(
        k_trial, k_opt_list, k_opt_dist, k_opt_ad, k_opt_md, False
    )
    (k_opt_list, k_opt_dist, k_opt_ad, k_opt_md) = k_opt_out

    k_trial = [0.5, 0, 0.5]  # F point
    k_opt_out = k_search.updateCandidateList(
        k_trial, k_opt_list, k_opt_dist, k_opt_ad, k_opt_md, False
    )
    (k_opt_list, k_opt_dist, k_opt_ad, k_opt_md) = k_opt_out

    assert np.allclose(k_opt_list[0], [0.5, 0.0, 0.5], atol=1e-5), (
        msg + f"\nk_opt_list[0] {k_opt_list[0]}, [.5, 0., .5]"
    )
    assert np.allclose(k_opt_dist[0], 0.08340, atol=1e-5), (
        msg + f"\nk_opt_dist[0] {k_opt_dist[0]}, 0.08340"
    )
    assert np.allclose(k_opt_list[1], [0.5, 0.5, 0.5], atol=1e-1), (
        msg + f"\nk_opt_list[1] {k_opt_list[1]}, [.5, .5, .5]"
    )
    assert np.allclose(k_opt_dist[1], 0.11751, atol=1e-1), (
    ...
    assert np.allclose(k_opt_list[0], [0.0, 0.0, 0.0], atol=1e-5), (
        msg + f"\nk_opt_list[0] {k_opt_list[0]}, [0, 0, 0]"
    )
    assert np.allclose(k_opt_dist[0], 0.0, atol=1e-5), (
        msg + f"\nk_opt_dist[0] {k_opt_dist[0]}, 0."
    )

    k_opt_final, kd_opt_final, _, _ = k_search.kOptFinder()
    assert np.allclose(k_opt_final[0], [0.0, 0.0, 0.0], atol=1e-5), (
        msg + f"\nk_opt_final[0] {k_opt_final[0]}, [0, 0, 0]"
    )
    assert np.allclose(kd_opt_final[0], 0.0, atol=1e-5), (
...
def test_KVecSearch():
    ...
    for two_theta in two_thetas:
        d_tmp = lambda_val / (2.0 * np.sin(np.deg2rad(two_theta) / 2.0))
        spos_gen.append(d_tmp)

    latt_params = [5.655600, 5.655600, 5.655600, 90.000000, 90.000000, 90.000000]
    ...
    assert np.allclose(k_opt_final[0], [0.0, 1.0, 0.0], atol=1e-5), msg
    assert np.allclose(k_opt[1][0], 0.0, atol=1e-5), msg
    ...
    latt_params = [5.598000, 5.598000, 8.955600, 90.000000, 90.000000, 120.000000]
    latt_vec = kvs.lat_params_to_vec(latt_params)

    atoms_labels = [1]

    atom_pos_p1 = [[0.000000, 0.000000, 0.000000]]
    ...
    assert np.allclose(k_opt_final[0], [0.14, 0.0, 0.0], atol=1e-5), msg
    assert np.allclose(k_opt[1][0], 0.0, atol=1e-5), msg
