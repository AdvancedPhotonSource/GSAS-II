...
class testSytSym(wx.Frame):
    def _init_ctrls(self, parent):
        wx.Frame.__init__(
            ...
            title="Test space group site symmetry",
        )
    ...
    def UpdateData(self, Data):
        ...
        def OnXYZ(event):
            ...
            try:
                ...
                CSIXtxt.SetLabel(
                    " site sym: %6s, mult: %3d, CSI-X: %s %s"
                    % (SytSym, Mul, StrXYZ, ValXYZ)
                )
                StrUIJ = [str(suij) for suij in CSIU[0]]
                ValUIJ = [str(val) for val in CSIU[1]]
                CSIUtxt.SetLabel(
                    " site sym: %6s, mult: %3d, CSI-U: %s %s"
        ...
        text, table = G2spc.SGPrint(SGData, AddInv=True)
        topSizer.Add(
            wx.StaticText(self.testSSPanel, label=" Special position test:"), 0, WACV
        )
        ...
        mainSizer.Add((5, 5), 0)
        CSIUtxt = wx.StaticText(
            self.testSSPanel,
            label=" site sym: %6s, mult: %3d, CSI-U: %s %s"
            % (SytSym, Mul, StrUIJ, ValUIJ),
        )
