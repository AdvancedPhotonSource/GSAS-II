...
if (
    importlib.util.find_spec("GSASII") is None
):  # hack path if GSASII not installed into Python
...
def test_MoveToUnitCell():
    ...
    assert abs(MoveToUnitCell(np.array([-0.1]))[0] - 0.9)[0] < 1e-6, msg
    assert abs(MoveToUnitCell(np.array([0.1]))[0] - 0.1)[0] < 1e-6, msg
...
def test_SpcGroup1():
    ...
    def CompareSpcGroup(spc, referr, refdict, reflist):
        ...
        for k, item in enumerate(refdict[key]):
            for i, j in enumerate(indices):
                if np.allclose(result[1][key][j][0], item[0]) and np.allclose(
                    result[1][key][j][1], item[1]
                ):
                    indices.pop(i)
                    break
            else:
            ...
            for inv in onebar:
                for cen in spc["SGCen"]:
                    ...
                    for refop in cctbx:
                        ...
                        if all(abs(noff - reftrans) < 1.0e-5):
                            cctbx.remove(refop)
                            break
                    else:
...
def test_SytSym():
    ...
    ExerciseSiteSym(
        ...
        [
            ((0.13, 0.22, 0.31), 8, "1"),
            ((0.25, 0.75, 0.31), 4, "2(z)"),
            ((0.5, 0.5, 0.5), 4, "-1"),
            ((0, 0.5, 0), 4, "-1"),
            ((0.25, 0.25, 0.31), 2, "4(z)"),
            ((0.25, 0.75, 0.5), 2, "-4(z)"),
            ((0.25, 0.75, 0.0), 2, "-4(z)"),
        ],
    )
