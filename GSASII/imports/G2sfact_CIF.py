...
class CIFhklReader(G2obj.ImportStructFactor):
    ...
    def Reader(self, filename, ParentFrame=None, **kwarg):
        ...
        """
        hklitems = [
            ("_refln_index_h", "_refln_index_k", "_refln_index_l", "_refln_index_m_1"),
        ...
        F2signames = (
            "_refln_f_squared_meas_sigma",
            "_refln.f_squared_meas_sigma",
            "_refln_f_squared_sigma",
            "_refln.f_squared_sigma",
            "_refln_intensity_meas_sigma",
            "_refln.intensity_meas_sigma",
        ...
        for blk in cf:  # scan for reflections, F or F2 values and cell lengths.
            ...
            for i in range(2):
                if hklitems[i][3] in blkkeys:  # Super lattice reflections h,k,l,m
                    im = 1
                if (
                    hklitems[i][0] in blkkeys
                    and hklitems[i][1] in blkkeys
                    and hklitems[i][2] in blkkeys
                ):
                    dnIndex = i
                    break
            else:
        ...
        if not blklist:
            ...
            if self.repeat:  # we were called to repeat the read
                ...
                self.repeatcount += 1
                if self.repeatcount >= len(blklist):
                    self.repeat = False
            else:
