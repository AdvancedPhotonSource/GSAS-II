...
class Exportbracket(G2fil.ExportBaseclass):
    ...
    def __init__(self, G2frame):
        G2fil.ExportBaseclass.__init__(
            self,
            G2frame=G2frame,
            formatName="Bracket notation CSV",
            extension=".csv",
            longFormatName="Export commonly needed parameters",
        )
    ...
    def Exporter(self, event=None):
        ...
        try:
            ...
            for phasedict in self.Phases.items():
                ...
                for i in range(len(cellList)):
                    if cellSig[i] > 0:
                        ...
                        model_parameters[
                            phasenam
                            + " "
                            + cell_quantity_symbols[i]
                            + " "
                            + cell_quantity_units[i]
                        ] = current_lp_bracket
                ...
                for hist_num, hist_name in enumerate(self.Histograms):
                    try:
                        ...
                        phasefrac_unc = self.sigDict[
                            str(phase_num) + ":" + str(hist_num) + ":Scale"
                        ]
                        ...
                        if len(self.Histograms) > 1:
                            model_parameters[
                                phasenam + " Phase Fraction in: " + hist_name
                    ...
                    try:
                        ...
                        depSigDict = self.OverallParms["Covariance"].get(
                            "depSigDict", {}
                        )
                        ...
                        if len(self.Histograms) > 1:
                            model_parameters[
                                phasenam + " Weight Fraction in: " + hist_name
                    ...
                    try:
                        pref_orr_props = phasedict[1]["Histograms"][hist_name][
                            "Pref.Ori."
                        ]
            ...
            for i, hist_name in enumerate(self.Histograms):  # noqa: B007
                ...
                GetParamSig(
                    "", hist_num, ":DisplaceX", "Sample X Displacement (micron)"
                )
                # Extract Debye-Scherrer sample Y displacement, if refined
                GetParamSig(
                    "", hist_num, ":DisplaceY", "Sample Y Displacement (micron)"
                )
                ...
                for phase_num, phase_name in enumerate(self.Phases):
                    ...
                    for atom_num, atom in enumerate(atom_list):
                        ...
                        for atom_axis in ("x", "y", "z"):
                            ...
                            try:
                                param_index = self.OverallParms["Covariance"][
                                    "varyList"
                                ].index(variable_code)
                                # Extract uncertainty
                                atom_axis_sig = self.OverallParms["Covariance"]["sig"][
                                    param_index
                                ]
                                # Extract value
                                atom_axis_val = list(self.Phases.values())[phase_num][
                                    "Atoms"
                                ][atom_num][ord(atom_axis) - 117]
...
class Export3col(G2fil.ExportBaseclass):
    ...
    def __init__(self, G2frame):
        G2fil.ExportBaseclass.__init__(
            self,
            G2frame=G2frame,
            formatName="common prm CSV",
            extension=".csv",
            longFormatName="Export commonly needed parameters with s.u. in a separate column",
        )
        self.exporttype = ["project"]
            ...
            for phasedict in self.Phases.items():
                ...
                for i in range(len(cellList)):
                    ...
                    if cellSig[i] > 0:
                        ...
                        model_parameters[phasenam + " " + lp_letter + " (Å)"] = (
                ...
                for hist_num, hist_name in enumerate(self.Histograms):
                    ...
                    try:
                        ...
                        if pref_orr_props[2] and pref_orr_props[0] == "MD":
                            # If so, first extract MD axis and write to dictionary to be exported
                            MD_axis = "'" + "".join(map(str, pref_orr_props[3]))
                            model_parameters[phasenam + " March Dollase Axis"] = (
                                MD_axis,
                                "",
                            )
                            # Next, extract MD ratio
