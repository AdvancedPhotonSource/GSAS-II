...
class scanCCD(wx.Frame):
    def _init_ctrls(self, parent):
        ...
        self.File.Append(
            wxID_FILEOPEN, "Open scanCCD files", "Open scanCCD image files (*.tif)"
        )
        self.File.Append(
            wxID_INTEGRATE, "Integrate scanCCD images", "Integrate scanCCD images"
        )
        self.File.Append(
            wxID_OUTPUT, "Output pattern", "Output fxye file from integration"
        )
    ...
    def OnImageIntegrate(self, event):
        def Make2ThetaMap(data, iLim, jLim):
            ...
            vecB /= norm.T
            tax = (
                npacosd(np.dot(vecB.T, vecA)) * tax / np.abs(tax)
            )  # to get sign of 2-theta
            tay += data["Zeros"][1]
        ...
        scaley = self.data["pixel"][1] / 1000.0
        ...
        nBlk = N**2 * len(
            self.imagefiles
        )  # assume 4Kx4K CCD images - done in 1Kx1K blocks
        t0 = time.time()
                ...
                if ifile >= self.data["skip"]:
                    ...
                    for iBlk in range(N):
                        ...
                        for jBlk in range(N):
                            ...
                            TA = Make2ThetaMap(
                                self.data, (iBeg, iFin), (jBeg, jFin)
                            )  # 2-theta & Y arrays & create position mask
                            Block = image[iBeg - iBegi : iFin - iBegi, jBeg:jFin]
                            tax, tay, taz = Fill2ThetaMap(
                                self.data, TA, Block
                            )  # and apply masks
                            NST, H0, Amat, Qmat = h2d.histosigma2d(
                                len(tax),
                                tax,
                                tay,
                                taz,
    ...
    def OnOutput(self, event):
        ...
        dlg = wx.FileDialog(
            self,
            "Choose output powder file name",
            ".",
            "",
    ...
    def UpdateControls(self, event):
        ...
        def ColorSizer(data):
            ...
            colorSizer.Add(
                wx.StaticText(self.SCCDPanel, label=" Color bar "),
                0,
            ...
            colorSizer.Add(
                wx.StaticText(self.SCCDPanel, label="image files to skip:"),
                0,
                wx.ALIGN_CENTER_VERTICAL,
            )
            skipFile = wx.ComboBox(
                self.SCCDPanel,
                value=str(data["skip"]),
        ...
        def MaxSizer(data):
            ...
            maxSizer.Add(
                wx.StaticText(parent=self.SCCDPanel, label=" Max intensity"),
                0,
                wx.ALIGN_CENTER_VERTICAL,
            )
            self.maxSel = wx.Slider(
        ...
        def ZSizer(data):
            ...
            zSizer.Add(
                wx.StaticText(self.SCCDPanel, label="Upper intensity mask:"),
                0,
                wx.ALIGN_CENTER_VERTICAL,
            )
            zMax = wx.TextCtrl(
            ...
            zSizer.Add(zMax)
            zSizer.Add(
                wx.StaticText(self.SCCDPanel, label="Intensity subtraction:"),
                0,
                wx.ALIGN_CENTER_VERTICAL,
            )
            zMin = wx.TextCtrl(
        ...
        def ZeroSizer(data):
            ...
            zeroSizer.Add(
                wx.StaticText(self.SCCDPanel, label="X-zero:"),
                0,
                wx.ALIGN_CENTER_VERTICAL,
            )
            zMax = wx.TextCtrl(
            ...
            zeroSizer.Add(zMax)
            zeroSizer.Add(
                wx.StaticText(self.SCCDPanel, label="Y-zero:"),
                0,
                wx.ALIGN_CENTER_VERTICAL,
            )
            zMin = wx.TextCtrl(
            ...
            zeroSizer.Add(zMin)
            zeroSizer.Add(
                wx.StaticText(self.SCCDPanel, label="mm per deg:"),
                0,
                wx.ALIGN_CENTER_VERTICAL,
            )
            zpdeg = wx.TextCtrl(
        ...
        def TBLRSizer(data):
            ...
            for i, item in enumerate(["Bottom", "Top"]):
                TBLRsizer.Add(
                    wx.StaticText(self.SCCDPanel, label=item + " limit, pixels:"),
                    0,
                    wx.ALIGN_CENTER_VERTICAL,
                )
                TBlim = wx.TextCtrl(
    ...
    def PlotImage(self):
        ...
        scalex = pixel[0] / 1000.0
        scaley = pixel[1] / 1000.0
            ...
            if event.xdata and event.ydata:  # avoid out of frame errors
                ...
                if item:
                    ...
                    if (0 <= xpix <= sizexy[0]) and (0 <= ypix <= sizexy[1]):
                        ...
                        vecB = np.array(
                            [
                                npcosd(tth) * self.data["radius"],
                                npsind(tth) * self.data["radius"],
                                (ypos - self.data["Zeros"][1]),
                            ]
                        )
                        vecB /= nl.norm(vecB)
        ...
        def OnPick(event):
            if self.itemPicked is not None:
                return
        ...
        Img = Plot.imshow(
            ...
            vmax=Imax,
            aspect="auto",
            cmap=self.data["color"],
            extent=[0, Xmax, 0, Ymax],
        )
        Lines.append(Plot.axvline(Zeros[0], color="b", dashes=(5, 5), picker=3.0))
        Lines.append(Plot.axhline(Zeros[1], color="b", dashes=(5, 5), picker=3.0))
        Lines.append(Plot.axhline(TBlimit[0], color="g", dashes=(5, 5), picker=3.0))
        Lines.append(Plot.axhline(TBlimit[1], color="r", dashes=(5, 5), picker=3.0))
        Lines.append(Plot.axvline(LRlimit[0], color="g", dashes=(5, 5), picker=3.0))
        Lines.append(Plot.axvline(LRlimit[1], color="r", dashes=(5, 5), picker=3.0))
