    ...
    def ContentsValidator(self, filename):
        ...
        for i, line in enumerate(fp):
            ...
            if line[:4] == "BANK":
                ...
                nBanks += 1
                continue
            if line[:7] == "Monitor" or line[:8] == "TIME_MAP":
                continue
        #            else:
    ...
    def Reader(self, filename, ParentFrame=None, **kwarg):  # noqa: ARG002
        ...
        def GetFXYEdata(File, Pos, Bank):  # noqa: ARG001
            ...
            while S and S[:4] != "BANK" and S[0] != "#":
                vals = S.split()
                x.append(float(vals[0]) / 100.0)  # CW: from centidegrees to degrees
                f = float(vals[1])
                s = float(vals[2])
                if f <= 0.0 or s <= 0.0:
                    y.append(0.0)
        ...
        def GetSTDdata(File, Pos, Bank):
            ...
            if "EDS" in cons[4]:
                ...
                self.Inst = {
                    "XE": [Ecoef[0], Ecoef[0], False],
                    "YE": [Ecoef[1], Ecoef[1], False],
                    "ZE": [Ecoef[2], Ecoef[2], False],
                    "WE": [Ecoef[3], Ecoef[3], False],
                    "2-theta": [5.0, 5.0, False],
                }
            while S and S[:4] != "BANK" and S[0] != "#":
        ...
        def GetALTdata(File, Pos, Bank):
            ...
            while S and S[:4] != "BANK" and S[0] != "#":
                for i in range(0, 80, 20):
                    if S[i : i + 8] == 8 * " ":
                        break
                    xi = sfloat(S[i : i + 8]) / 3200.0
                    if not xi:
                        break
                    yi = sfloat(S[i + 8 : i + 15]) / 1000.0
                    ei = sfloat(S[i + 15 : i + 21]) / 1000.0
                    x.append(xi)
                    if yi > 0.0:
                        y.append(yi)
                        w.append(1.0 / ei**2)
        ...
        if len(Banks) != len(Pos) or len(Banks) == 0:
            ...
            while True:
                ...
                if (
                    S[:8] == "TIME_MAP"
                ):  # assumes one time map; HIPPO has multiple time maps
                    if len(Banks) == 0:
