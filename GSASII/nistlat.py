...
else:  # binaries should be in path
    nistlattice = "LATTIC"
    convcell = "convcell"
...
def ReduceCell(center, cellin, mode=0, deltaV=0, output=None):
    ...
    try:
        for linenum, line in enumerate(cellout, 1):
            ...
            if len(s) > 1:
                mat = [
                    [float(j) for j in re.split(r" *([\d\.-]*) *", i, maxsplit=3)[1::2]]
                    for i in s[1:-1]
                ]
            ...
            if len(s) > 1:
                lat = [
                    float(i)
                    for i in re.split(r" *([\d\.-]*) *", s[1], maxsplit=6)[1::2]
                ]
...
def ConvCell(redcell):
    ...
    try:
        while out:
            ...
            if "**WARNING**" in line:
                print(
                    "Note: Warning generated in conversion of reduced\n  cell",
                    redcell,
...
def CompareCell(
    ...
    inp = "REL      1\n"
    inp += (
        "{:.1s}  {:2d}     {:10.5f}{:10.5f}{:10.5f}{:10.5f}{:10.5f}{:10.5f}\n".format(
...
def CellSymSearch(
    cellin, center, tolerance=3 * [0.2] + 3 * [1], mode=0, deltaV=2, output=None
):
            ...
            while lnum < len(lines):
                ...
                if len(s) > 1:
                    ...
                    if not np.allclose(cellstuff[2], lat):
                        print(
                            "Warning mismatch between list of reduced cells and"
            ...
            while lnum < len(lines):
                ...
                if 50 * "-" in line:
                    if xformCount:
                        ...
                        cnv2origMat = np.dot(
                            np.dot(red2convInp, im), np.linalg.inv(cnvCell[3])
                        )
...
if __name__ == "__main__":  # test code
    ...
    out = CompareCell(
        cell1, center1, cell2, center2, tolerance, output="/tmp/CompCell.txt"
    )
    for i in out:
        print(70 * "=")
        print(i[0], i[3], "\n", i[5])
    ...
    if cellList is None:
        ...
        sys.exit()
    print(
        "Input reduced cell", showCell(cellin, "P", " "), "\nout", showCell(*cellList)
    )
    mat = cellList[-1]
    print("test with\n", mat)
    ...
    print(
        "\ntest from [5,6,7,90,105,90] C-Centered"
    )  # ==>  [5,6,7,90,105,90] C-Centered
