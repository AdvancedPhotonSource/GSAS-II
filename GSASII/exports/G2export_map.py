...
class ExportMapASCII(G2fil.ExportBaseclass):
    ...
    def Exporter(self, event=None):
        ...
        for phasenam in self.phasenam:
            ...
            if (
                len(self.phasenam) > 1
            ):  # if more than one filename is written, add a phase # -- not in use yet
            ...
            self.OpenFile()
            self.Write(
                "Map of Phase " + phasenam + " from " + self.G2frame.GSASprojectfile
            )
...
class ExportMapCCP4(G2fil.ExportBaseclass):
    ...
    def Exporter(self, event=None):
        ...
        for phasenam in self.phasenam:
            ...
            for n in rho.shape:
                self.Write(n, "i")  # nX,nY,nZ
            self.Write(2, "i")  # mode=2 float map
            for i in [0, 0, 0]:
                self.Write(i, "i")  # 1st position on x,y,z
            for n in rho.shape:
                self.Write(n, "i")  # nX,nY,nZ
            for c in cell:
                self.Write(c, "f")
            for i in [1, 2, 3]:
                self.Write(i, "i")  # axes order = x,y,z
