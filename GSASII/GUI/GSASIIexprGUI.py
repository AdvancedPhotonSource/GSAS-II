...
class ExpressionDialog(wx.Dialog):
        ...
        * Values above 1 determine what variables will be shown
        ...
        if VarLabel:
            ...
            for i in range(1, len(choices)):  # remove empty menus from choice list
                if not len(self.depParmLists[i]):
                    choices[i] = ""
            self.depChoices = [i for i in range(len(choices)) if choices[i]]
            choice = wx.Choice(
                self, wx.ID_ANY, choices=[choices[i] for i in self.depChoices]
            )
    ...
    def SelectG2var(self, sel, var, parmList):
        ...
        dlg = G2G.G2SingleChoiceDialog(
            self,
            f'Select GSAS-II parameter for variable "{var}":',
            "Select parameter",
            varListlbl,
            monoFont=True,
        )
    ...
    def showError(self, msg1, msg2="", msg3=""):
        ...
        font1 = wx.Font(
            errMsg2.GetFont().GetPointSize(), wx.MODERN, wx.NORMAL, wx.NORMAL, False
        )
    ...
    def Repaint(self, exprObj):
        ...
        Siz = wx.BoxSizer(wx.VERTICAL)
        Siz.Add(
            wx.StaticText(self.varbox, wx.ID_ANY, "Assignment of variables to labels:"),
            0,
            wx.EXPAND,
            0,
        )
        ...
        try:
            ...
            if self.depVarDict:
                ...
                depVal = (
                    ...
                    + self.dependentVar
                    + '" = '
                    + str(self.depVarDict.get(self.dependentVar, "?"))
                )
            if self.wildCard:
...
class BondDialog(wx.Dialog):
    ...
    def OnSetRadii(self, event):
        ...
        dlg = G2G.DisAglDialog(
            self.parent, DisAglCtls, self.Phases[self.pName]["General"], Reset=False
        )
        if dlg.ShowModal() == wx.ID_OK:
    ...
    def Draw(self):
        ...
        if (
            not self.Oatom and len(aNames) > 0
        ):  # select an atom so distances get computed
            self.Oatom = aNames[0]
        origAtom = wx.ComboBox(
            self.panel,
            value=self.Oatom,
            choices=aNames,
            style=wx.CB_READONLY | wx.CB_DROPDOWN,
...
class AngleDialog(wx.Dialog):
    ...
    def Draw(self):
        ...
        atomSizer = wx.BoxSizer(wx.HORIZONTAL)
        atomSizer.Add(
            wx.StaticText(self.panel, label=" Origin atom (O in A-O-B): "), 0, WACV
        )
        ...
        atomSizer = wx.BoxSizer(wx.HORIZONTAL)
        atomSizer.Add(
            wx.StaticText(self.panel, label=" A-O-B angle for A,B: "), 0, WACV
        )
        neigh = []
        bNames = []
        if self.Oatom:
            neigh = G2mth.FindAllNeighbors(
                Phase, self.Oatom, aNames, searchType="Angle"
            )[0]
        if neigh:
        ...
        if bNames:
            ...
            if self.Tatoms:
                aobj = G2obj.makeAngleObj(
                    self.Phases[self.pName], self.Oatom, self.Tatoms
                )
                calcobj = G2obj.ExpressionCalcObj(aobj)
                calcobj.UpdateDict(self.parmDict)
                atomSizer.Add(
                    wx.StaticText(
                        self.panel, label=f" = {calcobj.EvalExpression():.2f} deg"
                    ),
                    0,
                    WACV,
                )
...
if __name__ == "__main__":
    ...
    frm = wx.Frame(None)
    frm.Show()
    PSvarDict = {"::a": 1.0, "::b": 1.1, "0::c": 1.2, "::AlongVaraiableName": 1000.0}
    ...
    print(dlg.GetDepVar())
    dlg = ExpressionDialog(
        frm, PSvarDict, header="Edit the PseudoVar expression", fit=True
    )
    newobj = dlg.Show(True)
    print(dlg.GetDepVar())
