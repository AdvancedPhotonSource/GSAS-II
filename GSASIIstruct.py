...
def HessRefine(values,HistoPhases,parmdict,varylist,calcControls,pawleyLookup,dlg):
    ...
    if MaxProcess > 1: 
        ...
        for Vec,Hess in mpPool.imap_unordered(ComputePowderHessian,argList):
            VecSum += Vec
            HessSum += Hess
    else:
        for arg in argList:
...
def ComputePowderProfile(args):
    Histogram,parmdict,varylist,Phases,calcControls,pawleyLookup = args
    hId = Histogram['hId']
    print 'ComputePowderProfile',hId
    hfx = ':%d:'%(hId)
    x,y,w,yc,yb,yd = Histogram['Data']
    Limits = calcControls[hfx+'Limits']
    xB = np.searchsorted(x,Limits[0])
...
def errRefine(values,HistoPhases,parmdict,varylist,calcControls,pawleyLookup,dlg):        
    ...
    if MaxProcess > 1: 
        ...
        for i,res in enumerate(
            mpPool.imap_unordered(ComputePowderProfile,argList)
            ):
            print 'process',i
            xB,xF,ycSect,ybSect,RL = res
            Histogram = argList[i][0]
...
def main():
    arg = sys.argv
    try: 
        mkl.set_num_threads(MaxThreads)
        print "Max threads ",mkl.get_max_threads()
        ActualThreads = mkl.get_max_threads()
    except:
        print "MKL module not present"
        ActualThreads = 1
    if len(arg) > 1:
        ...
        exit()
    print "Requested threads ",MaxThreads
    print "Actual threads ",ActualThreads
    print "Max subprocesses ",MaxProcess
    print "Done"
