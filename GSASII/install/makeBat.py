...
if __name__ == "__main__":
    ...
    if not os.path.exists:
        ...
        fp.write(
            "REM This script will reset GSAS-II to the latest version, even if the program can't be started\n"
        )
        pexe = pythonexe
        if " " in pythonexe:
            pexe = '"' + pythonexe + '"'
        ...
        if activate:
            fp.write(f"{activate}")
        fp.write(
            'choice /c yn /n /m "Reset any local changes and install latest GSAS-II version? (y/n)"\n'
        )
    ...
    try:
        oldgpx = winreg.OpenKey(
            winreg.HKEY_CURRENT_USER, r"Software\CLASSES\GSAS-II.project"
        ...
        if oldBat:
            print(
                f"old GPX assignment {oldBat} not found; registry entry will be made for new one"
            )
        new = True
    except NameError:
        pass
        ...
        try:
            if oldBat != G2bat:
                ...
                dlg = wx.MessageDialog(
                    None,
                    "gpx files already assigned in registry to: \n"
                    + oldBat
                    + "\n Replace with: "
                    + G2bat
                    + "?",
    ...
    if new:
        ...
        try:
            gpxkey = winreg.CreateKey(
                winreg.HKEY_CURRENT_USER, r"Software\CLASSES\.gpx"
            )
