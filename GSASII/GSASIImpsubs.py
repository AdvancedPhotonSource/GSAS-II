...
################################################################################
def InitFobsSqGlobals(
    x1, ratio1, shl1, xB1, xF1, im1, lamRatio1, kRatio1, xMask1, Ka21
):
...
def ComputeFobsSqCW(refl, iref):  # noqa: ARG001
    ...
    if ma.all(xMask[iBeg:iFin]):  # peak entirely masked - skip peak
        ...
        fp, sumfp = G2pwd.getFCJVoigt3(
            refl[5 + im], refl[6 + im], refl[7 + im], shl, x[iBeg:iFin]
        )
        yp[iBeg:iFin] = (
            100.0 * refl[11 + im] * refl[9 + im] * fp * cw[iBeg:iFin] / sumfp
        )
    ...
    refl8im = np.sum(
        np.where(
            ratio[iBeg:iFin2] > 0.0,
            yp[iBeg:iFin2] * ratio[iBeg:iFin2] / (refl[11 + im] * (1.0 + kRatio)),
            0.0,
        )
    )
    return refl8im, sInt
...
def ComputeFobsSqCWB(refl, iref):  # noqa: ARG001
    ...
    Wd, fmin, fmax = G2pwd.getWidthsTOF(
        refl[5 + im], refl[12 + im], refl[13 + im], refl[6 + im], refl[7 + im]
    )
    iBeg = max(xB, np.searchsorted(x, refl[5 + im] - fmin))
    iFin = max(xB, min(np.searchsorted(x, refl[5 + im] + fmax), xF))
    if not iBeg + iFin:  # peak below low limit - skip peak
    ...
    refl8im = np.sum(
        np.where(
            ratio[iBeg:iFin] > 0.0,
            yp[iBeg:iFin] * ratio[iBeg:iFin] / refl[11 + im],
            0.0,
        )
    )
...
def ComputeFobsSqCWA(refl, iref):  # noqa: ARG001
    ...
    Wd, fmin, fmax = G2pwd.getWidthsCWA(
        refl[5 + im], refl[12 + im], refl[13 + im], refl[6 + im], refl[7 + im], shl
    )
    iBeg = max(xB, np.searchsorted(x, refl[5 + im] - fmin))
    iFin = max(xB, min(np.searchsorted(x, refl[5 + im] + fmax), xF))
    if not iBeg + iFin:  # peak below low limit - skip peak
    ...
    if ma.all(xMask[iBeg:iFin]):  # peak entirely masked - skip peak
        ...
        return -2
    if iBeg < iFin:
        fp, sumfp = G2pwd.getExpFCJVoigt3(
            refl[5 + im],
            refl[12 + im],
            refl[13 + im],
            refl[6 + im],
...
def ComputeFobsSqED(refl, iref):  # noqa: ARG001
    ...
    if ma.all(xMask[iBeg:iFin]):  # peak entirely masked - skip peak
        ...
        return -2
    if iBeg < iFin:
        fp, sumfp = G2pwd.getPsVoigt(
            refl[5 + im], refl[6 + im] * 1.0e4, refl[6 + im] * 100.0, x[iBeg:iFin]
        )
        yp[iBeg:iFin] = 100.0 * refl[9 + im] * fp * cw[iBeg:iFin] / sumfp
    refl8im = np.sum(
        np.where(ratio[iBeg:iFin] > 0.0, yp[iBeg:iFin] * ratio[iBeg:iFin], 0.0)
    )
...
################################################################################
def InitPwdrProfGlobals(im1, x1):
    """Initialize for the computation of Fobs Squared for powder histograms.
    Puts lots of junk into the global namespace in this module.
    """
...
def ComputePwdrProfCW(profList):
    ...
    for pos, refl, iBeg, iFin, kRatio, shl in profList:
...
def ComputePwdrProfTOF(profList):
    ...
    for pos, refl, iBeg, iFin in profList:
        fp = G2pwd.getEpsVoigt(
            pos, refl[12 + im], refl[13 + im], refl[6 + im], refl[7 + im], x[iBeg:iFin]
        )[0]
...
def ComputePwdrProfCWA(profList):
    ...
    for pos, refl, iBeg, iFin, kRatio, shl in profList:
        ...
        yc[iBeg:iFin] += refl[11 + im] * refl[9 + im] * kRatio * fp
    return yc
...
def ComputePwdrProfED(profList):
    "Compute the peaks profile for a set of TOF peaks and add into the yc array"
    for pos, refl, iBeg, iFin in profList:
        fp = G2pwd.getPsVoigt(pos, refl[6 + im] * 1.0e4, 0.001, x[iBeg:iFin])[0]
