...
def UpdateDData(G2frame, DData, data, hist="", Scroll=0):
    ...
    def PlotSizer():
        ...
        if generalData["Data plot type"] == "Preferred orientation":
            ...
            POhklSizer.Add(
                wx.StaticText(
                    DData, wx.ID_ANY, " Plot preferred orientation for H K L: "
                ),
                0,
                WACV,
            )
            ...
            projSizer.Add(
                wx.StaticText(DData, wx.ID_ANY, " Show projections for: "), 0, WACV
            )
            proj = ["", "x", "y", "z", "xy", "xz", "yz", "xyz"]
            projType = wx.ComboBox(
                DData,
                wx.ID_ANY,
                value=generalData["3Dproj"],
                choices=proj,
    ...
    def ScaleSizer():
        ...
        scaleVal = G2G.ValidatedTxtCtrl(
            DData,
            UseList[G2frame.hist]["Scale"],
            0,
            xmin=0.0,
            nDig=(10, 4, "g"),
            typeHint=float,
            OnLeave=onChangeFraction,
        )
    ...
    def OnAddFixed(event):
        ...
        dlg = G2G.G2MultiChoiceDialog(
            G2frame,
            "Choose phase vars to fix for this histogram only",
            "Choose items to edit",
            phaseKeys,
            selected=selected,
        )
    ...
    def SetPOCoef(Order):
        ...
        cofNames = G2lat.GenSHCoeff(
            SGData["SGLaue"], "0", Order, False
        )  # cylindrical & no M
        newPOCoef = dict(zip(cofNames, len(cofNames) * [0.0], strict=False))
    ...
    def TopSizer(name, choices, parm, OnType):
        ...
        sizeType = wx.ComboBox(
            DData,
            wx.ID_ANY,
            value=UseList[G2frame.hist][parm][0],
            choices=choices,
            style=wx.CB_READONLY | wx.CB_DROPDOWN,
        )
    ...
    def LGmixSizer(name, Limits, OnRef):
        ...
        lgmixVal = G2G.ValidatedTxtCtrl(
            DData,
            UseList[G2frame.hist][name][1],
            2,
            nDig=(10, 3),
            xmin=Limits[0],
            xmax=Limits[1],
    ...
    def IsoSizer(name, parm, fmt, Limits, OnRef):
        ...
        sizeVal = G2G.ValidatedTxtCtrl(
            DData,
            UseList[G2frame.hist][parm][1],
            0,
            nDig=fmt,
            xmin=Limits[0],
            xmax=Limits[1],
    ...
    def UniDataSizer(parmName, parm, fmt, Limits, OnRef):
        ...
        parms = zip(
            [" Equatorial " + parmName, " Axial " + parmName],
            UseList[G2frame.hist][parm][2],
            range(2),
            strict=False,
        )
    ...
    def EllSizeDataSizer():
        parms = zip(
            ...
            UseList[G2frame.hist]["Size"][4],
            UseList[G2frame.hist]["Size"][5],
            range(6),
            strict=False,
        )
    ...
    def GenStrainDataSizer():
        ...
        while onumb < numb:
            ...
            onumb += 1
        muMean = G2spc.MuShklMean(
            SGData, Amat, UseList[G2frame.hist]["Mustrain"][4][:numb]
        )
        parms = zip(
            Snames, UseList[G2frame.hist]["Mustrain"][5], range(numb), strict=False
        )
        ...
        for Pa, ref, Id in parms:
            ...
            strainVal = G2G.ValidatedTxtCtrl(
                DData,
                UseList[G2frame.hist]["Mustrain"][4],
                Id,
                nDig=(10, 2),
                OnLeave=OnNewValueReDraw,
            )
    ...
    def HstrainSizer():
        ...
        parms = zip(
            Hsnames,
            UseList[G2frame.hist]["HStrain"][1],
            range(len(Hsnames)),
            strict=False,
        )
        allzero = True
        for Pa, ref, Id in parms:
            ...
            hstrainVal = G2G.ValidatedTxtCtrl(
                DData,
                UseList[G2frame.hist]["HStrain"][0],
                Id,
                nDig=(10, 3, "g"),
                OnLeave=OnNewValueReDraw,
            )
    ...
    def PoTopSizer(POData):
        ...
        poSizer.Add(
            wx.StaticText(DData, wx.ID_ANY, " Preferred orientation model "), 0, WACV
        )
        POType = wx.ComboBox(
            DData,
            wx.ID_ANY,
        ...
        if POData[0] == "SH":
            ...
            poOrder = wx.ComboBox(
                ...
                wx.ID_ANY,
                value=str(POData[4]),
                choices=[str(2 * i) for i in range(18)],
                style=wx.CB_READONLY | wx.CB_DROPDOWN,
            )
    ...
    def MDDataSizer(POData):
        ...
        poVal = G2G.ValidatedTxtCtrl(
            DData, POData, 1, nDig=(10, 3), typeHint=float, xmin=0.0
        )
    ...
    def SHDataSizer(POData):
        ...
        for odf in ODFkeys:
            ...
            ODFval = G2G.ValidatedTxtCtrl(
                DData, POData[5], odf, nDig=(8, 3), typeHint=float, OnLeave=OnNewValue
            )
        ...
        if generalData["Name"] in reflSets:
            ...
            if "RefList" in reflData:
                ...
                for ref in reflData["RefList"]:
                    if ref[nprfo + Super] < 0.0:
                        hkls += [
                            "%d %d %d: %.3f"
                            % (ref[0], ref[1], ref[2], ref[nprfo + Super]),
                        ]
                    if len(hkls) > 10:
                        break
        shPenalty = wx.BoxSizer(wx.HORIZONTAL)
        shPenalty.Add(
            wx.StaticText(DData, wx.ID_ANY, " Negative MRD penalty list: "), 0, WACV
        )
        shPenalty.Add(
            wx.ComboBox(
                DData, value=POData[6][0], choices=POData[6], style=wx.CB_DROPDOWN
            ),
            0,
            WACV | wx.LEFT,
            5,
    ...
    def ExtSizer(Type):
        ...
        def ExtVal(Ekey, valSizer, fmt, lim):
            for ekey in Ekey:
                ...
                Eval = G2G.ValidatedTxtCtrl(
                    DData,
                    UseList[G2frame.hist]["Extinction"][2][ekey],
                    0,
                    xmax=lim[1],
                    xmin=lim[0],
                    nDig=(10, 4, fmt),
        ...
        if Type == "HKLF":
            ...
            typeSizer.Add(
                wx.StaticText(DData, wx.ID_ANY, " Extinction type: "), 0, WACV
            )
            Choices = [
                "None",
                "Primary",
                "Secondary Type I",
                "Secondary Type II",
            ]  # remove 'Secondary Type I & II'
            typeTxt = wx.ComboBox(
                DData,
                wx.ID_ANY,
                choices=Choices,
                value=UseList[G2frame.hist]["Extinction"][1],
                style=wx.CB_READONLY | wx.CB_DROPDOWN,
            )
            ...
            approxTxT = wx.ComboBox(
                DData,
                wx.ID_ANY,
                choices=Choices,
                value=UseList[G2frame.hist]["Extinction"][0],
                style=wx.CB_READONLY | wx.CB_DROPDOWN,
            )
            ...
            if UseList[G2frame.hist]["Extinction"][1] == "None":
                ...
                if "Tbar" in UseList[G2frame.hist]["Extinction"][2]:  # skipped for TOF
                    ...
                    tbarVal = G2G.ValidatedTxtCtrl(
                        DData,
                        UseList[G2frame.hist]["Extinction"][2],
                        "Tbar",
                        xmin=0.0,
                        nDig=(10, 6),
                        typeHint=float,
                    ...
                    valSizer.Add(
                        wx.StaticText(DData, wx.ID_ANY, " cos(2ThM):"), 0, WACV
                    )
                    cos2tm = G2G.ValidatedTxtCtrl(
                        DData,
                        UseList[G2frame.hist]["Extinction"][2],
                        "Cos2TM",
                        xmin=0.0,
                ...
                if "Primary" in UseList[G2frame.hist]["Extinction"][1]:
                    ...
                    Ekey = ["Eg", "Es"]
                extSizer.Add(ExtVal(Ekey, val2Sizer, "g", [0.0, 100.0]), 0)
            ...
            extVal = G2G.ValidatedTxtCtrl(
                DData,
                UseList[G2frame.hist]["Extinction"],
                0,
                xmin=0.0,
                nDig=(10, 2),
                typeHint=float,
    ...
    def BabSizer():
        ...
        for bab in ["A", "U"]:
            ...
            babVal = G2G.ValidatedTxtCtrl(
                DData,
                UseList[G2frame.hist]["Babinet"]["Bab" + bab],
                0,
                nDig=(10, 3),
                xmin=0.0,
                typeHint=float,
    ...
    def FlackSizer():
        ...
        flackVal = G2G.ValidatedTxtCtrl(
            DData, UseList[G2frame.hist]["Flack"], 0, nDig=(10, 3), typeHint=float
        )
    ...
    def DispSizer():
        ...
        dispSizer.Add(
            G2G.ValidatedTxtCtrl(
                DData,
                UseList[G2frame.hist]["Layer Disp"],
                0,
                nDig=(10, 2),
                typeHint=float,
            ),
    ...
    def twinSizer():
        ...
        def OnTwinVal(invalid, value, tc):
            ...
            for it, twin in enumerate(UseList[G2frame.hist]["Twins"]):
                if it:
                    sumTw += twin[1]
            UseList[G2frame.hist]["Twins"][0][1][0] = 1.0 - sumTw
            wx.CallLater(100, RepaintHistogramInfo, DData.GetScrollPos(wx.VERTICAL))
        ...
        if nTwin > 1:
            for it, Twin in enumerate(UseList[G2frame.hist]["Twins"]):
                ...
                if "bool" not in str(type(Twin[0])):
                    matSizer.Add(
                        wx.StaticText(DData, -1, " Twin Law: "), 0, WACV | wx.LEFT, 5
                    )
                    for im, Mat in enumerate(twinMat):
                        mat = wx.TextCtrl(
                            DData,
                            wx.ID_ANY,
                            "%3d %3d %3d" % (Mat[0], Mat[1], Mat[2]),
                            style=Style,
                        )
                        if it:
                    ...
                    if not SGData["SGInv"]:
                        twinv = wx.CheckBox(
                            DData, wx.ID_ANY, label=" Use enantiomorph?"
                        )
                ...
                valSizer = wx.BoxSizer(wx.HORIZONTAL)
                valSizer.Add(
                    wx.StaticText(DData, -1, label=" Twin element fraction:"), 0, WACV
                )
                if it:
                    twinval = G2G.ValidatedTxtCtrl(
                        DData,
                        UseList[G2frame.hist]["Twins"][it],
                        1,
                        nDig=(10, 3),
                        xmin=0.0,
                        xmax=1.0,
        ...
        if "phoenix" in wx.version():
            ...
            try:
                ...
                if GSASIIpath.GetConfigValue("debug"):
                    print(
                        "DBG: DData window deleted. Ignoring RepaintHistogramInfo, forcing redraw"
                    )
    ...
    def ShowHistogramInfo():
        ...
        if G2frame.hist not in UseList:
            G2frame.ErrorDialog(
                "Missing data error", G2frame.hist + " not in GSAS-II data tree"
            )
            return None
        # patch
        ...
        if "HKLF" in UseList[G2frame.hist]["Histogram"]:
            ...
            if "Twins" not in UseList[G2frame.hist]:
                UseList[G2frame.hist]["Twins"] = [
                    [np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]]), [1.0, False]],
                ]
        ...
        useData = wx.CheckBox(
            DData, wx.ID_ANY, label="Use Histogram: " + G2frame.hist + " ?"
        )
        ...
        if not generalData["doPawley"] and "PWDR" in G2frame.hist[:4]:
            ...
            if UseList[G2frame.hist]["LeBail"]:
                ...
                G2frame.SetStatusText(
                    "To reset Le Bail extracted intensities, cycle Le Bail button.", 1
                )
        ...
        if G2frame.testSeqRefineMode() and not UseList[G2frame.hist]["LeBail"]:
            bottomSizer.Add(
                wx.StaticText(DData, label="     Sequential Refinement Options")
            )
            ...
            fixBox.Add(
                wx.StaticText(DData, label="     Fix these var types: "), 0, WACV
            )
            fixVals = wx.ComboBox(
                DData,
                value=UseList[G2frame.hist]["Fix FXU"],
                choices=parmChoice,
            ...
            fixBox.Add(
                wx.StaticText(
                    DData,
                    label=" in phase " + generalData["Name"] + " for this histogram",
                ),
                0,
                WACV,
            )
            ...
            if SeqId:
                ...
                fixBox.Add(
                    wx.StaticText(
                        DData,
                        label="     Specific phase variables to fix for this histogram: ",
                    ),
                    0,
                    WACV,
                )
                ...
                if len(fixedVars):
                    fixBox.Add(
                        wx.StaticText(
                            DData, label=f" (currently {len(fixedVars)} fixed)"
                        ),
                        0,
                        WACV,
                    )
        ...
        if G2frame.hist[:4] == "PWDR":
            if UseList[G2frame.hist]["Size"][0] == "isotropic":
                ...
                isoSizer.Add(
                    TopSizer(
                        " Domain size model: ",
                        ["isotropic", "uniaxial", "ellipsoidal"],
                        "Size",
                        OnSizeType,
                    ),
                    0,
                ...
                isoSizer.Add(LGmixSizer("Size", [0.0, 1.0], OnLGmixRef))
                isoSizer.Add(ResetSizer("isotropic", OnResetSize), 0, WACV)
                bottomSizer.Add(isoSizer)
                bottomSizer.Add(
                    IsoSizer("size(\xb5m): ", "Size", (10, 4), [0.0, 10.0], OnSizeRef),
                    0,
                    wx.BOTTOM,
                    5,
                )
                ...
                ellSizer.Add(LGmixSizer("Size", [0.0, 1.0], OnLGmixRef))
            ...
            if UseList[G2frame.hist]["Mustrain"][0] == "isotropic":
                ...
                isoSizer.Add(LGmixSizer("Mustrain", [0.0, 1.0], OnLGmixRef))
                isoSizer.Add(ResetSizer("isotropic", OnResetStrain), 0, WACV)
                bottomSizer.Add(isoSizer)
                bottomSizer.Add(
                    IsoSizer(
                        " microstrain: ", "Mustrain", (10, 1), [0.0, 1.0e5], OnStrainRef
                    ),
                    0,
                    wx.BOTTOM,
                    5,
                ...
                genSizer.Add(LGmixSizer("Mustrain", [0.0, 1.0], OnLGmixRef))
            ...
            bottomSizer.Add(
                wx.StaticText(DData, wx.ID_ANY, " Hydrostatic/elastic strain:")
            )
    ...
    ):  # no associated histograms, nothing to display here
        G2frame.hist = ""
    elif (
        hist and hist in G2frame.dataWindow.HistsInPhase
    ):  # something was input as a selection as an argument
        G2frame.hist = hist
    elif (not G2frame.hist) or (
        G2frame.hist not in G2frame.dataWindow.HistsInPhase
    ):  # no or bad selection but have data, take the first
    ...
    if G2frame.hist:
        ...
        DData.select = wx.ListBox(
            DData,
            choices=G2frame.dataWindow.HistsInPhase,
            style=wx.LB_SINGLE,
            size=(-1, 160),
        )
        ...
        mainSizer.Add(
            wx.StaticText(
                DData, wx.ID_ANY, "  (This project has no data; use Import to read it)"
            ),
            0,
            wx.TOP,
            10,
        ...
        mainSizer.Add(
            wx.StaticText(
                DData,
                wx.ID_ANY,
                "  (This phase has no associated data; use appropriate Edit/Add... menu item)",
            ),
            0,
            wx.TOP,
            10,
        ...
        mainSizer.Add(
            wx.StaticText(DData, wx.ID_ANY, "  (Strange, how did we get here?)"),
            0,
            wx.TOP,
            10,
        )
    ...
    def OnSelectPage(event):
        ...
        for i, page in enumerate(phaseList):
            if tabname == page:
                ...
                wx.CallAfter(
                    FillDDataWindow, i
                )  # may result in a double paint on some OSs
                return
