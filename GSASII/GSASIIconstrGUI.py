...
class RBDataTable(wg.GridTableBase):
    ...
    def MoveRow(self, frm, to):
        ...
        if grid:
            ...
            # Notify the grid
            grid.BeginBatch()
            msg = wg.GridTableMessage(self, wg.GRIDTABLE_NOTIFY_ROWS_DELETED, frm, 1)
            grid.ProcessTableMessage(msg)
            msg = wg.GridTableMessage(self, wg.GRIDTABLE_NOTIFY_ROWS_INSERTED, to, 1)
            grid.ProcessTableMessage(msg)
            grid.EndBatch()
        if self.onChange:
            self.onChange()
...
class ConstraintDialog(wx.Dialog):
    ...
    def __init__(
        self, parent, title, text, data, separator="*", varname="", varyflag=False
    ):
        wx.Dialog.__init__(
            self,
            parent,
        ...
        subpanel = wxscroll.ScrolledPanel(
        ...
        for id in range(len(self.data)):
            ...
            scale = G2G.ValidatedTxtCtrl(
                subpanel, self.data[id], 0, OKcontrol=self.DisableOK
            )
...
def CheckConstraints(
    ...
    constrDict, fixedList, ignored = G2mv.ProcessConstraints(
        constrDict, seqhst=seqhst, seqmode=seqmode
    )
    parmDict = {}
    ...
    )  # generates atom symmetry constraints
    ...
    errmsg, warnmsg, groups, parmlist = G2mv.GenerateConstraints(
...
def UpdateConstraints(G2frame, data, selectTab=None, Clear=False):
    ...
    def FindEquivVarb(name, nameList):
        ...
        for item in nameList:
            ...
            if (
                items[1] == "*" and keys[2] in namelist
            ):  # wildcard -- select only sequential options
                keys[1] = "*"
                mitem = ":".join(keys)
                if mitem == name:
                    continue
                if mitem not in outList:
                    outList.append(mitem)
            elif (
                keys[2] in namelist or keys[2].split(";")[0] in namelist
            ) and item != name:
                outList.append(item)
        return outList
    ...
    def SelectVarbs(page, FrstVarb, varList, legend, constType):
        ...
        if l[1] == "*":
            ...
            if len(Histograms) > 1:
                ...
                typ = Histograms[G2obj.LookupHistName(l[1])[0]][
                    "Instrument Parameters"
                ][0]["Type"][1]
                i = 0
                for hist in Histograms:
                    if Histograms[hist]["Instrument Parameters"][0]["Type"][1] == typ:
                        i += 1
                if i > 1:
                    histlist += ["all=" + typ]
        ...
        if len(choices):
            ...
            atchoices = [
                fmt.format(*i1) for i1 in choices
            ]  # reformat list as str with columns
            dlg = G2G.G2MultiChoiceDialog(
                G2frame,
                legend,
                "Constrain " + str(FrstVarb) + " with...",
                atchoices,
                toggle=False,
                size=(625, 400),
                monoFont=True,
        ...
        for sel in Selections:
            ...
            if l[1] == "all":
                ...
                hstlist = [
                    ...
                    for hist in Histograms
                    if Histograms[hist]["Instrument Parameters"][0]["Type"][1] == htyp
                ]
            else:
                hstlist = [l[1]]
            if len(l) == 3:
    ...
    def CheckAddedConstraint(newcons):
        ...
        errmsg, warnmsg = CheckConstraints(
            G2frame,
            Phases,
            Histograms,
            data,
        ...
        if errmsg:
            G2frame.ErrorDialog(
                ...
                + errmsg
                + "\nIgnoring newly added constraint",
                parent=G2frame,
            )
            # reset error status
            errmsg, warnmsg = CheckConstraints(
                G2frame, Phases, Histograms, data, seqhst=seqhistnum, seqmode=seqmode
            )
            if errmsg:
                print(errmsg)
            ...
            txt += warnmsg + "\n\nKeep this addition?"
            ans = G2G.ShowScrolledInfo(
                header="Constraint Warning",
                txt=txt,
                buttonlist=[wx.ID_YES, wx.ID_NO],
                parent=G2frame,
                height=250,
            )
            if ans == wx.ID_NO:
    ...
    def CheckChangedConstraint():
        ...
        if errmsg:
            G2frame.ErrorDialog(
                ...
                + errmsg
                + "\nDiscarding last constraint edit",
                parent=G2frame,
            )
            # reset error status
            errmsg, warnmsg = CheckConstraints(
    ...
    def PageSelection(page):
        ...
        if page[1] == "phs":
            vartype = "phase"
            varList = G2obj.removeNonRefined(
                phaseList
            )  # remove any non-refinable prms from list
            constrDictEnt = "Phase"
        elif page[1] == "hap":
    ...
    def OnAddHold(event):
        ...
        if not varList:
            G2frame.ErrorDialog(
                "No variables",
                "There are no variables of type " + vartype,
                parent=G2frame,
            )
            return
        l2 = l1 = 1
        ...
        legend = (
            "Select variables to hold (Will not be varied, even if vary flag is set)"
        )
        dlg = G2G.G2MultiChoiceDialog(
            G2frame,
            legend,
    ...
    def OnAddConstraint(event):
        ...
        title2 = (
            "Select additional "
            + vartype
            + " variable(s) to include in constraint with "
        )
        if not varList:
            G2frame.ErrorDialog(
                "No variables",
    ...
    def FindNeighbors(phase, FrstName, AtNames):
        ...
        IndB = ma.nonzero(ma.masked_greater(dist - 0.85 * sumR, 0.0))
        for j in IndB[0]:
            if j != Orig:
    ...
    def GetAddAtomVars(page, title1, title2, varList, constrDictEnt, constType):
        ...
        """
        Atoms = {
            G2obj.VarDescr(i)[0]: [] for i in varList if "Atom" in G2obj.VarDescr(i)[0]
        }
        for item in varList:
        ...
        AtNames.sort()
        dlg = G2G.G2SingleChoiceDialog(
            G2frame, "Select 1st atom:", title1, AtNames, monoFont=True, size=(625, 400)
        )
        dlg.CenterOnParent()
        FrstAtom = ""
        if dlg.ShowModal() == wx.ID_OK:
        ...
        for name in Atoms[FrstAtom]:
            ...
            )  # get stuff between phase id & atom id
            for sel in Selections:
    ...
    def MakeConstraintsSizer(name, panel):
        ...
        if name == "Sym-Generated":  # show symmetry generated constraints
            ...
            if symHolds:
                ...
                for var in symHolds:
                    ...
                    Sizer.Add(ch)
                    Sizer.Add(
                        wx.StaticText(panel, label="FIXED"),
                        0,
                        WACV | wx.ALIGN_CENTER | wx.RIGHT | wx.LEFT,
                        2,
                    )
                    Sizer.Add(
                        wx.StaticText(panel, label=var),
                        0,
                        WACV | wx.ALIGN_CENTER | wx.RIGHT | wx.LEFT,
                    )
            else:
            ...
            Sizer1.Add(
                wx.StaticText(
                    panel,
                    label="Equivalences generated based on cell/space group input",
                )
            )
            Sizer1.Add((-1, 5))
            ...
            Sizer1.Add(Sizer)
            helptext = ""
            for sym, (warnmsg, note), helptext in zip(
                symGen, SymErr, SymHelp, strict=False
            ):
                if warnmsg:
                    if helptext:
                        helptext += "\n\n"
                ...
                if helptext:
                    ...
                    Sizer.Add((-1, -1))
                Sizer.Add(
                    wx.StaticText(panel, wx.ID_ANY, "EQUIV"),
                    0,
                    WACV | wx.ALIGN_CENTER | wx.RIGHT | wx.LEFT,
                    2,
                )
                Sizer.Add(
                    wx.StaticText(panel, wx.ID_ANY, sym),
                    0,
                    WACV | wx.ALIGN_LEFT | wx.RIGHT | wx.LEFT,
                    2,
                )
                if note:
                    Sizer.Add(
                        wx.StaticText(panel, wx.ID_ANY, note),
                        0,
                        WACV | wx.ALIGN_LEFT | wx.RIGHT | wx.LEFT,
                        2,
                    )
                ...
                Sizer.Add((-1, -1))
            for sym, (warnmsg, note), helptext in zip(
                symGenD, SymErrD, SymHelpD, strict=False
            ):
                if warnmsg:
                    if helptext:
                        helptext += "\n\n"
        ...
        for Id, item in enumerate(data[name]):
            ...
            if item[-1] == "h":  # Hold on variable
                ...
                typeString = "FIXED"
                # var = str(item[0][1])
                var, explain, note, warnmsg = item[0][1].fmtVarByMode(
                    seqmode, note, warnmsg
                )
                ...
                item[-1] == "f" or item[-1] == "e" or item[-1] == "c"
            ):  # not true on original-style (2011?) constraints
                ...
                if item[-1] == "f":
                    ...
                    for term in item[:-3]:
                        ...
                        if np.isclose(m, 0):
                            continue
                        # var = str(term[1])
                        var, explain, note, warnmsg = term[1].fmtVarByMode(
                            seqmode, note, warnmsg
                        )
                        # if '?' in var: badVar = True
                    ...
                    if "_Explain" in data:
                        ...
                        try:
                            ...
                            hlptxt = data["_Explain"].get(
                                str(item[-3].phase) + item[-3].name
                            )
                        if hlptxt:
                            helptext += "\n\n" + hlptxt
                    if item[-3]:
                    ...
                    helptext = (
                        "The following variables are constrained to equal a constant:"
                    )
                    for term in item[:-3]:
                        # var = str(term[1])
                        var, explain, note, warnmsg = term[1].fmtVarByMode(
                    ...
                    m = item[0][0] / item[1][0]
                    # var1 = str(item[1][1])
                    var1, explain, note, warnmsg = item[1][1].fmtVarByMode(
                        seqmode, note, warnmsg
                    )
                    helptext += (
                    ...
                    for i, term in enumerate(item[:-3]):
                        ...
                        if (
                            i == 0
                        ):  # move independent variable to end, as requested by Bob
                            helptext += f"\n{var} " + " (" + varMean + ")"
    ...
    def OnConstEdit(event):
        ...
        if data[name][Id][-1] == "f":
            ...
            dlg = ConstraintDialog(
                ...
                items,
                varname=varname,
                varyflag=data[name][Id][-2],
            )
        elif data[name][Id][-1] == "c":
            items = data[name][Id][:-3] + [[str(data[name][Id][-3]), "fixed value ="]]
    ...
    def UpdateConstraintPanel(panel, typ):
        ...
        if typ != "Sym-Generated":
            ...
            if G2frame.testSeqRefineMode():
                ...
                butSizer.Add(
                    wx.StaticText(
                        panel, wx.ID_ANY, "  Sequential Ref. Settings.  Wildcard use: "
                    ),
                    0,
                    WACV,
                )
                btn = G2G.EnumSelector(
                    panel,
                    data,
                    "_seqmode",
                ...
                butSizer.Add(btn, 0, wx.ALIGN_CENTER_VERTICAL)
                butSizer.Add(
                    wx.StaticText(panel, wx.ID_ANY, "  Selected histogram: "), 0, WACV
                )
                btn = G2G.EnumSelector(
                    panel,
                    data,
    ...
    def OnPageChanged(event, selectTab=None):
        ...
        if text == "Histogram/Phase":
            ...
            if "DELETED" in str(
                PhaseConstr
            ):  # seems to be no other way to do this (wx bug)
                if GSASIIpath.GetConfigValue("debug"):
        ...
        for obj, flag in zip(
            G2frame.dataWindow.ConstraintEdit.GetMenuItems(),
            enableEditCons,
            strict=False,
        ):
            obj.Enable(flag)
    ...
    for key, value in data.items():
        ...
        if j:
            print(
                ...
                + ": "
                + str(j)
                + " variable(s) as strings converted to objects"
            )
    ##### end patch #############################################################################
    rigidbodyDict = G2frame.GPXtree.GetItemPyData(
        G2gd.GetGPXtreeItemId(G2frame, G2frame.root, "Rigid bodies")
    ...
    try:
        AtomDict = dict(
            [Phases[phase]["pId"], Phases[phase]["Atoms"]] for phase in Phases
        )
    except KeyError:
        G2frame.ErrorDialog(
            "Constraint Error",
            "Constraints cannot be set until a cycle of least squares"
        ...
        for h, hist in problems:
            ...
            for p in phases:
                if hist not in phases[p]["Histograms"]:
                    continue
                if not phases[p]["Histograms"][hist]["Use"]:
                    continue
                constr += [
                    [
                        1.0,
...
def CheckScalePhaseFractions(G2frame, hist, histograms, phases, Constraints):
    ...
    for p in phases:
        if hist not in phases[p]["Histograms"]:
            continue
        if (
            phases[p]["Histograms"][hist]["Use"]
            and not phases[p]["Histograms"][hist]["Scale"][1]
        ):
            return False
        else:
            phaseCount += 1
...
def TransConstraints(G2frame, oldPhase, newPhase, Trans, Vec, atCodes):
    ...
    def SetUniqAj(pId, iA, SGData):
        ...
        if SGLaue in ["m3", "m3m"]:
            ...
            if (
                iA in [0, 1, 2]
                or (iA == 3 and SGUniq == "c")
                or (iA == 4 and SGUniq == "b")
                or (iA == 5 and SGUniq == "a")
            ):
                parm = "%d::A%s" % (pId, iA)
            else:
                parm = None
    ...
    for ia, code in enumerate(atCodes):
        ...
        if not ia and atom[cia] == "A":
            wx.MessageDialog(
                ...
                "Anisotropic thermal constraint?",
        ...
        for i, ix in enumerate(list(CSX[0])):
            ...
            for iop, opval in enumerate(invOpr[i]):
                if abs(opval) > 1e-6:
                    DepCons.append(
                        [opval, G2obj.G2VarObj("%d::%s:%s" % (opId, xnames[iop], iat))]
                    )
            if len(DepCons) == 1:
    ...
    if True:  # debug
        constraints["Phase"] += G2lat.GenCellConstraints(
...
def UpdateRigidBodies(G2frame, data):
    ...
    def OnPageChanged(event):
        ...
        if text == "Vector rigid bodies":
            ...
            G2frame.Bind(
                wx.EVT_MENU, OnDefineTorsSeq, id=G2G.wxID_RESIDUETORSSEQ
            )  # enable only if residue RBs exist?
    ...
    def getMacroFile(macName):
        ...
        dlg = wx.FileDialog(
            G2frame,
            message="Choose " + macName + " rigid body macro file",
            defaultDir=defDir,
            defaultFile="",
            wildcard="GSAS-II macro file (*.mac)|*.mac",
        ...
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                if macName not in head:
                    print(head)
                    print(
                        "**** ERROR - wrong restraint macro file selected, try again ****"
                    )
                    macro = []
    ...
    def DumpVectorRB(event):
        ...
        dlg = wx.FileDialog(
            G2frame,
            "Choose file to save residue rigid body",
            pth,
            "",
            "RRB files (*.resbody)|*.resbody",
        ...
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                for i, sym in enumerate(rbData["rbTypes"]):
                    fp.write(f"{sym:3s}")
                    fp.write("{:8.5f}{:9.5f}{:9.5f}   ".format(*rbData["rbXYZ"][i]))
                    fp.write("\n")
                fp.close()
    ...
    def LoadVectorRB(event):
        ...
        dlg = wx.FileDialog(
            G2frame,
            "Choose file to read vector rigid body",
            pth,
            "",
            "RRB files (*.resbody)|*.resbody",
            wx.FD_OPEN,
    ...
    def AddVectorRB(event):
        ...
        dlg = G2G.MultiIntegerDialog(
            G2frame, "New Rigid Body", ["No. atoms", "No. translations"], [3, 1]
        )
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if nAtoms < 3:
                dlg.Destroy()
                G2G.G2MessageBox(
                    G2frame, "A vector rigid body must have 3 or more atoms"
                )
                return
            rbid = ran.randint(0, sys.maxsize)
            ...
            name = "UNKRB"
            namelist = [
                data["Vector"][key]["RBname"]
                for key in data["Vector"]
    ...
    def ExtractPhaseRB(event):
        ...
        def SetupDrawing(atmData):
            ...
            for iat, atom in enumerate(atmData["Atoms"]):
                ...
                if atom[ct] not in generalData["AtomTypes"] and atom[ct] != "UNK":
                    ...
                    atom[ct] = Info[
                        "Symbol"
                    ]  # N.B. symbol might be changed by GetAtomInfo
                    ...
                    if atom[ct] in generalData["Isotope"]:
                        if (
                            generalData["Isotope"][atom[ct]]
                            not in generalData["Isotopes"][atom[ct]]
                        ):
        ...
        def Page1():
            ...
            if rd.Phase["General"]["Type"] == "macromolecular":
                ...
                rd.Phase["General"]["AtomPtrs"] = [
                    i - 3 for i in rd.Phase["General"]["AtomPtrs"]
                ]
        ...
        def Page2():
            ...
            def onSetX(event):
                ...
                for i in range(len(rd.Phase["RBselection"])):
                    if rd.Phase["RBselection"][i]:
                        ...
                        center += rd.Phase["RBcoords"][i]
                if not count:
                    G2G.G2MessageBox(G2frame, "No atoms selected", "Selection required")
                    return
                XYZP = center / count
            ...
            def onSetPlane(event):
                ...
                if Natoms < 3:
                    G2G.G2MessageBox(
                        G2frame,
                        "A plane requires three or more atoms",
                        "Need more atoms",
                    )
                    return
                Zmat = np.zeros((3, 3))
            ...
            def onAddVector(event):
                ...
                if len(rb["rbTypes"]) < 3:
                    return  # must have at least 3 atoms
                rbid = ran.randint(0, sys.maxsize)
                data["Vector"][rbid] = rb
                data["RBIds"]["Vector"].append(rbid)
                for t in rb["rbTypes"]:
                    if t in data["Vector"]["AtInfo"]:
                        continue
                    Info = G2elem.GetAtomInfo(t)
            ...
            DrawCallback = G2plt.PlotRigidBody(
                G2frame, "Vector", AtInfo, rbData, plotDefaults
            )
...
"""
            ...
            inSizer.Add(
                G2G.G2ChoiceButton(
                    RBImpPnl, ("xy", "yz", "xz"), None, None, bntOpts, "plane"
                )
            )
            ...
            inSizer.Add(
                G2G.G2ChoiceButton(
                    RBImpPnl, ("x", "y", "z"), None, None, bntOpts, "direction"
                )
            )
            btnSizer.Add(inSizer, 0, wx.ALIGN_CENTER)
        ...
        def GetCoords(atmsel):
            ...
            Amat, Bmat = G2lat.cell2AB(cell)
            rd.Phase["RBcoords"] = np.array(
                [np.inner(Amat, atms[i][3:6]) for i in atmsel]
            )
            rd.Phase["RBcoords"] -= rd.Phase["RBcoords"].mean(
                axis=0
            )  # origin to middle
        ...
        def UpdateVectorBody(rb, useSelection=False):
            ...
            for i in rd.Phase["RBindex"]:
                use = True
                if useSelection and not rd.Phase["RBselection"][i]:
                    use = False
                if use:
        ...
        if wx.__version__.split(".")[0] != "4":
            wx.MessageBox(
                "Sorry, wxPython 4.x is required to run this command",
        ...
        if platform.python_version()[:1] == "2":
            wx.MessageBox(
                "Sorry, Python >=3.x is required to run this command",
                caption="Update Python",
                style=wx.ICON_EXCLAMATION,
            )
            return
        ...
        dlg = G2G.G2SingleChoiceDialog(
            G2frame, "Select the format of the file", "select format", choices
        )
        dlg.CenterOnParent()
        try:
            if dlg.ShowModal() == wx.ID_OK:
        ...
        with open(filename):
            ...
            if len(rd.selections) > 1:
                print(
                    f"File {filename} has {len(rd.selections)} phases. This is unexpected."
                )
                return
    ...
    def AddVectTrans(event):
        ...
        if len(choices) == 0:
            G2G.G2MessageBox(G2frame, "No Vector Rigid Bodies found", "No VR Bodies")
            return
        elif len(choices) == 1:
            rbid = rbIdlist[0]
        else:
            dlg = G2G.G2SingleChoiceDialog(
                G2frame, "Select the rigid body to save", "select format", choices
            )
            try:
                if dlg.ShowModal() == wx.ID_OK:
    ...
    def SaveVectorRB(event):
        ...
        dlg = wx.FileDialog(
            G2frame,
            "Choose file to save vector rigid body",
            pth,
            "",
            "VRB files (*.vecbody)|*.vecbody",
            wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT,
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                for i, sym in enumerate(data["Vector"][rbid]["rbTypes"]):
                    ...
                    for j in range(ntrans):
                        fp.write(
                            "{:8.5f}{:9.5f}{:9.5f}   ".format(
                                *data["Vector"][rbid]["rbVect"][j][i]
                            )
                        )
                    fp.write("\n")
                fp.close()
    ...
    def AddResidueRB(event):
        ...
        while macStr:
            ...
            if items[0] == "I":
                ...
                data["Residue"][resRBsel] = {
                    "RBname": rbName,
                    "rbXYZ": rbXYZ,
                    "rbTypes": rbTypes,
                    "atNames": atNames,
                    "rbRef": [nOrig - 1, mRef - 1, nRef - 1, True],
                    "rbSeq": rbSeq,
                    "SelSeq": [0, 0],
                    "useCount": 0,
                    "molCent": None,
    ...
    def ImportResidueRB():
        ...
        while len(items):
            ...
            if "mol2" in ext and "BOND" in txtStr:
                break
            if "pdb" in ext and (
                "ATOM" not in txtStr[:6] and "HETATM" not in txtStr[:6]
            ):
                break
            items = txtStr.split()
            nat += 1
        if len(atNames) < 3:
            G2G.G2MessageBox(
                G2frame, "Not enough atoms in rigid body; must be 3 or more"
            )
        else:
        ...
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                for iat, xyz in enumerate(rbData["rbXYZ"]):
                    File.write(
                        "ATOM %6d  %-4s%3s     1    %8.3f%8.3f%8.3f  1.00  0.00          %2s\n"
                        % (
                            iat,
                            rbData["atNames"][iat],
                            rbData["RBname"][:3],
    ...
    def OnDefineTorsSeq(event):
        ...
        dlg = wx.SingleChoiceDialog(
            G2frame,
            "Select origin atom for torsion sequence",
            "Origin atom",
            rbData["atNames"],
        )
        if dlg.ShowModal() == wx.ID_OK:
        ...
        if not len(TargList):
            return
        dlg = wx.SingleChoiceDialog(
            G2frame, "Select pivot atom for torsion sequence", "Pivot atom", TargList
        )
        if dlg.ShowModal() == wx.ID_OK:
    ...
    def UpdateVectorRB(Scroll=0):
        ...
        def rbNameSizer(rbid, rbData):
            ...
            nameSizer.Add(
                wx.StaticText(VectorRBDisplay, -1, "Rigid body name: "), 0, WACV
            )
            RBname = wx.TextCtrl(
                VectorRBDisplay, -1, rbData["RBname"], style=wx.TE_PROCESS_ENTER
            )
            ...
            if not rbData["useCount"]:
                delRB = wx.Button(
                    VectorRBDisplay, wx.ID_ANY, "Delete", style=wx.BU_EXACTFIT
                )
        ...
        def rbRefAtmSizer(rbid, rbData):
            ...
            if rbData["useCount"]:
                ...
                refAtmSizer.Add(
                    wx.StaticText(
                        VectorRBDisplay, -1, "Orientation reference atoms A-B-C: "
                    ),
                    0,
                    WACV,
                )
...
"""
                ...
                refAtmSizer.Add(
                    hlp, 0, wx.LEFT | wx.RIGHT | wx.ALIGN_CENTER_VERTICAL, 2
                )
            return refAtmSizer
        ...
        def rbVectMag(rbid, imag, rbData):
            ...
            magSizer.Add(
                wx.StaticText(VectorRBDisplay, -1, "Translation magnitude: "), 0, WACV
            )
        ...
        if "DELETED" in str(
            G2frame.GetStatusBar()
        ):  # seems to be no other way to do this (wx bug)
            if GSASIIpath.GetConfigValue("debug"):
                print(
                    "DBG_wx error: Rigid Body/Status not cleanly deleted after Refine"
                )
            return
        ...
        XYZ = np.array([[0.0, 0.0, 0.0] for Ty in rbData["rbTypes"]])
    ...
    def UpdateSpinRB():
        ...
        SpinRBSizer.Add(
            wx.StaticText(
                SpinRBDisplay,
                label=" Spinning rigid body shells/nonspherical atoms (Atom=Q & select Orbitals):",
            )
        )
        nQ = 0
        ...
        for ibod, spinID in enumerate(data["Spin"]):
            ...
            bodSizer.Add(
                G2G.ValidatedTxtCtrl(SpinRBDisplay, data["Spin"][spinID], "RBname")
            )
            ...
            bodSizer.Add(
                G2G.ValidatedTxtCtrl(SpinRBDisplay, data["Spin"][spinID], "Natoms")
            )
            if "Q" in data["Spin"][spinID]["atType"]:
                data["Spin"][spinID]["elType"] = data["Spin"][spinID].get("elType", "C")
                elSel = wx.TextCtrl(
                    SpinRBDisplay,
                    value=data["Spin"][spinID]["elType"],
                    style=wx.TE_PROCESS_ENTER,
                )
    ...
    def UpdateResidueRB():
        ...
        def rbNameSizer(rbid, rbData):
            ...
            nameSizer = wx.BoxSizer(wx.HORIZONTAL)
            nameSizer.Add(
                wx.StaticText(ResidueRBDisplay, -1, "Residue name: "), 0, WACV
            )
            RBname = wx.TextCtrl(
                ResidueRBDisplay, -1, rbData["RBname"], style=wx.TE_PROCESS_ENTER
            )
            RBname.Bind(wx.EVT_LEAVE_WINDOW, OnRBName)
            RBname.Bind(wx.EVT_TEXT_ENTER, OnRBName)
            RBname.Bind(wx.EVT_KILL_FOCUS, OnRBName)
            ...
            plotRB = wx.Button(
                ResidueRBDisplay, wx.ID_ANY, "Plot", style=wx.BU_EXACTFIT
            )
            plotRB.Bind(wx.EVT_BUTTON, OnPlotRB)
            Indx[plotRB.GetId()] = rbid
            nameSizer.Add(plotRB, 0, WACV)
            ...
            if not rbData["useCount"]:
                delRB = wx.Button(
                    ResidueRBDisplay, wx.ID_ANY, "Delete", style=wx.BU_EXACTFIT
                )
                delRB.Bind(wx.EVT_BUTTON, OnDelRB)
                Indx[delRB.GetId()] = rbid
                nameSizer.Add(delRB, 0, WACV)
            ...
            nameSizer.Add(
                wx.StaticText(
                    ResidueRBDisplay,
                    -1,
                    "  body type #" + str(data["RBIds"]["Residue"].index(rbid)),
                ),
                0,
                WACV,
            )
        ...
        def rbResidues(rbid, rbData):
            ...
            def OnRefSel(event):
                ...
                if rbData["rbTypes"][ind] == "H":
                    G2G.G2MessageBox(
                        G2frame,
                        "You should not select an H-atom for rigid body orientation",
                    )
            ...
            def OnMolCent(event):
                ...
                if rbData["molCent"]:
                    ...
                    res.ForceRefresh()
                    G2plt.PlotRigidBody(
                        G2frame, "Residue", AtInfo, rbData, plotDefaults
                    )
...
"""
            ...
            if rbData["rbRef"][3] or rbData["useCount"]:
                ...
                refAtmSizer.Add(
                    wx.StaticText(
                        ResidueRBDisplay,
                        -1,
                        "Orientation reference non-H atoms A-B-C: ",
                    ),
                    0,
                    WACV,
                )
                ...
                if "molCent" not in rbData:
                    rbData["molCent"] = False  # patch
        ...
        def Add2SeqSizer(seqSizer, angSlide, rbid, iSeq, Seq, atNames):
            ...
            if not iSeq:
                radBt = wx.RadioButton(
                    ResidueRBDisplay, wx.ID_ANY, "", style=wx.RB_GROUP
                )
        ...
        def SlideSizer():
            ...
            slideSizer.Add(
                wx.StaticText(ResidueRBDisplay, -1, "Selected torsion angle:"), 0
            )
            iSeq, angId = rbData["SelSeq"]
            angSlide = G2G.G2Slider(
                ResidueRBDisplay,
                -1,
                int(100 * rbData["rbSeq"][iSeq][2]),
                0,
                36000,
                size=(200, 20),
        ...
        ResidueRB.SetScrollbars(
            10, 10, int(Size[0] / 10 - 4), int(Size[1] / 10 - 1)
        )  # dataframe already scrolls
        G2frame.dataWindow.SendSizeEvent()
...
def ShowIsoDistortCalc(G2frame, phase=None):
    ...
    if phase and phase not in isophases:
        ...
        dlg = wx.SingleChoiceDialog(
            G2frame, "Select phase from ISODISTORT phases", "Select Phase", isophases
        )
        if dlg.ShowModal() == wx.ID_OK:
            sel = dlg.GetSelection()
            phase = isophases[sel]
    ...
    dlg = wx.Dialog(
        G2frame,
        wx.ID_ANY,
        "ISODISTORT mode values",  # size=(630,400),
        style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER,
    )
    mainSizer = wx.BoxSizer(wx.VERTICAL)
    data = Phases[phase]
    ...
    mainSizer.Add(
        wx.StaticText(
            dlg,
            wx.ID_ANY,
            "ISODISTORT mode computation for coordinates in phase "
    ...
    if "G2OccVarList" in ISO:
        ...
        for gv, Ilbl in zip(ISO["G2OccVarList"], ISO["OccVarList"], strict=False):
            ...
            if var in parmDict:
                ...
                dlg.EndModal(wx.ID_CANCEL)
                G2frame.ErrorDialog(
                    "Atom not found", "No value found for parameter " + str(var)
                )
                return
...
def ShowIsoModes(G2frame, phase):
    ...
    dlg = wx.Dialog(
        G2frame,
        wx.ID_ANY,
        "ISODISTORT modes and displacements",  # size=(630,400),
        style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER,
    )
    mainSizer = wx.BoxSizer(wx.VERTICAL)
    mainSizer.Add(
        wx.StaticText(
            dlg,
            wx.ID_ANY,
            "ISODISTORT modes and displacements in phase "
            + str(data["General"].get("Name", "?")),
        )
    )
    # ISODISTORT displacive modes
    if "G2VarList" in ISO:
        ...
        subSizer1.Add(
            wx.StaticText(panel1, label="ISODISTORT\nvariable"), 0, wx.ALIGN_CENTER
        )
        subSizer1.Add((8, -1))  # spacer
        subSizer1.Add(
            wx.StaticText(panel1, wx.ID_ANY, "GSAS-II\nequiv."), 0, wx.ALIGN_CENTER
        )
        subSizer1.Add((8, -1))  # spacer
        subSizer1.Add(
            wx.StaticText(panel1, wx.ID_ANY, "expression"), 0, wx.ALIGN_CENTER
        )
        subSizer2.Add((-1, -1))
        subSizer2.Add(
            wx.StaticText(panel2, wx.ID_ANY, "ISODISTORT\nMode name"),
            0,
            wx.ALIGN_CENTER,
        )
        subSizer2.Add((8, -1))  # spacer
        subSizer2.Add(
            wx.StaticText(panel2, wx.ID_ANY, "GSAS-II\nequiv."), 0, wx.ALIGN_CENTER
        )
        subSizer2.Add((8, -1))  # spacer
        subSizer2.Add(
