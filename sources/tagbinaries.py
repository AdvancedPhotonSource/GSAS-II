...
source = os.environ.get("MESON_SOURCE_ROOT", os.path.dirname(os.path.dirname(__file__)))
build = os.environ.get("MESON_BUILD_ROOT", os.path.dirname(os.path.dirname(__file__)))

if shutil.which("git"):
    out = subprocess.run(
        ["git", "tag", "-l", "--sort=-authordate", "v*"],
    ...
    out = subprocess.run(
        ["git", "log", "HEAD", "-1"], stdout=subprocess.PIPE, cwd=source, check=False
    )
