...
def SetNewPhase(Name="New Phase", SGData=None, cell=None, Super=None):
    ...
    if SGData is None:
        SGData = P1SGData
    if cell is None:
        cell = [1.0, 1.0, 1.0, 90.0, 90.0, 90.0, 1.0]
    phaseData = {
    ...
    if Super and Super.get("Use", False):
        phaseData["General"].update(
            {"Modulated": True, "Super": True, "SuperSg": Super["ssSymb"]}
        )
...
def LookupAtomId(pId, ranId):
    ...
    if ranId not in AtomRanIdLookup[pId]:
        raise KeyError(
            "Error: LookupAtomId, ranId "
            + str(ranId)
            + " not in AtomRanIdLookup["
            + pId
            + "]"
        )
...
class G2VarObj:
    ...
    def __init__(self, *args):
        ...
        if (
            len(args) == 1
            and (type(args[0]) is list or type(args[0]) is tuple)
            and len(args[0]) == 4
        ):
            ...
            if lst[0] == "*":
                ...
                if len(lst) == 4:
                    ...
                    raise Exception(
                        "Incorrect number of colons in var name " + str(args[0])
                    )
            self.name = lst[2]
        elif len(args) == 4:
            if args[0] == "*":
...
def SetDefaultSample():
    ...
    return {
        ...
        "Thick": 1.0,
        "Contrast": [0.0, 0.0],  # contrast & anomalous contrast
...
class ImportBaseclass:
    ...
    def ExtensionValidator(self, filename):
        ...
        if filename:
            ...
            if not ext and self.strictExtension:
                return False
...
class ImportStructFactor(ImportBaseclass):
    ...
    def InitParameters(self):
        ...
        self.Parameters = [
            {
                "Type": [HistType, HistType],  # create the structure
                "Lam": [Lambda, Lambda],
            },
            {},
        ]
...
class ImportImage(ImportBaseclass):
    ...
    def LoadImage(self, ParentFrame, imagefile, imagetag=None):
        ...
        if ParentFrame:
            ...
            ParentFrame.oldImagefile = (
                imagefile  # save the name of the last image file read
            )
            ParentFrame.oldImageTag = (
                imagetag  # save the tag of the last image file read
            )
...
class ExpressionObj:
    ...
    def LoadExpression(
        self, expr, exprVarLst, varSelect, varName, varValue, varRefflag
    ):
    ...
    def GetVaried(self):
        ...
        return [
            "::" + self.freeVars[v][0] for v in self.freeVars if self.freeVars[v][2]
        ]

    def GetVariedVarVal(self):
        "Returns the names and values of the free parameters that will be refined"
        return [
            ("::" + self.freeVars[v][0], self.freeVars[v][1])
            for v in self.freeVars
            if self.freeVars[v][2]
        ]
    ...
    def UpdateVariedVars(self, varyList, values):
        ...
        for v in self.freeVars:
            if not self.freeVars[v][2]:
                continue
            if "::" + self.freeVars[v][0] not in varyList:
                continue
    ...
    def ParseExpression(self, expr):
            ...
            if isinstance(node, list):
                ...
                for a, b in ast.iter_fields(node):
                    if isinstance(b, ast.AST):
                        ...
                        varlist += v
                        fxnlist += f
                    elif node.__class__.__name__ == "Name" or (
                        fxn and node.__class__.__name__ == "Attribute"
                    ):
                        varlist += [b]
        ...
        for fxn in fxnlist:
            ...
            if not fxnobj:
                self.lastError = ("Error: Invalid function", fxn, "is not defined")
                return None
...
class ExpressionCalcObj:
    ...
    def SetupCalc(self, parmDict):
        ...
        """
        if self.eObj.expression.startswith("Dist") or self.eObj.expression.startswith(
            "Angle"
        ):
            return
    ...
    def EvalExpression(self):
        ...
        if self.eObj.expression.startswith("Dist"):
            #            GSASIIpath.IPyBreak()
            dist = G2mth.CalcDist(
                self.eObj.distance_dict, self.eObj.distance_atoms, self.parmDict
            )
            return dist
        elif self.eObj.expression.startswith("Angle"):
            angle = G2mth.CalcAngle(
                self.eObj.angle_dict, self.eObj.angle_atoms, self.parmDict
            )
            return angle
...
def HowDidIgetHere(wherecalledonly=False):
    ...
    if wherecalledonly:
        ...
        print(70 * "*")
        for i in traceback.format_list(traceback.extract_stack()[:-1]):
            print(i.strip().rstrip())
        print(70 * "*")
...
def CreatePDFitems(G2frame, PWDRtree, ElList, Qlimits, numAtm=1, FltBkg=0, PDFnames=[]):
    ...
    G2frame.GPXtree.SetItemPyData(
        G2frame.GPXtree.AppendItem(Id, text="PDF Controls"), Data
    )
    G2frame.GPXtree.SetItemPyData(
...
def patchControls(Controls):
    ...
    for k in Controls["parmFrozen"]:
        for item in Controls["parmFrozen"][k]:
            if type(item) is str:
                conv = True
                Controls["parmFrozen"][k] = [
                    G2VarObj(i) for i in Controls["parmFrozen"][k]
                ]
                break
    if conv:
        G2fil.G2Print("Applying patch to Controls['parmFrozen']")
