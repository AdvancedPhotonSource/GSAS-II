...
#### Dysnomia (MEM) Data page ##############################################################################
    ...
    def OnSetupRMC(event):
        '''Writes the files used in the PDF programs in response to the Setup... menu command"
        '''
        G2phsG2.SetupRMC(event,G2frame,data)

    def OnRunRMC(event):
        '''Run a previously created RMCProfile/fullrmc/PDFfit2 script
        '''
        if G2frame.RMCchoice == 'fullrmc':
             G2phsG2.Runfullrmc(event,G2frame,data)
        elif G2frame.RMCchoice == 'RMCProfile':
            G2phsG2.RunRMCProfile(event,G2frame,data)
        elif G2frame.RMCchoice == 'PDFfit':
            G2phsG2.RunPDFfit(event,G2frame,data)
    ...
    def OnLoadRMC(event):
        ...
        original cell
        '''
        if G2phsG2.fullrmcLoadPhase(super=False):
    ...
    def OnLoadRMCsuper(event):
        ...
        supercell cell
        '''
        if G2phsG2.fullrmcLoadPhase(super=True):
            SetupGeneral()  # index elements

    def OnViewRMC(event):
        if G2frame.RMCchoice == 'fullrmc':
            G2phsG2.Viewfullrmc(event,G2frame,data)
        elif G2frame.RMCchoice == 'RMCProfile':
            G2phsG2.ViewRMCprofile(event,G2frame,data)
        elif G2frame.RMCchoice == 'PDFfit':
            G2phsG2.ViewPDFfit(event,G2frame,data)
