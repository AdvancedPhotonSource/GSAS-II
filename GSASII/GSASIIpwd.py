...
from . import NIST_profile as FP  # noqa: E402
...
def SurfaceRoughDerv(SRA, SRB, Tth):
    ...
    T1 = np.exp(-SRB / sth)
    T2 = SRA + (1.0 - SRA) * np.exp(-SRB)
    Trans = (SRA + (1.0 - SRA) * T1) / T2
    dydSRA = ((1.0 - T1) * T2 - (1.0 - np.exp(-SRB)) * Trans) / T2**2
    dydSRB = ((SRA - 1.0) * T1 * T2 / sth - Trans * (SRA - T2)) / T2**2
    return [dydSRA, dydSRB]


def Absorb(Geometry, MuR, Tth, Phi=0, Psi=0):  # noqa: ARG001
    ...
    def muRunder3(MuR, Sth2):
        T0 = 16.0 / (3.0 * np.pi)
        T1 = (
            (25.99978 - 0.01911 * Sth2**0.25) * np.exp(-0.024551 * Sth2)
            + 0.109561 * np.sqrt(Sth2)
            - 26.04556
        )
        T2 = (
            -0.02489
    ...
    def muRover3(MuR, Sth2):
        ...
        T3 = (
            1.933433 / (1.0 + 23.12967 * Sth2) ** 1.686715
            - 0.13576 * np.sqrt(Sth2)
            + 1.163198
        )
...
def Oblique(ObCoeff, Tth):
    ...
    if ObCoeff:
        K = (1.0 - ObCoeff) / (1.0 - np.exp(np.log(ObCoeff) / npcosd(Tth)))
        return K
    else:
        return 1.0
...
def Ruland(RulCoff, wave, Q, Compton):
    ...
    return 1.0 / (
        (1.0 + dlam / RulCoff) * (1.0 + (np.pi * dlam_c / (dlam + RulCoff)) ** 2)
    )
...
def KleinNishina(wave, Q):
    hmc = 0.024262734687  # Compton wavelength in A
    TTh = npq2T(Q, wave)
    P = 1.0 / (1.0 + (1.0 - npcosd(TTh) * (hmc / wave)))
    KN = (P**3 - (P * npsind(TTh)) ** 2 + P) / (1.0 + npcosd(TTh) ** 2)
    return KN
...
def CalcPDF(data, inst, limits, xydata):
    ...
    IofQ[1] = np.array([I[Ibeg:Ifin] for I in IofQ[1]])  # noqa: E741
    if data["Sample Bkg."]["Name"]:
        try:  # fails if background differs in number of points
            IofQ[1][1] += (
                xydata["Sample Bkg."][1][1][Ibeg:Ifin] * data["Sample Bkg."]["Mult"]
            )
        except ValueError:
            print("Interpolating Sample background since points don't match")
            interpF = si.interp1d(
                xydata["Sample Bkg."][1][0],
                xydata["Sample Bkg."][1][1],
    ...
    if data["Container"]["Name"]:
        ...
        if data["Container Bkg."]["Name"]:
            try:
                xycontainer += (
                    xydata["Container Bkg."][1][1][Ibeg:Ifin]
                    * data["Container Bkg."]["Mult"]
                )
            except ValueError:
                print(
                    "Number of points do not agree between Container and Container Bkg."
                )
                return None
        try:  # fails if background differs in number of points
            ...
            interpF = si.interp1d(
                xydata["Container"][1][0], xycontainer, fill_value="extrapolate"
            )
            IofQ[1][1] += interpF(IofQ[1][0])

    data["IofQmin"] = IofQ[1][1][-1]
    IofQ[1][1] -= data.get("Flat Bkg", 0.0)
    ...
    if "X" in inst["Type"][0]:
        ...
        for iel, El in enumerate(Els):
            BL = BLtables[El][1]
            SA = (
                BL["SA"] * wave / 1.798197 + 4.0 * np.pi * FPP[iel] ** 2
            )  # +BL['SL'][1]?
            SA *= ElList[El]["FormulaNo"] / data["Form Vol"]
            Abs += SA
        MuR = Abs * data["Diam"] / 2.0
        IofQ[1][1] /= Absorb(
            data["Geometry"], MuR, inst["2-theta"][1] * np.ones(len(wave))
        )
    ...
    if "C" in inst["Type"][0]:
        ...
        Qpoints = np.linspace(0.0, maxQ, nQpoints, endpoint=True)
        dq = Qpoints[1] - Qpoints[0]
        XY[0] = npT2q(XY[0], wave)
        Qdata = si.griddata(
            XY[0], XY[1], Qpoints, method="linear", fill_value=XY[1][0]
        )  # interpolate I(Q)
    elif "T" in inst["Type"][0]:
        difC = inst["difC"][1]
        minQ = 2.0 * np.pi * difC / Tth[-1]
        maxQ = 2.0 * np.pi * difC / Tth[0]
        Qpoints = np.linspace(0.0, maxQ, nQpoints, endpoint=True)
        dq = Qpoints[1] - Qpoints[0]
        XY[0] = 2.0 * np.pi * difC / XY[0]
        Qdata = si.griddata(
            XY[0], XY[1], Qpoints, method="linear", fill_value=XY[1][-1]
        )  # interpolate I(Q)
    ...
    if "XC" in inst["Type"][0]:
        FFSq, SqFF, CF = GetAsfMean(
            ElList, (xydata["SofQ"][1][0] / (4.0 * np.pi)) ** 2
        )  # these are <f^2>,<f>^2,Cf
    else:  # TOF
    ...
    Rmax = GSASIIpath.GetConfigValue("PDF_Rmax", 100.0)
    mul = int(round(2.0 * np.pi * nR / (Rmax * qLimits[1])))
    #    mul = int(round(2.*np.pi*nR/(data.get('Rmax',100.)*qLimits[1])))
    R = 2.0 * np.pi * np.linspace(0, nR, nR, endpoint=True) / (mul * qLimits[1])
    xydata["GofR"][1][0] = R
    xydata["gofr"][1][0] = R
    GR = (
        -(2.0 / np.pi)
        * dq
        * np.imag(fft.fft(xydata["FofQ"][1][1], mul * nR)[:nR])
        * data.get("GR Scale", 1.0)
    )
    ...
    if "ElList" in data:
        ...
        gr = GR / (4.0 * np.pi * numbDen * R) + 1.0
        #        gr = GR/(np.pi*R) ##mising numberdensity
        xydata["gofr"][1][1] = gr
    if data.get("noRing", True):
        Rmin = data["Rmin"]
        xydata["gofr"][1][1] = np.where(Rmin > R, -4.0 * numbDen, xydata["gofr"][1][1])
        xydata["GofR"][1][1] = np.where(
            Rmin > R, -4.0 * R * np.pi * numbDen, xydata["GofR"][1][1]
        )
    return auxPlot


def PDFPeakFit(peaks, data):
    rs2pi = 1.0 / np.sqrt(2 * np.pi)
    ...
    Rvals["Rwp"] = np.sqrt(chisq / np.sum(Y**2)) * 100.0  # to %
    chisq = np.sum(result[2]["fvec"] ** 2) / (
        len(X) - len(values)
...
def MakeRDF(RDFcontrols, background, inst, pwddata):
    ...
    if "C" in inst["Type"][0] or "B" in inst["Type"][0]:
        ...
        TOF = pwddata[0]
        difC = inst["difC"][1]
        minQ = 2.0 * np.pi * difC / TOF[-1]
        maxQ = 2.0 * np.pi * difC / TOF[0]
        powQ = 2.0 * np.pi * difC / TOF
    ...
    dq = Qpoints[1] - Qpoints[0]
    nR = len(Qdata)
    R = 0.5 * np.pi * np.linspace(0, nR, nR) / (4.0 * maxQ)
...
def OptimizePDF(data, xydata, limits, inst, showFit=True, maxCycles=25):
    ...
    if showFit:
        ...
        if data["Sample Bkg."].get("Refine", False):
            #            data['Flat Bkg'] = 0.
            G2fil.G2Print(
                "  start: Ruland={:.3f}, Sample Bkg mult={:.3f} (RMS:{:.4f})".format(
                    data["Ruland"], data["Sample Bkg."]["Mult"], rms
                )
            )
        else:
            G2fil.G2Print(
                "  start: Flat Bkg={:.1f}, BackRatio={:.3f}, Ruland={:.3f} (RMS:{:.4f})".format(
    ...
    if data["Sample Bkg."].get("Refine", False):
        res = opt.minimize(
            Min,
            xstart,
            bounds=([0.01, 1.0], [1.2 * bakMul, 0.8 * bakMul]),
            method="L-BFGS-B",
    ...
    if showFit:
        ...
        if data["Sample Bkg."].get("Refine", False):
            ...
            G2fil.G2Print(
                "  end:   Flat Bkg={:.1f}, BackRatio={:.3f}, Ruland={:.3f} RMS:{:.4f}) *** {} ***\n".format(
...
def SetupPDFEval(data, xydata, limits, inst, numbDen):
    ...
    def EvalLowPDF(arg):
        ...
        if Data["Sample Bkg."].get("Refine", False):
            ...
            F, B, R = arg
            Data["Flat Bkg"] = BkgMax * (2.0 * F - 1.0)
            Data["BackRatio"] = B
        Data["Ruland"] = R
    ...
    def GetCurrentVals():
        ...
        if data["Sample Bkg."].get("Refine", False):
            return [max(data["Ruland"], 0.05), data["Sample"]["Mult"]]
        try:
            F = 0.5 + 0.5 * data["Flat Bkg"] / BkgMax
        except:  # noqa: E722
            F = 0
        return [F, data["BackRatio"], max(data["Ruland"], 0.05)]
    ...
    BkgMax = max(xydata["IofQ"][1][1]) / 50.0
...
# loc = mu, scale = std
_norm_pdf_C = 1.0 / math.sqrt(2 * math.pi)
...
class norm_gen(st.rv_continuous):
    ...
    def pdf(self, x, *args, **kwds):  # noqa: ARG002
...
class fcjde_gen(st.rv_continuous):
    ...
    def _pdf(self, x, t, s, dx):
        ...
        fx = np.where(ax > bx, (np.sqrt(bx / (ax - bx)) - 1.0 / s) / ax2, 0.0)
        fx = np.where(fx > 0.0, fx, 0.0)
        return fx
...
def getWidthsCW(pos, sig, gam, shl):
    ...
    """
    widths = [np.sqrt(sig) / 100.0, gam / 100.0]
    fwhm = max(2.355 * widths[0] + widths[1], 0.0001)
    fmin = 50.0 * (fwhm + shl * abs(npcosd(pos)))
    fmax = 75.0 * fwhm
    if pos > 90.0:
        fmin, fmax = [fmax, fmin]
...
def getWidthsED(pos, sig, gam):  # noqa: ARG001
    """Compute the peak widths used for computing the range of a peak
...
def getWidthsTOF(pos, alp, bet, sig, gam):  # noqa: ARG001
    """Compute the peak widths used for computing the range of a peak
    ...
    """
    widths = [np.sqrt(sig), gam]
    fwhm = 2.355 * widths[0] + 2.0 * widths[1]
    fmin = 50.0 * fwhm * (1.0 + 1.0 / alp)
    fmax = 50.0 * fwhm * (1.0 + 1.0 / bet)
    return widths, fmin, fmax
...
def getWidthsCWA(pos, alp, bet, sig, gam, shl):
    ...
    fwhm = 2.355 * widths[0] + 2.0 * widths[1]
    if pos < 90.0:
        fmin = 50.0 * (fwhm * (1.0 + 1.0 / alp) + shl * abs(npcosd(pos)))
        fmax = 75.0 * fwhm * (1.0 + 1.0 / bet)
    else:
        fmin = 75.0 * fwhm * (1.0 + 1.0 / alp)
        fmax = 50.0 * (fwhm * (1.0 + 1.0 / bet) + shl * abs(npcosd(pos)))
    return widths, fmin, fmax


def getWidthsCWB(pos, alp, bet, sig, gam):  # noqa: ARG001
    """Compute the peak widths used for computing the range of a peak
...
def getFWHM(pos, Inst, N=1):
    ...
    if "LF" in Inst["Type"][0]:
        ...
        s = sigTOF(
            dsp, Inst["sig-0"][N], Inst["sig-1"][N], Inst["sig-2"][N], Inst["sig-q"][N]
        )
        g = gamTOF(dsp, Inst["X"][N], Inst["Y"][N], Inst["Z"][N])
        return getgamFW(g, s) + np.log(2.0) * (alp + bet) / (alp * bet)
    elif "C" in Inst["Type"][0]:
        s = sig(pos / 2.0, Inst["U"][N], Inst["V"][N], Inst["W"][N])
        g = gam(pos / 2.0, Inst["X"][N], Inst["Y"][N], Inst["Z"][N])
        return getgamFW(g, s) / 100.0  # returns FWHM in deg
        ...
        s = sig(pos / 2.0, Inst["U"][N], Inst["V"][N], Inst["W"][N])
        g = gam(pos / 2.0, Inst["X"][N], Inst["Y"][N], Inst["Z"][N])
        return getgamFW(g, s) / 100.0 + np.log(2.0) * (alp + bet) / (
            alp * bet
        )  # returns FWHM in deg
...
def getBackground(pfx, parmDict, bakType, dataType, xdata, fixback=None):
    ...
    """
    if "T" in dataType:
        q = 2.0 * np.pi * parmDict[pfx + "difC"] / xdata
    elif "E" in dataType:
        const = 4.0 * np.pi * npsind(parmDict[pfx + "2-theta"] / 2.0)
        q = const * xdata
    else:
    ...
    if bakType in ["chebyschev", "cosine", "chebyschev-1"]:
        ...
        for iBak in range(nBak):
            ...
            if bakType == "chebyschev":
                ybi = parmDict[key] * (-1.0 + 2.0 * (xdata - xdata[0]) / dt) ** iBak
            elif bakType == "chebyschev-1":
                xpos = -1.0 + 2.0 * (xdata - xdata[0]) / dt
                ybi = parmDict[key] * np.cos(iBak * np.arccos(xpos))
            elif bakType == "cosine":
                ybi = parmDict[key] * npcosd(180.0 * xdata * iBak / xdata[-1])
            yb += ybi
        sumBk[0] = np.sum(yb)
        ...
        if nBak == 1:
            ...
            if bakType == "lin interpolate":
                ...
                bakPos = 1.0 / np.linspace(1.0 / xmax, 1.0 / xmin, nBak, True)
    ...
    while True:
        try:
            ...
            if "C" in dataType:
                ...
                alp = max(
                    0.1, parmDict[pfx + "alpha-0"] + parmDict[pfx + "alpha-1"] * sinPos
                )
                bet = max(
                    0.001, parmDict[pfx + "beta-0"] + parmDict[pfx + "beta-1"] * sinPos
                )
                ...
                Wd, fmin, fmax = getWidthsTOF(pkP, 1.0, 1.0, pkS, pkG)
            ...
            if "C" in dataType:
                ...
                ybi = (
                    pkI
                    * getEpsVoigt(
                        pkP, alp, bet, pkS / 1.0e4, pkG / 100.0, xdata[iBeg:iFin]
                    )[0]
                    / 100.0
                )
            elif "A" in dataType:
                ybi = (
                    pkI
                    * getExpFCJVoigt3(pkP, alp, bet, pkS, pkG, shl, xdata[iBeg:iFin])[0]
                )
            elif "T" in dataType:
                ybi = pkI * getEpsVoigt(pkP, 1.0, 1.0, pkS, pkG, xdata[iBeg:iFin])[0]
            elif "E" in dataType:
                ybi = (
                    pkI
                    * getPsVoigt(pkP, pkS * 10.0**4, pkG * 100.0, xdata[iBeg:iFin])[0]
                )
            else:
                msg = f"dataType of {dataType} should not happen!"
                raise Exception(msg)
            yb[iBeg:iFin] += ybi
            sumBk[2] += np.sum(ybi)
            iD += 1
...
def getBackgroundDerv(hfx, parmDict, bakType, dataType, xdata, fixback=None):
    ...
    """
    if "T" in dataType:
        q = 2.0 * np.pi * parmDict[hfx + "difC"] / xdata
    elif "E" in dataType:
        const = 4.0 * np.pi * npsind(parmDict[hfx + "2-theta"] / 2.0)
        q = const * xdata
    else:
        wave = parmDict.get(hfx + "Lam", parmDict.get(hfx + "Lam1", 1.0))
        q = 2.0 * np.pi * npsind(xdata / 2.0) / wave
    nBak = 0
    while True:
    ...
    if bakType in ["chebyschev", "cosine", "chebyschev-1"]:
        ...
        for iBak in range(nBak):
            if bakType == "chebyschev":
                dydb[iBak] = (-1.0 + 2.0 * (xdata - xdata[0]) / dt) ** iBak
            elif bakType == "chebyschev-1":
                xpos = -1.0 + 2.0 * (xdata - xdata[0]) / dt
                dydb[iBak] = np.cos(iBak * np.arccos(xpos))
            elif bakType == "cosine":
                dydb[iBak] = npcosd(180.0 * xdata * iBak / xdata[-1])
    elif bakType in ["Q^2 power series", "Q^-2 power series"]:
        QT = 1.0
        ...
        if nBak == 1:
            ...
            for i, pos in enumerate(bakPos):
                if i == 0:
                    dydb[0] = np.where(
                        xdata < bakPos[1],
                        (bakPos[1] - xdata) / (bakPos[1] - bakPos[0]),
                        0.0,
                    )
                elif i == len(bakPos) - 1:
                    dydb[i] = np.where(
                        xdata > bakPos[-2],
                        (bakPos[-1] - xdata) / (bakPos[-1] - bakPos[-2]),
                        0.0,
                    )
                else:
                    dydb[i] = np.where(
                        ...
                        np.where(
                            xdata < bakPos[i + 1],
                            (bakPos[i + 1] - xdata) / (bakPos[i + 1] - pos),
                            0.0,
                        ),
                        np.where(
                            xdata > bakPos[i - 1],
                            (xdata - bakPos[i - 1]) / (pos - bakPos[i - 1]),
                            0.0,
                        ),
                    )
    if hfx + "difC" in parmDict:
        ...
        FF = G2elem.GetFormFactorCoeff("Si")[0]
        ff = (
            np.array(G2elem.ScatFac(FF, SQ)[0]) * np.pi**2
        )  # needs pi^2~10. for cw data (why?)
    iD = 0
    while True:
        try:
            if hfx + "difC" in parmDict:
    ...
    while True:
        try:
            ...
            if "C" in dataType:
                ...
                sinPos = npsind(pkP / 2.0)
                alp = max(
                    0.1, parmDict[hfx + "alpha-0"] + parmDict[hfx + "alpha-1"] * sinPos
                )
                bet = max(
                    0.001, parmDict[hfx + "beta-0"] + parmDict[hfx + "beta-1"] * sinPos
                )
                Wd, fmin, fmax = getWidthsCWB(pkP, alp, bet, pkS, pkG)
            elif "A in dataType":
                shl = parmDict[hfx + "SH/L"]
            ...
            if not iBeg:
                ...
                iFin = np.searchsorted(xdata, pkP + fmax)
            if "C" in dataType:
                Df, dFdp, dFds, dFdg, x = getdFCJVoigt3(
                    pkP, pkS, pkG, shl, xdata[iBeg:iFin]
                )
            elif "B" in dataType:
                Df, dFdp, x, x, dFds, dFdg = getdEpsVoigt(  # noqa: PLW0128
                    pkP, alp, bet, pkS / 1.0e4, pkG / 100.0, xdata[iBeg:iFin]
                )
                dFdp /= 100.0
                dFds /= 1.0e6
                ...
                Df, dFdp, x, x, dFds, dFdg, x = getdExpFCJVoigt3(  # noqa: PLW0128
                    pkP, alp, bet, pkS, pkG, shl, xdata[iBeg:iFin]
                )
            elif "E" in dataType:
                Df, dFdp, dFds, dFdg = getdPsVoigt(
                    pkP, pkS * 10.0**4, pkG * 100.0, xdata[iBeg:iFin]
                )
            else:  #'T'OF
                Df, dFdp, x, x, dFds, dFdg = getdEpsVoigt(  # noqa: PLW0128
                    pkP, 1.0, 1.0, pkS, pkG, xdata[iBeg:iFin]
                )
...
def getdFCJVoigt3(pos, sig, gam, shl, xdata):
    ...
    """
    Df, dFdp, dFds, dFdg, dFdsh = pyd.pydpsvfcjo(
        len(xdata), xdata - pos, pos, sig, gam, shl
    )
...
def getdExpFCJVoigt3(pos, alp, bet, sig, gam, shl, xdata):
    ...
    """
    Df, dFdp, dFda, dFdb, dFds, dFdg, dFdsh = pyd.pydpsvfcjexpo(
        len(xdata), xdata - pos, pos, alp, bet, sig, gam, shl
    )
...
def getdEpsVoigt(pos, alp, bet, sig, gam, xdata):
    ...
    Df, dFdp, dFda, dFdb, dFds, dFdg = pyd.pydepsvoigt(
        len(xdata), xdata - pos, alp, bet, sig, gam
    )
...
def ellipseSize(H, Sij, GB):
    ...
    R = (
        np.inner(HX / lenHX, Rsize) ** 2 * Esize
    )  # want column length for hkl in crystal
    lenR = 1.0 / np.sqrt(np.sum(R))
    return lenR
...
def getCrSize(HKL, G, GB, sizeData):
    if sizeData[0] == "isotropic":
        ...
        H = np.array(HKL)
        return 1.0 / np.array([ellipseSize(h, Sij, GB) for h in H.T]) ** 2
    ...
    for h, k, l, d in HKL:  # noqa: E741
        ...
        for dH in SSdH:
            if dH:
                ...
                d = float(1 / np.sqrt(G2lat.calc_rDsq(H, A)))  # noqa: PLW2901
                if d >= dmin:
...
def setPeakInstPrmMode(normal=True):
    ...
    """
    global peakInstPrmMode  # noqa: PLW0603
    ...
    if "LF" in dataType:
        ...
        while True:
            ...
            try:
                ...
                l = parmDict["l" + str(iPeak)]  # noqa: E741
                ...
                fmin = parmDict.get(
                    "fitRange", 8.0
                )  # width for peak computation: defaults to 8 deg.
                fmin = min(
                    0.9 * abs(xdata[-1] - xdata[0]), fmin
                ...
                if (
                    not iBeg - iFin
                ):  # got peak above high limit (peaks sorted, so we can stop)
                    break
                LaueFringePeakCalc(
                    xdata,
                    yc,
        ...
        if "Lam1" in parmDict:
            Ka2 = True
            lamRatio = (
                360 * (parmDict["Lam2"] - parmDict["Lam1"]) / (np.pi * parmDict["Lam1"])
            )
            kRatio = parmDict["I(L2)/I(L1)"]
        iPeak = 0
        while True:
            try:
                ...
                if "C" in dataType:
                    ...
                    fp = (
                        getEpsVoigt(
                            pos, alp, bet, sig / 1.0e4, gam / 100.0, xdata[iBeg:iFin]
                        )[0]
                        / 100.0
                    )
                else:  #'A'
                    fp = getExpFCJVoigt3(
                        pos, alp, bet, sig, gam, shl, xdata[iBeg:iFin]
                    )[0]
                ...
                if Ka2:
                    ...
                    if iBeg - iFin:
                        if "C" in dataType:
                            ...
                            fp2 = getExpFCJVoigt3(
                                pos2, alp, bet, sig, gam, shl, xdata[iBeg:iFin]
                            )[0]
        ...
        while True:
            try:
                ...
                yc[iBeg:iFin] += (
                    intens
                    * getPsVoigt(pos, sig * 10.0**4, gam * 100.0, xdata[iBeg:iFin])[0]
                )
                iPeak += 1
            except KeyError:  # no more peaks to process
                return yb + yc
        ...
        while True:
            try:
                ...
                if not iBeg - iFin:  # peak above high limit
                    return yb + yc
                yc[iBeg:iFin] += (
                    intens * getEpsVoigt(pos, alp, bet, sig, gam, xdata[iBeg:iFin])[0]
                )
                iPeak += 1
            except KeyError:  # no more peaks to process
                return yb + yc
...
def getPeakProfileDerv(dataType, parmDict, xdata, fixback, varyList, bakType):
    ...
    dMdb, dMddb, dMdpk, dMdfb = getBackgroundDerv(
        "", parmDict, bakType, dataType, xdata, fixback
    )
    ...
    if "LF" in dataType:
        for i, name in enumerate(varyList):
            ...
            intArrP = getPeakProfile(
                dataType, deltaParmDict, xdata, fixback, varyList, bakType
            )
            deltaParmDict[name] -= 2 * delta
            intArrM = getPeakProfile(
                dataType, deltaParmDict, xdata, fixback, varyList, bakType
            )
    ...
    if dataType[2] in ["A", "B", "C"]:
        ...
        while True:
            try:
                ...
                if "C" in dataType:
                    ...
                    for i in range(1, 6):
                        dMdpk[i][iBeg:iFin] += intens * dMdipk[i] / 100.0
                    dMdpk[0][iBeg:iFin] += dMdipk[0] / 100.0
                    dervDict = {
                        "int": dMdpk[0],
                        "pos": dMdpk[1],
                        "alp": dMdpk[2],
                ...
                if Ka2:
                    ...
                    if iBeg - iFin:
                        if "C" in dataType:
                            ...
                            for i in range(1, 6):
                                dMdpk2[i][iBeg:iFin] += (
                                    intens * kRatio * dMdipk2[i] / 100.0
                                )
                            dMdpk[0][iBeg:iFin] += kRatio * dMdipk2[0] / 100.0
                            dervDict = {
                                "int": dMdpk[0],
                                "pos": dMdpk[1],
                                "alp": dMdpk[2],
                                ...
                                "sig": dMdpk[4] / 1.0e4,
                                "gam": dMdpk[5] / 100.0,
                                "L1/L2": dMdpk[6] * intens,
                            }
                        else:  #'A'
                            dMdpk2 = np.zeros(shape=(8, len(xdata)))
                            dMdipk2 = getdExpFCJVoigt3(
                                pos, alp, bet, sig, gam, shl, xdata[iBeg:iFin]
                            )
                            for i in range(1, 7):
                                dMdpk2[i][iBeg:iFin] += intens * kRatio * dMdipk2[i]
        ...
        while True:
            try:
                ...
                dMdipk = getdPsVoigt(pos, sig * 10.0**4, gam * 100.0, xdata[iBeg:iFin])
                dMdpk[0][iBeg:iFin] += dMdipk[0]
                for i in range(1, 4):
                    dMdpk[i][iBeg:iFin] += intens * dMdipk[i]
                dervDict = {
...
def SetBackgroundParms(Background):
    ...
    for i in range(Debye["nDebye"]):
        ...
        debyeDict.update(
            dict(zip(debyeNames, Debye["debyeTerms"][i][::2], strict=False))
        )
    ...
    for i in range(Debye["nPeaks"]):
        ...
        peaksDict.update(
            dict(zip(peaksNames, Debye["peaksList"][i][::2], strict=False))
        )
...
def autoBkgCalc(bkgdict, ydata):
    ...
    try:
        ...
        raise Exception("import of pybaselines failed. autobackground requires this")  # noqa: B904
...
def DoCalibInst(IndexPeaks, Inst, Sample):
    ...
    def GetInstParms(parmDict, varyList):  # noqa: ARG001
        for name in Inst:
    ...
    def errPeakPos(values, peakDsp, peakPos, peakWt, dataType, parmDict, varyList):
        ...
        if "C" in dataType or "B" in dataType:
            const = 18.0e-2 / (np.pi * parmDict["radius"])
            shft = (
                -const
                * (
                    parmDict["DisplaceX"] * npcosd(calcPos)
                    + parmDict["DisplaceY"] * npsind(calcPos)
                )
                + parmDict["Zero"]
            )
    ...
    for peak, sig in zip(IndexPeaks[0], IndexPeaks[1], strict=False):
        if peak[2] and peak[3] and sig > 0.0:
            peakPos.append(peak[0])
            peakDsp.append(peak[-1])  # d-calc
            peakWt.append(1.0 / (sig * peak[-1]))
    ...
    while True:
        ...
        G2fil.G2Print(f"calib time = {runtime:8.3f}s, {runtime / ncyc:8.3f}s/cycle")
        G2fil.G2Print(f"chi**2 = {chisq:12.6g}, reduced chi**2 = {GOF:6.2f}")
        try:
...
def DoPeakFit(
    ...
    def BackgroundPrint(Background, sigDict):
        ...
        if "BF mult" in sigDict:
            print(
                "Background file mult: %.3f(%d)"
                % (Background[1]["background PWDR"][1], int(1000 * sigDict["BF mult"]))
            )
    ...
    def GetPeaksParms(Inst, parmDict, Peaks, varyList):
        ...
        for i, peak in enumerate(Peaks):
            ...
            for j in range(len(names)):
                parName = names[j] + str(i)
                if peak[2 * j + off + 1] or not peakInstPrmMode:
                    continue
                if "alp" in parName:
                    if "T" in Inst["Type"][0] and len(Pdabc):
    ...
    def PeaksPrint(dataType, parmDict, sigDict, varyList, ptsperFW):
        if "clat" in varyList:
            print(f"c = {parmDict['clat']:.6f} esd {sigDict['clat']:.6f}")
        ...
        for i, peak in enumerate(Peaks):
            ...
            for j in range(len(names)):
                ...
                if parName in varyList:
                    ...
                    ptstr += 10 * " "
            ptstr += f"{ptsperFW[i]:8.1f}"
            print(f"{('Peak' + str(i + 1)).center(8)}", ptstr)
    ...
    def devPeakProfile(
        ...
        """
        parmdict.update(zip(varylist, values, strict=False))
        return np.sqrt(weights) * getPeakProfileDerv(
            dataType, parmdict, xdata, fixback, varylist, bakType
        )
    ...
    def errPeakProfile(
        ...
        parmdict.update(zip(varylist, values, strict=False))
        M = np.sqrt(weights) * (
            getPeakProfile(dataType, parmdict, xdata, fixback, varylist, bakType)
            - ydata
        )
        Rwp = min(100.0, np.sqrt(np.sum(M**2) / np.sum(weights * ydata**2)) * 100.0)
        if dlg:
            dlg.Raise()
            GoOn = dlg.Update(int(Rwp), newmsg=f"Peak fit Rwp ={Rwp:8.3f}%")[0]
            if not GoOn:
                return -M  # abort!!
        return M
    ...
    yc.fill(0.0)  # set calcd ones to zero
    yb.fill(0.0)
    yd.fill(0.0)
    ...
    if not peakInstPrmMode:
        ...
        if remVary:
            print(
                "Instrumental profile terms cannot be varied "
    ...
    if "LF" in Inst["Type"][0]:
        ...
        if warn:
            print(
                "Instrumental profile terms cannot be varied " + "in Laue Fringe fits:",
                warn,
            )
    ...
    while not noFit:
        ...
        Values2Dict(parmDict, varyList, result[0])
        Rvals["Rwp"] = (
            np.sqrt(chisq / np.sum(wtFactor * w[xBeg:xFin] * y[xBeg:xFin] ** 2)) * 100.0
        )  # to %
    ...
    yb[xBeg:xFin] = getBackground(
        "", parmDict, bakType, dataType, x[xBeg:xFin], fixback[xBeg:xFin]
    )[0]
    yc[xBeg:xFin] = getPeakProfile(
        dataType, parmDict, x[xBeg:xFin], fixback[xBeg:xFin], varyList, bakType
    )
    ...
    if binsperFWHM:
        if min(binsperFWHM) < 1.0:
            ...
            G2fil.G2Print(
                " Manually make individually refined sigma/gamma terms positive"
            )
            if "T" in Inst["Type"][0]:
                G2fil.G2Print(
...
def calcIncident(Iparm, xdata):
    ...
    def IfunAdv(Iparm, xdata):
        ...
        x = xdata / 1000.0  # expressions are in ms
...
def MakeInst(PWDdata, Name, Size, Mustrain, useSamBrd):
    ...
    if "T" in inst["Type"][1]:
        ...
        fname = Name + ".inst"
        fl = open(fname, "w")  # noqa: SIM115
        fl.write("1\n")
        fl.write(f"{int(inst[prms[0]][1])}\n")
        fl.write(
            f"{inst[prms[1]][1]:19.11f}{inst[prms[2]][1]:19.11f}{inst[prms[3]][1]:19.11f}{inst[prms[4]][1]:19.11f}{inst[prms[5]][1]:19.11f}\n"
        )
        fl.write(
            f"{inst[prms[6]][1]:12.6e}{inst[prms[7]][1]:14.6e}{inst[prms[8]][1]:14.6e}{inst[prms[9]][1]:14.6e}\n"
        )
        fl.write(
            f"{inst[prms[10]][1]:12.6e}{inst[prms[11]][1]:14.6e}{inst[prms[12]][1]:14.6e}{inst[prms[13]][1]:14.6e}\n"
        )
        fl.write(
            f"{inst[prms[14]][1]:12.6e}{inst[prms[15]][1] + Ysb:14.6e}{inst[prms[16]][1] + Xsb:14.6e}{0.0:14.6e}{0.0:14.6e}\n"
        )
        fl.write(f"{sample['Absorption'][0]:12.6e}\n\n\n")
        fl.close()
    else:
        if useSamBrd[0]:
            wave = G2mth.getWave(inst)
        ...
        prms = ["Bank", "Lam", "Zero", "Polariz.", "U", "V", "W", "X", "Y"]
        fname = Name + ".inst"
        fl = open(fname, "w")  # noqa: SIM115
        fl.write("1\n")
        fl.write(f"{int(inst[prms[0]][1])}\n")
        fl.write(
            "%10.5f%10.5f%10.4f%10d\n"
            % (inst[prms[1]][1], 100.0 * inst[prms[2]][1], inst[prms[3]][1], 0)
        )
        fl.write(
            f"{inst[prms[4]][1]:10.3f}{inst[prms[5]][1]:10.3f}{inst[prms[6]][1]:10.3f}\n"
        )
        fl.write(
            f"{inst[prms[7]][1] + Xsb:10.3f}{inst[prms[8]][1] + Ysb:10.3f}{0.0:10.3f}\n"
        )
        fl.write(f"{0.0:10.3f}{0.0:10.3f}{0.0:10.3f}\n")
        fl.write(f"{sample['Absorption'][0]:12.6e}\n\n\n")
        fl.close()
    return fname
...
def MakeBack(PWDdata, Name):
    ...
    for val in BackVals:
        if "T" in inst["Type"][1]:
            fl.write(f"{float(val):12.6g}\n")
        else:
            fl.write(f"{val:12.6g}\n")
    fl.close()
    return fname
...
def findDup(Atoms):
    ...
    for iat1, at1 in enumerate(Atoms):
        ...
        for _iat2, at2 in enumerate(Atoms[(iat1 + 1) :]):
...
def MakeRMC6f(PWDdata, Name, Phase, RMCPdict):
    ...
    ifSfracs = any(np.any(sfracs - 1.0) for sfracs in Sfracs)
    Sample = PWDdata["Sample Parameters"]
    ...
    newPhase, Atcodes = G2lat.TransformPhase(
        Phase, newPhase, Trans, np.zeros(3), np.zeros(3), ifMag=False, Force=True
    )
    Natm = np.core.defchararray.count(
        np.array(Atcodes), "+"
    )  # no. atoms in original unit cell
    ...
    if ifSfracs:
        ...
        for i in range(len(Atoms) // Natm):
            ...
            Satoms.append(
                G2mth.sortArray(
                    G2mth.sortArray(G2mth.sortArray(Atoms[ind : ind + Natm], 5), 4), 3
                )
            )
        Natoms = []
    ...
    for item in Meta:
        fl.write(f"{'Metadata ' + item + ':':20}{Meta[item]}\n")
    fl.write(f"Atom types present:                 {'    '.join(Atseq)}\n")
    fl.write(f"Number of each atom type:       {''.join(NAstr)}\n")
    fl.write(f"Number of atoms:                {len(Natoms)}\n")
    fl.write(
        "%-35s%4d%4d%4d\n"
        % ("Supercell dimensions:", Supercell[0], Supercell[1], Supercell[2])
    )
    fl.write(
        f"Cell (Ang/deg): {Cell[0]:12.6f}{Cell[1]:12.6f}{Cell[2]:12.6f}{Cell[3]:12.6f}{Cell[4]:12.6f}{Cell[5]:12.6f}\n"
    )
    A, B = G2lat.cell2AB(Cell, True)
    fl.write("Lattice vectors (Ang):\n")
    for i in [0, 1, 2]:
        fl.write(f"{A[i, 0]:12.6f}{A[i, 1]:12.6f}{A[i, 2]:12.6f}\n")
    ...
    for atm in Atseq:
        for iat, atom in enumerate(Natoms):
            if atom[1] == atm:
                ...
                fl.write(
                    "%6d%4s  [%s]%19.15f%19.15f%19.15f%6d%4d%4d%4d\n"
                    % (
                        nat,
                        atom[1].strip(),
                        atcode[0],
...
def MakeBragg(PWDdata, Name, Phase):
    ...
    if "T" in Inst["Type"][0]:
        ...
        for i in range(Ibeg, Ifin - 1):
            fl.write(f"{Data[0][i] / 1000.0:12.8f}{Data[1][i]:12.6f}\n")
    else:
        fl.write("%12s%12s\n" % ("   2-theta, deg", "  I(obs)"))
        for i in range(Ibeg, Ifin - 1):
            fl.write(f"{Data[0][i]:11.6f}{Data[1][i]:15.2f}\n")
    fl.close()
    return fname


def MakeRMCPdat(PWDdata, Name, Phase, RMCPdict):  # noqa: ARG001
    Meta = RMCPdict["metadata"]
    ...
    if "T" in inst["Type"][1]:
        ...
        Xfl = open(Name + ".xray", "w")  # noqa: SIM115
        for atm in Atseq:
            fa = XFF[atm]["fa"]
            fb = XFF[atm]["fb"]
            fc = XFF[atm]["fc"]
            Xfl.write(
                "%2s  %8.4f%8.4f%8.4f%8.4f%8.4f%8.4f%8.4f%8.4f%8.4f\n"
                % (
                    atm.upper(),
                    fa[0],
                    fb[0],
    ...
    for pair in [
        ...
        Pairs += pair
    for pair in Pairs:
        pair = pair.replace(" ", "")  # noqa: PLW2901
        at1, at2 = pair.split("-")
        if at1 == "Va" or at2 == "Va":
            ncoef = 0.0
        else:
    ...
    pairMin = [atPairs.get(pair, [0.0, 0.0, 0.0]) for pair in Pairs]
    maxMoves = [Atypes.get(atm, 0.0) for atm in Atseq]
    fname = Name + ".dat"
    fl = open(fname, "w")  # noqa: SIM115
    ...
    minHD = " ".join([f"{dist[0]:6.3f}" for dist in pairMin])
    minD = " ".join([f"{dist[1]:6.3f}" for dist in pairMin])
    maxD = " ".join([f"{dist[2]:6.3f}" for dist in pairMin])
    fl.write(f"MINIMUM_DISTANCES ::   {minHD}  Angstrom\n")
    maxMv = " ".join([f"{mov:6.3f}" for mov in maxMoves])
    fl.write(f"MAXIMUM_MOVES ::   {maxMv} Angstrom\n")
    fl.write("R_SPACING ::  0.0200 Angstrom\n")
    fl.write("PRINT_PERIOD :: 100\n")
    fl.write(f"TIME_LIMIT ::     {Times[0]:.2f} MINUTES\n")
    fl.write(f"SAVE_PERIOD ::    {Times[1]:.2f} MINUTES\n")
    fl.write("\n")
    ...
    if "T" in inst["Type"][1]:
        fl.write(
            "NEUTRON_COEFFICIENTS :: "
            + "".join([f"{coeff:9.5f}" for coeff in Ncoeff])
            + "\n"
        )
    fl.write("DISTANCE_WINDOW ::\n")
    fl.write(f"  > MNDIST :: {minD}\n")
    fl.write(f"  > MXDIST :: {maxD}\n")
    if len(RMCPdict["Potentials"]["Stretch"]) or len(RMCPdict["Potentials"]["Stretch"]):
        fl.write("\n")
        fl.write("POTENTIALS ::\n")
        fl.write(f"  > TEMPERATURE :: {RMCPdict['Potentials']['Pot. Temp.']:.1f} K\n")
        ...
        if len(RMCPdict["Potentials"]["Stretch"]):
            ...
            for bond in RMCPdict["Potentials"]["Stretch"]:
                fl.write(
                    f"  > STRETCH :: {bond[0]} {bond[1]} {bond[3]:.2f} eV {bond[2]:.2f} Ang\n"
                )
        if len(RMCPdict["Potentials"]["Angles"]):
            fl.write(
                f"  > ANGLE_SEARCH :: {RMCPdict['Potentials']['Angle search']:.1f}%\n"
            )
            for angle in RMCPdict["Potentials"]["Angles"]:
                fl.write(
                    f"  > ANGLE :: {angle[1]} {angle[0]} {angle[2]} {angle[6]:.2f} eV {angle[3]:.2f} deg {angle[4]:.2f} {angle[5]:.2f} Ang\n"
                )
    if RMCPdict["useBVS"]:
        fl.write("BVS ::\n")
    ...
    if len(RMCPdict["FxCN"]):
        fl.write(f"FIXED_COORDINATION_CONSTRAINTS ::  {len(RMCPdict['FxCN'])}\n")
    ...
    if len(RMCPdict["AveCN"]):
        fl.write(f"AVERAGE_COORDINATION_CONSTRAINTS ::  {len(RMCPdict['AveCN'])}\n")
        for iav, avcn in enumerate(RMCPdict["AveCN"]):
            try:
                ...
                break
            fl.write(
                "  > CAVSTR%d ::   %d %d %.2f %.2f %.2f %.6f\n"
                % (iav + 1, at1 + 1, at2 + 1, avcn[2], avcn[3], avcn[4], avcn[5])
            )
    for File in Files:
        if Files[File][0] and Files[File][0] != "Select":
            if "Xray" in File and "F(Q)" in File:
                fqdata = open(Files[File][0])  # noqa: SIM115
                lines = int(fqdata.readline()[:-1])
                fqdata.close()
            fl.write("\n")
            fl.write(f"{File.split(';')[0].upper().replace(' ', '_')} ::\n")
            fl.write(f"  > FILENAME :: {Files[File][0]}\n")
            fl.write(f"  > DATA_TYPE :: {Files[File][2]}\n")
            fl.write(f"  > FIT_TYPE :: {Files[File][2]}\n")
            if "Xray" not in File:
                fl.write("  > START_POINT :: 1\n")
                fl.write("  > END_POINT :: 3000\n")
                fl.write(f"  > WEIGHT :: {Files[File][1]:.4f}\n")
            ...
            if Files[File][3] != "RMC":
                fl.write(f"  > {Files[File][3]}\n")
            if "reciprocal" in File:
                fl.write("  > CONVOLVE ::\n")
                if "Xray" in File:
                    fl.write("  > RECIPROCAL_SPACE_FIT :: 1 %d 1\n" % lines)
                    fl.write(
                        "  > RECIPROCAL_SPACE_PARAMETERS :: 1 %d %.4f\n"
                        % (lines, Files[File][1])
                    )
                    fl.write("  > REAL_SPACE_FIT :: 1 %d 1\n" % (3 * lines // 2))
                    fl.write(
                        "  > REAL_SPACE_PARAMETERS :: 1 %d %.4f\n"
                        % (3 * lines // 2, 1.0 / Files[File][1])
                    )
    fl.write("\n")
    fl.write("BRAGG ::\n")
    fl.write(f"  > BRAGG_SHAPE :: {gsasType}\n")
    fl.write("  > RECALCUATE\n")
    fl.write(f"  > DMIN :: {dMin - 0.02:.2f}\n")
    fl.write(f"  > WEIGHT :: {BraggWt:10.3f}\n")
    if "T" in inst["Type"][1]:
        fl.write(
            "  > SCATTERING LENGTH :: "
            + "".join([f"{blen:8.4f}" for blen in Nblen])
            + "\n"
        )
    fl.write("\n")
    fl.write("END  ::\n")
...
def findrmcprofile():
    ...
    if rmcprofile_exe is not None:
        if is_exe(rmcprofile_exe):
            ...
            print(
                f"Config var rmcprofile_exec defined as {rmcprofile_exe!r} but file not found or not exe."
            )
...
def fullrmcDownload():
    ...
    print(
        "Starting installation of fullrmc\nDownloading from",
...
def MakePDFfitAtomsFile(Phase, RMCPdict):
    ...
    shape = ""
    if RMCPdict["shape"] == "sphere" and RMCPdict["spdiameter"][0] > 0.0:
        shape = f"   sphere, {RMCPdict['spdiameter'][0]:10.6f}\n"
    elif RMCPdict["stepcut"] > 0.0:
        shape = f"stepcut, {RMCPdict['stepcut']:10.6f}\n"
    if shape:
        fatm.write("shape  " + shape)
    fatm.write(f"spcgr   {RMCPdict['SGData']['SpGrp'].replace(' ', '')}\n")
    cell = General["Cell"][1:7]
    fatm.write(
        f"cell  {cell[0]:10.6f},{cell[1]:10.6f},{cell[2]:10.6f},{cell[3]:10.6f},{cell[4]:10.6f},{cell[5]:10.6f}\n"
    )
    ...
    for atom in Atoms:
        ...
        fatm.write("    " + f"{0.0:18.8f}{0.0:18.8f}{0.0:18.8f}{0.0:13.4f}\n")
        fatm.write(
            "    "
...
"""
    ...
    if "sequential" in RMCPdict["refinement"]:
        ...
        for fil in RMCPdict["files"]:
            ...
            if "Neutron" in fil:
                ...
                Nd += 1
                dType = "Xdata"
            rundata += "pf.read_data(pathWrap(r'{}'), '{}', 30.0, {:.4f})\n".format(
                filNam,
                dType[0],
                RMCPdict[dType]["qdamp"][0],
            )
            rundata += "pf.setdata(%d)\n" % Nd
            rundata += "pf.pdfrange(%d, %6.2f, %6.2f)\n" % (
                Nd,
                RMCPdict[dType]["Fitrange"][0],
                RMCPdict[dType]["Fitrange"][1],
            )
            ...
            if it > 1 and item:
                ...
                for itm in itms[1:]:
                    try:
                        ...
                        print(
                            f" *** ERROR - invalid string in atom constraint {item} ***"
                        )
                        return None
                    if it < 6:
                        if not once:
                            rundata += f'pf.constrain({names[it - 2]},"{item}")\n'
                            once = True
                        ...
                        for i in range(3):
                            rundata += f'pf.constrain({uijs[i]},"{item}")\n'
                            if itnum not in used:
                                parms[itnum] = AtomVar["@%d" % itnum]
    ...
    if "sequential" in RMCPdict["refinement"]:
        ...
        for iprm in parms:  # noqa: PLC0206
    ...
    if "sequential" in RMCPdict["refinement"]:
        ...
        rfile = open("Seq_PDFfit_template.py", "w")  # noqa: SIM115
        rundata += f'pf.save_pdf(1, pathWrap("{fName + ".fgr"}"))\n'
    else:
        fName = General["Name"].replace(" ", "_") + "-PDFfit"
        rfile = open(fName + ".py", "w")  # noqa: SIM115
        Nd = 0
    ...
    rundata += f'pf.save_struct(1, pathWrap("{fName + ".rstr"}"))\n'
    rundata += f'pf.save_res(pathWrap("{fName + ".res"}"))\n'
...
def UpdatePDFfit(Phase, RMCPdict):
    ...
    if RMCPdict["refinement"] == "normal":
        ...
        try:
            rstr = open(fName.replace(" ", "_"))  # noqa: SIM115
        except FileNotFoundError:
    ...
    try:
        res = open(fName.replace(" ", "_"))  # noqa: SIM115
    except FileNotFoundError:
        return [fName, "Not found - PDFfit failed"]
    lines = res.readlines()
    res.close()
    ...
    RMCPdict["Parms"] = dict(
        [[item[0][1:-1], float(item[1])] for item in results]
    )  # {'n':val,...}
    if RMCPdict["refinement"] == "normal":
        fName = General["Name"] + "-PDFfit.py"
        py = open(fName.replace(" ", "_"))  # noqa: SIM115
        pylines = py.readlines()
        py.close()
        py = open(fName.replace(" ", "_"), "w")  # noqa: SIM115
        newpy = []
        for pyline in pylines:
            if "setpar" in pyline:
                parm = pyline.split("(")[1].split(",")[0]
                newpy.append(f"pf.setpar({parm},{RMCPdict['Parms'][parm]:.5f})\n")
            else:
                newpy.append(pyline)
        py.writelines(newpy)
        py.close()
        ...
        results = dict(
            [
                [item[0][:-1], float(item[1])]
                for item in results
                if item[0][:-1] in RMCPdict["AtomVar"]
            ]
        )
        ...
        newParms = dict(
            [[item[0][1:-1], [float(item[1]), float(item[2])]] for item in results]
        )  # {'n':[val,esd],...}
        return newParms, Rwp
...
def MakefullrmcSupercell(Phase, RMCPdict):
    ...
    cellnum = -1
    for a in range(
        int(0.5 - RMCPdict["SuperCell"][0] / 2), int(1 + RMCPdict["SuperCell"][0] / 2)
    ):
        for b in range(
            int(0.5 - RMCPdict["SuperCell"][1] / 2),
            int(1 + RMCPdict["SuperCell"][1] / 2),
        ):
            for c in range(
                int(0.5 - RMCPdict["SuperCell"][2] / 2),
                int(1 + RMCPdict["SuperCell"][2] / 2),
            ):
                cellnum += 1
                for i, atom in enumerate(Phase["Atoms"]):
...
def MakefullrmcRun(pName, Phase, RMCPdict):
    ...
    Files = RMCPdict["files"]
    rundata = ""
    rundata += f"## fullrmc {scrname} file ##\n## OK to edit this by hand ##\n"
...
FRESH_START = {RMCPdict["ReStart"][0]}
dirName = "{projDir}"
prefix = "{projName}"
...
"""
    ...
    if RMCPdict.get("Groups", []):
        ...
        for i in range(3):
            for val in (
                int(0.5 - RMCPdict["SuperCell"][i] / 2),
                int(1 + RMCPdict["SuperCell"][0] / 2),
            ):
                fpos = [0, 0, 0]
                fpos[i] = val
...
"""
    ...
    for File in Files:
        ...
        if not os.path.exists(filDat[0]):
            continue
        sfwt = "neutronCohb"
        if "Xray" in File:
            sfwt = "atomicNumber"
        if "G(r)" in File:
            rundata += f'    GR = np.loadtxt(os.path.join(dirName,"{filDat[0]}")).T\n'
            if filDat[3] == 0:
            ...
            if RMCPdict["addThermalBroadening"]:
                rundata += (
                    "    GofR.set_thermal_corrections({'defaultFactor': 0.001})\n"
                )
                ...
                for atm1 in RMCPdict["aTypes"]:
                    for atm2 in RMCPdict["aTypes"]:
                        rundata += "         ('{}', '{}'): {},\n".format(
                            ...
                            atm2,
                            (
                                RMCPdict["ThermalU"].get(atm1, 0.005)
                                + RMCPdict["ThermalU"].get(atm2, 0.005)
                            )
                            / 2,
                        )
                rundata += "    }\n"
...
"""
        for pair in BondList:  # noqa: PLC0206
            e1, e2 = pair.split("-")
            d1, d2 = BondList[pair]
            if d1 == 0:
                continue
        ...
        rundata += "             ])\n"
    rundata += (
        "    D_CONSTRAINT = DistanceConstraint(defaultLowerDistance={})\n".format(
            RMCPdict["min Contact"]
        )
    )
    if minDists:
        rundata += (
            "    D_CONSTRAINT.set_pairs_definition( {'inter':[" + minDists + "]})\n"
        )
...
"""
    if RMCPdict["Swaps"]:
        ...
        for swap in RMCPdict["Swaps"]:
            rundata += (
                f'SwapA = [[idx] for idx in range(len(aN)) if aN[idx]=="{swap[0]}"]\n'
            )
            rundata += (
...
"""
    rundata += f"steps = {RMCPdict['Steps/cycle']}\n"
    rundata += f"for _ in range({RMCPdict['Cycles']}):\n"
    rundata += "    expected = ENGINE.generated+steps\n"

    rundata += (
        "    ENGINE.run(restartPdb=pdbFile,numberOfSteps=steps, saveFrequency=steps)\n"
    )
...
"""
    rfile = open(scrname, "w")  # noqa: SIM115
    rfile.writelines(rundata)
    rfile.close()
    return scrname
...
def ISO2PDFfit(Phase):
    ...
    Atoms = Phase["Atoms"]
    parentXYZ = Phase["ISODISTORT"][
        "G2parentCoords"
    ]  # starting point for mode displacements
    cx, ct, cs, cia = Phase["General"]["AtomPtrs"]
    ...
    newPhase["General"]["SGData"] = G2spc.SpcGroup("P 1")[
        1
    ]  # this is for filled unit cell
    ...
    RMCPdict["AtomConstr"] = []
    RMCPdict["SGData"] = copy.deepcopy(
        SGData
    )  # this is from the ISODISTORT child; defines PDFfit constraints
    Norms = ISOdict["NormList"]
    ModeMatrix = ISOdict["Mode2VarMatrix"]
    RMCPdict["AtomVar"] = {
        "@%d" % (itm + 21): val for itm, val in enumerate(ISOdict["modeDispl"])
    }
    for iatm, [atom, atcode] in enumerate(zip(Atoms, atCodes, strict=False)):
        ...
        for ip, pname in enumerate([f"{atom[ct - 1]}_d{x}" for x in ["x", "y", "z"]]):
            try:
                conStr = ""
                if Atoms[iatm][cx + ip]:
                    conStr += f"{Atoms[iatm][cx + ip]:.5f}"
                pid = ISOdict["IsoVarList"].index(pname)
                consVec = ModeMatrix[pid]
                for ic, citm in enumerate(
                    consVec
                ):  # NB: this assumes orthorhombic or lower symmetry
                    if opid < 0:
                        citm *= -SGOps[100 - opid % 100 - 1][0][ip][
                            ip
                        ]  # remove centering, if any
                    else:
                        citm *= SGOps[opid % 100 - 1][0][ip][ip]  # noqa: PLW2901
                    if citm > 0.0:
...
def REFDRefine(Profile, ProfDict, Inst, Limits, Substances, data):  # noqa: ARG001
    G2fil.G2Print(
        "fit REFD data by "
        + data["Minimizer"]
        + f" using {data['Resolution'][0]:.2f}% data resolution"
    )
        ...
        def __call__(self, x):
            ...
            while True:
                ...
                xnew = x + np.random.uniform(
                    -self.stepsize * steps, self.stepsize * steps, np.shape(x)
                )
    ...
    def GetModelParms():
        ...
        parmDict["Res"] = data["Resolution"][0] / (
            100.0 * sateln2
        )  # % FWHM-->decimal sig
        ...
        for ilay, layer in enumerate(data["Layers"]):
            ...
            for parm in ["Thick", "Rough", "DenMul", "Mag SLD", "iDenMul"]:
                parmDict[cid + parm] = layer.get(parm, [0.0, False])[0]
                if layer.get(parm, [0.0, False])[1]:
            ...
            if name not in ["vacuum", "unit scatter"]:
                ...
                parmDict[cid + "irho"] = Substances[name].get("XImag density", 0.0)
        return parmDict, varyList, values, bounds

    def SetModelParms():
        line = f" Refined parameters: Histogram scale: {parmDict['Scale']:.4g}"
        if "Scale" in varyList:
            data["Scale"][0] = parmDict["Scale"]
            line += f" esd: {sigDict['Scale']:.4g}"
        G2fil.G2Print(line)
        line = f" Flat background: {parmDict['FltBack']:15.4g}"
        if "FltBack" in varyList:
            data["FltBack"][0] = parmDict["FltBack"]
            line += f" esd: {sigDict['FltBack']:15.3g}"
        G2fil.G2Print(line)
        for ilay, layer in enumerate(data["Layers"]):
            name = layer["Name"]
            ...
            line2 += " Imag %.5g" % (
                Substances[name].get("XImag density", 0.0) * parmDict[cid + "DenMul"]
            )
            for parm in ["Thick", "Rough", "DenMul", "Mag SLD", "iDenMul"]:
                if parm in layer:
                    if parm == "Rough":
                    ...
                    line += f" {parm}: {layer[parm][0]:.3f}"
                    if cid + parm in varyList:
                        line += f" esd: {sigDict[cid + parm]:.3g}"
            G2fil.G2Print(line)
            G2fil.G2Print(line2)
    ...
    def getREFD(Q, Qsig, parmDict):
        ...
        if parmDict["dQ type"] == "None":
            AB = abeles(
                0.5 * Q, depth, rho, irho, sigma[1:]
            )  # Q --> k, offset roughness for abeles
    ...
    if varyList:
        if data["Minimizer"] == "LMLS":
            ...
            G2fil.G2Print(f" Estimated temperature: {T0:.3g}")
            result = so.basinhopping(
                sumREFD,
                values,
                take_step=take_step,
                disp=True,
            ...
            ncalc = result[3]
            covM = []
            G2fil.G2Print(f" MC/SA final temperature: {result[2]:.4g}")
    ...
    Rvals = {}
    Rvals["Rwp"] = (
        np.sqrt(chisq / np.sum(wt[Ibeg:Ifin] * Io[Ibeg:Ifin] ** 2)) * 100.0
    )  # to %
    ...
    try:
        ...
        if np.any(Nans):
            ...
            raise ValueError
        Negs = np.less_equal(newVals, 0.0)
        if np.any(Negs):
            indx = Negs.nonzero()
...
def REFDModelFxn(Profile, Inst, Limits, Substances, data):  # noqa: ARG001
    Q, Io, wt, Ic, Ib, Qsig = Profile[:6]
    Qmin = Limits[1][0]
    Qmax = Limits[1][1]
    iBeg = np.searchsorted(Q, Qmin)
    ...
    for ilay, lay in enumerate(np.array(laySeq, dtype=int)):
        ...
        if name == "unit scatter":
            ...
            irho[ilay] = Substances[name].get("XImag density", 0.0) * layer["DenMul"][0]
        if "Mag SLD" in layer:
            rho[ilay] += layer["Mag SLD"][0]
    if data["dQ type"] == "None":
        AB = abeles(
            0.5 * Q[iBeg:iFin], depth, rho, irho, sigma[1:]
        )  # Q --> k, offset roughness for abeles
    elif "const" in data["dQ type"]:
        res = data["Resolution"][0] / (100.0 * sateln2)
        AB = SmearAbeles(
            0.5 * Q[iBeg:iFin], res * Q[iBeg:iFin], depth, rho, irho, sigma[1:]
        )
    else:  # dQ/Q in data
        AB = SmearAbeles(
            0.5 * Q[iBeg:iFin], Qsig[iBeg:iFin], depth, rho, irho, sigma[1:]
        )
...
def abeles(kz, depth, rho, irho=0, sigma=0):
    ...
    def calc(kz, depth, rho, irho, sigma):
        ...
        for i in range(len(depth) - 1):
            k_next = np.sqrt(
                kz_sq - 4e-6 * np.pi * (rho[:, i + 1] + 1j * irho[:, i + 1])
            )
            F = (k - k_next) / (k + k_next)
    ...
    sigma = (
        sigma * np.ones(m - 1, "d") if np.isscalar(sigma) else np.asarray(sigma, "d")
    )
...
def SmearAbeles(kz, dq, depth, rho, irho=0, sigma=0):
    ...
    y += 0.24935 * (
        abeles(kz - 5 * s / 3.0, depth, rho, irho, sigma)
        + abeles(kz + 5 * s / 3.0, depth, rho, irho, sigma)
    )
    y += 0.4111 * (
        abeles(kz - 4 * s / 3.0, depth, rho, irho, sigma)
        + abeles(kz + 4 * s / 3.0, depth, rho, irho, sigma)
    )
    y += 0.60653 * (
    ...
    y += 0.80074 * (
        abeles(kz - 2 * s / 3.0, depth, rho, irho, sigma)
        + abeles(kz - 2 * s / 3.0, depth, rho, irho, sigma)
    )
    y += 0.94596 * (
        abeles(kz - s / 3.0, depth, rho, irho, sigma)
        + abeles(kz - s / 3.0, depth, rho, irho, sigma)
    )
    y *= 0.137023
    return y
...
def StackSim(
    ...
    sf = open("data.sfc", "w")  # noqa: SIM115
    ...
    for atType in atTypes:
        ...
        for i in range(4):
            text += f"{Adat['fa'][i]:11.6f}{Adat['fb'][i]:11.6f}"
        text += f"{Adat['fc']:11.6f}{blen:11.6f}"
        text += "%3d\n" % (Adat["Z"])
        sf.write(text)
    sf.close()
    # make DIFFaX control.dif file - future use GUI to set some of these flags
    cf = open("control.dif", "w")  # noqa: SIM115
    if ctrls in ("0\n0\n3\n", "0\n1\n3\n"):
        x0 = profile[0]
        ...
        cf.write(f"{x0[iBeg]:.6f} {x0[iFin]:.6f} {Dx:.6f}\n1\n1\nend\n")
    else:
        cf.write("GSASII-DIFFaX.dat\n" + ctrls)
        inst = {
            "Type": [
    ...
    df = open("GSASII-DIFFaX.dat", "w")  # noqa: SIM115, PD901
    ...
    if "X" in inst["Type"][0]:
        ...
        df.write("NEUTRON\n")
    if ctrls in ("0\n0\n3\n", "0\n1\n3\n"):
        df.write(f"{G2mth.getMeanWave(inst):.4f}\n")
        U = ateln2 * inst["U"][1] / 10000.0
        V = ateln2 * inst["V"][1] / 10000.0
        W = ateln2 * inst["W"][1] / 10000.0
        HWHM = (
            U * nptand(x0[iBeg:iFin] / 2.0) ** 2 + V * nptand(x0[iBeg:iFin] / 2.0) + W
        )
        HW = np.sqrt(np.mean(HWHM))
        #    df.write('PSEUDO-VOIGT 0.015 -0.0036 0.009 0.605 TRIM\n')
        if "Mean" in Layers["selInst"]:
            df.write(
                f"GAUSSIAN {HW:.6f} TRIM\n"
            )  # fast option - might not really matter
        elif "Gaussian" in Layers["selInst"]:
            df.write(
                f"GAUSSIAN {U:.6f} {V:.6f} {W:.6f} TRIM\n"
    ...
    gam = Layers["Cell"][6]
    df.write(f"{a:.4f} {b:.4f} {c:.4f} {gam:.3f}\n")
    laue = Layers["Laue"]
    if laue == "2/m(ab)":
        laue = "2/m(1)"
    elif laue == "2/m(c)":
    ...
    if "unknown" in Layers["Laue"]:
        df.write(f"{laue} {Layers['Toler']:.3f}\n")
    else:
        df.write(f"{laue}\n")
    df.write(f"{len(Layers['Layers'])}\n")
    ...
    for il, layer in enumerate(Layers["Layers"]):
        ...
        for ia, atom in enumerate(layer["Atoms"]):
            ...
            if "-1" in layer["Symm"] and [x, y, z] == [0.0, 0.0, 0.0]:
                frac /= 2.0
            df.write(
                "%4s %3d %.5f %.5f %.5f %.4f %.2f\n"
                % (atype.ljust(6), ia, x, y, z, 78.9568 * Uiso, frac)
            )
    df.write("STACKING\n")
    df.write(f"{Layers['Stacking'][0]}\n")
    if "recursive" in Layers["Stacking"][0]:
        df.write(f"{Layers['Stacking'][1]}\n")
        ...
        while True:
            ...
            if iF >= Slen:
                break
            iF = min(iF, Slen)
            df.write(f"{Layers['Stacking'][2][iB:iF]}\n")
            iB = iF
    else:
        df.write(f"{Layers['Stacking'][1]}\n")
    ...
    for iY in range(len(Layers["Layers"])):
        ...
        for iX in range(len(Layers["Layers"])):
            ...
            p = round(p, 3)
            df.write(f"{p:.3f} {dx:.5f} {dy:.5f} {dz:.5f}\n")
            sumPx += p
        if sumPx != 1.0:  # this has to be picky since DIFFaX is.
            G2fil.G2Print(
                f"ERROR - Layer probabilities sum to {sumPx:.3f} DIFFaX will insist it = 1.0"
            )
            df.close()
            os.remove("data.sfc")
    ...
    G2fil.G2Print(f" DIFFaX time = {time.time() - time0:.2f}s")
    if os.path.exists("GSASII-DIFFaX.spc"):
        ...
        profile[4][iBeg:iFin] = getBackground(
            "", backDict, bakType, inst["Type"][0], profile[0][iBeg:iFin]
        )[0]
        ...
        if not np.any(profile[1]):  # fill dummy data x,y,w,yc,yb,yd
            ...
            profile[1][iBeg:iFin] = (
                profile[3][iBeg:iFin]
                + np.abs(profile[1][iBeg:iFin] - profile[3][iBeg:iFin]) * Z
            )
            profile[2][iBeg:iFin] = np.where(
                profile[1][iBeg:iFin] > 0.0, 1.0 / profile[1][iBeg:iFin], 1.0
            )
...
def SetCellAtoms(Layers):
    ...
    for il, layer in enumerate(Layers["Layers"]):
        ...
        for ia, atom in enumerate(layer["Atoms"]):
            ...
            AtomXOU.append(
                [float(Nuniq), float(ia + 1), float(Ta), x, y, z, frac, Uiso * 78.9568]
            )
...
def CalcStackingPWDR(Layers, scale, background, limits, inst, profile, debug):
    ...
    G2fil.G2Print(f" GETSPC time = {time.time() - time0:.2f}s")
    time0 = time.time()
    U = ateln2 * inst["U"][1] / 10000.0
    V = ateln2 * inst["V"][1] / 10000.0
    W = ateln2 * inst["W"][1] / 10000.0
    HWHM = U * nptand(x0[iBeg:iFin] / 2.0) ** 2 + V * nptand(x0[iBeg:iFin] / 2.0) + W
    HW = np.sqrt(np.mean(HWHM))
    ...
    profile[5][iBeg:iFin] = profile[1][iBeg:iFin] - profile[3][iBeg:iFin]
    G2fil.G2Print(f" Broadening time = {time.time() - time0:.2f}s")
...
def CalcStackingSADP(Layers, debug):
    ...
    G2fil.G2Print(f" GETSAD time = {time.time() - time0:.2f}s")
...
def makePRFfile(data, MEMtype):
    ...
    prfName = pName + ".prf"
    prf = open(prfName, "w")  # noqa: SIM115
    ...
    prf.write(f"{DysData['Lagrange'][2]:.3f}\n")
    prf.write(f"{DysData['E_factor']:.2f}\n")
    prf.write("1\n")
    prf.write("0\n")
    prf.write("%d\n" % DysData["Ncyc"])
    prf.write("1\n")
...
def makeMEMfile(data, reflData, MEMtype, DYSNOMIA):  # noqa: ARG001
    ...
    Type = Map["Type"]
    UseList = Map["RefList"]
    mem = open(memName, "w")  # noqa: SIM115
    mem.write(f"{generalData['Name'] + ' from ' + UseList[0]}\n")
    a, b, c, alp, bet, gam = cell
    mem.write(f"{a:10.5f}{b:10.5f}{c:10.5f}{alp:10.5f}{bet:10.5f}{gam:10.5f}\n")
    mem.write(
    ...
    for atm in generalData["NoAtoms"]:
        ...
        if blen < 0.0:
            ...
            sumpos += blen * Nat
    if "X" in Type:
        mem.write(f"{sumZ:10.2f}  0.001\n")
    elif "N" in Type and MEMtype:
        mem.write(f"{sumpos:10.3f}{sumneg:10.3f} 0.001\n")
    else:
        mem.write(f"{sumpos:10.3f} 0.001\n")
    ...
    for ref in reflData:
        if ref[3] < 0:
            continue
        if "T" in Type:
            h, k, l, mult, dsp, pos, sig, gam, Fobs, Fcalc, phase, x, x, x, x, prfo = (  # noqa: E741, PLW0128
                ref[:16]
            )
            ...
            if dsp < dmin:
                continue
            theta = npasind(TOFlam / (2.0 * dsp))
            FWHM *= nptand(theta) / pos
            pos = 2.0 * theta
        else:
            h, k, l, mult, dsp, pos, sig, gam, Fobs, Fcalc, phase, x, prfo = ref[:13]  # noqa: E741
    ...
    while True and nref2:
        if not len(refs2[-1]):
            ...
            break
    mem.write(f"{len(refs2):5}\n")
    for iref2, ref2 in enumerate(refs2):
        mem.write("#%5d\n" % iref2)
        mem.write(f"{len(ref2):5}\n")
        Gsum = 0.0
        Msum = 0
        for ref in ref2:
            Gsum += ref[6] * ref[3]
        ...
        for ref in ref2[1:]:
            h, k, l, m = ref[:4]  # noqa: E741
            mem.write("%5d%5d%5d%5d\n" % (h, k, l, m))
            hkl = "%d %d %d" % (h, k, l)
            refDict.pop(hkl, None)
    if refDict:
        mem.write(f"{len(refDict)}\n")
        for hkl in list(refDict.keys()):
            h, k, l = refDict[hkl][:3]  # noqa: E741
            mem.write("%5d%5d%5d\n" % (h, k, l))
    else:
        mem.write("0\n")
    mem.close()
...
class profileObj(FP.FP_profile):
    def conv_Lauefringe(self):
        ...
        wave = self.param_dicts["conv_global"]["dominant_wavelength"] * 1.0e10  # in A
        pos = np.rad2deg(
            self.param_dicts["conv_global"]["twotheta0"]
        )  # peak position as 2theta in deg
        posQ = (
            np.pi * 4 * np.sin(self.param_dicts["conv_global"]["twotheta0"] / 2) / wave
        )  # peak position as Q
        ttwid = self.twotheta_window_fullwidth_deg
        ncell = self.param_dicts[me]["Ncells"]
        co2 = self.param_dicts[me]["clat"] / 2.0
        ...
        weqdiv[: np.searchsorted(Qs, posQ - np.pi / self.param_dicts[me]["clat"])] = (
            0  # isolate central peak, if needed
        )
    ...
    def conv_Lorentzian(self):
        ...
        eqdiv = (0.5 * gamma / np.pi) / (gamma**2 / 4.0 + ttlist**2)
    ...
    **  If term is <= zero, item is removed from convolution
    ...
    NISTparms = {
        "": {
            ...
            "dominant_wavelength": 1.0e-10 * lam,
            "diffractometer_radius": 1e-3 * diffRadius,  # diffractometer radius in m
            "oversampling": 8,
        },
        "emission": {
            "emiss_wavelengths": 1.0e-10 * np.array([lam]),
            "emiss_intensities": np.array([1.0]),
            "emiss_gauss_widths": 1.0e-10 * 1.0e-3 * np.array([0.001]),
            "emiss_lor_widths": 1.0e-10 * 1.0e-3 * np.array([0.001]),
            "crystallite_size_gauss": 1.0e-9 * 1e6,
            "crystallite_size_lor": 1.0e-9 * 1e6,
        },
        "axial": {
            "axDiv": "full",
            "slit_length_source": 1e-3 * diffRadius * shol * axial_factor,
            "slit_length_target": 1e-3
            * diffRadius
            * shol
            * 1.00001
            * axial_factor,  # != 'slit_length_source'
    ...
    for key in NISTparms:  # set parameters for each convolver  # noqa: PLC0206
        if key:
    ...
    NISTpk.set_optimized_window(
        twotheta_exact_bin_spacing_deg=step,
    ...
    if ncells:
        convList += ["conv_Lauefringe"]
    if sigma2 > 0:
        convList += ["conv_Gaussian"]
    if gamma > 0:
        convList += ["conv_Lorentzian"]
    if shol > 0:
        convList += ["conv_axial"]
...
def LaueSatellite(peakpos, wave, c, ncell, j=None):
...
def SetDefaultSASDModel():
    ...
    return {
        ...
        "Size": {
            ...
            "IPG": {"Niter": 100, "Approach": 0.8, "Power": -1},
            "Reg": {},
        },
        "Pair": {
            "Method": "Moore",
        ...
        "Particle": {
            "Matrix": {"Name": "vacuum", "VolFrac": [0.0, False]},
            "Levels": [],
        },
        "Shapes": {
            "outName": "run",
...
def SetDefaultREFDModel():
    ...
    * DenMul: multiplier for layer scattering density (default = 1.0)
    ...
    return {
        ...
        "Layer Seq": [],  # globals
        "Minimizer": "LMLS",
        "Resolution": [0.0, "Const dq/q"],
        "Recomb": 0.5,
        "Toler": 0.5,  # minimizer controls
        "DualFitFiles": [
            "",
        ],
        "DualFltBacks": [
    ...
    }  # optional stuff for multidat fits?
...
def TestData():
    "needs a doc string"
    #    global NeedTestData
    global bakType  # noqa: PLW0603
    bakType = "chebyschev"
    global xdata  # noqa: PLW0603
    xdata = np.linspace(4.0, 40.0, 36000)
    global parmDict0  # noqa: PLW0603
    parmDict0 = {
        "pos0": 5.6964,
        "int0": 8835.8,
        "sig0": 1.0,
...
def test0():
    ...
    gplot.plot(
        xdata, getPeakProfile(parmDict0, xdata, np.zeros_like(xdata), varyList, bakType)
    )
    fplot = plotter.add("FCJ-Voigt, Ka1+2").gca()
    fplot.plot(xdata, getBackground("", parmDict1, bakType, "PXC", xdata)[0])
    fplot.plot(
        xdata, getPeakProfile(parmDict1, xdata, np.zeros_like(xdata), varyList, bakType)
    )
...
def test1():
    ...
    G2fil.G2Print(f"100+6*Ka1-2 peaks=1200 peaks {time.time():.2f}" - time0)
...
def test2(name, delt):
    ...
    hplot.plot(
        xdata,
        getPeakProfileDerv(parmDict2, xdata, np.zeros_like(xdata), varyList, bakType)[
            0
        ],
    )
...
def test3(name, delt):
    ...
    hplot.plot(
        ...
        * dx
        * getdFCJVoigt3(
            myDict["pos"], myDict["sig"], myDict["gam"], myDict["shl"], xdata
        )[idx + 1],
    )
    y0 = getFCJVoigt3(
        myDict["pos"], myDict["sig"], myDict["gam"], myDict["shl"], xdata
