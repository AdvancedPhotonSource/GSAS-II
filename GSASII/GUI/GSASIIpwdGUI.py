...
def SetLattice(controls):
    ...
    if controls[5] in ["Fm3m", "Im3m", "Pm3m"]:
        ...
        controls[9] = controls[11] = 90.0  # b unique
        ...
        def RefreshGrid(event):
            ...
            if c == 0:
                ...
                if "magAtms" in phase:
                    ...
                    text[3] = (
                        " The magnetic lattice point group is " + mSGData["MagPtGp"]
                    )
                ...
                if "magAtms" in phase:
                    ...
                    mAtoms = TestMagAtoms(
                        phase, magAtms, self.SGData, Uvec, Trans, allmom, maxequiv
                    )
                else:
                    mAtoms = TestAtoms(
                        phase, self.controls[15], self.SGData, Uvec, Trans, maxequiv
                    )
                Atms = []
                AtCods = []
                atMxyz = []
                ...
                G2phsG.UseMagAtomDialog(
                    self.panel, pname, Atms, AtCods, atMxyz, ifMag=ifMag, ifOK=True
                ).ShowModal()
            elif c in [3, 4]:
                if c == 3:
                    ...
                    if not items[0]:
                        wx.MessageBox(
                            pname + " is a maximal subgroup",
                            caption="Super group is parent",
                            style=wx.ICON_INFORMATION,
                        )
                        return
                SubCellsDialog(
                    self.panel, title, self.controls, self.SGData, items, self.phaseDict
                ).Show()
...
class RDFDialog(wx.Dialog):
    ...
    def Draw(self):
        ...
        dataSizer.Add(
            wx.StaticText(self.panel, label=" Maximum radial dist.: "), 0, WACV
        )
        maxR = G2G.ValidatedTxtCtrl(
            self.panel,
            self.result,
            "maxR",
            nDig=(10, 1),
    ...
    if "background PWDR" in background[1]:
        ...
        if backfile:
            ...
            if bId:
                ...
                print(f"Error: background PWDR {backfile} not found")
        ...
        if dataType == "Debye-Scherrer":
            if "T" in histType:
                parms += [
                    [
                        "Absorption",
                        "Sample absorption (\xb5r/" + Gklambda + "): ",
                        [10, 4],
                    ],
                ]
            else:
                parms += [
                    ["DisplaceX", "Sample X displ. perp. to beam (\xb5m): ", [10, 3]],
                    ["DisplaceY", "Sample Y displ. || to beam (\xb5m): ", [10, 3]],
                    ["Absorption", "Sample absorption (\xb5\xb7r): ", [10, 4]],
                ]
        elif dataType == "Bragg-Brentano":
            parms += [
                ["Shift", "Sample displacement(\xb5m): ", [10, 4]],
    ...
    while item:
        ...
        if name.startswith(hPrefix) and name != hstName:
            ...
            if cLam == hLam and cType == hType:  # and cGeom == hGeom:
                if name not in histList:
                    histList.append(name)
        item, cookie = G2frame.GPXtree.GetNextChild(G2frame.root, cookie)
    return histList


def SetCopyNames(histName, dataType, addNames=None):
...
def CopyPlotCtrls(G2frame):
    ...
    source = dict(zip(keys, [sourceData[0][item] for item in keys], strict=False))
...
def TestMagAtoms(
    phase, magAtms, SGData, Uvec, Trans, allmom, maxequiv=100, maximal=False
):
    ...
    atCodes = []
    Phase = {
        "General": {"AtomPtrs": [2, 1], "SGData": copy.deepcopy(phase["SGData"])},
        "Atoms": [],
    }
    for matm in magAtms:
        ...
        for _ix, x in enumerate(xyzs):
            xyz = G2lat.TransformXYZ(x - Uvec, invTrans.T, np.zeros(3)) % 1.0
...
def RefineCell(G2frame):
    ...
    def cellPrint(ibrav, A):
        ...
        if ibrav in ["Fm3m", "Im3m", "Pm3m"]:
            ...
            print(
                " {}{:10.6f} {}{:10.6f} {}{:10.6f} {}{:8.3f} {}{:12.3f}".format(
                    "a =",
                    cell[0],
                    "b =",
                    cell[1],
    ...
    peaks = G2frame.GPXtree.GetItemPyData(
        G2gd.GetGPXtreeItemId(G2frame, PatternId, "Index Peak List")
    )
    controls, bravais, cells, dminx, ssopt, magcells = G2frame.GPXtree.GetItemPyData(
        G2gd.GetGPXtreeItemId(G2frame, PatternId, "Unit Cells List")
    )
    cell = controls[6:12]
    A = G2lat.cell2A(cell)
    ibrav = bravaisSymb.index(controls[5])
    ...
    if "T" in Inst["Type"][0]:
        if ssopt.get("Use", False):
            ...
            peaks = [
                G2indx.IndexSSPeaks(peaks[0], G2frame.HKL)[1],
                peaks[1],
            ]  # put peak fit esds back in peaks
            Lhkl, M20, X20, Aref, Vec, Zero = G2indx.refinePeaksTSS(
                peaks[0],
                difC,
                Inst,
                SGData,
            ...
            peaks = [
                G2indx.IndexPeaks(peaks[0], G2frame.HKL)[1],
                peaks[1],
            ]  # put peak fit esds back in peaks
            Lhkl, M20, X20, Aref, Zero = G2indx.refinePeaksT(
                peaks[0], difC, ibrav, A, controls[1], controls[0]
            )
    elif (
        "E" in Inst["Type"][0]
    ):  # no super lattice stuff for EDX data - resolution too low
        G2frame.HKL = G2pwd.getHKLpeak(dmin, SGData, A, Inst)
        peaks = [
            G2indx.IndexPeaks(peaks[0], G2frame.HKL)[1],
            peaks[1],
        ...
        Lhkl, M20, X20, Aref, Vec, Zero = G2indx.refinePeaksZSS(
            peaks[0],
            wave,
            Inst,
            SGData,
        ...
        Lhkl, M20, X20, Aref, Zero = G2indx.refinePeaksZ(
            peaks[0], wave, ibrav, A, controls[1], controls[0]
        )
    controls[1] = Zero
    ...
    if "PKS" in G2frame.GPXtree.GetItemText(G2frame.PatternId):
        G2plt.PlotPowderLines(
            G2frame, indexFrom=f"Indexing from refine cell, M20={M20:.3f}"
        )
    else:
        G2pwpl.PlotPatterns(
            G2frame, indexFrom=f"Indexing from refine cell, M20={M20:.3f}"
...
def UpdatePeakGrid(G2frame, data):
    ...
    def OnAutoSearch(event):
        ...
        PatternId = G2frame.PatternId
        limits = G2frame.GPXtree.GetItemPyData(
            G2gd.GetGPXtreeItemId(G2frame, PatternId, "Limits")
        )[1]
        background = G2frame.GPXtree.GetItemPyData(
            G2gd.GetGPXtreeItemId(G2frame, PatternId, "Background")
        )
        inst, inst2 = G2frame.GPXtree.GetItemPyData(
        ...
        for off in offset:
            ymask = ma.array(ymask, mask=(ymask - np.roll(y0, off) <= 0.0))
        indx = ymask.nonzero()
        mags = ymask[indx]
        poss = x[indx]
        refs = list(zip(poss, mags, strict=False))
        if "T" in Inst["Type"][0]:
            refs = G2mth.sortArray(refs, 0, reverse=True)  # big TOFs first
        else:  #'C', 'E' or 'B'
            refs = G2mth.sortArray(
                refs, 0, reverse=False
            )  # small 2-Thetas or energies first
        for i, ref1 in enumerate(refs):  # reject picks closer than 1 FWHM
            for ref2 in refs[i + 1 :]:
                if abs(ref2[0] - ref1[0]) < 2.0 * G2pwd.getFWHM(ref1[0], inst):
                    del refs[i]
        if "T" in Inst["Type"][0]:
    ...
    def OnLoadPeaks(event):
        ...
        dlg = wx.FileDialog(
            G2frame,
            "Choose GSAS-II PWDR peaks list file",
            pth,
            "",
            "PWDR peak list files (*.pkslst)|*.pkslst",
            wx.FD_OPEN,
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
    ...
    def SaveState():
        ...
        for item in ["Background", "Instrument Parameters", "Peak List"]:
            pickle.dump(
                G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, PatternId, item)
                ),
                file,
                1,
            )
    ...
    def OnLSQPeakFit(event):
        ...
        ):  # force a save of the gpx file so SaveState can write in the same directory
    ...
    def OnSeqPeakFit(event):
        ...
        od = {
            "label_1": "Copy to next",
            "value_1": False,
            "label_2": "Reverse order",
            "value_2": False,
        }
        dlg = G2G.G2MultiChoiceDialog(
            G2frame,
            "Sequential peak fits",
        ...
        if Id:
            ...
            Id = G2frame.GPXtree.AppendItem(
                parent=G2frame.root, text="Sequential peak fit results"
            )
        SeqResult = {"SeqPseudoVars": {}, "SeqParFitEqList": []}
        SeqResult["histNames"] = names
        dlg = wx.ProgressDialog(
            "Sequential peak fit",
            "Data set name = " + names[0],
            len(names),
        ...
        try:
            for i, name in enumerate(names):
                ...
                if i and od["value_1"]:
                    G2frame.GPXtree.SetItemPyData(
                        G2gd.GetGPXtreeItemId(G2frame, PatternId, "Peak List"),
                        copy.deepcopy(peaks),
                    )
                ...
                SeqResult[name] = {
                    ...
                    "sig": sig,
                    "Rvals": Rvals,
                    "covMatrix": np.eye(len(result[0])),
                    "title": name,
                    "parmDict": parmDict,
                    "fullVary": fullvaryList,
                    "badVary": badVary,
                }
    ...
    def OnDelPeaks(event):
        ...
        sel = []
        dlg = G2G.G2MultiChoiceDialog(
            G2frame, "Select peaks to delete", "Delete peaks", choices
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
    ...
    def OnPeakFit(oneCycle=False, noFit=False):
                ...
                for i in peaks["peaks"]:
                    ...
                    peaks["LaueFringe"]["satellites"].extend(
                        G2pwd.LaueSatellite(
                            i[0],
                            wave,
                            peaks["LaueFringe"]["clat"],
                            peaks["LaueFringe"]["ncell"],
                            pks,
                        )
                    )

        if (
            G2frame.dataWindow.XtraPeakMode.IsChecked()
        ):  # adding peaks to computed pattern
        ...
        text = "Peak fit: Rwp=%.2f%% Nobs= %d Nparm= %d Npeaks= %d" % (
            results[3]["Rwp"],
            results[1][2]["fjac"].shape[1],
            len(results[0]),
            len(peaks["peaks"]),
        )
        newpeaks = copy.copy(peaks)
        G2frame.GPXtree.SetItemPyData(
            G2gd.GetGPXtreeItemId(G2frame, PatternId, "Peak List"), newpeaks
        )
    ...
    def setBackgroundColors():
        ...
        for r in range(reflGrid.GetNumberRows()):
            for c in range(reflGrid.GetNumberCols()):
                if reflGrid.GetColLabelValue(c) in [
                    "position",
                    "intensity",
                    "alpha",
                    "beta",
                    ...
                    try:
                        if float(reflGrid.GetCellValue(r, c)) < 0.0:
    ...
    def KeyEditPeakGrid(event):
        ...
        if (
            ...
            event.GetKeyCode() == wx.WXK_DELETE or event.GetKeyCode() == 8
        ):
            ...
            if nDel:
                msg = wg.GridTableMessage(
                    G2frame.PeakTable, wg.GRIDTABLE_NOTIFY_ROWS_DELETED, 0, nDel
                )
            ...
            for col in colList:
                if G2frame.PeakTable.GetTypeName(0, col) == wg.GRID_VALUE_BOOL:
                    if key == 89:  #'Y'
                        for row in range(G2frame.PeakTable.GetNumberRows()):
    ...
    def SelectVars(rows):
        ...
        refOpts = {
            reflGrid.GetColLabelValue(i): i + 1
            for i in range(reflGrid.GetNumberCols())
            if reflGrid.GetColLabelValue(i) != "refine"
        }
        dlg = G2G.G2MultiChoiceDialog(
            G2frame,
    ...
    def OnRefineSelected(event):
        ...
        rows = list(
            set(
                [row for row, col in reflGrid.GetSelectedCells()]
        ...
        if not rows:
            choices = [f"{i[0]:.2f}" for i in tbl]
            dlg = G2G.G2MultiChoiceDialog(
                G2frame, "Select peaks to refine", "select peaks", choices
            )
            try:
                if dlg.ShowModal() == wx.ID_OK:
    ...
    def onCellListDClick(event):
        ...
        if r < 0 and c < 0:
            ...
            if reflGrid.GetColLabelValue(c) != "refine":
                return
            choice = [
                "Y - vary all",
                "N - vary none",
            ...
            dlg = wx.SingleChoiceDialog(
                G2frame,
                "Select refinement option for " + reflGrid.GetColLabelValue(c - 1),
                "Refinement controls",
                choice,
            )
            dlg.CenterOnParent()
    ...
    phCount = 0
    # isHistMag = False
    for _ph in GetPhasesforHistogram(G2frame, histoName):
        phCount += 1
    ...
    if G2frame.dataWindow.XtraPeakMode.IsChecked():
        ...
        for peak in data["xtraPeaks"]:
            T.append(peak[0])
        D = dict(zip(T, data["xtraPeaks"], strict=False))
        T.sort()
        ...
        data["xtraPeaks"] = X
        G2frame.GPXtree.SetItemPyData(G2frame.PickId, data)
        kPeakTable = G2G.Table(
            data["xtraPeaks"], rowLabels=krowLabels, colLabels=kcolLabels, types=Types
        )
    ...
    for peak in data["peaks"]:
        T.append(peak[0])
    D = dict(zip(T, data["peaks"], strict=False))
    T.sort()
    if "T" in Inst["Type"][0]:  # want big TOF's first
        T.reverse()
    X = []
        ...
        for i in range(len(data["peaks"])):
            if len(data["peaks"][i]) == 8:  # need to extend the entry
                if "Lam" in Inst:
                    ...
                    lam = (
                        Inst["Lam1"][0] + Inst["I(L2)/I(L1)"][0] * Inst["Lam2"][0]
                    ) / (1 + Inst["I(L2)/I(L1)"][0])
                if i == 0:
                    pos = data["peaks"][i][0]
                    data["LaueFringe"]["clat"] = (
                        data["LaueFringe"]["lmin"]
                        * 0.5
                        * lam
                ...
                data["peaks"][i] += [0.0, False, 0.0, False, l, None]
            peakline = copy.deepcopy(data["peaks"][i][2:])
            peakline[-1] = data["peaks"][i][0]
            peakline[-2] = int(peakline[-2] + 0.5)
            data["LFpeaks"].append(peakline)
        G2frame.PeakTable = G2G.Table(
            data["LFpeaks"], rowLabels=rowLabels, colLabels=colLabels, types=Types
        )
    else:
        G2frame.PeakTable = G2G.Table(
            data["peaks"], rowLabels=rowLabels, colLabels=colLabels, types=Types
        )
    ...
    parent = G2frame.dataWindow.topPanel
    topSizer.Add(
        wx.StaticText(parent, label="List of peaks to fit individually"), 0, WACV
    )
    ...
    if "LF" in Inst["Type"][0]:
        ...
        topSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" Overall parms: "), 0, WACV
        )
        ...
        cellSpin = wx.SpinButton(
            G2frame.dataWindow, style=wx.SP_VERTICAL, size=wx.Size(20, 20)
        )
        cellSpin.SetValue(0)
        ...
        prmSizer.Add((15, -1))
        prmSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" fit exponent, minus side"),
            0,
            WACV,
        )
        cVal = G2G.ValidatedTxtCtrl(
...
def UpdateBackground(G2frame, data):
    ...
    def OnBackFlagCopy(event):
        ...
        for item in copyList:
            Id = G2gd.GetGPXtreeItemId(G2frame, G2frame.root, item)
            backData = G2frame.GPXtree.GetItemPyData(
    ...
    def OnBackSave(event):
        ...
        dlg = wx.FileDialog(
            G2frame,
            "Set name to save GSAS-II background parameters file",
            pth,
            "",
            "background parameter files (*.pwdrbck)|*.pwdrbck",
            wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT,
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                out_tmp = list()
                for item in data[0]:
                    if isinstance(item, np.float64 | float):
                        out_tmp.append(float(item))
                    else:
                        out_tmp.append(item)
                File.write(str(out_tmp) + "\n")
                for item in data[1]:
                    if item in ["nPeaks", "background PWDR", "nDebye"] or not len(
                        data[1][item]
                    ):
    ...
    def OnBackLoad(event):
        ...
        dlg = wx.FileDialog(
            G2frame,
            "Choose GSAS-II background parameters file",
            pth,
            "",
            "background parameter files (*.pwdrbck)|*.pwdrbck",
            wx.FD_OPEN,
    ...
    def OnBkgFit(event):
        ...
        if "FixedPoints" not in background[1] or not len(background[1]["FixedPoints"]):
            msg = (
                "You have not defined any fixed background points. "
            ...
            return
        background[1]["FixedPoints"] = sorted(
            background[1]["FixedPoints"], key=lambda pair: pair[0]
        )
        ...
        if len(bakVary) * 1.5 > len(X):
            msg = (
                "You are attempting to vary "
                + str(len(bakVary))
                + " background terms with only "
                + str(len(X))
                + " background points"
        ...
        pwddata[3][xBeg:xFin] *= 0.0
        pwddata[5][xBeg:xFin] *= 0.0
        pwddata[4][xBeg:xFin] = G2pwd.getBackground(
            "", parmDict, bakType, dataType, xdata, bxye
        )[0]
        G2pwpl.PlotPatterns(G2frame, plotType="PWDR")
    ...
    def BackSizer():
        ...
        topSizer = wx.BoxSizer(wx.HORIZONTAL)
        topSizer.Add(
            wx.StaticText(G2frame.dataWindow, -1, " Background function: "), 0, WACV
        )
        bakType = wx.ComboBox(
            G2frame.dataWindow,
            value=data[0][0],
        ...
        topSizer = wx.BoxSizer(wx.HORIZONTAL)
        topSizer.Add(
            wx.StaticText(G2frame.dataWindow, -1, " Number of coeff.: "), 0, WACV
        )
        bakTerms = wx.ComboBox(
            G2frame.dataWindow,
            -1,
            value=str(data[0][2]),
            choices=[str(i + 1) for i in range(36)],
            style=wx.CB_READONLY | wx.CB_DROPDOWN,
        )
        ...
        backSizer.Add(topSizer)
        backSizer.Add(
            wx.StaticText(G2frame.dataWindow, -1, " Background coefficients:"), 0
        )
        bakSizer = wx.FlexGridSizer(0, 5, 5, 5)
        for i, _value in enumerate(data[0][3:]):
            bakVal = G2G.ValidatedTxtCtrl(
                G2frame.dataWindow, data[0], i + 3, nDig=(10, 4), OnLeave=AfterChange
            )
    ...
    def DebyeSizer():
                    ...
                    if debyeTable.GetTypeName(0, col) == wg.GRID_VALUE_BOOL:
                        if key == 89:  #'Y'
                            for row in range(debyeGrid.GetNumberRows()):
                                data[1]["debyeTerms"][row][col] = True
                        elif key == 78:  #'N'
                            for row in range(debyeGrid.GetNumberRows()):
        ...
        topSizer = wx.BoxSizer(wx.HORIZONTAL)
        topSizer.Add(
            wx.StaticText(G2frame.dataWindow, -1, " Debye scattering: "), 0, WACV
        )
        topSizer.Add(
            wx.StaticText(G2frame.dataWindow, -1, " Number of terms: "), 0, WACV
        )
        debTerms = wx.ComboBox(
            G2frame.dataWindow,
            -1,
            value=str(data[1]["nDebye"]),
        ...
        if data[1]["nDebye"]:
            debSizer.Add(
                wx.StaticText(G2frame.dataWindow, -1, " Debye diffuse terms:"), 0
            )
            rowLabels = []
            for i in range(len(data[1]["debyeTerms"])):
    ...
    def PeaksSizer():
        def OnPeaks(event):
            ...
            if N > M:  # add terms
                for _i in range(M, N):
                    data[1]["peaksList"].append(
                        [1.0, False, 1.0, False, 0.10, False, 0.10, False]
                    )
            elif N < M:  # delete terms
                for _i in range(N, M):
        ...
        topSizer = wx.BoxSizer(wx.HORIZONTAL)
        topSizer.Add(
            wx.StaticText(G2frame.dataWindow, -1, " Peaks in background: "), 0, WACV
        )
        topSizer.Add(
            wx.StaticText(G2frame.dataWindow, -1, " Number of peaks: "), 0, WACV
        )
        peaks = wx.ComboBox(
            G2frame.dataWindow,
            -1,
            value=str(data[1]["nPeaks"]),
        ...
        if data[1]["nPeaks"]:
            ...
            rowLabels = []
            for i in range(len(data[1]["peaksList"])):
                rowLabels.append(str(i))
            colLabels = [
                "pos",
                "refine",
    ...
    def BackFileSizer():
        def OnBackPWDR(event):
            ...
            if len(data[1]["background PWDR"][0]):
                ...
                Id = G2gd.GetGPXtreeItemId(
                    G2frame, G2frame.root, data[1]["background PWDR"][0]
                )
                if not Id:
                    G2G.G2MessageBox(
                        G2frame,
                        "Histogram not found -- how did this happen?",
                        "Missing histogram",
                    )
                    back.SetValue("")
                    data[1]["background PWDR"][0] = back.GetValue()
                ...
                if len(bkgHist[1][0]) != len(curHist[1][0]):
                    G2G.G2MessageBox(
                        G2frame,
                        "Histogram have different lengths",
        ...
        fileSizer.Add(
            wx.StaticText(
                G2frame.dataWindow,
                -1,
                " Fixed background histogram (for point-by-point subtraction):",
            ),
            0,
        )
        if "background PWDR" not in data[1]:
            data[1]["background PWDR"] = ["", -1.0, False]
        backSizer = wx.BoxSizer(wx.HORIZONTAL)
        ...
        back = wx.ComboBox(
            ...
            value=data[1]["background PWDR"][0],
            choices=Choices,
            style=wx.CB_READONLY | wx.CB_DROPDOWN,
        )
        ...
        backMult = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            data[1]["background PWDR"],
            1,
            nDig=(10, 3),
            OnLeave=AfterChange,
        )
    ...
    def copyAutoBack(event):
        ...
        if not autoBkgDict.get("Mode"):
            G2frame.ErrorDialog(
                "No auto bkg setting",
                "This is unexpected, no auto bkg parms for histogram " + hst,
                G2frame,
            )
            return
        ...
        histList = [i for i in GetHistsLikeSelected(G2frame) if "Autobkg for" not in i]
        if not histList:
            G2frame.ErrorDialog("No match", "No histograms match " + hst, G2frame)
            return
        dlg = G2G.G2MultiChoiceDialog(
    ...
    def CalcBack(PatternId=G2frame.PatternId):
        ...
        pwddata[1][4][xBeg:xFin] = G2pwd.getBackground(
            "",
            parmDict,
            bakType,
            dataType,
            pwddata[1][0][xBeg:xFin],
            fixBack[xBeg:xFin],
        )[0]
    ...
    if "nPeaks" not in data[1]:
        data[1].update(
            {"nPeaks": 0, "peaksList": [], "background PWDR": ["", 1.0, False]}
        )
    if "background PWDR" not in data[1]:
...
def addAutoBack(G2frame, data, xydata):
    ...
    """
    bkgHistName = (
        "PWDR Autobkg for " + G2frame.GPXtree.GetItemText(G2frame.PatternId)[5:]
    )
...
class autoBackground(wx.Dialog):
    ...
    def __init__(self, G2frame, *args, **kwargs):
        ...
        self.bkgdict["autoPrms"]["logLam"] = self.bkgdict["autoPrms"].get(
            "logLam", logLam
        )
        self.bkgdict["autoPrms"]["Mode"] = None
        maxLam = min(15.0, 1.0 * int(3 * self.bkgdict["autoPrms"]["logLam"] + 0.9))
        # save starting point info
        self.startingBackground = copy.deepcopy(self.xydata[4])
        # start process
        wx.Dialog.__init__(
            self, parent=G2frame, style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER
        )
        self.CenterOnParent()
        ...
        for w in G2G.G2RadioButtons(
            self, self.bkgdict["autoPrms"], "opt", choices, OnChange=self._calcBkg
        ):
            subSiz.Add(w, 0, wx.ALIGN_CENTER_VERTICAL, 0)
        mainSizer.Add(subSiz)
        siz = G2G.G2SliderWidget(
            self,
            self.bkgdict["autoPrms"],
            "logLam",
            "log(Lambda)",
            1.0,
            maxLam,
...
def UpdateLimitsGrid(G2frame, data, datatype):
    ...
    def AfterChange(invalid, value, tc):
        ...
        )  # unfortunately this resets the plot width
    ...
    def LimitSizer():
        ...
        for i in [0, 1]:
            limits.Add(
                wx.StaticText(
                    G2frame.dataWindow, label=f" Original {labels[i]} {data[0][i]:.4f}"
                ),
                0,
                WACV,
            )
    ...
    def onSetLimExcl(event):
        txt = event.EventObject.FindItemById(
            event.GetId()
        ).GetItemLabel()  # gets menu command text
        if "lower" in txt:
    ...
    def Draw():
        ...
        parent = G2frame.dataWindow.topPanel
        topSizer.Add(
            wx.StaticText(parent, label=" Data range to be used in fits"), 0, WACV
        )
        # add help button to bring up help web page - at right side of window
        topSizer.Add((-1, -1), 1, wx.EXPAND)
        topSizer.Add(G2G.HelpButton(parent, helpIndex=G2frame.dataWindow.helpKey))
...
def UpdateInstrumentGrid(G2frame, data):
    ...
    def RefreshInstrumentGrid(event, doAnyway=False):
        if doAnyway or event.GetRow() == 1:
            peaks = G2frame.GPXtree.GetItemPyData(
                G2gd.GetGPXtreeItemId(G2frame, G2frame.PatternId, "Peak List")
            )
            newpeaks = []
            for peak in peaks["peaks"]:
    ...
    def OnCalibrate(event):
        ...
        cw = np.diff(xye[0])
        IndexPeaks = G2frame.GPXtree.GetItemPyData(
            G2gd.GetGPXtreeItemId(G2frame, G2frame.PatternId, "Index Peak List")
        )
        ...
        if "Debye" not in Sample["Type"]:
            G2frame.ErrorDialog(
                "Cannot calibrate", "Only apropriate for Debye-Scherrer geometry"
            )
            return None
        ...
        if G2pwd.DoCalibInst(IndexPeaks, data, Sample):
            ...
            if "C" in data["Type"][0] or "B" in data["Type"][0]:
                const = 18.0e-2 / (np.pi * Sample["Gonio. radius"])
            ...
            for ip, peak in enumerate(IndexPeaks[0]):
                ...
                if peak[2] and peak[3]:
                    ...
                    if const:
                        shft = -const * (
                            Sample["DisplaceX"][0] * npcosd(peak[0])
                            + Sample["DisplaceY"][0] * npsind(peak[0])
                        )
            ...
            G2frame.ErrorDialog(
                "Cannot calibrate",
                "Nothing selected for refinement or refinement failed",
            )
            return None
    ...
    def OnLoad(event):
            ...
            while True:  # loop until we get a choice
                ...
                if "Generic TOF" in choices[res]:
                    ...
                    if dlg.ShowModal() == wx.ID_OK:  # strictly empirical approx.
                        ...
                        Inst = G2frame.ReadPowderInstprm(
                    ...
                    inst1, inst2 = G2frame.ReadPowderInstprm(
                        dI.defaultIparms[res], bank, rd
                    )
                    return [inst1, inst2]
        ...
        dlg = wx.FileDialog(
            G2frame,
            "Choose GSAS-II instrument parameters file",
            pth,
            "",
            "instrument parameter files (*.instprm)|*.instprm",
            wx.FD_OPEN,
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                if Found:
                    ...
                    if (
                        "Bank" not in Inst
                    ):  # patch for old .instprm files - may cause faults for TOF data
                        Inst["Bank"] = [1, 1, 0]
    ...
    def OnSave(event):
        ...
        dlg = wx.FileDialog(
            G2frame,
            "Set name to save GSAS-II instrument parameters file",
            pth,
            "",
            "instrument parameter files (*.instprm)|*.instprm",
            wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT,
        ...
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                for hist in saveList:
                    ...
                    inst1 = G2frame.GPXtree.GetItemPyData(
                        G2gd.GetGPXtreeItemId(G2frame, Id, "Instrument Parameters")
                    )[1]  # pdabc dictionary
                    Sample = G2frame.GPXtree.GetItemPyData(
                        G2gd.GetGPXtreeItemId(G2frame, Id, "Sample Parameters")
    ...
    def OnInstFlagCopy(event):
        ...
        try:
            ...
            pass
        flags = dict(zip(keys, [data[key][2] for key in keys], strict=False))
        instType = data["Type"][0]
        copyList = []
        dlg = G2G.G2MultiChoiceDialog(
            G2frame,
            "Copy inst ref. flags from\n" + hst[5:],
            "Copy refinement flags",
            histList,
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
        ...
        for item in copyList:
            Id = G2gd.GetGPXtreeItemId(G2frame, G2frame.root, item)
            instData = G2frame.GPXtree.GetItemPyData(
                G2gd.GetGPXtreeItemId(G2frame, Id, "Instrument Parameters")
            )[0]
    ...
    def OnInstCopy(event):
        ...
        instType = data["Type"][0]
        dlg = G2G.G2MultiChoiceDialog(
            G2frame, "Copy inst params from\n" + hst, "Copy parameters", histList
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
                for i in dlg.GetSelections():
        ...
        for item in copyList:
            ...
            if "Z" not in instData:
                instData["Z"] = [0.0, 0.0, False]
            if (
                len(data) == len(instData) and instType == instData["Type"][0]
            ):  # don't mix data types or lam & lam1/lam2 parms!
                instData.update(copyData)
            elif len(data) != len(instData):
                print(
                    item
                    + " not copied - %d instrument parameters do not match source # %d"
                    % (len(instData), len(data))
                )
            else:
                print(
                    item
    ...
    def MakeParameterWindow():
        ...
        def MakeLamSizer():
            if "Lam1" in insVal:
                ...
                waveSizer.Add(
                    wx.StaticText(G2frame.dataWindow, -1, "  Source type: "), 0, WACV
                )
                ...
                if "Lam1" in data and data["Source"][1] not in choice:
                    indxs = [
                        ...
                        if t.lower().startswith(data["Source"][1].lower())
                    ]
                    if len(indxs) == 1:
                        data["Source"][1] = choice[indxs[0]]
                lamPick = wx.ComboBox(
                    G2frame.dataWindow,
                    value=data["Source"][1],
                    choices=choice,
                ...
                instSizer.Add(
                    wx.StaticText(
                        G2frame.dataWindow, -1, f" Lam (\xc5): ({insDef[key]:10.6f})"
                    ),
                    0,
                    WACV,
                )
                waveVal = G2G.ValidatedTxtCtrl(
                    G2frame.dataWindow,
                    insVal,
                    key,
                    nDig=(10, 6),
        ...
        if "P" in insVal["Type"]:  # powder data
            [
                instSizer.Add(wx.StaticText(G2frame.dataWindow, -1, txt), 0, WACV)
                for txt in [" Name (default)", " Value", "Refine?"]
            ]
            Reference = "Reference?"
            if insVal["Type"][2] in ["A", "B", "C"]:  # constant wavelength
                ...
                for item in ["Zero", "Polariz."]:
                    if item in insDef:
                        ...
                        instSizer.Add(
                            wx.StaticText(
                                G2frame.dataWindow, -1, lblWdef(item, 4, insDef[item])
                            ),
                            0,
                            WACV,
                        )
                        itemVal = G2G.ValidatedTxtCtrl(
                            G2frame.dataWindow,
                            insVal,
                            item,
                            nDig=(10, 4),
    ...
    Von Dreele, R.B., Clarke, S.M. & Walsh, J.P.S. (2021). J. Appl. Cryst., 54, 3-6.
                ...
                for item in itemList:
                    ...
                    instSizer.Add(
                        wx.StaticText(
                            G2frame.dataWindow, -1, lblWdef(item, nDig[1], insDef[item])
                        ),
                        0,
                        WACV,
                    )
                ...
                instSizer.Add(
                    wx.StaticText(
                        G2frame.dataWindow, -1, f" 2-theta ({insDef[key]:10.6f}):"
                    ),
                    0,
                    WACV,
                )
                tthVal = G2G.ValidatedTxtCtrl(
                    G2frame.dataWindow,
                    insVal,
                    key,
                    nDig=(10, 6),
                ...
                for item in ["XE", "YE", "ZE", "WE", "A", "B", "C", "X", "Y", "Z"]:
                    ...
                    if item in ["XE", "YE", "ZE", "WE"]:
                        instSizer.Add(
                            wx.StaticText(
                                G2frame.dataWindow, label=f"{insVal[item]:10.6g}"
                            )
                        )
                        instSizer.Add((5, 5), 0)
                    else:
    ...
    def MakeMultiParameterWindow(selected=None):
        ...
        histoList, histIdList = G2frame.GetHistogramNamesID(
            [
                "PWDR",
            ]
        )
        ...
        for i in selected:
            ...
            hid = histIdList[h]
            inst, inst2 = G2frame.GPXtree.GetItemPyData(
                G2gd.GetGPXtreeItemId(G2frame, hid, "Instrument Parameters")
            )
            histnames.append(h)
            histdict[h] = inst
        ...
        for i, h in enumerate(histnames):
            if len(h[:5].strip()) > 20:
                fgs.Add(
                    ...
                    0,
                    WACV | wx.LEFT | wx.RIGHT,
                    5,
                )
            else:
                fgs.Add(
                    wx.StaticText(sdlg, wx.ID_ANY, h[5:]),
                    0,
                    WACV | wx.LEFT | wx.RIGHT,
                    5,
                )
        ...
        if "T" in data["Type"][1]:
            keylist += ["2-theta"]
            lbllist += ["2-theta"]
        for key, lbl in zip(keylist, lbllist, strict=False):
            ...
            for h in histnames:
                ...
                fgs.Add(
                    wx.StaticText(sdlg, wx.ID_ANY, str(val)),
                    0,
                    wx.ALIGN_CENTER | WACV,
                    0,
                )
                plotvals.append(val)
        ...
        for k in Items:
            ...
            if firstRadio:
                ...
                firstRadio = 0
            fgs.Add(
                G2G.G2CheckBox(sdlg, "", plotYsel, k, OnChange=onPrmPlot),
                0,
                wx.ALIGN_CENTER | WACV,
            )
            ...
            for h in histnames:
                ...
                itemVal = G2G.ValidatedTxtCtrl(
                    sdlg, histdict[h][k], 1, nDig=(10, 4), typeHint=float
                )
                plotvals.append(histdict[h][k][1])
                miniSizer.Add(itemVal)
                miniSizer.Add((2, -1))
                miniSizer.Add(
                    G2G.G2CheckBox(sdlg, "", histdict[h][k], 2), 0, WACV | wx.RIGHT, 15
                )
    ...
    for key in data:
        if type(data[key]) is tuple:
            ...
            patched += 1
    if patched:
        print(patched, " instrument parameters changed from tuples")
    ...
    if "P" in data["Type"][0]:  # powder data
        insVal = dict(zip(instkeys, [data[key][1] for key in instkeys], strict=False))
        insDef = dict(zip(instkeys, [data[key][0] for key in instkeys], strict=False))
        insRef = dict(zip(instkeys, [data[key][2] for key in instkeys], strict=False))
        if "NC" in data["Type"][0]:
            del insDef["Polariz."]
            del insVal["Polariz."]
            del insRef["Polariz."]
    elif (
        "S" in data["Type"][0] or "L" in data["Type"][0] or "R" in data["Type"][0]
    ):  # single crystal data
        insVal = dict(zip(instkeys, [data[key][1] for key in instkeys], strict=False))
        insDef = dict(zip(instkeys, [data[key][0] for key in instkeys], strict=False))
        insRef = {}
    RefObj = {}
    Inst2 = G2frame.GPXtree.GetItemPyData(
        G2gd.GetGPXtreeItemId(G2frame, G2frame.PatternId, "Instrument Parameters")
    )[1]
    ...
    if "P" in insVal["Type"]:  # powder data menu commands
        ...
        G2frame.GetStatusBar().SetStatusText(
            "NB: Azimuth is used for polarization only", 1
        )
        ...
        "L" in insVal["Type"] or "R" in insVal["Type"] or "S" in insVal["Type"]
    ):  # HKLF, SASD & REFD data menu commands
        MakeParameterWindow()
...
def UpdateSampleGrid(G2frame, data):
    ...
    def OnSampleSave(event):
        ...
        dlg = wx.FileDialog(
            G2frame,
            "Choose GSAS-II sample parameters file",
            pth,
            "",
            "sample parameter files (*.samprm)|*.samprm",
            wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT,
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
    ...
    def OnAllSampleLoad(event):
        ...
        dlg = wx.FileDialog(
            G2frame,
            "Choose multihistogram metadata text file",
            pth,
            "",
            "metadata file (*.*)|*.*",
            wx.FD_OPEN,
        ...
        dlg = G2G.G2ColumnIDDialog(
            G2frame,
            " Choose multihistogram metadata columns:",
            "Select columns",
            Comments,
            Names,
            np.array(newItems).T,
        ...
        for ih, hist in enumerate(histList):
            ...
            for item in colIds:
                ...
                try:
                    ...
                    try:
                        newItems[key] = float(dataDict[f"{ih + 1}"])
                    except KeyError:
                        break
            Id = G2gd.GetGPXtreeItemId(G2frame, G2frame.root, hist)
    ...
    def OnSetScale(event):
        ...
        dlg = wx.SingleChoiceDialog(
            G2frame,
            "Select reference histogram for scaling",
            "Reference histogram",
            histList,
        )
        try:
        ...
        refSample = G2frame.GPXtree.GetItemPyData(
            G2gd.GetGPXtreeItemId(G2frame, refId, "Sample Parameters")
        )
        refLimits = G2frame.GPXtree.GetItemPyData(
            G2gd.GetGPXtreeItemId(G2frame, refId, "Limits")
        )
    ...
    def OnRescaleAll(event):
        ...
        if not histList:
            ...
            return
        od = {
            "label_1": "Scaling range min",
            "value_1": 0.0,
            "label_2": "Scaling range max",
            "value_2": 10.0,
        }
        dlg = G2G.G2MultiChoiceDialog(
            G2frame,
        ...
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                if iBeg > iFin:
                    wx.MessageBox(
                        "Wrong order for Xmin, Xmax", "Error", style=wx.ICON_EXCLAMATION
                    )
                else:
    ...
    def OnSampleCopySelected(event):
        ...
        TextTable = {
            key: label
            for key, label, dig in SetupSampleLabels(
                hst, data.get("Type"), Inst["Type"][0]
            )
        }
        # get flexible labels
        TextTable.update(
        ...
        keyList = [i for i in data if i in TextTable]
        keyText = [TextTable[i] for i in keyList]
        # sort both lists together, ordered by keyText
        keyText, keyList = zip(
            *sorted(zip(keyText, keyList, strict=False)), strict=False
        )  # sort lists
        selectedKeys = []
        dlg = G2G.G2MultiChoiceDialog(
            G2frame,
    ...
    def SearchAllComments(value, tc, *args, **kwargs):
        ...
        while Id:
            name = G2frame.GPXtree.GetItemText(Id)
            if "PWDR" in name:
                Comments = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, Id, "Comments")
                )
                Sample = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, Id, "Sample Parameters")
                )
                for _i, item in enumerate(Comments):
                    itemSp = item.split("=")
    ...
    if "SurfRoughA" not in data and "PWDR" in histName:
        data["SurfRoughA"] = [0.0, False]
        data["SurfRoughB"] = [0.0, False]
    ...
    parent = G2frame.dataWindow.topPanel
    topSizer.Add(
        wx.StaticText(parent, label=" Sample and Experimental Parameters"), 0, WACV
    )
    topSizer.Add((-1, -1), 1, wx.EXPAND)
    topSizer.Add(G2G.HelpButton(parent, helpIndex=G2frame.dataWindow.helpKey))
    mainSizer = wx.BoxSizer(wx.VERTICAL)
    ...
    nameSizer.Add(
        wx.StaticText(G2frame.dataWindow, wx.ID_ANY, " Instrument Name "), 0, WACV
    )
    nameSizer.Add((-1, -1), 1, WACV)
    instNameVal = wx.TextCtrl(
        G2frame.dataWindow,
        wx.ID_ANY,
        data["InstrName"],
        size=(200, -1),
    ...
    if "PWDR" in histName:
        nameSizer = wx.BoxSizer(wx.HORIZONTAL)
        nameSizer.Add(
            wx.StaticText(G2frame.dataWindow, wx.ID_ANY, " Diffractometer type: "),
            0,
            WACV,
        )
    ...
    for key, lbl, nDig in parms:
        ...
        if "list" in str(type(data[key])):
            ...
            parmSizer.Add(
                wx.StaticText(G2frame.dataWindow, label=" " + lbl), 0, wx.EXPAND
            )
            parmVal = G2G.ValidatedTxtCtrl(
                G2frame.dataWindow, data, key, typeHint=float, OnLeave=AfterChange
            )
    ...
    if histName[:4] in [
        ...
        for Id, item in enumerate(data["Materials"]):
            subSizer.Add(
                wx.StaticText(G2frame.dataWindow, label=" Material: "), 0, WACV
            )
            matsel = wx.ComboBox(
                G2frame.dataWindow,
                value=item["Name"],
                choices=list(Substances["Substances"].keys()),
                style=wx.CB_READONLY | wx.CB_DROPDOWN,
            )
            ...
            subSizer.Add(
                wx.StaticText(G2frame.dataWindow, label=" Volume fraction: "), 0, WACV
            )
            volfrac = G2G.ValidatedTxtCtrl(
                G2frame.dataWindow,
                item,
                "VolFrac",
                xmin=0.0,
            ...
            mu += item["VolFrac"] * material.get("XAbsorption", 0.0)
            rho[Id] = material["Scatt density"]
            anomrho[Id] = material.get("XAnom density", 0.0)
        ...
        conSizer.Add(
            wx.StaticText(
                G2frame.dataWindow,
                label=f" Transmission (calc): {np.exp(-mut):10.3f}  ",
            ),
            0,
            WACV,
        )
...
def UpdateIndexPeaksGrid(G2frame, data):
    ...
    def OnSave(event):
        ...
        dlg = wx.FileDialog(
            G2frame,
            "Choose Index peaks csv file",
            pth,
            "",
            "indexing peaks file (*.csv)|*.csv",
            wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT,
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
    ...
    def OnExportPreDICT(event):
        ...
        if wx.TheClipboard.Open():
            ...
            G2frame.ErrorDialog(
                "Clipboard locked",
                "Sorry, unable to access the clipboard, try again later. You might need to restart GSAS-II or reboot",
            )
            return
        G2G.G2MessageBox(
            G2frame,
            f"{c} reflection positions placed in clipboard. "
    ...
    def KeyEditPickGrid(event):
        ...
        if (
            ...
            key = event.GetKeyCode()
            for col in colList:
                if G2frame.IndexPeaksTable.GetColLabelValue(col) in [
                    "use",
                ]:
                    if key == 89:  #'Y'
    ...
    if len(data[0]):
        ...
        Unit = G2frame.GPXtree.GetItemPyData(
            G2gd.GetGPXtreeItemId(G2frame, G2frame.PatternId, "Unit Cells List")
        )
        if Unit:
            if len(Unit) == 4:  # patch
                Unit.append({})
            ...
            if ssopt.get("Use", False):
                ...
                maxH = ssopt["maxH"]
                G2frame.HKL = np.array(
                    G2pwd.getHKLMpeak(dmin, Inst, SGData, SSGData, Vec, maxH, A)
                )
    ...
    G2frame.GPXtree.SetItemPyData(IndexId, data)
    G2frame.IndexPeaksTable = G2G.Table(
        data[0], rowLabels=rowLabels, colLabels=colLabels, types=Types
    )
    G2frame.dataWindow.currentGrids = []
    ...
    if controls:
        ...
        for cellGUI in cellGUIlist:
            if ibrav in cellGUI[0]:
                useGUI = cellGUI
        botSizer.Add(
            wx.StaticText(parent, label="Cell: ", style=wx.ALIGN_RIGHT), 0, WACV
        )
        ...
        for txt, fmt, _ifEdit, Id in zip(*useGUI[2], strict=False):
            if "Vol" in txt:
                val = fmt % controls[12]
                botSizer.Add(
                    wx.StaticText(parent, label=txt, style=wx.ALIGN_RIGHT), 0, WACV
                )
                volVal = wx.TextCtrl(
                    parent, value=val, style=wx.TE_READONLY, size=(65, -1)
                )
                botSizer.Add(volVal, 0, WACV)
            else:
                val = f"%.{fmt[1]}f" % controls[6 + Id]
                cellSizer.Add(
                    wx.StaticText(parent, label=txt, style=wx.ALIGN_RIGHT),
                    0,
                    wx.ALIGN_RIGHT | WACV,
                )
                volVal = wx.TextCtrl(
        ...
        if not ssopt.get("Use", False):  # zero for super lattice doesn't work!
            ...
            vcSizer.Add(
                wx.StaticText(parent, label="Zero offset", style=wx.ALIGN_CENTER),
                0,
                wx.EXPAND,
            )
            hcSizer = wx.BoxSizer(wx.HORIZONTAL)
            zero = G2G.ValidatedTxtCtrl(
                parent,
                controls,
                1,
                nDig=(10, 4),
...
def UpdateUnitCellsGrid(
    G2frame, data, callSeaResSelected=False, New=False, showUse=False
):
    ...
    def OnExportCells(event):
        ...
        dlg = wx.FileDialog(
            G2frame,
            "Choose Indexing Result csv file",
            pth,
            "",
            "indexing result file (*.csv)|*.csv",
            wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT,
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
    ...
    def OnHklShow(event=None, Print=True, Plot=True, indexFrom=""):
        ...
        if ssopt.get("Use", False) and ssopt.get("ssSymb", ""):
            ...
            if cellDisplayOpts["showExtinct"]:
                ...
                for (
                    peak
                ) in allpeaks:  # show one reflection only if in a region with no others
                    pos = peak[4].round(3)
                    if pos in alreadyShown:
                        continue
            ...
            if len(peaks[0]):  # put hkl values into the Index Peak List
                ...
                G2frame.GPXtree.SetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, PatternId, "Index Peak List"), peaks
                )
        G2frame.HKL = np.array(G2frame.HKL)
        frfnd = 0.0
        ...
        if Nhkl:
            ...
            if Print:
                print(
                    " new M20,X20: %.2f %d, fraction found: %.3f for %s"
                    % (M20, X20, frfnd, Symb)
                )
    ...
    def CopyUnitCell(event):
        ...
        if len(cells):
            ...
            if controls[5][1:] == "m3":
                controls[5] += "m"
            if "P3" in controls[5] or "P-3" in controls[5]:
                controls[5] = "P6/mmm"
            if "R" in controls[5]:
                controls[5] = "R3-H"
    ...
    def LoadUnitCell(event):
        ...
        while item:
            ...
            Phase = G2frame.GPXtree.GetItemPyData(
                G2gd.GetGPXtreeItemId(G2frame, pId, pName)
            )
        ...
        if pNum is None:
            return
        Phase = G2frame.GPXtree.GetItemPyData(
            G2gd.GetGPXtreeItemId(G2frame, pId, Phases[pNum])
        )
        Phase["magPhases"] = G2frame.GPXtree.GetItemText(
            G2frame.PatternId
        )  # use as reference for recovering possible phases
        ...
        if "SuperSg" in Phase["General"] or SGData.get("SGGray", False):
            ssopt.update(
                {
                    "SGData": SGData,
                    "ssSymb": Phase["General"]["SuperSg"],
                    "ModVec": Phase["General"]["SuperVec"][0],
                    "Use": True,
                    "maxH": 1,
                }
            )
            ...
            if ssSym not in G2spc.SSChoice(SGData):
                ...
                wx.MessageBox(
                    ...
                    + " not valid;\n It is set to "
                    + ssSym,
                    caption="Unusable super space group",
                    style=wx.ICON_EXCLAMATION,
                )
        ...
        if "mono" in SGData["SGSys"]:
            ...
            if SpGrp is None:
                wx.MessageBox(
                    "Monoclinic " + SGData["SpGrp"] + " not usable here",
                    caption="Unusable space group",
                    style=wx.ICON_EXCLAMATION,
                )
                return
        controls[13] = SpGrp
        ...
        if controls[5][1:] == "m3":
            controls[5] += "m"
        if controls[5] in ["P3", "P3m1", "P31m", "P6/m"]:
            controls[5] = "P6/mmm"
        if controls[5] in [
            "P4/m",
        ]:
    ...
    def OnIndexPeaks(event):
        ...
        if ssopt.get("Use", False):
            G2frame.ErrorDialog(
                "Super lattice error", "Indexing not available for super lattices"
            )
            return
        ...
        dlg = wx.ProgressDialog(
            ...
            "0 " + " cell search for " + bravaisNames[ibrav],
            101,
            style=wx.PD_ELAPSED_TIME
            | wx.PD_AUTO_HIDE
            | wx.PD_REMAINING_TIME
            | wx.PD_CAN_SKIP
            | wx.PD_CAN_ABORT,
        )  # desn't work in 32 bit versions
        #            style = wx.PD_ELAPSED_TIME|wx.PD_AUTO_HIDE|wx.PD_REMAINING_TIME|wx.PD_CAN_ABORT)
        try:
            OK, dmin, newcells = G2indx.DoIndexPeaks(
                peaks[0], controls, bravais, dlg, G2frame.ifX20
            )
        finally:
            dlg.Destroy()
        ...
        if OK:
            ...
            bestCell = cells[0]
            if bestCell[0] > 10.0:
                G2frame.HKL = G2lat.GenHBravais(
                    dmin, bestCell[2], G2lat.cell2A(bestCell[3:9]), ifList=True
                )
                for hkl in G2frame.HKL:
                ...
                if "PKS" in G2frame.GPXtree.GetItemText(G2frame.PatternId):
                    G2plt.PlotPowderLines(
                        G2frame,
                        indexFrom=f" Indexing from result #0, M20= {cells[0][0]:.3f}",
                    )
                else:
                    G2pwpl.PlotPatterns(
                        G2frame,
            ...
            if cells[0][0] == "?":  # k-vector table
                ...
                if G2frame.kvecSearch["mode"]:
                    ...
                    newPhase, _ = G2lat.TransformPhase(
                        Phase, newPhase, Trans, Uvec, Vvec, False
                    )
                    ...
                    satellite_peaks = sorted(set(satellite_peaks), reverse=True)
        ...
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                G2frame.GPXtree.SetItemPyData(
                    sub,
                    G2obj.SetNewPhase(
                        Name=PhaseName, SGData=SGData, cell=cell[1:], Super=ssopt
                    ),
                )
                G2frame.GetStatusBar().SetStatusText(
                    "Change space group from " + str(controls[13]) + " if needed", 1
                )
        finally:
            dlg.Destroy()
    ...
    def TransformUnitCell(event):
        ...
        try:
            if dlg.ShowModal() == wx.ID_OK:
                newPhase, Trans, Uvec, Vvec, ifMag, ifConstr, Common = (
                    dlg.GetSelection()
                )
            ...
            dlg.Destroy()
        OnHklShow(
            None, indexFrom=" Indexing from transformed unit cell & symmetry settings"
        )
        wx.CallAfter(UpdateUnitCellsGrid, G2frame, data)
    ...
    def OnLatSym(event):
        ...
        controls, bravais, cells, dminx, ssopt, magcells = (
            G2frame.GPXtree.GetItemPyData(pUCid)
        )
        sgNum = bravaisSPG.get(controls[5], 0)
        if sgNum < 1:
            wx.MessageBox(
                "Sorry, only standard cell settings are allowed, please transform axes",
                caption="Bilbao requires standard settings",
                style=wx.ICON_EXCLAMATION,
            )
            return
        cell = controls[6:12]
        ...
        dlg = G2G.SingleFloatDialog(
            G2frame,
            "Tolerance",
            "Enter angular tolerance for search",
            5.0,
            [0.1, 30.0],
            "%.1f",
        )
        ...
        if not page:
            return
        cells.clear()
        for _i, (cell, mat) in enumerate(kSUB.parseBilbaoCheckLattice(page)):
            cells.append([])
            cells[-1] += [mat, 0, 16]
            cells[-1] += cell
    ...
    def OnNISTLatSym(event):
        ...
        tableSizer.Add(
            wx.StaticText(dlg, label="Cell length tolerance (A) "),
            0,
            WACV | wx.ALIGN_LEFT,
        )
        ...
        tableSizer.Add(w)
        tableSizer.Add(
            wx.StaticText(dlg, label="Cell angle tolerance (deg) "),
            0,
            WACV | wx.ALIGN_LEFT,
        )
        ...
        # tableSizer.Add(w)
        tableSizer.Add(
            wx.StaticText(dlg, label="Search mode: Generate "), 0, WACV | wx.ALIGN_LEFT
        )
        tableSizer.Add(
            G2G.EnumSelector(
                dlg,
        ...
        out = nistlat.CellSymSearch(
            cell, center, tolerance=tol, mode=mode, deltaV=delta
        )
        wx.EndBusyCursor()
    ...
    def OnRunSubs(event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if nkvec not in [0, 3, 6, 9]:
                wx.MessageBox(
                    "Error: check your propagation vector(s)",
                    caption="Bilbao SUBGROUPS setup error",
                    style=wx.ICON_EXCLAMATION,
                )
                return
            if nkvec in [6, 9] and Landau:
                wx.MessageBox(
                    "Error, multi k-vectors & Landau not compatible",
                    caption="Bilbao SUBGROUPS setup error",
                    style=wx.ICON_EXCLAMATION,
                )
                return
            ...
            SubGroups, baseList = kSUB.GetNonStdSubgroups(
                SGData, kvec[:9], star, Landau
            )
            wx.EndBusyCursor()
            ...
            if not SubGroups:
                if Landau:
                    wx.MessageBox(
                        "No results from SUBGROUPS, multi k-vectors & Landau not compatible",
                        caption="Bilbao SUBGROUPS error",
                        style=wx.ICON_EXCLAMATION,
                    )
                else:
                    wx.MessageBox(
                        "No results from SUBGROUPS, check your propagation vector(s)",
                        caption="Bilbao SUBGROUPS error",
                        style=wx.ICON_EXCLAMATION,
                    )
                return
            ...
            for ir, result in enumerate(SubGroups):
                ...
                phase["nAtoms"] = len(
                    TestAtoms(
                        phase, controls[15], SGData, Uvec, Trans, maxequiv, maximal
                    )
                )
    ...
    def OnRunSubsMag(event, kvec1=None):
        def strTest(text):
            if "." in text:  # no decimals
                ...
                nums = text.split("/")
                return (0 < int(nums[1]) < 10) and (
                    0 < abs(int(nums[0])) < int(nums[1])
                )
            return False
        ...
        ]  # skip "magnetic" O atoms
        kvec = [["0", "0", "0"], [" ", " ", " "], [" ", " ", " ", " "]]
        msg = (
            f"Run k-SUBGROUPSMAG on space group {SGData['SpGrp']}\n"
            + "For k-vectors, enter rational fraction, 0, -1 or 1; no decimals"
        )
        dlg = G2G.MultiDataDialog(
            G2frame,
            title="k-SUBGROUPSMAG options",
            prompts=[
                " k-vector 1",
                " k-vector 2",
                " k-vector 3",
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if nkvec not in [0, 3, 6, 9]:
                wx.MessageBox(
                    "Error: check your propagation vector(s)",
                    caption="Bilbao k-SUBGROUPSMAG setup error",
                    style=wx.ICON_EXCLAMATION,
                )
                return
            if nkvec in [6, 9] and Landau:
            ...
            MAXMAGN, baseList = kSUB.GetNonStdSubgroupsmag(
                SGData, kvec[:9], star, Landau
            )
            wx.EndBusyCursor()
            if MAXMAGN is None:
                wx.MessageBox(
                    "Check your internet connection?",
                    caption="Bilbao k-SUBGROUPSMAG error",
                    style=wx.ICON_EXCLAMATION,
                )
                return
            if not MAXMAGN:
                if Landau:
                    wx.MessageBox(
                        "No results from k-SUBGROUPSMAG, multi k-vectors & Landau not compatible",
                        caption="Bilbao k-SUBGROUPSMAG error",
                        style=wx.ICON_EXCLAMATION,
                    )
                else:
                    wx.MessageBox(
                        "No results from k-SUBGROUPSMAG, check your propagation vector(s)",
                        caption="Bilbao k-SUBGROUPSMAG error",
                        style=wx.ICON_EXCLAMATION,
                    )
                return
            ...
            try:
                ...
                controls.append(baseList)
            dlg = wx.ProgressDialog(
                "k-SUBGROUPSMAG results",
                "Processing " + MAXMAGN[0][0],
                len(MAXMAGN),
                style=wx.PD_ELAPSED_TIME | wx.PD_AUTO_HIDE | wx.PD_REMAINING_TIME,
            )
            ...
            for ir, result in enumerate(MAXMAGN):
                ...
                phase["nAtoms"] = len(
                    TestMagAtoms(
                        phase, magAtms, SGData, Uvec, Trans, allmom, maxequiv, maximal
                    )
                )
                magcells.append(phase)
            dlg.Destroy()
    ...
    def OnISODISTORT_kvec(phase_nam):  # needs attention from Yuanpeng
        ...
        Phase = G2frame.GPXtree.GetItemPyData(
            G2gd.GetGPXtreeItemId(G2frame, phaseID, phase_nam)
        )
        data = Phase
    ...
    def clearShowFlags():
        ...
        for _i, grid in enumerate(resizeGrids):
            try:
    ...
    def _onResizeUnitCellsList(event):
        ...
        try:
            ...
            for _i, grid in enumerate(resizeGrids):
                ...
                grid.SetMaxSize(
                    (wid - 15, int(hgt / 2.5))
                )  # leave inside on right room for scrollbar
    ...
    def firstSizer():
        ...
        firstSizer.Add(
            wx.StaticText(
                G2frame.dataWindow, label=" Bravais Lattice(s) for autoindexing trials:"
            ),
            0,
        )
        ...
        bravList = []
        bravs = zip(bravais, bravaisNames, strict=False)
        ...
        littleSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" Max Nc/Nobs "), 0, WACV
        )
        ...
        littleSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" Start Volume "), 0, WACV
        )
        startVol = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow, controls, 3, typeHint=int, xmin=25
        )
    ...
    def kvecSizer():
        def OnKvecSearch(event):
            ...
            wx.CallAfter(
                UpdateUnitCellsGrid, G2frame, data, select
            )  # refresh & select 1st result

        kvecSizer = wx.BoxSizer(wx.VERTICAL)
        kvecSizer.Add(
            wx.StaticText(
                parent=G2frame.dataWindow,
                label="k-Vector Search Mode",
                style=wx.ALIGN_CENTER,
            ),
            0,
            wx.EXPAND,
        ...
        if len(Phases) == 0:
            ...
            G2frame.kvecSearch["phase"] = next(iter(Phases.keys()))
        else:
            littleSizer.Add(
        ...
        littleSizer1x.Add(
            wx.StaticText(G2frame.dataWindow, label="Number of processors"), 0, WACV
        )
        ...
        littleSizer2.Add(
            wx.StaticText(G2frame.dataWindow, label="Search tolerance"), 0, WACV
        )
        ...
        ch1 = G2G.EnumSelector(
            G2frame.dataWindow, G2frame.kvecSearch, "soption", search_opts
        )
    ...
    def unitSizer():
        ...
        def OnSelMG(event):
            ...
            OnHklShow(
                event,
                indexFrom=" Indexing from new super group %s"
        ...
        def OnMoveMod(event):
            ...
            move = Obj.GetValue() * inc / 100.0
            Obj.SetValue(0)
            value = min(0.98, max(-0.98, float(valObj.GetValue()) + move))
            valObj.SetValue(f"{value:.4f}")
        ...
        def OnFindOneMV(event):
            ...
            dlg = wx.ProgressDialog(
                "Elapsed time",
                "Modulation vector search",
                style=wx.PD_ELAPSED_TIME | wx.PD_AUTO_HIDE,
            )
            try:
                ssopt["ModVec"], result = G2indx.findMV(
                    Peaks, controls, ssopt, Inst, dlg
                )
                if len(result[0]) == 2:
                    G2plt.PlotXYZ(
                        G2frame,
        ...
        def OnFindMV(event):
            ...
            for ssSym in ssChoice:
                ...
                if len(Vref) > 1:
                    print(f" {ssSym} skipped - too many variables to search this way")
                    continue
                ssopt["ssSymb"] = ssSym
                Peaks = np.copy(peaks[0])
                ssopt["ModVec"] = G2spc.SSGModCheck(
                    ssopt["ModVec"], G2spc.splitSSsym(ssSym)[0], True
                )[0]
                print(" Trying: " + controls[13] + ssSym + " maxH: 1")
                dlg = wx.ProgressDialog(
                    "Elapsed time",
                    f"Modulation vector search with {ssSym}",
            ...
            if bestSS != "":
                ssopt["ssSymb"] = bestSS
                ssopt["ModVec"], result = G2indx.findMV(
                    Peaks, controls, ssopt, Inst, dlg=None
                )
                G2plt.PlotXY(
                    G2frame,
                    [
                    ...
                    labelY="fit",
                    newPlot=True,
                    Title=f"Modulation vector search for {bestSS}",
                )
        ...
        def OnBravSel(event):
            ...
            ssopt["Use"] = False
            OnHklShow(event, indexFrom=f" Indexing from Bravais lattice {brav}")
        ...
        def OnSpcSel(event):
            ...
            # ssopt['Use'] = False
            G2frame.dataWindow.RefineCell.Enable(True)
            OnHklShow(event, indexFrom=f" Indexing from space group {controls[13]}")
            wx.CallLater(100, UpdateUnitCellsGrid, G2frame, data)
        ...
        def OnTryAllSG(event):
            ...
            if ssopt["SgResults"]:
                ssopt["SgResults"] = G2mth.sortArray(
                    ssopt["SgResults"], 2, reverse=True
                )
                ...
                ssopt["SgSettings"] = (
                    "Brav. Lat.: {}, cell: {:.3f} {:.3f} {:.3f} {:.2f} {:.2f} {:.2f}".format(
                        *controls[5:12]
                    )
                )
                OnHklShow(
                    ...
                    False,
                    False,
                    indexFrom=f" Indexing from space group {controls[13]}",
                )
            wx.CallLater(100, UpdateUnitCellsGrid, G2frame, data)
        ...
        def SetCellValue(Obj, ObjId, value):
            ...
            if controls[5] in ["Fm3m", "Im3m", "Pm3m"]:
                ...
                if ObjId < 3:
                    ...
                    setval(controls[6 + ObjId])
            controls[12] = G2lat.calc_V(G2lat.cell2A(controls[6:12]))
            volVal.SetValue(f"{controls[12]:.3f}")
        ...
        def OnMoveCell(event):
            ...
            Obj.SetValue(0)
            value = float(valObj.GetValue()) * (1.0 + move * inc / 100.0)
        ...
        def OnMakePks(event):
            ...
            dlg = wx.MessageDialog(
                G2frame, msg, "Replace peaks?", wx.YES_NO | wx.ICON_QUESTION
            )
            try:
            ...
            G2frame.GPXtree.SetItemPyData(
                G2gd.GetGPXtreeItemId(G2frame, PatternId, "Peak List"), peaks
            )
        ...
        def OnExtHklShow(event):
            ...
            if Obj.GetValue():
                ...
                OnHklShow(
                    event, indexFrom=" Indexing from unit cell & symmetry settings"
                )

        def OnAxHklShow():
            mode = Sel.GetStringSelection()
            if "None" not in mode:
                OnHklShow(None, indexFrom=f" Indexing showing only {mode} hkls")
            else:
                OnHklShow(
                    None, indexFrom=" Indexing from unit cell & symmetry settings"
                )
        ...
        bravSizer.Add(
            wx.StaticText(
                G2frame.dataWindow, label=" Bravais  \n lattice ", style=wx.ALIGN_CENTER
            ),
            0,
            WACV,
            5,
        ...
        bravSizer.Add(
            wx.StaticText(
                G2frame.dataWindow, label=" Space  \n group  ", style=wx.ALIGN_CENTER
            ),
            0,
            WACV,
            5,
        ...
        spcSel = wx.Choice(
            G2frame.dataWindow, choices=SPGlist[controls[5]], size=(100, -1)
        )
        ...
        for txt, fmt, ifEdit, Id in zip(*useGUI[2], strict=False):
            cellSizer.Add(
                wx.StaticText(G2frame.dataWindow, label=txt, style=wx.ALIGN_RIGHT),
                0,
                wx.ALIGN_RIGHT | WACV,
            )
            if ifEdit:  # a,b,c,etc.
        ...
        vcSizer = wx.BoxSizer(wx.VERTICAL)
        vcSizer.Add(
            wx.StaticText(G2frame.dataWindow, label="cell step", style=wx.ALIGN_CENTER),
            0,
            wx.EXPAND,
        )
        ...
        if not ssopt.get("Use", False):  # zero for super lattice doesn't work!
            ...
            vcSizer.Add(
                wx.StaticText(
                    G2frame.dataWindow, label="Zero offset", style=wx.ALIGN_CENTER
                ),
                0,
                wx.EXPAND,
            )
        ...
        if ssopt.get("Use", False):  # super lattice display
            ...
            ssSizer.Add(
                wx.StaticText(
                    G2frame.dataWindow,
                    label=" Supersymmetry space group: " + SpSg + " ",
                ),
                0,
                WACV,
            )
            ...
            ssSizer = wx.BoxSizer(wx.HORIZONTAL)
            ssSizer.Add(
                wx.StaticText(G2frame.dataWindow, label=" Mod. vector: "), 0, WACV
            )
            modS = G2spc.splitSSsym(ssopt["ssSymb"])[0]
            ssopt["ModVec"], ifShow = G2spc.SSGModCheck(ssopt["ModVec"], modS)
            for i, [val, show] in enumerate(zip(ssopt["ModVec"], ifShow, strict=False)):
                if show:
                    valSizer = wx.BoxSizer(wx.HORIZONTAL)
            ...
            ssSizer.Add(
                wx.StaticText(G2frame.dataWindow, label=" in steps of cell step above"),
                0,
                WACV,
            )
            unitSizer.Add(ssSizer, 0)
        ...
        if "E" not in Inst["Type"][0]:
            ...
            if not ssopt.get(
                "Use", False
            ):  # Show Extinct not available for super lattice
                showExt = G2G.G2CheckBox(
                    G2frame.dataWindow,
                    "Show Extinct",
                    cellDisplayOpts,
                    "showExtinct",
                    OnChange=OnExtHklShow,
                )
                unitSizerWidgetList.append(showExt)
                littleSizer.Add(showExt, 0, WACV)
        littleSizer.Add(
            wx.StaticText(
                G2frame.dataWindow, label=" highlight ", style=wx.ALIGN_RIGHT
            ),
            0,
            WACV,
        )
        G2frame.PlotOpts["hklHighlight"] = G2frame.PlotOpts.get("hklHighlight", 0)
        Sel = G2G.G2ChoiceButton(
            G2frame.dataWindow,
            [
                "None",
            ]
            + [c + notEq0 for c in ("h", "k", "l")],
            indLoc=G2frame.PlotOpts,
            indKey="hklHighlight",
            onChoice=OnAxHklShow,
        )
        ...
        if "E" not in Inst["Type"][0]:
            ...
            if len(G2frame.HKL) and "PKS" not in G2frame.GPXtree.GetItemText(
                G2frame.PatternId
            ):
                littleSizer.Add((5, -1))
        ...
        if "N" in Inst["Type"][0] and "MagSpGrp" in SGData:
            ...
            SGData["SGGray"] = False
            neutSizer.Add(
                wx.StaticText(G2frame.dataWindow, label=" BNS lattice: "), 0, WACV
            )
            ...
            for isym, sym in enumerate(GenSym[1:]):
                neutSizer.Add(
                    wx.StaticText(G2frame.dataWindow, label=f" {sym.strip()}: "),
                    0,
                    WACV,
                )
                spinOp = wx.ComboBox(
    ...
    def SpGrpGrid():
        def OnSelectSgrp(event):
            ...
            OnHklShow(
                event,
                True,
                indexFrom=" Space group selection %s #%d" % (controls[13], r),
            )

        SpGrpGrid = wx.BoxSizer(wx.VERTICAL)
        lbl = (
            ' Space group search results from "Try all"'
            + "\n  "
    ...
    def SSGrid():
        def OnSelectSSG(event):
            ...
            OnHklShow(
                event,
                True,
                indexFrom=" Super Space group selection %s #%d" % (controls[13], r),
            )

        SSGrpGrid = wx.BoxSizer(wx.VERTICAL)
        lbl = (
            ' Super Space group search results from "Try all"'
            + "\n  "
            + ssopt.get("SgSettings", "")
        )
    ...
    def MagSubGrid():
        ...
        KeyList += [
            [
                "j",
                ClearCurrentShowNext,
                "Show next Mag. Spc. Group, clear keep flag on current",
            ]
        ]
        ...
        KeyList += [
            ["k", KeepCurrentShowNext, "Show next Mag. Spc. Group, keep current"]
        ]
        ...
        def KeepShowNext(KeepCurrent=True):
            ...
            yscroll = (
                magDisplay.CellToRect(next, 1)[1]
                / G2frame.dataWindow.GetScrollPixelsPerUnit()[1]
            )
            ...
            if magcells:
                if c == 0:
                    ...
                    if "magAtms" in phase:
                        ...
                        magAtms = phase.get("magAtms", "")
                        mAtoms = TestMagAtoms(
                            phase, magAtms, SGData, Uvec, Trans, allmom, maxequiv
                        )
                    else:
                        mAtoms = TestAtoms(
                            phase, controls[15], SGData, Uvec, Trans, maxequiv
                        )
                    Atms = []
                    AtCods = []
                    atMxyz = []
                    for ia, atom in enumerate(mAtoms):
                        ...
                        if "magAtms" in phase:
                            ...
                            atMxyz.append([SytSym, CSI[0]])
                    G2phsG.UseMagAtomDialog(
                        G2frame, pname, Atms, AtCods, atMxyz, ifMag=ifMag, ifOK=True
                    ).ShowModal()
                elif c in [4, 5]:
                    if "altList" not in phase:
                        return
                    if c == 4:
                        title = "Conjugacy list for " + pname
                        items = phase["altList"]
                    ...
                    Pick = -1
                    dlg = SubCellsDialog(
                        G2frame, title, controls, SGData, items, phaseDict, ifPick
                    )
                    try:
                        if dlg.ShowModal() == wx.ID_OK:
        ...
        def OnRefreshKeep(event):
            ...
            E, SGData = G2spc.SpcGroup(controls[13])
            if c == 2:
                testAtoms = ["", *list({atom[1] for atom in controls[15]})]
                ifMag = False
                ...
                if "magAtms" in magcells[0]:
                    ...
                    dlg = G2G.MultiDataDialog(
                        ...
                        values=[maxequiv, "", allmom, False],
                        limits=[[1, 100], testAtoms, [True, False], [True, False]],
        ...
        phaseDict = dict(zip(itemList, magcells, strict=False))
        G2frame.dataWindow.CopyCell.Enable(False)
        ...
        G2frame.GetStatusBar().SetStatusText(
            "Double click Keep to refresh Keep flags; click Space Gp to see sym. ops., Uniq to see unique atoms list; Try to trigger K & J keys on plot",
            1,
        )
    ...
    if peaks:
        G2frame.dataWindow.IndexPeaks.Enable(True)
    G2frame.dataWindow.RefineCell.Enable(False)
    if controls[12] > 1.0 and len(
        peaks[0]
    ):  # if a "real" volume (i.e. not default) and peaks
        G2frame.dataWindow.RefineCell.Enable(True)
    G2frame.dataWindow.CopyCell.Enable(False)
    ...
    if cells:
        ...
        if mode == 2:
            ...
            mainSizer.Add(
                wx.StaticText(
                    parent=G2frame.dataWindow, label="\n k-vector search results:"
                )
            )
        elif mode == 1:
            mainSizer.Add(
            ...
            mainSizer.Add(
                wx.StaticText(parent=G2frame.dataWindow, label="\n Indexing Result:")
            )
...
def UpdateReflectionGrid(G2frame, data, HKLF=False, Name=""):
    ...
    def OnPlot1DHKL(event):
        ...
        elif "RefList" in data[phaseName]:
            refList = np.array(data[phaseName]["RefList"])
        else:
            wx.MessageBox(
                "No reflection list - do Refine first", caption="Reflection plotting"
            )
            return
        G2plt.Plot1DSngl(
    ...
    def OnPlot3DHKL(event):
        ...
        if "list" in str(type(data)):  # single crystal data is 2 dict in list
            ...
            return
        refList.T[3 + Super] = np.where(
            refList.T[4 + Super] < dMin, -refList.T[3 + Super], refList.T[3 + Super]
        )
        FoMax = np.max(refList.T[8 + Super])
        Hmin = np.array(
            [
    ...
    def OnWilsonStat(event):
        ...
        print(
            f" Wilson statistics   : <|E|>: {Estat[0] / Estat[1]:.3f}, <|E^2|>: {1.0:.3f}, <|E^2-1|>: {Estat[2] / Estat[1]:.3f}"
        )
        ...
        G2plt.PlotXY(
            G2frame,
            XY,
            labelX=f"sin$^2${GkTheta}/{Gklambda}$^2$",
            labelY=rf"ln(<|F$_o$|$^2$>/{GkSigma}f$^2$)",
            newPlot=True,
            Title="Wilson plot",
        )
    ...
    def OnMakeCSV(event):
        ...
        dlg = wx.FileDialog(
            G2frame,
            "Choose Reflection List csv file",
            pth,
            "",
            "Reflection List (*.csv)|*.csv",
            wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT,
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
    ...
    def MakeReflectionTable(phaseName):
            ...
            if pId:  # phase section missing from file (unusual)
                ...
                if phaseId:  # is phase deleted?
                    ...
                    if General.get("Modulated", False):
                        Super = 1
                    try:
                        Histograms = G2frame.GPXtree.GetItemPyData(phaseId)[
                            "Histograms"
                        ]
                        histName = G2frame.GPXtree.GetItemText(G2frame.PatternId)
        ...
        if HKLF:
            ...
            try:  # patch for old reflection lists
                ...
                I100 = refList.T[8 + Super] * np.array(
                    [refl[11 + Super] for refl in data[phaseName]]
                )
            ...
            if "C" in Inst["Type"][0]:
                ...
                refs = np.vstack(
                    (refList.T[: 12 + Super], I100, MuStr, CrSize)
                ).T  # last two not shown for now
        ...
        if HKLF:
            ...
            if Super:
                colLabels.insert(3, "M")
        refs.T[3 + Super] = np.where(
            refs.T[4 + Super] < dMin, -refs.T[3 + Super], refs.T[3 + Super]
        )
    ...
    def ShowReflTable(phaseName):
        ...
        def setBackgroundColors(im, it):
            for r in range(G2frame.refTable[phaseName].GetNumberRows()):
                if HKLF:
                    if (
                        float(G2frame.refTable[phaseName].GetCellValue(r, 3 + im))
                        <= 0.0
                    ):
                    ...
                    if rat > 10.0:
                        ...
                        G2frame.refTable[phaseName].SetCellBackgroundColour(
                            r, 7 + im, wx.Colour(255, 255, 0)
                        )
                else:  # PWDR
                    if (
                        float(
                            G2frame.refTable[phaseName].GetCellValue(r, 12 + im + itof)
                        )
                        < 0.0
                    ):
                        G2frame.refTable[phaseName].SetCellBackgroundColour(
        ...
        if HKLF:
            G2frame.GetStatusBar().SetStatusText(
                "abs(DF)/sig > 10 red; > 3 yellow; flag:>0 twin no., 0 sp.gp absent, -1 user rejected, -2 Rfree",
                1,
            )
        else:
            G2frame.GetStatusBar().SetStatusText(
                "Prfo < 0. in red; if excluded Fosq in red & mul < 0", 1
            )
        itof = 0
        if HKLF:
        ...
        if HKLF:
            if G2frame.Hide:
                refList = np.array(
                    [refl[: 6 + im] for refl in data[1]["RefList"] if refl[3]]
                )
            else:
    ...
    G2frame.refTable = {}
    G2frame.dataWindow.currentGrids = []
    for _tabnum, phase in enumerate(phases):
        if isinstance(data, list):  # single crystal HKLF
            ...
            G2frame.refTable[phase].SetScrollRate(
                10, 10
            )  # reflection grids (inside tab) need scroll bars
...
def UpdateSubstanceGrid(G2frame, data):
    ...
    def LoadSubstance(name):
        ...
        for El in ElList:
            ...
            if "Volume" in subst:
                ...
                data["Substances"][name]["Density"] = G2mth.Vol2Den(
                    data["Substances"][name]["Elements"],
                    data["Substances"][name]["Volume"],
                )
            elif "Density" in subst:
                data["Substances"][name]["Density"] = subst["Density"]
                data["Substances"][name]["Volume"] = G2mth.Den2Vol(
                    data["Substances"][name]["Elements"],
                    data["Substances"][name]["Density"],
                )
            else:
                data["Substances"][name]["Volume"] = G2mth.El2EstVol(
                    data["Substances"][name]["Elements"]
                )
                data["Substances"][name]["Density"] = G2mth.Vol2Den(
                    data["Substances"][name]["Elements"],
            ...
            if "X" in Inst["Type"][0]:
                data["Substances"][name]["Scatt density"] = G2mth.XScattDen(
                    data["Substances"][name]["Elements"],
                    data["Substances"][name]["Volume"],
                )[0]
                recontrst, absorb, imcontrst = G2mth.XScattDen(
                    data["Substances"][name]["Elements"],
                    data["Substances"][name]["Volume"],
                    wave,
                )
                ...
                data["Substances"][name]["Scatt density"] = G2mth.NCScattDen(
                    data["Substances"][name]["Elements"],
                    data["Substances"][name]["Volume"],
                )[0]
                recontrst, absorb, imcontrst = G2mth.NCScattDen(
                    data["Substances"][name]["Elements"],
                    data["Substances"][name]["Volume"],
                    wave,
                )
    ...
    def OnLoadSubstance(event):
        ...
        names.sort()
        dlg = wx.SingleChoiceDialog(
            G2frame, "Which substance?", "Select substance", names, wx.CHOICEDLG_STYLE
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
    ...
    def AddElement(name):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            data["Substances"][name]["Density"] = G2mth.Vol2Den(
                data["Substances"][name]["Elements"], data["Substances"][name]["Volume"]
            )
            if "X" in Inst["Type"][0]:
                data["Substances"][name]["Scatt density"] = G2mth.XScattDen(
                    data["Substances"][name]["Elements"],
    ...
    def SubstSizer():
        ...
        def OnVolDen(invalid, value, tc):
            ...
            if keyId in "Volume":
                data["Substances"][name]["Density"] = G2mth.Vol2Den(
                    data["Substances"][name]["Elements"], value
                )
        ...
        substSizer.Add(
            wx.StaticText(
                parent=G2frame.dataWindow,
                label=f" Substance list: wavelength: {wave:.5f}A",
            )
        )
        for name in data["Substances"]:
            ...
            substSizer.Add(
                wx.StaticText(
                    parent=G2frame.dataWindow, label=" Data for " + name + ":"
                ),
                0,
            )
            if name == "vacuum":
                substSizer.Add(
                    wx.StaticText(
                        parent=G2frame.dataWindow, label="        Not applicable"
                    ),
                    0,
                )
                ...
                for El in Elems:  # do elements as pull downs for isotopes for neutrons
                    elSizer.Add(
                        wx.StaticText(parent=G2frame.dataWindow, label=" " + El + ": "),
                        0,
                        WACV,
                    )
                    num = G2G.ValidatedTxtCtrl(
                        G2frame.dataWindow,
                        data["Substances"][name]["Elements"][El],
                        "Num",
                        nDig=(10, 2, "f"),
                        typeHint=float,
                        OnLeave=OnNum,
                    ...
                    if "N" in Inst["Type"][0]:
                        elSizer.Add(
                            wx.StaticText(G2frame.dataWindow, label=" Isotope: "),
                            0,
                            WACV,
                        )
                        ...
                        isotope = wx.ComboBox(
                            G2frame.dataWindow,
                            choices=isotopes,
                            value=Elems[El].get("Isotope", "Nat. Abund."),
                            style=wx.CB_READONLY | wx.CB_DROPDOWN,
                        )
                ...
                vdsSizer.Add(
                    wx.StaticText(parent=G2frame.dataWindow, label=" Volume: "), 0, WACV
                )
                vol = G2G.ValidatedTxtCtrl(
                    G2frame.dataWindow,
                    data["Substances"][name],
                    "Volume",
                    nDig=(10, 2),
                ...
                vdsSizer.Add(
                    wx.StaticText(parent=G2frame.dataWindow, label=" Density: "),
                    0,
                    WACV,
                )
                den = G2G.ValidatedTxtCtrl(
                    G2frame.dataWindow,
                    data["Substances"][name],
                    "Density",
                    nDig=(10, 2),
                ...
                denSizer.Add(
                    wx.StaticText(G2frame.dataWindow, label=" Scattering density,f"),
                    0,
                    WACV,
                )
                denSizer.Add(
                ...
                denSizer.Add(
                    wx.StaticText(G2frame.dataWindow, label=" Real density,f+f'"),
                    0,
                    WACV,
                )
                denSizer.Add(
                ...
                denSizer.Add(
                    wx.StaticText(G2frame.dataWindow, label=' Imaginary density,f"'),
                    0,
                    WACV,
                )
                denSizer.Add(
                ...
                denSizer.Add(
                    wx.StaticText(G2frame.dataWindow, label=" Absorption"), 0, WACV
                )
                denSizer.Add(
                    wx.StaticText(
                        G2frame.dataWindow,
...
def UpdateModelsGrid(G2frame, data):
    ...
    if "MinMaxDiam" in data["Size"]:
        data["Size"]["MinDiam"] = 50.0
        data["Size"]["MaxDiam"] = 10000.0
    ...
    def OnCopyModel(event):
        ...
        copyList = []
        dlg = G2G.G2MultiChoiceDialog(
            G2frame, "Copy models from\n" + hst[5:] + " to...", "Copy models", histList
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
                for i in dlg.GetSelections():
        ...
        for item in copyList:
            ...
            G2frame.GPXtree.SetItemPyData(
                G2gd.GetGPXtreeItemId(G2frame, Id, "Models"), newdata
            )
            if newdata["BackFile"]:
                Profile = G2frame.GPXtree.GetItemPyData(Id)[1]
                BackId = G2gd.GetGPXtreeItemId(
                    G2frame, G2frame.root, newdata["BackFile"]
                )
                BackSample = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, BackId, "Sample Parameters")
                )
                Profile[5] = (
                    BackSample["Scale"][0] * G2frame.GPXtree.GetItemPyData(BackId)[1][1]
                )
        ...
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                for i in result:
                    item = histList[i]
                    Id = G2gd.GetGPXtreeItemId(G2frame, G2frame.root, item)
                    newModel = G2frame.GPXtree.GetItemPyData(
                        G2gd.GetGPXtreeItemId(G2frame, Id, "Models")
                    )
                    ...
                    for ilev, level in enumerate(newModel["Particle"]["Levels"]):
                        for form in level:
                            if form in distChoice:
                                ...
                                for item in parmOrder:
                                    if item in thisForm:
                                        level[form]["SFargs"][item][1] = copy.copy(
                                            thisForm[item][1]
                                        )
        finally:
            dlg.Destroy()
    ...
    def OnFitModelAll(event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if Id:
                ...
                SeqResult = {}
                Id = G2frame.GPXtree.AppendItem(
                    parent=G2frame.root, text="Sequential SASD results"
                )
            SeqResult = {"SeqPseudoVars": {}, "SeqParFitEqList": []}
            SeqResult["histNames"] = names
        else:
        ...
        try:
            for i, name in enumerate(names):
                ...
                if i and od["value_1"]:
                    G2frame.GPXtree.SetItemPyData(
                        G2gd.GetGPXtreeItemId(G2frame, sId, "Models"), JModel
                    )
                IProfDict, IProfile = G2frame.GPXtree.GetItemPyData(sId)[:2]
                IModel = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, sId, "Models")
                )
                ISample = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, sId, "Sample Parameters")
                )
                ILimits = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, sId, "Limits")
                )
                IfOK, result, varyList, sig, Rvals, covMatrix, parmDict, Msg = (
                    G2sasd.ModelFit(IProfile, IProfDict, ILimits, ISample, IModel)
                )
                JModel = copy.deepcopy(IModel)
                if not IfOK:
                    G2frame.ErrorDialog(
                        "Failed sequential refinement for data " + name,
                        " Msg: "
                        + Msg
                        + "\nYou need to rethink your selection of parameters\n"
                        + " Model restored to previous version for"
                        + name,
                    )
                    ...
                    break
                G2frame.GPXtree.SetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, sId, "Models"), copy.deepcopy(IModel)
                )
    ...
    def OnFitModel(event):
        if data["Current"] == "Size dist.":
            if not any(Sample["Contrast"]):
                G2frame.ErrorDialog(
                    "No contrast; your sample is a vacuum!",
            ...
            data["Pair"]["Result"] = G2shapes.G2shapes(
                Profile, ProfDict, Limits, data, dlg
            )
            wx.CallAfter(UpdateModelsGrid, G2frame, data)
    ...
    def SizeSizer():
        ...
        sizeSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" Size distribution parameters: "),
            0,
        )
        binSizer = wx.FlexGridSizer(0, 7, 5, 5)
        binSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" No. size bins: "), 0, WACV
        )
        bins = ["50", "100", "150", "200"]
        nbins = wx.ComboBox(
            G2frame.dataWindow,
            value=str(data["Size"]["Nbins"]),
            choices=bins,
            style=wx.CB_READONLY | wx.CB_DROPDOWN,
        )
        ...
        partSizer.Add(
            wx.StaticText(G2frame.dataWindow, label="Particle description: "), 0, WACV
        )
        shapes = {
            "Spheroid": " Aspect ratio: ",
            "Cylinder": " Diameter ",
            "Cylinder AR": " Aspect ratio: ",
            "Unified sphere": "",
            "Unified rod": " Diameter: ",
            "Unified rod AR": " Aspect ratio: ",
            "Unified disk": " Thickness: ",
            "Spherical shell": " Shell thickness",
        }
        partsh = wx.ComboBox(
            G2frame.dataWindow,
            value=str(data["Size"]["Shape"][0]),
            choices=list(shapes.keys()),
            style=wx.CB_READONLY | wx.CB_DROPDOWN,
        )
        partsh.Bind(wx.EVT_COMBOBOX, OnShape)
        partSizer.Add(partsh, 0, WACV)
        if data["Size"]["Shape"][0] not in [
            "Unified sphere",
        ]:
            partSizer.Add(
                wx.StaticText(
                    G2frame.dataWindow, label=shapes[data["Size"]["Shape"][0]]
                ),
                0,
                WACV,
            )
            partprm = G2G.ValidatedTxtCtrl(
                G2frame.dataWindow,
                data["Size"]["Shape"],
                1,
                nDig=(10, 3),
        ...
        fitSizer.Add(
            wx.StaticText(G2frame.dataWindow, label="Fitting method: "), 0, WACV
        )
        method = wx.ComboBox(
            G2frame.dataWindow,
            value=data["Size"]["Method"],
            choices=methods,
            style=wx.CB_READONLY | wx.CB_DROPDOWN,
        )
        ...
        fitSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" No. iterations: "), 0, WACV
        )
        Method = data["Size"]["Method"]
        iter = wx.ComboBox(
            G2frame.dataWindow,
            value=str(data["Size"][Method]["Niter"]),
            choices=iters,
            style=wx.CB_READONLY | wx.CB_DROPDOWN,
        )
        ...
        if "MaxEnt" in data["Size"]["Method"]:
            fitSizer.Add(
                wx.StaticText(G2frame.dataWindow, label=" Log floor factor: "), 0, WACV
            )
            floors = [str(-i) for i in range(9)]
            floor = wx.ComboBox(
                G2frame.dataWindow,
                value=str(data["Size"]["MaxEnt"]["Sky"]),
                choices=floors,
                style=wx.CB_READONLY | wx.CB_DROPDOWN,
            )
            ...
            fitSizer.Add(
                wx.StaticText(
                    G2frame.dataWindow, label=" Q power weight (-1 for sigma): "
                ),
                0,
                WACV,
            )
    ...
    def PairSizer():
        ...
        pairSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" Pair distribution parameters: "),
            0,
        )
        ...
        if data["Pair"]["Method"] == "Moore":
            fitSizer.Add(
                wx.StaticText(
                    G2frame.dataWindow,
                    label=" P.B. Moore, J. Appl. Cryst., 13, 168-175 (1980)",
                ),
                0,
                WACV,
            )
            ...
            fitSizer.Add(
                wx.StaticText(
                    G2frame.dataWindow,
                    label=" D.I. Svergun, J. Appl. Cryst., 24, 485-492 (1991)",
                ),
                0,
                WACV,
            )
        ...
        if "Moore" in data["Pair"]["Method"]:
            ...
            mooreSizer.Add(
                wx.StaticText(G2frame.dataWindow, label="Number of functions: "),
                0,
                WACV,
            )
            moore = G2G.ValidatedTxtCtrl(
                G2frame.dataWindow, data["Pair"], "Moore", xmin=2, xmax=20
            )
        ...
        errorSizer.Add(
            wx.StaticText(G2frame.dataWindow, label="Error method: "), 0, WACV
        )
        errors = ["User", "Sqrt", "Percent"]
        error = wx.ComboBox(
            G2frame.dataWindow,
            value=data["Pair"]["Errors"],
            choices=errors,
            style=wx.CB_READONLY | wx.CB_DROPDOWN,
        )
        error.Bind(wx.EVT_COMBOBOX, OnError)
        if "Percent" in data["Pair"]["Errors"]:
            percent = G2G.ValidatedTxtCtrl(
                G2frame.dataWindow,
                data["Pair"],
                "Percent error",
                xmin=0.5,
    ...
    def ShapesSizer():
        ...
        # 1st row
        parmSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" No. amino acids: "), 0, WACV
        )
        numAA = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow, data["Shapes"], "NumAA", xmin=10
        )
        parmSizer.Add(numAA, 0, WACV)
        parmSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" Nballs=no. amino acids*"), 0, WACV
        )
        scaleAA = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            data["Shapes"],
            "AAscale",
            xmin=0.01,
        ...
        # 2nd row
        parmSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" Inflate by (1.-1.4): "), 0, WACV
        )
        inflate = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            data["Shapes"],
            "inflateV",
            xmin=1.0,
        ...
        parmSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" Axial symmetry (1-12): "), 0, WACV
        )
        symm = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow, data["Shapes"], "Symm", xmin=1, xmax=12
        )
        parmSizer.Add(symm, 0, WACV)
        # 3rd row
        parmSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" z-axis bias (-2 to 2): "), 0, WACV
        )
        zaxis = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            data["Shapes"],
            "bias-z",
            xmin=-2.0,
        ...
        parmSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" elongation (0-20): "), 0, WACV
        )
        glue = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            data["Shapes"],
            "AAglue",
            xmin=0.0,
        ...
        # 4th row
        parmSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" No. iterations (1-10): "), 0, WACV
        )
        niter = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow, data["Shapes"], "Niter", xmin=1, xmax=10
        )
        parmSizer.Add(niter, 0, WACV)
        parmSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" Output name: "), 0, WACV
        )
        name = G2G.ValidatedTxtCtrl(G2frame.dataWindow, data["Shapes"], "outName")
        parmSizer.Add(name, 0, WACV)
        # last row
        parmSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" Bead separation (3.5-5): "),
            0,
            WACV,
        )
        beadsep = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            data["Shapes"],
            "boxStep",
            xmin=3.5,
        ...
        if len(data["Pair"].get("Result", [])):
            shapeSizer.Add(
                wx.StaticText(G2frame.dataWindow, label=" SHAPES run results:"), 0
            )
            ...
            shapeTable = G2G.Table(
                tableVals, rowLabels=rowLabels, colLabels=colLabels, types=Types
            )
    ...
    def PartSizer():
        FormFactors = {
            "Sphere": {},
            "Spheroid": {"Aspect ratio": [1.0, False]},
            "Cylinder": {"Length": [100.0, False]},
            "Cylinder diam": {"Diameter": [100.0, False]},
            "Cylinder AR": {"Aspect ratio": [1.0, False]},
            "Unified sphere": {},
            "Unified rod": {"Length": [100.0, False]},
            "Unified rod AR": {"Aspect ratio": [1.0, False]},
            "Unified disk": {"Thickness": [100.0, False]},
            "Unified tube": {"Length": [100.0, False], "Thickness": [10.0, False]},
            "Spherical shell": {"Shell thickness": [1.5, False]},
        }

        StructureFactors = {
            "Dilute": {},
            "Hard sphere": {"VolFr": [0.1, False], "Dist": [100.0, False]},
            "Sticky hard sphere": {
                "VolFr": [0.1, False],
                "Dist": [100.0, False],
                "epis": [0.05, False],
            ...
            "InterPrecipitate": {"VolFr": [0.1, False], "Dist": [100.0, False]},
        }
        ...
        def OnParmSlider(event):
            ...
            if key in ["P", "epis", "Sticky", "Depth", "Width", "VolFr", "Dist"]:
                ...
                value = 10.0 ** float(slide / slMult)
            item[key][0] = value
            pvObj.SetValue(f"{item[key][0]:.3g}")
            G2sasd.ModelFxn(Profile, ProfDict, Limits, Sample, data)
            RefreshPlots(True)
        ...
        def SizeSizer():
            ...
            sizeSizer.Add(
                wx.StaticText(G2frame.dataWindow, label=" Distribution: "), 0, WACV
            )
            Distchoice = [
                "LogNormal",
                "Gaussian",
            ...
            distChoice = wx.ComboBox(
                G2frame.dataWindow,
                value=level["Controls"]["DistType"],
                choices=Distchoice,
                style=wx.CB_READONLY | wx.CB_DROPDOWN,
            )
            ...
            if level["Controls"]["DistType"] not in [
                "Bragg",
                "Unified",
                "Porod",
            ]:
                sizeSizer.Add(
                    wx.StaticText(G2frame.dataWindow, label=" Form Factor: "), 0, WACV
                )
                if "Mono" in level["Controls"]["DistType"]:
                    ffChoice = wx.ComboBox(
                        G2frame.dataWindow,
                        value=level["Controls"]["FormFact"],
                        choices=ffMonoChoices,
                        style=wx.CB_READONLY | wx.CB_DROPDOWN,
                    )
                else:
                    ffChoice = wx.ComboBox(
                ...
                rho = Substances["Substances"][level["Controls"]["Material"]].get(
                    "XAnom density", 0.0
                )
                level["Controls"]["Contrast"] = contrast = (rho - rhoMat) ** 2
                sizeSizer.Add(
                    wx.StaticText(
                        G2frame.dataWindow, label=" Resonant X-ray contrast: "
                    ),
                    0,
                    WACV,
                )
                sizeSizer.Add(
                    wx.StaticText(
                        G2frame.dataWindow,
                        label=f"  {contrast:.2f} 10{Pwr20}cm{Pwrm4}",
                    ),
                    0,
                    WACV,
                )
                if "Mono" not in level["Controls"]["DistType"]:
                    sizeSizer.Add(
                        wx.StaticText(G2frame.dataWindow, label=" Num. radii: "),
                        0,
                        WACV,
                    )
                    ...
                    nRadii = wx.ComboBox(
                        G2frame.dataWindow,
                        value=str(level["Controls"]["NumPoints"]),
                        choices=radii,
                        style=wx.CB_READONLY | wx.CB_DROPDOWN,
                    )
                    ...
                    sizeSizer.Add(
                        wx.StaticText(G2frame.dataWindow, label=" R dist. cutoff: "),
                        0,
                        WACV,
                    )
                    rCutoff = G2G.ValidatedTxtCtrl(
                        G2frame.dataWindow,
                        level["Controls"],
                        "Cutoff",
                        xmin=0.001,
            ...
            elif level["Controls"]["DistType"] in [
                "Unified",
            ]:
                Parms = level["Unified"]
                ...
                sizeSizer.Add(
                    wx.StaticText(
                        G2frame.dataWindow, label=f" Estimated Dist B: {Best:12.4g}"
                    ),
                    0,
                    WACV,
                )
            ...
            for parm in parmOrder:
                if parm in Parms:
                    if parm == "MinSize":
                        ...
                        parmVar = wx.CheckBox(
                            G2frame.dataWindow, label="Refine? Dist " + parm
                        )
                    ...
                    parmValue = wx.TextCtrl(
                        G2frame.dataWindow,
                        value=f"{Parms[parm][0]:.3g}",
                        style=wx.TE_PROCESS_ENTER,
                    )
                    ...
                    parmSldr = G2G.G2Slider(
                        G2frame.dataWindow,
                        minValue=slMult * valMinMax[0],
                        maxValue=slMult * valMinMax[1],
                        value=slMult * value,
                    )
                    ...
                    for parm in parmOrder:
                        if parm in Args:
                            ...
                            if parm == "epis":
                                ...
                                valMinMax = [100.0, 1000.0]
                            elif parm == "VolFr":
                                valMinMax = [1.0e-4, 1.0]
                            else:
        ...
        topSizer = wx.BoxSizer(wx.HORIZONTAL)
        topSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" Particle fit parameters: "),
            0,
            WACV,
        )
        ...
        volfrac = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow, data["Particle"]["Matrix"]["VolFrac"], 0, typeHint=float
        )
        ...
        for ilev, level in enumerate(data["Particle"]["Levels"]):
            ...
            if level["Controls"]["DistType"] not in [
                ...
                topLevel.Add(
                    wx.StaticText(G2frame.dataWindow, label=" Structure factor: "),
                    0,
                    WACV,
                )
                strfctr = wx.ComboBox(
                    G2frame.dataWindow,
                    value=level["Controls"]["StrFact"],
                    choices=sfChoices,
                    style=wx.CB_READONLY | wx.CB_DROPDOWN,
                )
    ...
    def OnEsdScale(event):
        ...
        try:
            ...
            value = 1.0 / np.sqrt(ProfDict["wtFactor"])
        ProfDict["wtFactor"] = 1.0 / value**2
    ...
    topSizer.Add(
        wx.StaticText(G2frame.dataWindow, label=" Error multiplier: "), 0, WACV
    )
    #        azmthOff = G2G.ValidatedTxtCtrl(G2frame.dataWindow,data,'azmthOff',nDig=(10,2),typeHint=float,OnLeave=OnAzmthOff)
    ...
    topSizer.Add((-1, -1), 1, wx.EXPAND)
    topSizer.Add(
        G2G.HelpButton(G2frame.dataWindow, helpIndex=G2frame.dataWindow.helpKey)
    )
    ...
    if "Size" in data["Current"]:
        ...
        if "MaxEnt" in data["Size"]["Method"]:
            G2frame.GetStatusBar().SetStatusText(
                "Size distribution by Maximum entropy", 1
            )
        elif "IPG" in data["Size"]["Method"]:
            G2frame.GetStatusBar().SetStatusText(
                "Size distribution by Interior-Point Gradient", 1
            )
    ...
    backVal = G2G.ValidatedTxtCtrl(
        G2frame.dataWindow, data["Back"], 0, nDig=(10, 3, "g"), OnLeave=OnBackChange
    )
...
def UpdateREFDModelsGrid(G2frame, data):
    ...
    def OnModelPlot(event):
        ...
        plotList = []
        od = {
            "label_1": "Zero at substrate",
            "value_1": False,
            "label_2": "Show layer transitions",
            "value_2": True,
        }
        dlg = G2G.G2MultiChoiceDialog(
            G2frame,
            "Plot reflectivity models for:",
            "Plot SLD models",
            histList,
            extraOpts=od,
        )
        ...
        for item in plotList:
            ...
            model = G2frame.GPXtree.GetItemPyData(
                G2gd.GetGPXtreeItemId(G2frame, mId, "Models")
            )
            if len(model["Layers"]) == 2:  # see if any real layers defined; will be > 2
                continue
            Substances = G2frame.GPXtree.GetItemPyData(
                G2gd.GetGPXtreeItemId(G2frame, mId, "Substances")
            )["Substances"]
            ...
            if od["value_2"]:
                ...
                for ilay, lay in enumerate(
                    np.fromstring(data["Layer Seq"], dtype=int, sep=" ")
                ):
                    linePos[ilay + 1 :] += model["Layers"][lay].get(
                        "Thick", [0.0, False]
                    )[0]
                if od["value_1"]:
        ...
        G2plt.PlotXY(
            ...
            XY,
            labelX=disLabel,
            labelY=r"$SLD,\ 10^{10}cm^{-2}$",
            newPlot=True,
            Title="Scattering length density",
            lines=True,
            names=[],
            vertLines=LinePos,
        )
    ...
    def OnFitModelAll(event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if Id:
                ...
                SeqResult = {}
                Id = G2frame.GPXtree.AppendItem(
                    parent=G2frame.root, text="Sequential REFD results"
                )
            SeqResult = {"SeqPseudoVars": {}, "SeqParFitEqList": []}
            SeqResult["histNames"] = names
        else:
        ...
        try:
            for i, name in enumerate(names):
                ...
                ISubstances = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, sId, "Substances")
                )["Substances"]
                ILimits = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, sId, "Limits")
                )
                IfOK, result, varyList, sig, Rvals, covMatrix, parmDict, Msg = (
                    G2pwd.REFDRefine(
                        IProfile, IProfDict, Inst, ILimits, ISubstances, IModel
                    )
                )
                JModel = copy.deepcopy(IModel)
                if not IfOK:
    ...
    def ModelPlot(data, x, xr, y):
        ...
        for ilay, lay in enumerate(
            ...
            linePos[ilay + 1 :] += data["Layers"][lay].get("Thick", [0.0, False])[0]
        if data["Zero"] == "Top":
            XY = [
                [x, y],
            ]
    ...
    def SaveState():
        ...
        pickle.dump(
            G2frame.GPXtree.GetItemPyData(
                G2gd.GetGPXtreeItemId(G2frame, PatternId, "Models")
            ),
            file,
            1,
        )
    ...
    def ControlSizer():
        ...
        minimiz.Add(
            wx.StaticText(G2frame.dataWindow, label=" Bounds factor: "), 0, WACV
        )
        minimiz.Add(
            G2G.ValidatedTxtCtrl(
                G2frame.dataWindow, data, "Toler", nDig=(10, 2), xmax=0.99, xmin=0.1
            ),
            0,
            WACV,
        )
        ...
        plotSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" Plot controls: "), 0, WACV
        )
        sld = wx.Button(G2frame.dataWindow, label="Plot SLD?")
        sld.Bind(wx.EVT_BUTTON, OnSLDplot)
        plotSizer.Add(sld, 0, WACV)
        plotSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" Zero position location: "),
            0,
            WACV,
        )
        poslist = ["Top", "Bottom"]
    ...
    def OverallSizer():
        ...
        overall.Add(
            wx.StaticText(G2frame.dataWindow, label=" Select slider range 0-"), 0, WACV
        )
        choice = ["1000", "2000", "5000", "10000"]
        slidermax = wx.ComboBox(
            G2frame.dataWindow,
            value="%d" % data["slider max"],
            choices=choice,
            style=wx.CB_READONLY | wx.CB_DROPDOWN,
        )
    ...
    def LayerSizer():
        ...
        def OnSelect(event):
            ...
            if "Rough" not in data["Layers"][item]:
                data["Layers"][item]["Rough"] = [0.0, False]
            if "Thick" not in data["Layers"][item]:
                data["Layers"][item]["Thick"] = [10.0, False]
            if "N" in Inst["Type"][0]:
                data["Layers"][item]["Mag SLD"] = [0.0, False]
            if Name == "unit scatter":
                data["Layers"][item]["iDenMul"] = [0.0, False]
            G2pwd.REFDModelFxn(Profile, Inst, Limits, Substances, data)
            G2pwpl.PlotPatterns(G2frame, plotType="REFD")
            wx.CallAfter(UpdateREFDModelsGrid, G2frame, data)
        ...
        def OnInsertLayer(event):
            ...
            data["Layer Seq"] = " ".join(
                [str(i + 1) for i in range(len(data["Layers"]) - 2)]
            )
            G2pwd.REFDModelFxn(Profile, Inst, Limits, Substances, data)
            G2pwpl.PlotPatterns(G2frame, plotType="REFD")
            wx.CallAfter(UpdateREFDModelsGrid, G2frame, data)
        ...
        for ilay, _layer in enumerate(data["Layers"]):
            if not ilay:
                layerSizer.Add(
                    wx.StaticText(G2frame.dataWindow, label=" Top layer (superphase):"),
                    0,
                )
                ...
                layerSizer.Add(
                    wx.StaticText(
                        G2frame.dataWindow, label=" Bottom layer (substrate):"
                    ),
                    0,
                )
            midlayer = wx.BoxSizer(wx.HORIZONTAL)
            midlayer.Add(
                wx.StaticText(G2frame.dataWindow, label=" Substance: "), 0, WACV
            )
            midName = data["Layers"][ilay]["Name"]
            midSel = wx.ComboBox(
                G2frame.dataWindow,
                value=midName,
                choices=list(Substances.keys()),
                style=wx.CB_READONLY | wx.CB_DROPDOWN,
            )
            ...
            if midName != "vacuum":
                if midName != "unit scatter":
                    midlayer.Add(
                        wx.StaticText(G2frame.dataWindow, label=" Den. Mult.: "),
                        0,
                        WACV,
                    )
                    midlayer.Add(
                    ...
                    realScatt = (
                        data["Layers"][ilay]["DenMul"][0]
                        * Substances[midName]["Scatt density"]
                    )
                    midlayer.Add(
                        wx.StaticText(
                            G2frame.dataWindow,
                            label=f" Real scat. den.: {realScatt:.4g}",
                        ),
                        0,
                        WACV,
                    )
                    imagScatt = (
                        data["Layers"][ilay]["DenMul"][0]
                        * Substances[midName]["XImag density"]
                    )
                    midlayer.Add(
                        wx.StaticText(
                            G2frame.dataWindow,
                            label=f" Imag scat. den.: {imagScatt:.4g}",
                        ),
                        0,
                        WACV,
                    )
                ...
                midlayer.Add(
                    wx.StaticText(G2frame.dataWindow, label=", air or gas"), 0, WACV
                )
            layerSizer.Add(midlayer)
            if midName == "unit scatter":
                nxtlayer = wx.BoxSizer(wx.HORIZONTAL)
                nxtlayer.Add(
                    wx.StaticText(G2frame.dataWindow, label=" Real Den. : "), 0, WACV
                )
                nxtlayer.Add(
                    G2G.ValidatedTxtCtrl(
                        G2frame.dataWindow,
                ...
                nxtlayer.Add(
                    wx.StaticText(G2frame.dataWindow, label=" Imag Den. : "), 0, WACV
                )
                nxtlayer.Add(
                    G2G.ValidatedTxtCtrl(
                        G2frame.dataWindow,
                        data["Layers"][ilay]["iDenMul"],
                        0,
                        nDig=(10, 4),
                        OnLeave=Recalculate,
                    ),
            ...
            if midName != "vacuum":
                if "N" in Inst["Type"][0] and midName not in ["vacuum", "unit scatter"]:
                    ...
                    magLayer.Add(
                        wx.StaticText(G2frame.dataWindow, label=" Magnetic SLD: "),
                        0,
                        WACV,
                    )
                    magLayer.Add(
                    ...
                    magLayer.Add(
                        wx.StaticText(
                            G2frame.dataWindow,
                            label=" Real+mag scat. den.: %.4g"
                            % (realScatt + data["Layers"][ilay]["Mag SLD"][0]),
                        ),
                        0,
                        WACV,
                    )
                ...
                if ilay:
                    names = {
                        "Rough": "Upper surface Roughness, " + Angstr,
                        "Thick": "Layer Thickness, " + Angstr,
                    }
                    ...
                    if len(parms) > 1:
                        ...
                        slide.Add(
                            wx.StaticText(
                                G2frame.dataWindow, label=" Layer thickness: "
                            ),
                            0,
                            WACV,
                        )
                        parmSldr = G2G.G2Slider(
                            G2frame.dataWindow,
                            minValue=0,
                            maxValue=data["slider max"],
                            value=data["Layers"][ilay]["Thick"][0],
                        )
                        parmSldr.Bind(wx.EVT_SLIDER, OnParmSlider)
                    ...
                    for parm in parms:
                        parmsline.Add(
                            wx.StaticText(
                                G2frame.dataWindow, label=f" {names[parm]}: "
                            ),
                            0,
                            WACV,
                        )
    ...
    parent = G2frame.dataWindow.topPanel
    topSizer.Add(
        wx.StaticText(parent, label=" Reflectometry fitting for: " + Name), 0, WACV
    )
    topSizer.Add((-1, -1), 1, wx.EXPAND)
    topSizer.Add(G2G.HelpButton(parent, helpIndex=G2frame.dataWindow.helpKey))
    mainSizer = wx.BoxSizer(wx.VERTICAL)
    ...
    if Nlayers > 2:
        ...
        lineSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" Layer sequence: "), 0, WACV
        )
        repseq = wx.TextCtrl(
            G2frame.dataWindow,
            value=data["Layer Seq"],
            style=wx.TE_PROCESS_ENTER,
            size=(500, 25),
        )
        ...
        mainSizer.Add(
            wx.StaticText(
                G2frame.dataWindow, label=" NB: Repeat sequence by e.g. 6*(1 2) "
            )
        )
    G2G.HorizontalLine(mainSizer, G2frame.dataWindow)
    mainSizer.Add(
        wx.StaticText(
            G2frame.dataWindow,
            label=f" Layers: scatt. densities are 10{Pwr10}cm{Pwrm2} = 10{Pwrm6}{Angstr}{Pwrm2}",
        )
    )
...
def computePDF(G2frame, data):
    ...
    limits = G2frame.GPXtree.GetItemPyData(
        G2gd.GetGPXtreeItemId(G2frame, powId, "Limits")
    )
    Xlimits = [limits[1][0], limits[0][1]]  # use lower limit but ignore upper limit
    inst = G2frame.GPXtree.GetItemPyData(
        G2gd.GetGPXtreeItemId(G2frame, powId, "Instrument Parameters")
    )[0]
...
def OptimizePDF(G2frame, data, showFit=True, maxCycles=5):
    ...
    for key in ["Sample", "Sample Bkg.", "Container", "Container Bkg."]:
        name = data[key]["Name"]
        if name:
            xydata[key] = G2frame.GPXtree.GetItemPyData(
                G2gd.GetGPXtreeItemId(G2frame, G2frame.root, name)
            )
...
def UpdatePDFGrid(G2frame, data):
    ...
    def PDFFileSizer():
        def FillFileSizer(fileSizer, key):
            ...
            def OnRefMult(event):
                ...
                if item["Refine"]:
                    G2frame.GetStatusBar().SetStatusText(
                        "Be sure Mult is close to anticipated value. "
                        + "Suggest setting Flat Bkg. to 0 before Optimize Mult",
                        1,
                    )

            def GetExposure(backFile):
                dataId = G2gd.GetGPXtreeItemId(
                    G2frame, G2frame.root, "PWDR" + dataFile[4:]
                )
                dataComments = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, dataId, "Comments")
                )
                if not backFile:
                    return -1.0
                backId = G2gd.GetGPXtreeItemId(G2frame, G2frame.root, backFile)
                backComments = G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, backId, "Comments")
                )
                expTime = 1.0
                sumExp = 1.0
            ...
            fileSizer.Add(
                wx.StaticText(parent=G2frame.dataWindow, label=" " + key + " file:"),
                0,
                WACV,
            )
            fileName = wx.ComboBox(
                G2frame.dataWindow,
                value=item["Name"],
                choices=fileList,
                style=wx.CB_READONLY | wx.CB_DROPDOWN,
            ...
            fileSizer.Add(
                wx.StaticText(parent=G2frame.dataWindow, label="Multiplier:"), 0, WACV
            )
            mulBox = wx.BoxSizer(wx.HORIZONTAL)
            mult = G2G.ValidatedTxtCtrl(
                G2frame.dataWindow,
                item,
                "Mult",
                nDig=(10, 3),
            ...
            multSpin = wx.SpinButton(
                G2frame.dataWindow, style=wx.SP_VERTICAL, size=wx.Size(20, 25)
            )
        ...
        def ResetFlatBkg():
            Smin = np.min(
                G2frame.GPXtree.GetItemPyData(
                    G2gd.GetGPXtreeItemId(G2frame, G2frame.root, "PWDR" + dataFile[4:])
                )[1][1]
            )
            Bmin = 0
            Cmin = 0.0
            ...
            if data["Sample Bkg."]["Name"]:
                Bmin = np.min(
                    G2frame.GPXtree.GetItemPyData(
                        G2gd.GetGPXtreeItemId(
                            G2frame, G2frame.root, data["Sample Bkg."]["Name"]
                        )
                    )[1][1]
                )
            ...
            if data["Container"]["Name"]:
                Cmin = np.min(
                    G2frame.GPXtree.GetItemPyData(
                        G2gd.GetGPXtreeItemId(
                            G2frame, G2frame.root, data["Container"]["Name"]
                        )
                    )[1][1]
                )
                ...
                if data["Container Bkg."]["Name"]:
                    CBmin = np.min(
                        G2frame.GPXtree.GetItemPyData(
                            G2gd.GetGPXtreeItemId(
                                G2frame, G2frame.root, data["Container Bkg."]["Name"]
                            )
                        )[1][1]
                    )
        ...
        PDFfileSizer.Add(
            wx.StaticText(parent=G2frame.dataWindow, label=" PDF data files: "), 0
        )
    ...
    def SampleSizer():
        def FillElemSizer(elemSizer, ElData):
            ...
            elemSizer.Add(
                wx.StaticText(
                    parent=G2frame.dataWindow,
                    label=" Element: " + "%2s" % (ElData["Symbol"]) + " * ",
                ),
                0,
                WACV,
            )
        ...
        if not ElList:
            sampleSizer.Add(
                wx.StaticText(
                    G2frame.dataWindow, label=" Sample information: fill in this 1st"
                ),
                0,
            )
        else:
            sampleSizer.Add(
                wx.StaticText(G2frame.dataWindow, label=" Sample information: "), 0
            )
        ...
        midSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" Formula volume: "), 0, WACV
        )
        formVol = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            data,
            "Form Vol",
            nDig=(10, 3),
        ...
        geoBox.Add(
            wx.StaticText(G2frame.dataWindow, label=" Sample geometry: "), 0, WACV
        )
        if "C" in inst["Type"][0]:
            choice = [
                "Cylinder",
                "Bragg-Brentano",
                "Tilting flat plate in transmission",
                "Fixed flat plate",
            ]
        else:
            choice = [
        ...
        geoBox.Add(
            wx.StaticText(G2frame.dataWindow, label=" Sample diameter/thickness, mm: "),
            0,
            WACV,
        )
        diam = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            data,
            "Diam",
            nDig=(10, 3),
    ...
    def SFGctrlSizer():
        ...
        sqBox.Add(
            wx.StaticText(G2frame.dataWindow, label=" S(Q)->F(Q)->G(r) controls: "),
            0,
            WACV,
        )
        ...
        if data["DetType"] == "Area detector":
            sqBox.Add(
                wx.StaticText(G2frame.dataWindow, label=" IP transmission coeff.: "),
                0,
                WACV,
            )
            obliqCoeff = G2G.ValidatedTxtCtrl(
                G2frame.dataWindow,
                data,
                "ObliqCoeff",
                nDig=(10, 3),
        ...
        bkBox.Add(
            wx.StaticText(G2frame.dataWindow, label=" Background ratio: "), 0, WACV
        )
        backSldr = G2G.G2Slider(
            parent=G2frame.dataWindow,
        ...
        if "XC" in inst["Type"][0]:
            sqBox = wx.BoxSizer(wx.HORIZONTAL)
            sqBox.Add(
                wx.StaticText(G2frame.dataWindow, label=" Ruland width: "), 0, WACV
            )
            rulandSldr = G2G.G2Slider(
                parent=G2frame.dataWindow,
                style=wx.SL_HORIZONTAL,
            ...
            rulandWdt = G2G.ValidatedTxtCtrl(
                G2frame.dataWindow,
                data,
                "Ruland",
                nDig=(10, 3),
        ...
        sqBox = wx.BoxSizer(wx.HORIZONTAL)
        sqBox.Add(
            wx.StaticText(G2frame.dataWindow, label=" Scaling Q-range: "), 0, WACV
        )
        SQmin = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            data["QScaleLim"],
            0,
            nDig=(10, 3),
            xmin=qLimits[0],
            xmax=0.95 * data["QScaleLim"][1],
            typeHint=float,
            OnLeave=AfterChangeNoRefresh,
        )
        sqBox.Add(SQmin, 0, WACV)
        sqBox.Add(wx.StaticText(G2frame.dataWindow, label=" to Qmax "), 0, WACV)
        SQmax = G2G.ValidatedTxtCtrl(
            G2frame.dataWindow,
            data["QScaleLim"],
            1,
            nDig=(10, 3),
    ...
    def DiffSizer():
        def OnSelectGR(event):
            ...
            if newName:
                ...
                if subData[1][0][-1] != data["G(R)"][1][0][-1]:
                    G2frame.ErrorDialog(
                        "delt-G(R) Error", " G(R) for " + newName + " not same R range"
                    )
                    grName.SetValue(data["diffGRname"])
                    return
                data["diffGRname"] = newName
                data["delt-G(R)"][1] = np.array(
                    [subData[1][0], data["G(R)"][1][1] - subData[1][1]]
                )
                data["delt-G(R)"][2] += "-\n" + subData[2]
        ...
        def OnMult(invalid, value, tc):
            ...
            subData = G2frame.GPXtree.GetItemPyData(pId)["G(R)"]
            data["delt-G(R)"][1] = np.array(
                [subData[1][0], data["G(R)"][1][1] - data["diffMult"] * subData[1][1]]
            )
            G2plt.PlotISFG(G2frame, data, newPlot=True, plotType="delt-G(R)")
        ...
        diffSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=" Subtract G(R) for: "), 0, WACV
        )
        grName = wx.ComboBox(
            G2frame.dataWindow,
            value=data["diffGRname"],
            choices=fileList,
            style=wx.CB_READONLY | wx.CB_DROPDOWN,
        )
    ...
    def SumElementVolumes():
        ...
        ElList = data["ElList"]
        for El in ElList:
            Avol = (4.0 * math.pi / 3.0) * ElList[El]["Drad"] ** 3
    ...
    def OnCopyPDFControls(event):
        ...
        if len(TextList) == 1:
            G2frame.ErrorDialog(
                "Nothing to copy controls to",
                'There must be more than one "PDF" pattern',
            )
            return
        od = {
            "label_1": "Only refine flag",
            "value_1": False,
            "label_2": "Only Lorch flag",
            "value_2": False,
        }
        dlg = G2G.G2MultiChoiceDialog(
            G2frame,
            "Copy PDF controls",
            "Copy controls from " + Source + " to:",
            TextList,
            extraOpts=od,
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                for item in PDFlist:
                    Id = G2gd.GetGPXtreeItemId(G2frame, G2frame.root, item)
                    olddata = G2frame.GPXtree.GetItemPyData(
                        G2gd.GetGPXtreeItemId(G2frame, Id, "PDF Controls")
                    )
                    if od["value_1"]:
                        olddata["Sample Bkg."]["Refine"] = data["Sample Bkg."][
                            "Refine"
                        ]  # only one flag
                    elif od["value_2"]:
                    ...
                    G2frame.GPXtree.SetItemPyData(
                        G2gd.GetGPXtreeItemId(G2frame, Id, "PDF Controls"), olddata
                    )
    ...
    def OnSavePDFControls(event):
        ...
        dlg = wx.FileDialog(
            G2frame,
            "Choose GSAS-II PDF controls file",
            pth,
            "",
            "PDF controls files (*.pdfprm)|*.pdfprm",
            wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT,
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
    ...
    def OnComputeAllPDF(event):
        ...
        if G2frame.GPXtree.GetCount():
            ...
            while Id:
                ...
                if Name.startswith("PDF "):
                    ...
                    if not Data["ElList"]:
                        print(f"  No chemical formula for {Name}")
                    else:
                        choices.append(Name)
                Id, cookie = G2frame.GPXtree.GetNextChild(G2frame.root, cookie)
        if not choices:
        ...
        dlg = G2G.G2MultiChoiceDialog(
            G2frame, "Select PDFs to compute", "Select PDFs", choices, extraOpts=od
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
                results = dlg.GetSelections()
        ...
        try:
            while Id:
                ...
                if Name in Names:
                    N += 1
                    msg = f"PDFs done: {N - 1} of {len(Names)}"
            ...
            pgbar.Destroy()
        if od["value_1"]:
            msg = f"{N}/{len(Names)} PDFs computed; {notConverged} unconverged"
        else:
            msg = f"{N}/{len(Names)} PDFs computed"
    ...
    if powId:  # skip if no matching PWDR entry
        fullLimits, limits = G2frame.GPXtree.GetItemPyData(
            G2gd.GetGPXtreeItemId(G2frame, powId, "Limits")
        )[:2]
        inst = G2frame.GPXtree.GetItemPyData(
            G2gd.GetGPXtreeItemId(G2frame, powId, "Instrument Parameters")
        ...
        if "C" in inst["Type"][0]:
            ...
            qLimits = [
                tof2q(fullLimits[1], inst["difC"][1]),
                tof2q(fullLimits[0], inst["difC"][1]),
            ]
            polariz = 1.0
    ...
    if powId:
        ...
        mainSizer.Add(
            wx.StaticText(G2frame.dataWindow, label=f"Controls for {powName}:\n")
        )
        mainSizer.Add(DiffSizer())
    G2frame.dataWindow.SetDataSize()
...
def UpdatePDFPeaks(G2frame, peaks, data):
    ...
    def backSizer():
        ...
        backBox.Add(
            wx.StaticText(G2frame.dataWindow, label=" Background slope: "), 0, WACV
        )
    ...
    def peakSizer():
        ...
        Types = (
            ...
            * [
            ...
            + [
                wg.GRID_VALUE_CHOICE + ": ,P,M,S,PM,PS,MS,PMS",
            ]
            + 2
            * [
                wg.GRID_VALUE_STRING,
        ...
        peakTable = G2G.Table(
            peaks["Peaks"], rowLabels=rowLabels, colLabels=colLabels, types=Types
        )
        ...
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                for item in PDFlist:
                    Id = G2gd.GetGPXtreeItemId(G2frame, G2frame.root, item)
                    olddata = G2frame.GPXtree.GetItemPyData(
                        G2gd.GetGPXtreeItemId(G2frame, Id, "PDF Peaks")
                    )
                    if od["value_1"]:
                    ...
                    G2frame.GPXtree.SetItemPyData(
                        G2gd.GetGPXtreeItemId(G2frame, Id, "PDF Peaks"), olddata
                    )
    ...
    def OnFitPDFpeaks(event):
        ...
        peaks = G2frame.GPXtree.GetItemPyData(
            G2gd.GetGPXtreeItemId(G2frame, PatternId, "PDF Peaks")
        )
        if not peaks:
            G2frame.ErrorDialog("No peaks!", "Nothing to fit!")
            return
        ...
        G2frame.GPXtree.SetItemPyData(
            G2gd.GetGPXtreeItemId(G2frame, PatternId, "PDF Peaks"), newpeaks
        )
    ...
    def OnFitAllPDFpeaks(event):
        ...
        dlg = G2G.G2MultiChoiceDialog(
            G2frame, "PDF peak fitting", "Select PDFs to fit:", Names, extraOpts=od
        )
        try:
            if dlg.ShowModal() == wx.ID_OK:
                Id = G2gd.GetGPXtreeItemId(
                    G2frame, G2frame.root, "Sequential PDF peak fit results"
                )
                if Id:
                    SeqResult = G2frame.GPXtree.GetItemPyData(Id)
                else:
                    SeqResult = {}
                    Id = G2frame.GPXtree.AppendItem(
                        parent=G2frame.root, text="Sequential PDF peak fit results"
                    )
                SeqResult = {"SeqPseudoVars": {}, "SeqParFitEqList": []}
                ...
                for item in items:
                    ...
                    if od["value_1"] and newpeaks is not None:
                        ...
                        peaks = G2frame.GPXtree.GetItemPyData(
                            G2gd.GetGPXtreeItemId(G2frame, pId, "PDF Peaks")
                        )
                    newpeaks, vals, varyList, sigList, parmDict, Rvals = (
                        G2pwd.PDFPeakFit(peaks, data["G(R)"])
                    )
                    if vals is None:
                    ...
                    SeqResult[name] = {
                        ...
                        "sig": sigList,
                        "Rvals": Rvals,
                        "covMatrix": np.eye(len(varyList)),
                        "title": name,
                        "parmDict": parmDict,
                    }
                    G2frame.GPXtree.SetItemPyData(
                        G2gd.GetGPXtreeItemId(G2frame, pId, "PDF Peaks"), newpeaks
                    )
                SeqResult["histNames"] = Names
                G2frame.G2plotNB.Delete(
                    "Sequential refinement"
