...
class testSSymbols(wx.Frame):
    ...
    def UpdateData(self, Data):
        ...
        if Data["SGData"]["SGGray"]:
            SpGrp += " 1'"
        SGTxt = wx.TextCtrl(
            self.testSSPanel, -1, value=SpGrp, style=wx.TE_PROCESS_ENTER
        )
        SGTxt.Bind(wx.EVT_TEXT_ENTER, OnSpaceGroup)
        mainSizer.Add(SGTxt, 0, WACV)
        mainSizer.Add(
            wx.StaticText(self.testSSPanel, label=" Superspace group: " + SpGrp),
            0,
            WACV,
        )
        ssChoice = G2spc.SSChoice(Data["SGData"])
        ...
        if ssChoice:
            superGp = wx.ComboBox(
                self.testSSPanel, value=ssSym, choices=ssChoice, style=wx.CB_DROPDOWN
            )  # wx.CB_READONLY|
            ...
            superGp = wx.TextCtrl(
                self.testSSPanel, value=ssSym, style=wx.TE_PROCESS_ENTER
            )
            superGp.Bind(wx.EVT_TEXT_ENTER, OnSuperGp)
