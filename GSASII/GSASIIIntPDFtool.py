...
class AutoIntFrame(wx.Frame):
    ...
    def __init__(self, G2frame, PollTime=30.0):
        ...
        def GetGPXInputFile(event):
            ...
            if event is not None:
                dlg = wx.FileDialog(self, lbl, pth, style=wx.FD_OPEN, wildcard=filtyp)
                dlg.CenterOnParent()
                if dlg.ShowModal() == wx.ID_OK:
        ...
        def TestInput(*args, **kwargs):
            ...
            for dfmt, obj in self.fmtlist.items():
                fmt = dfmt[1:]
                if not self.params["outsel"][fmt]:
                    continue
                if fmt in self.multipleFmtChoices:
                    continue
            ...
            if not self.params["TableMode"]:
                if (
                    not self.gpxin[3]
                    or not os.path.exists(self.gpxin[3])
                    or len(self.imgList) == 0
                ):
                    self.EnableIntButtons(False)
                    return
                else:
        ...
        def checkPDFselection():
            ...
            for i, lbl in enumerate(("Sample Bkg.", "Container", "Container Bkg.")):
                ...
                try:
                    ...
                    if PDFobj.data["PDF Controls"][lbl]["Name"]:
                        print(
                            "PDF {} hist entry {} not found".format(
        ...
        def ShowbyMode():
            ...
            if not self.params["TableMode"]:
                ...
                self.gpxin[1] = G2G.ValidatedTxtCtrl(
                    mnpnl, self.gpxin, 3, OKcontrol=TestInput, OnLeave=TestInput
                )
                ...
                self.imprm[3] = 0
                self.imprm[1] = G2G.G2ChoiceButton(
                    mnpnl, [""], self.imprm, 3, onChoice=TestInput
                )
                ...
                self.maskfl[3] = 0
                self.maskfl[1] = G2G.G2ChoiceButton(
                    mnpnl, [""], self.maskfl, 3, onChoice=TestInput
                )
        ...
        def OnTableButton(event):
            ...
            try:
                ...
                if dlg.ShowModal() == wx.ID_OK:
                    ...
                    for f, m in zip(
                        self.IMfileList, self.ImgTblParms[-1], strict=False
                    ):
                        n = os.path.split(f)[1]
        ...
        wx.Frame.__init__(
            self, None, title="Automatic Integration", style=wxMainFrameStyle
        )
        ...
        sizer = wx.BoxSizer(wx.HORIZONTAL)
        sizer.Add(
            wx.StaticText(mnpnl, wx.ID_ANY, "Write to: "), 0, wx.ALIGN_CENTER_VERTICAL
        )
        ...
        for dfmt in sorted(self.fmtlist):
            ...
            btn = G2G.G2CheckBox(
                mnpnl, dfmt, self.params["outsel"], fmt, OnChange=TestInput
            )
            sizer.Add(btn)
        lblsizr.Add(sizer)
        sizer = wx.BoxSizer(wx.HORIZONTAL)
        ...
        sizer = wx.BoxSizer(wx.HORIZONTAL)
        sizer.Add(
            wx.StaticText(mnpnl, wx.ID_ANY, "Autocompute PDF:"),
            0,
            wx.ALIGN_CENTER_VERTICAL,
        )
        lbl4b = G2G.G2CheckBox(
            mnpnl, "", self.params, "ComputePDF", OnChange=showPDFctrls
        )
        sizer.Add(lbl4b)
        ...
        self.pdfSel = G2G.G2ChoiceButton(
            mnpnl, [""], self.params, "pdfprm", onChoice=checkPDFselection
        )
        ...
        for fmt in self.PDFformats:
            ...
            if fmt not in self.params["outsel"]:
                self.params["outsel"][fmt] = False
            btn = G2G.G2CheckBox(
                mnpnl, fmt, self.params["outsel"], fmt, OnChange=TestInput
            )
            sizer.Add(btn)
        ...
        for i, lbl in enumerate((" Sample bkg:", " Container:", "Container bkg:")):
            ...
            self.pbkg[i][4] = G2G.ValidatedTxtCtrl(
                mnpnl,
                self.pbkg[i],
                6,
                (6, 3),
    ...
    def EnableIntButtons(self, flag):
        for item in (self.btnstart, self.btnreset):
            item.Enable(flag)
    ...
    def StartLoop(self):
        ...
        if self.params["SeparateDir"]:
            for dfmt in self.fmtlist:
                if not self.params["outsel"][dfmt[1:]]:
                    continue
        ...
        if self.params["ComputePDF"] and self.params["SeparateDir"]:
            for fmt in self.PDFformats:
                if not self.params["outsel"][fmt]:
                    continue
                dir = os.path.join(
                    self.params["outdir"], fmt.replace("(", "_").replace(")", "")
                )
                if not os.path.exists(dir):
                    os.makedirs(dir)
        return False
...
def ProcessImage(
    newImage, imgprms, mskprms, xydata, PDFdict, InterpVals, calcModes, outputModes
):
    ...
    if SeparateDir:
        ...
        savedir = outdir
    outgpx = os.path.join(
        savedir, os.path.split(os.path.splitext(newImage)[0] + ".gpx")[1]
    )
    ...
    for im in gpxout.add_image(newImage):
        ...
        setdist = "{:.2f}".format(
            im.getControls()["setdist"]
        )  # ignore differences in position less than 0.01 mm
        ...
        #    im.initMasks()
        hists = im.Integrate(
            MaskMap=MapCache["maskMap"][setdist],
            ThetaAzimMap=MapCache["ThetaAzimMap"][setdist],
        )
        # write requested files
        for dfmt in fmtlist:
            fmt = dfmt[1:]
            if not outputSelect[fmt]:
                continue
            if fmtlist[dfmt] is None:
                continue
            hint = ""
...
def SetupInterpolation(dlg):
    ...
    return copy.deepcopy(
        [cols, parms, IMfileList, ParmList, nonInterpVars, ControlsTable, MaskTable]
    )
