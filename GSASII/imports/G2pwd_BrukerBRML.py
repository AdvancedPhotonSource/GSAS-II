...
class brml_ReaderClass(G2obj.ImportPowderData):
    ...
    def __init__(self):
        ...
        if xml is None:
            ...
            msg = (
                "Bruker .brml Reader skipped because xmltodict module is not installed."
            )
    ...
    def Reader(self, filename, ParentFrame=None, **kwarg):  # noqa: ARG002
        ...
        def XtractXMLScan():
            ...
            try:
                nSteps = int(
                    data["RawData"]["DataRoutes"]["DataRoute"][datano][
                        "ScanInformation"
                    ]["MeasurementPoints"]
                )
            except KeyError:
                datano = 0
                nSteps = int(
                    data["RawData"]["DataRoutes"]["DataRoute"]["ScanInformation"][
                        "MeasurementPoints"
                    ]
                )
            if nSteps <= 10:
            ...
            while i < nSteps:
                if datano:
                    entry = data["RawData"]["DataRoutes"]["DataRoute"][datano]["Datum"][
                        i
                    ].split(",")
                else:
                    entry = data["RawData"]["DataRoutes"]["DataRoute"]["Datum"][
                        i
                    ].split(",")
                x[i] = float(entry[2])
        ...
        def XtractXMLNoscan():
            ...
            self.comments = []
            try:
                scandata = data["RawData"]["DataRoutes"]["DataRoute"][
                    "ScanInformation"
                ]["ScaleAxes"]
                if not scandata:
                    return None
        ...
        while filNum <= self.buffer["maxNum"]:
            ...
            files = [
                i
                for i, j in enumerate(self.buffer["filesFound"])
