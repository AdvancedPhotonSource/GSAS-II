...
def ReadCheckConstraints(GPXfile, seqHist=None, Histograms=None, Phases=None):
    ...
    if seqHist:
        Histograms = {
            seqHist: Histograms[seqHist]
        }  # sequential fit: only need one histogram
        hId = Histograms[seqHist]["hId"]
    else:
        hId = None
    constrDict, fixedList = ReadConstraints(
        GPXfile, hId
    )  # load user constraints from file (uses ProcessConstraints)
    parmDict = {}
    ...
    (
        ...
        Phases, RestraintDict=None, seqHistName=seqHist, rbIds=rbIds, Print=False
    )  # generates atom symmetry constraints
    parmDict.update(phaseDict)
    hapVary, hapDict, controlDict = GetHistogramPhaseData(
        Phases, Histograms, Print=False, resetRefList=False
    )
    parmDict.update(hapDict)
    histVary, histDict, histDict1, controlDict = GetHistogramData(
    ...
    errmsg, warnmsg, groups, parmlist = G2mv.GenerateConstraints(
        varyList, constrDict, fixedList, parmDict, seqHistNum=hId
    )
        ...
        for Histogram in Phases[Phase]["Histograms"]:
            try:
                ...
                if phfx + "TwinFr:0" in hapVary:
                    ...
                    TwConstr.append(
                        {phfx + "TwinFr:" + str(i): "1.0" for i in range(nTwin)}
                    )
    ...
    for histogram in histoList:
        if histogram[:4] in ["PWDR", "HKLF"]:
            ...
            if "E" in controlDict[hfx + "histType"]:
                for El in FFtables:
                    FFtables[El][hfx + "FP"] = 0.0
                    FFtables[El][hfx + "FPP"] = 0.0
...
def GetHistograms(GPXfile, hNames):
    ...
    for hist in hNames:
        ...
        if pos is None:
            msg = f"Histogram {hist} not found in GPX file"
            raise Exception(msg)
        fl.seek(pos)
        data = pickleLoad(fl)
        datum = data[0]
        if "PWDR" in hist[:4]:
            ...
            PWDRdata["Data"] = ma.array(
                ma.getdata(datum[1][1])
            )  # masked powder data arrays/clear previous masks
    ...
    for hist in Histograms:
        ...
        if fixedBkg:
            ...
            try:
                ...
                KeyError
            ):  # would happen if a referenced histogram were renamed or deleted
                G2fil.G2Print(
                    f'Warning: For hist "{hist}" unresolved background reference to hist "{fixedBkg[0]}"'
                )
...
def getBackupName(GPXfile, makeBack):
    ...
    GPXback = ospath.join(
        GPXpath, ospath.splitext(GPXname)[0] + ".bak" + str(last) + ".gpx"
    )
    return GPXback
...
def SaveUsedHistogramsAndPhases(
    GPXfile, Histograms, Phases, RigidBodies, CovData, parmFrozenList, makeBack=True
):
    ...
    while True:
        ...
        if datum[0] == "Phases":
            ...
            Controls["parmFrozen"]["FrozenList"] = [
                i if type(i) is G2obj.G2VarObj else G2obj.G2VarObj(i)
        ...
        try:
            ...
            for datus in data[1:]:
                ...
                if datus[0] == "Background":  # remove fixed background from file
                    d1 = {
                        key: histogram["Background"][1][key]
...
def SaveUpdatedHistogramsAndPhases(
    GPXfile, Histograms, Phases, RigidBodies, CovData, parmFrozen
):
    """
    ...
    for key in (
        ...
        if key == "Background":  # remove fixed background from file
            xfer_dict["Background"][1] = {
                k: hist["Background"][1][k]
...
def SetSeqResult(GPXfile, Histograms, SeqResult):
    ...
    name, parmFrozenDict = pickleLoad(fp)[
        0
    ...
    for i, varyI in enumerate(pawleyVary):
        ...
        for varyJ in pawleyVary[i + 1 :]:
            ...
            if SGLaue in ["4/m", "4/mmm"]:
                ...
                if isum == jsum:
                    eqvDict[varyI].append(varyJ)
            elif abs(dspI - dspJ) / dspI < 1.0e-4:
                eqvDict[varyI].append(varyJ)
...
def GetRigidBodyModels(rigidbodyDict, Print=True, pFile=None):
    ...
    def PrintSpnRBModel(RBModel):
        pFile.write(
            "Spinning RB name: %s atom type: %s, no.atoms: %d, No. times used: %d\n"
            % (
                RBModel["RBname"],
                RBModel["atType"],
                RBModel["Natoms"],
    ...
    def PrintVecRBModel(RBModel):
        ...
        for i in WriteVecRBModel(RBModel):
            pFile.write(i)
        pFile.write(
            "Orientation defined by: atom {} -> atom {} & atom {} -> atom {}\n".format(
                RBModel["rbRef"][0],
                RBModel["rbRef"][1],
                RBModel["rbRef"][0],
                RBModel["rbRef"][2],
    ...
    if Print and pFile is None:
        raise Exception("specify pFile or Print=False")
    rbVary = []
    rbDict = {}
    rbIds = rigidbodyDict.get("RBIds", {"Vector": [], "Residue": [], "Spin": []})
...
def SetRigidBodyModels(parmDict, sigDict, rigidbodyDict, pFile=None):
    ...
    for irb, item in enumerate(RBIds["Vector"]):
        if rigidbodyDict["Vector"][item]["useCount"]:
            VectSig = []
            RBmags = rigidbodyDict["Vector"][item]["VectMag"]
            for i, _mag in enumerate(RBmags):
...
def GetPhaseData(
    ...
    def PrintFFtable(FFtable):
        ...
        for Ename in FFtable:
            if "Q" not in Ename:
                ...
                fb = ffdata["fb"]
                pFile.write(
                    " %8s %9.5f %9.5f %9.5f %9.5f %9.5f %9.5f %9.5f %9.5f %9.5f\n"
                    % (
                        Ename.ljust(8),
                        fa[0],
                        fa[1],
        ...
        for Ename in ORBtable:
            ...
            for item in orbdata:
                if "core" in item or "<" in item or "Sl " in item:
                    ...
                    fb = orbdata[item]["fb"]
                    if "core" in item:
                        name = (f"{Ename} {item}").ljust(13)
                    else:
                        name = "   " + item.ljust(10)
                    pFile.write(
                        " %13s %9.5f %9.5f %9.5f %9.5f %9.5f %9.5f %9.5f %9.5f %9.5f\n"
                        % (
                            name,
                            fa[0],
                            fa[1],
    ...
    def PrintRBObjects(resRBData, vecRBData, spnRBData):
        def PrintRBThermals():
            ...
            if "T" in RB["ThermalMotion"][0]:
                ...
                text = ""
                for i in range(6):
                    text += "T" + tlstr[i] + f" {TLS[i]:8.4f} {str(TLSvar[i])[0]} "
                pFile.write(text + "\n")
            ...
            if "U" in RB["ThermalMotion"][0]:
                ...
                text = "Uiso" + f" {TLS[0]:10.3f} {str(TLSvar[0])[0]}"
                pFile.write(text + "\n")
            ...
            for RB in resRBData:
                ...
                if len(Torsions):
                    ...
                    for torsion in Torsions:
                        text += f"{torsion[0]:10.4f} Refine? {torsion[1]}"
                    pFile.write(text + "\n")
                PrintRBThermals()
    ...
    def PrintAtoms(General, Atoms):
        ...
        if General["Type"] in [
            ...
            for _i, at in enumerate(Atoms):
                ...
                if at[cia] == "I":
                    line += f"{at[cia + 1]:8.5f}" + 48 * " "
                else:
                    line += 8 * " "
                    for j in range(6):
                        line += f"{at[cia + 2 + j]:8.5f}"
                pFile.write(line + "\n")
            ...
            for _i, at in enumerate(Atoms):
                ...
                if at[cia] == "I":
                    line += f"{at[cia + 1]:8.4f}" + 48 * " "
                else:
                    line += 8 * " "
                    for j in range(6):
                        line += f"{at[cia + 2 + j]:8.4f}"
                pFile.write(line + "\n")
    ...
    def PrintDeformations(General, Atoms, Deformations):
        ...
        for AtDef in Deformations:
            ...
            pFile.write(
                f"\n Atom: {atom[ct - 1]} at {atom[cx]:10.5f}, {atom[cx + 1]:10.5f}, {atom[cx + 2]:10.5f} sytsym: {atom[cs]}\n"
            )
            pFile.write(135 * "-" + "\n")
            for orb in DefTable:
    ...
    def PrintWaves(General, Atoms):
        ...
        names = {
            ...
            "Smag": ["MXsin", "MYsin", "MZsin", "MXcos", "MYcos", "MZcos"],
        }
        pFile.write(135 * "-" + "\n")
        for _i, at in enumerate(Atoms):
            AtomSS = at[-1]["SS1"]
            for Stype in ["Sfrac", "Spos", "Sadp", "Smag"]:
                ...
                if len(Waves):
                    pFile.write(
                        f" atom: {at[ct - 1]}, site sym: {at[cs]}, type: {Stype} wave type: {Waves[0]}:\n"
                    )
                else:
                    continue
    ...
    def PrintTexture(textureData):
        ...
        for _block in range(nBlock):
            ptlbls = " names :"
            ptstr = " values:"
            for item in SHkeys[iBeg:iFin]:
                ptlbls += "%12s" % (item)
                ptstr += f"{SHcoeff[item]:12.4f}"
            pFile.write(ptlbls + "\n")
            pFile.write(ptstr + "\n")
            iBeg += 10
            iFin = min(iBeg + 10, nCoeff)
    ...
    def MakeRBParms(rbKey, phaseVary, phaseDict):
        ...
        if "AtomFrac" not in RB and rbKey != "S":
            raise Exception("out of date RB: edit in RB Models")
        # end patch
        if rbKey == "S":
        ...
        for i in range(4):
            ...
            if (
                (RB["Orient"][1] == "AV" and i)
                or ("A" in RB["Orient"][1] and not i)
                or (RB["Orient"][1] == "V" and i)
            ):
                phaseVary += [
                    name,
                ]
        if rbKey != "S":
            ...
            for ish, _atype in enumerate(RB["atType"]):
    ...
    def MakeRBSphHarm(rbKey, phaseVary, phaseDict):
        ...
        for ish, Shcof in enumerate(RB["SHC"]):
            ...
            for _i, shcof in enumerate(Shcof):
                SHcof = Shcof[shcof]
                name = (
                    pfxRB + shcof.strip("+").strip("-") + ":" + iAt + ":" + rbid
                )  # patch; remove sign
    ...
    for name in PhaseData:
        if (
            seqHistName is not None and seqHistName != "All"
        ):  # sequential: load only used phases
            if seqHistName not in PhaseData[name]["Histograms"]:
                continue
            if not PhaseData[name]["Histograms"][seqHistName]["Use"]:
                continue
        ...
        if General["Type"] == "magnetic":
            ...
            for itm, text in enumerate(SGtable):
                opr = text.split(")")[1]
                newTable += [
                    "(%2d)%s   %2d" % (itm + 1, opr, int(SpnFlp[itm])),
                ]
            SGtable = newTable
        ...
        phaseDict.update(
            {
                ...
                pfx + "A4": A[4],
                pfx + "A5": A[5],
                pfx + "Vol": G2lat.calc_V(A),
            }
        )
        if cell[0]:
            phaseVary += cellVary(
                pfx, SGData
            )  # also fills in symmetry required constraints
        ...
        if General.get("Modulated", False):
            ...
            phaseDict.update(
                {pfx + "mV0": Vec[0], pfx + "mV1": Vec[1], pfx + "mV2": Vec[2]}
            )
        ...
        if Atoms and not General.get("doPawley"):
            ...
            for i, at in enumerate(Atoms):
                ...
                phaseDict.update(
                    {
                        pfx + "Atype:" + str(i): at[ct],
                        pfx + "Afrac:" + str(i): at[cx + 3],
                        pfx + "Amul:" + str(i): at[cs + 1],
                        pfx + "Ax:" + str(i): at[cx],
                        pfx + "Ay:" + str(i): at[cx + 1],
                        pfx + "Az:" + str(i): at[cx + 2],
                        pfx + "dAx:" + str(i): 0.0,
                        pfx + "dAy:" + str(i): 0.0,
                        pfx + "dAz:" + str(i): 0.0,  # refined shifts for x,y,z
                ...
                if at[cia] == "I":
                    ...
                    phaseDict.update(
                        {
                            pfx + "AU11:" + str(i): at[cia + 2],
                            pfx + "AU22:" + str(i): at[cia + 3],
                            pfx + "AU33:" + str(i): at[cia + 4],
                            pfx + "AU12:" + str(i): at[cia + 5],
                            pfx + "AU13:" + str(i): at[cia + 6],
                            pfx + "AU23:" + str(i): at[cia + 7],
                        }
                    )
                if General["Type"] == "magnetic":
                    phaseDict.update(
                        {
                            pfx + "AMx:" + str(i): at[cx + 4],
                            pfx + "AMy:" + str(i): at[cx + 5],
                            pfx + "AMz:" + str(i): at[cx + 6],
                        }
                    )
                if "F" in at[ct + 1]:
                ...
                if General.get("Modulated", False):
                    ...
                    for Stype in ["Sfrac", "Spos", "Sadp", "Smag"]:
                        ...
                        if len(Waves):
                            ...
                            continue
                        phaseDict[pfx + Stype[1].upper() + "waveType:" + str(i)] = (
                            waveType
                        )
                        nx = 0
                        for iw, wave in enumerate(Waves[1:]):
                            if not iw:
                                if waveType in ["ZigZag", "Block"]:
                                    nx = 1
                                CSI = G2spc.GetSSfxuinel(
                                    waveType, Stype, 1, at[cx : cx + 3], SGData, SSGData
                                )
                            else:
                                CSI = G2spc.GetSSfxuinel(
                                    "Fourier",
                            ...
                            if wave[
                                1
                            ]:  # what do we do here for multiple terms in modulation constraints?
                                for j in range(len(equivs)):
                                    if uId[j][0] > 0:
                                        phaseVary.append(names[j])
                                        equivs[uId[j][0]].append(
                                            [names[j], uCoef[j][0]]
                                        )
                                for equiv in equivs:
                                    if len(equivs[equiv]) > 1:
            ...
            if len(Deformations) and not General.get("doPawley"):
                for iAt in Deformations:
                    ...
                    radial = phaseDict[pfx + "Radial:%d" % AtId] = Deformations[-iAt][
                        "Radial"
                    ]
            ...
            if Print:
                ...
                pFile.write(
                    f"\n Unit cell: a = {cell[1]:.5f} b = {cell[2]:.5f} c = {cell[3]:.5f} alpha = {cell[4]:.3f} beta = {cell[5]:.3f} gamma = {cell[6]:.3f} volume = {cell[7]:.3f} Refine? {cell[0]}\n"
                )
                if len(SSGtext):  # if superstructure
                    pFile.write(
            ...
            for i, refl in enumerate(PawleyRef):
                ...
                if im:
                    pawleyLookup[
                        pfx + "%d,%d,%d,%d" % (refl[0], refl[1], refl[2], refl[3])
                    ] = i
                else:
            ...
            GetPawleyConstr(
                SGData["SGLaue"], PawleyRef, im, pawleyVary
            )  # does G2mv.StoreEquivalence
...
def PrintRestraints(
    cell, SGData, AtPtrs, Atoms, AtLookup, textureData, phaseRest, pFile
):
        ...
        for name, rest in names:
            ...
            if rest in itemRest and itemRest[rest] and itemRest["Use"]:
                ...
                if name in ["Bond", "Angle", "Plane", "Chiral"]:
                    pFile.write(
                        "     calc       obs      sig   delt/sig  atoms(symOp): \n"
                    )
                    for indx, ops, obs, esd in itemRest[rest]:
                        try:
                            AtNames = G2mth.GetAtomItemsById(
                                Atoms, AtLookup, indx, ct - 1
                            )
                            AtName = ""
                            for i, Aname in enumerate(AtNames):
                                ...
                                if ops[i] == "1":
                                    ...
                                    AtName += "+(" + ops[i] + ")-"
                            XYZ = np.array(
                                G2mth.GetAtomItemsById(Atoms, AtLookup, indx, cx, 3)
                            )
                            ...
                            pFile.write(
                                f" {calc:9.3f} {obs:9.3f} {esd:8.3f} {(obs - calc) / esd:8.3f}   {AtName[:-1]}\n"
                            )
                        except KeyError:
                            del itemRest[rest]
                elif name in ["Torsion", "Rama"]:
                    pFile.write(
                        "  atoms(symOp)  calc  obs  sig  delt/sig  torsions: \n"
                    )
                    ...
                    for indx, ops, cofName, esd in itemRest[rest]:
                        ...
                        if name == "Torsion":
                            ...
                            pFile.write(
                                f" {calc:8.3f} {obs:8.3f} {esd:.3f} {(obs - calc) / esd:8.3f} {tor:8.3f} {AtName[:-1]}\n"
                            )
                        else:
                            ...
                            pFile.write(
                                f" {calc:8.3f} {obs:8.3f} {esd:8.3f} {(obs - calc) / esd:8.3f} {phi:8.3f} {psi:8.3f} {AtName[:-1]}\n"
                            )
                elif name == "ChemComp":
                    pFile.write("     atoms   mul*frac  factor     prod\n")
                    for indx, factors, obs, esd in itemRest[rest]:
                        try:
                            atoms = G2mth.GetAtomItemsById(
                                Atoms, AtLookup, indx, ct - 1
                            )
                            mul = np.array(
                            ...
                            for _iatm, [atom, mf, fr, clc] in enumerate(
                                zip(atoms, mulfrac, factors, calcs, strict=False)
                            ):
                                pFile.write(
                                    " %10s %8.3f %8.3f %8.3f\n" % (atom, mf, fr, clc)
                                )
                            pFile.write(
                                f" Sum:                   calc: {np.sum(calcs):8.3f} obs: {obs:8.3f} esd: {esd:8.3f}\n"
                            )
                        except KeyError:
                            del itemRest[rest]
                elif name == "Moments":
                    for item in itemRest["Moments"]:
                        nMom = len(item[0])
                        AtNames = G2mth.GetAtomItemsById(
                            Atoms, AtLookup, item[0], ct - 1
                        )
                        moms = G2mth.GetAtomItemsById(
                            Atoms, AtLookup, item[0], cx + 4, 3
                        )
                        obs = 0.0
                        ...
                        for i, mom in enumerate(moms):
                            ...
                            line += f" {AtNames[i]}  {Mom:8.3f}"
                            obs += Mom
                        obs /= nMom
                        line += f"{obs:8.3f} {item[-1]:6.3f}\n"
                        pFile.write(line)
                    ...
                    pFile.write(
                        "    HKL  grid  neg esd  sum neg  num neg use unit?  unit esd \n"
                    )
                    ...
                    pFile.write(
                        "  target   sig     obs  expression        variables \n"
                    )
                    for expObj, target, esd in itemRest[rest]:
                        ...
                        if len(txt) > 80:
                            txt = txt[:77] + "..."
                        pFile.write(
                            f"{target:8.5g}{esd:6.3g}{val:>8s}  {expObj.expression:17s} {txt}\n"
                        )
...
def SetPhaseData(
    parmDict, sigDict, Phases, RBIds, covData, RestraintDict=None, pFile=None
):
    ...
    def PrintAtomsAndSig(General, Atoms, sigDict, sigKey):
        ...
        for atyp in General["AtomTypes"]:  # zero composition
            General["NoAtoms"][atyp] = 0.0
        for i, at in enumerate(Atoms):
            General["NoAtoms"][at[ct]] += at[cx + 3] * float(
                at[cx + 5]
            )  # new composition
            if General["Type"] == "macromolecular":
                name = f" {at[0]} {at[1]} {at[2]} {at[3]}:"
                valstr = " values:          "
                sigstr = " sig   :           "
            else:
                ...
                if ("B" in radial and "S" not in orb[0]) or (
                    ...
                    for item in orb[1]:
                        ...
                        if pName in deformSig:
                            sigstr += f"{deformSig[pName]:10.5f}"
                        else:
                            sigstr += 10 * " "
                    pFile.write(names + "\n")
                    pFile.write(values + "\n")
    ...
    def PrintWavesAndSig(General, Atoms, wavesSig):
        ...
        for i, at in enumerate(Atoms):
            ...
            for Stype in ["Sfrac", "Spos", "Sadp", "Smag"]:
                ...
                if len(Waves):
                    pFile.write(
                        f" atom: {at[ct - 1]}, site sym: {at[cs]}, type: {Stype} wave type: {waveType}:\n"
                    )
                    for iw, wave in enumerate(Waves[1:]):
                        stiw = ":" + str(i) + ":" + str(iw)
                        namstr = "  names :"
                        ...
                        if Stype == "Spos":
                            ...
                            for j in range(nt):
                                ...
                                namstr += "%12s" % (name)
                                valstr += f"{wave[0][j]:12.4f}"
                                if name + stiw in wavesSig:
                                    sigstr += f"{wavesSig[name + stiw]:12.4f}"
                                else:
                                    sigstr += 12 * " "
                        elif Stype == "Sfrac":
                            ot = 0
                            ...
                            for j in range(12):
                                ...
                                valstr += f"{wave[0][j]:10.6f}"
                                if name + stiw in wavesSig:
                                    sigstr += f"{wavesSig[name + stiw]:10.6f}"
                                else:
                                    sigstr += 10 * " "
                        elif Stype == "Smag":
                            for j in range(6):
    ...
    for phase in Phases:
        if pFile:
            pFile.write(f" Result for phase: {phase}\n")
        if pFile:
            pFile.write(135 * "=" + "\n")
        Phase = Phases[phase]
        ...
        if cell[0]:
            ...
            cellSig = G2lat.getCellEsd(
                pfx, SGData, A, covData, unique=True
            )  # includes sigVol
            if pFile:
            ...
            ptstr = "  values:"
            sigstr = "  esds  :"
            for name, fmt, a, siga in zip(
                names, ptfmt, cell[1:8], cellSig, strict=False
            ):
                namstr += "%12s" % (name)
                ptstr += fmt % (a)
                if siga and siga > 0:
        ...
        if General.get("Modulated", False):
            ...
            if vRef:
                ...
                for iv, var in enumerate(["mV0", "mV1", "mV2"]):
                    namstr += "%12s" % (pfx + var)
                    ptstr += f"{parmDict[pfx + var]:12.6f}"
                    if pfx + var in sigDict:
                        Vec[iv] = parmDict[pfx + var]
                        sigstr += f"{sigDict[pfx + var]:12.6f}"
                    else:
                        sigstr += 12 * " "
                if pFile:
                    pFile.write(namstr + "\n")
        ...
        if Phase["General"].get("doPawley"):
            ...
            if pFile:
                ...
                for irb, RBObj in enumerate(RBModels.get("Spin", [])):
                    ...
                    for ish in range(len(RBObj["RBId"])):  # shell no.
                        ...
                        rbsx = ":%d:%d" % (iAt, jrb)
                        pFile.write(
                            " Spin rigid body parameters for at. no. %d; shell %d:\n"
                            % (iAt, ish)
                        )
                        if not ish:
                ...
                if General.get("Modulated", False):
                    ...
                    for Stype in ["Sfrac", "Spos", "Sadp", "Smag"]:
                        ...
                        if len(Waves):
                            ...
                            continue
                        for iw, _wave in enumerate(Waves[1:]):
                            stiw = str(i) + ":" + str(iw)
                            if Stype == "Spos":
                                if (
                                    waveType
            ...
            if pFile:
                PrintAtomsAndSig(General, Atoms, sigDict, sigKey)
...
def GetHistogramPhaseData(
    Phases, Histograms, Controls=None, Print=True, pFile=None, resetRefList=True
):
    ...
    def PrintSize(hapData):
        if hapData[0] in ["isotropic", "uniaxial"]:
            ...
            line += (
                f"\n\t LG mixing coeff.: {hapData[1][2]:12.4f}"
                + " Refine? "
                + str(hapData[2][2])
            )
            pFile.write(line + "\n")
            ...
            pFile.write(
                f"\n Size model    : {hapData[0]}\n\t LG mixing coeff.:{hapData[1][2]:12.4f} Refine? {hapData[2][2]}\n"
            )
    ...
    for phase in Phases:
        ...
        for histogram in Histograms:
            if not histogram.startswith("PWDR"):
                continue
            if (
                histogram not in HistoPhase
                and phase in Histograms[histogram]["Reflection Lists"]
            ):
        ...
        for histogram in histoList:
            ...
            try:
                if (
                    not HistoPhase[histogram]["Use"]
                ) and "Reflection Lists" in Histograms[
                    histogram
                ]:  # remove previously created  & now unused phase reflection list
            ...
            if "PWDR" in histogram:
                ...
                inst = Histogram["Instrument Parameters"][
                    0
                ]  # TODO - grab table here if present
                ...
                if Phases[phase]["General"]["Type"] == "magnetic":
                    dmin = max(dmin, Phases[phase]["General"].get("MagDmin", 0.0))
                ...
                if "E" not in inst["Type"][0]:
                    ...
                    if hapData["Pref.Ori."][0] == "MD":
                        ...
                        controlDict[pfx + "SHnames"] = G2lat.GenSHCoeff(
                            SGData["SGLaue"], "0", controlDict[pfx + "SHord"], False
                        )
                ...
                if Print:
                    ...
                    if hapDict.get(pfx + "LeBail"):
                        ...
                        if hapData["Pref.Ori."][0] == "MD":
                            ...
                            pFile.write(
                                " March-Dollase PO: %10.4f Refine? %s Axis: %d %d %d\n"
                                % (
                                    hapData["Pref.Ori."][1],
                                    hapData["Pref.Ori."][2],
                                    Ax[0],
                            ...
                            pFile.write(
                                " Penalty hkl list: {} tolerance: {:.2f}\n".format(
                                    controlDict[pfx + "SHhkl"],
                                    controlDict[pfx + "SHtoler"],
                                )
                            )
                ...
                if resetRefList and (
                    not hapDict.get(pfx + "LeBail")
                    or (
                        hapData.get("LeBail", False)
                        and Controls.get("newLeBail", False)
                    )
                ):
                    Scale = Histogram["Sample Parameters"]["Scale"][
                        0
                    ]  # for initializing reflection structure factors.
                    StartI = hapData["Scale"][0] * Scale
                    #### TODO: apply random distribution to StartI - optional??
                    refList = []
                    useExt = (
                        "magnetic" in Phases[phase]["General"]["Type"]
                        and "N" in inst["Type"][0]
                    )
                    if Phases[phase]["General"].get("Modulated", False):
                        ifSuper = True
                        HKLd = np.array(
                ...
                hfx = ":%d:" % (hId)
                for item in inst:
                    if type(inst) is not list and item != "Type":
                        continue  # skip over non-refined items (such as InstName)
                ...
                if (
                    "Primary" in extType
                ):  # makes no sense to have dynamical effects with anything but primary
                    Ekey = ["Ep", "Ma", "Mb"]
                ...
                if len(Twins) > 1:  # force sum to unity
                    hapDict[pfx + "TwinFr:0"] = 1.0 - sumTwFr
                if Print:
                    pFile.write(f"\n Phase: {phase} in histogram: {histogram}\n")
                    pFile.write(135 * "=" + "\n")
                    pFile.write(
                    ...
                    if extType != "None":
                        pFile.write(
                            " Extinction  Type: %15s approx: %10s\n"
                            % (extType, extApprox)
                        )
                        text = " Parameters       :"
                        for eKey in Ekey:
                            text += " %4s : %10.3e Refine? " % (
                                eKey,
                                extParms[eKey][0],
                            ) + str(extParms[eKey][1])
                        pFile.write(text + "\n")
                    ...
                    if len(Twins) > 1:
                        for it, twin in enumerate(Twins):
                            if "bool" in str(type(twin[0])):
                                pFile.write(
                                    " Nonmerohedral twin fr.: {:5.3f} Inv? {} Refine? {}\n".format(
                                        hapDict[pfx + "TwinFr:" + str(it)],
                                        str(controlDict[pfx + "TwinInv"][it]),
                                        str(Twins[0][1][1]),
                                    )
                                )
                            else:
                                pFile.write(
                                    " Twin law: {} Twin fr.: {:5.3f} Refine? {}\n".format(
                                        str(twin[0]).replace("\n", ","),
                                        hapDict[pfx + "TwinFr:" + str(it)],
                                        str(Twins[0][1][1]),
                                    )
...
def SetHistogramPhaseData(
    ...
    def PrintSizeAndSig(hapData, sizeSig):
        ...
        refine = False
        if hapData[0] in ["isotropic", "uniaxial"]:
            line += f" equatorial:{hapData[1][0]:12.5f}"
            if sizeSig[0][0]:
                line += f", sig:{sizeSig[0][0]:8.4f}"
                refine = True
            if hapData[0] == "uniaxial":
                line += f" axial:{hapData[1][1]:12.4f}"
                if sizeSig[0][1]:
                    refine = True
                    line += f", sig:{sizeSig[0][1]:8.4f}"
            line += f" LG mix coeff.:{hapData[1][2]:12.4f}"
            if sizeSig[0][2]:
                refine = True
                line += f", sig:{sizeSig[0][2]:8.4f}"
            if refine:
                pFile.write(line + "\n")
        else:
            line += f" LG mix coeff.:{hapData[1][2]:12.4f}"
            ...
            for i, name in enumerate(Snames):
                ptlbls += "%12s" % (name)
                ptstr += f"{hapData[4][i]:12.6f}"
                if sizeSig[1][i]:
                    refine = True
                    sigstr += f"{sizeSig[1][i]:12.6f}"
                else:
                    sigstr += 12 * " "
            if refine:
                pFile.write(line + "\n")
    ...
    def PrintMuStrainAndSig(hapData, mustrainSig, SGData):
        ...
        refine = False
        if hapData[0] in ["isotropic", "uniaxial"]:
            line += f" equatorial:{hapData[1][0]:12.1f}"
            if mustrainSig[0][0]:
                line += f", sig:{mustrainSig[0][0]:8.1f}"
                refine = True
            if hapData[0] == "uniaxial":
                line += f" axial:{hapData[1][1]:12.1f}"
                if mustrainSig[0][1]:
                    line += f", sig:{mustrainSig[0][1]:8.1f}"
            line += f" LG mix coeff.:{hapData[1][2]:12.4f}"
            if mustrainSig[0][2]:
                refine = True
                line += f", sig:{mustrainSig[0][2]:8.3f}"
            if refine:
                pFile.write(line + "\n")
        else:
            line += f" LG mix coeff.:{hapData[1][2]:12.4f}"
            ...
            for i, name in enumerate(Snames):
                ...
                if mustrainSig[1][i]:
                    refine = True
                    sigstr += f"{mustrainSig[1][i]:12.1f}"
                else:
                    sigstr += 12 * " "
            if refine:
                pFile.write(line + "\n")
    ...
    def PrintExtAndSig(pfx, hapData, ScalExtSig):
        ...
        for item in hapData[2]:
            if pfx + item in ScalExtSig:
                text += f"       {item}: "
                text += f"{hapData[2][item][0]:12.2e}"
                if pfx + item in ScalExtSig:
                    text += f" sig: {ScalExtSig[pfx + item]:12.2e}"
        pFile.write(text + "\n")
    ...
    def PrintBabinetAndSig(pfx, hapData, BabSig):
        ...
        for item in hapData:
            ptlbls += "%12s" % (item)
            ptstr += f"{hapData[item][0]:12.3f}"
            if pfx + item in BabSig:
                sigstr += f"{BabSig[pfx + item]:12.3f}"
            else:
                sigstr += 12 * " "
        pFile.write(ptlbls + "\n")
        pFile.write(ptstr + "\n")
            ...
            if "PWDR" in histogram:
                ...
                if "E" not in inst["Type"][0]:
                    ...
                    if hapData["Pref.Ori."][0] == "MD":
                        ...
                        if pfx + "MD" in sigDict and not parmDict.get(pfx + "LeBail"):
                            PhFrExtPOSig.update(
                                {
                                    pfx + "MD": sigDict[pfx + "MD"],
                                }
                            )
                    else:  #'SH' spherical harmonics
                        for item in hapData["Pref.Ori."][5]:
                            hapData["Pref.Ori."][5][item] = parmDict[pfx + item]
                            if pfx + item in sigDict and not parmDict.get(
                                pfx + "LeBail"
                            ):
                                PhFrExtPOSig.update(
                                    {
                ...
                SizeMuStrSig.update(
                    {
                        pfx + "Mustrain": [
                            [0, 0, 0],
                            [0 for i in range(len(hapData["Mustrain"][4]))],
                        ],
                        pfx + "Size": [
                            [0, 0, 0],
                            [0 for i in range(len(hapData["Size"][4]))],
                        ],
                        pfx + "HStrain": {},
                    }
                )
                for item in ["Mustrain", "Size"]:
                    ...
                    if hapData[item][0] in ["isotropic", "uniaxial"]:
                        ...
                        if item == "Size":
                            hapData[item][1][0] = min(
                                10.0, max(0.001, hapData[item][1][0])
                            )
                        ...
                        if hapData[item][0] == "uniaxial":
                            ...
                            if item == "Size":
                                hapData[item][1][1] = min(
                                    10.0, max(0.001, hapData[item][1][1])
                                )
                            if pfx + item + ";a" in sigDict:
                                SizeMuStrSig[pfx + item][0][1] = sigDict[
                                    pfx + item + ";a"
                                ]
                    else:  # generalized for mustrain or ellipsoidal for size
                        Nterms = len(hapData[item][4])
                        for i in range(Nterms):
                            ...
                            if pfx + item + sfx in sigDict:
                                SizeMuStrSig[pfx + item][1][i] = sigDict[
                                    pfx + item + sfx
                                ]
                ...
                if "Twins" in hapData:
                    ...
                    while True:
                        try:
                            hapData["Twins"][it][1] = parmDict[
                                pfx + "TwinFr:" + str(it)
                            ]
                            if pfx + "TwinFr:" + str(it) in sigDict:
                                TwinFrSig[pfx + "TwinFr:" + str(it)] = sigDict[
                                    pfx + "TwinFr:" + str(it)
                                ]
                            if it:
                            ...
                            break
                    hapData["Twins"][0][1][0] = 1.0 - sumTwFr
    ...
    for hist in Phases[phase]["Histograms"]:
        try:
            ...
            continue
        vDict, sDict = G2stMth.calcMassFracs(
            varyList, covMatrix, Phases, hist, Histograms[hist]["hId"]
        )
    ...
    if Print:
        for phase in Phases:
            ...
            for histogram in histoList:
                ...
                hfx = f":{hId}:"
                if (
                    pfx + "Nref" not in Histogram["Residuals"]
                ):  # skip not used phase in histogram
                    continue
                pFile.write(f"\n Phase: {phase} in histogram: {histogram}\n")
                pFile.write(135 * "=" + "\n")
                Inst = Histogram["Instrument Parameters"][0]
                if "PWDR" in histogram:
                    pFile.write(
                        " Final refinement RF, RF^2 = %.2f%%, %.2f%% on %d reflections\n"
                        % (
                            Histogram["Residuals"][pfx + "Rf"],
                            Histogram["Residuals"][pfx + "Rf^2"],
                            Histogram["Residuals"][pfx + "Nref"],
                        )
                    )
                    pFile.write(
                    ...
                    if parmDict.get(pfx + "LeBail") or "E" in Inst["Type"][0]:
                        pFile.write(
                            f" Performed LeBail extraction for phase {phase} in histogram {histogram}\n"
                        )
                    elif "E" not in Inst["Type"][0]:
                        var = pfx + "WgtFrac"
                        ...
                        if pfx + "Extinction" in PhFrExtPOSig:
                            pFile.write(
                                " Extinction coeff: {:10.4f}, sig {:10.4f}\n".format(
                                    hapData["Extinction"][0],
                                    PhFrExtPOSig[pfx + "Extinction"],
                                )
                            )
                        if hapData["Pref.Ori."][0] == "MD":
                            if pfx + "MD" in PhFrExtPOSig:
                                pFile.write(
                                    " March-Dollase PO: {:10.4f}, sig {:10.4f}\n".format(
                                        hapData["Pref.Ori."][1],
                                        PhFrExtPOSig[pfx + "MD"],
                                    )
                                )
                    ...
                    PrintMuStrainAndSig(
                        hapData["Mustrain"], SizeMuStrSig[pfx + "Mustrain"], SGData
                    )
                    ref = PrintHStrainAndSig(
                        hapData["HStrain"], SizeMuStrSig[pfx + "HStrain"], SGData
                    )
                    if ref:
                    ...
                    if pfx + "LayerDisp" in SizeMuStrSig:
                        pFile.write(
                            " Layer displacement : {:10.3f}, sig {:10.3f}\n".format(
                                hapData["Layer Disp"][0],
                                SizeMuStrSig[pfx + "LayerDisp"],
                            )
                        )
                    if (
                        Phases[phase]["General"]["Type"] != "magnetic"
                    ...
                    pFile.write(
                        " Final refinement RF, RF^2 = %.2f%%, %.2f%% on %d reflections (%d user rejected, %d sp.gp.extinct)\n"
                        % (
                            Histogram["Residuals"][pfx + "Rf"],
                            Histogram["Residuals"][pfx + "Rf^2"],
                            Histogram["Residuals"][pfx + "Nref"],
                            Histogram["Residuals"][pfx + "Nrej"],
                            Histogram["Residuals"][pfx + "Next"],
                        )
                    )
...
def GetHistogramData(Histograms, Print=True, pFile=None):
    ...
    def GetBackgroundParms(hId, Background):
        ...
        for i in range(DebyePeaks["nDebye"]):
            debyeNames = [
                ":" + str(hId) + ":DebyeA;" + str(i),
                ":" + str(hId) + ":DebyeR;" + str(i),
                ":" + str(hId) + ":DebyeU;" + str(i),
            ]
            debyeDict.update(
                dict(zip(debyeNames, DebyePeaks["debyeTerms"][i][::2], strict=False))
            )
            debyeList += zip(
                debyeNames, DebyePeaks["debyeTerms"][i][1::2], strict=False
            )
        debyeVary = []
        ...
        for i in range(DebyePeaks["nPeaks"]):
            peakNames = [
                ":" + str(hId) + ":BkPkpos;" + str(i),
                ":" + str(hId) + ":BkPkint;" + str(i),
                ":" + str(hId) + ":BkPksig;" + str(i),
                ":" + str(hId) + ":BkPkgam;" + str(i),
            ]
            peakDict.update(
                dict(zip(peakNames, DebyePeaks["peaksList"][i][::2], strict=False))
            )
        ...
        if "background PWDR" in Background[1]:
            backDict[":" + str(hId) + ":Back File"] = Background[1]["background PWDR"][
                0
            ]
            backDict[":" + str(hId) + ":BF mult"] = Background[1]["background PWDR"][1]
            try:
                if (
                    Background[1]["background PWDR"][0]
                    and Background[1]["background PWDR"][2]
                ):
    ...
    def GetSampleParms(hId, Sample):
        ...
        hfx = ":" + str(hId) + ":"
        sampDict = {
            hfx + "Gonio. radius": Sample["Gonio. radius"],
            hfx + "Omega": Sample["Omega"],
            hfx + "Chi": Sample["Chi"],
            hfx + "Phi": Sample["Phi"],
            hfx + "Azimuth": Sample["Azimuth"],
        }
    ...
    def PrintBackground(Background):
        Back = Background[0]
        DebyePeaks = Background[1]
        pFile.write(f"\n Background function: {Back[0]} Refine? {Back[1]}\n")
        line = " Coefficients: "
        ...
        if "background PWDR" in DebyePeaks:
            try:
                pFile.write(
                    " Fixed background file: {}; mult: {:.3f}  Refine? {}\n".format(
                        DebyePeaks["background PWDR"][0],
                        DebyePeaks["background PWDR"][1],
    ...
    for histogram in histoList:
        if "PWDR" in histogram:
            ...
            for excl in controlDict[pfx + "Exclude"]:
                Histogram["Data"][0] = ma.masked_inside(
                    Histogram["Data"][0], excl[0], excl[1]
                )
            ...
            if Print:
                ...
                if len(controlDict[pfx + "Exclude"]):
                    ...
                    for excl in excls:
                        pFile.write(
                            f" Excluded region:  {excl[0]:8.2f}{units} to {excl[1]:8.2f}{units}\n"
                        )
            ...
            if "X" in Inst["Type"][1]:
                ...
                "SEC" in Inst["Type"][1]
                or "NC" in Inst["Type"][1]
                or "NB" in Inst["Type"][1]
            ):
                histDict[pfx + "Lam"] = Inst["Lam"][1]
...
def SetHistogramData(
    parmDict, sigDict, Histograms, calcControls, Print=True, pFile=None, seq=False
):
    ...
    def SetBackgroundParms(pfx, Background, parmDict, sigDict):
        ...
        backSig = [
            0
            for i in range(
                lenBack + 3 * DebyePeaks["nDebye"] + 4 * DebyePeaks["nPeaks"]
            )
        ]
        ...
        if DebyePeaks["nPeaks"]:
            for i in range(DebyePeaks["nPeaks"]):
                ...
                for j, name in enumerate(names):
                    ...
                    if name in sigDict:
                        backSig[lenBack + 3 * DebyePeaks["nDebye"] + 4 * i + j] = (
                            sigDict[name]
                        )
    ...
    def SetSampleParms(pfx, Sample, parmDict, sigDict):
        if "Bragg" in Sample["Type"]:  # Bragg-Brentano
            ...
            for i, item in enumerate(
                ["Scale", "Shift", "Transparency", "SurfRoughA", "SurfRoughB"]
            ):
    ...
    def PrintBackgroundSig(Background, backSig):
        ...
        if DebyePeaks["nPeaks"]:
            ...
            for ip in range(DebyePeaks["nPeaks"]):
                ...
                pFile.write(ptstr + "\n")
        if (
            "background PWDR" in DebyePeaks
            and len(DebyePeaks["background PWDR"]) >= 3
            and DebyePeaks["background PWDR"][2]
        ):
            pFile.write(
                " Fixed background scale: %.3f(%d)\n"
                % (DebyePeaks["background PWDR"][1], int(1000 * backSig[-1]))
            )
        sumBk = np.array(Histogram["sumBk"])
        pFile.write(
            f" Background sums: empirical {sumBk[0]:.3g}, Debye {sumBk[1]:.3g}, peaks {sumBk[2]:.3g}, Total {np.sum(sumBk):.3g}\n"
        )
    ...
    def PrintInstParmsSig(Inst, instSig):
        ...
        insKeys = [
            ...
            for item in instSig
            if item
            not in ["Type", "Lam1", "Lam2", "Azimuth", "Source", "fltPath", "Bank"]
        ]
        insKeys.sort()
        iBeg = 0
        Ok = True
        while Ok:
            ...
            for name in insKeys[iBeg:iFin]:
                ptlbls += "%12s" % (name)
                ptstr += f"{Inst[name][1]:12.6f}"
                if instSig[name]:
                    refine = True
                    sigstr += f"{instSig[name]:12.6f}"
                else:
                    sigstr += 12 * " "
            if refine:
                pFile.write("\n Instrument Parameters:\n")
    ...
    for histogram in histoList:
        if "PWDR" in histogram:
            ...
            Sample = Histogram["Sample Parameters"]
            parmDict[pfx + "Scale"] = max(
                1.0e-12, parmDict[pfx + "Scale"]
            ...
            if Print:
                ...
                pFile.write(
                    " Other residuals: R = {:.2f}%, R-bkg = {:.2f}%, wR-bkg = {:.2f}% wRmin = {:.2f}%\n".format(
                        Histogram["Residuals"]["R"],
                        Histogram["Residuals"]["Rb"],
...
def WriteRBObjPOAndSig(pfx, rbfx, rbsx, parmDict, sigDict):
    ...
    valstr = "  values:"
    sigstr = "  esds  :"
    for _i, px in enumerate(["Px:", "Py:", "Pz:"]):
        name = pfx + rbfx + px + rbsx
        if name not in parmDict:
            continue
        namstr += "%12s" % ("Pos " + px[1])
        valstr += f"{parmDict[name]:12.5f}"
        if name in sigDict:
            sigstr += f"{sigDict[name]:12.5f}"
        else:
            sigstr += 12 * " "
    for _i, po in enumerate(["Oa:", "Oi:", "Oj:", "Ok:"]):
...
def WriteRBObjTLSAndSig(pfx, rbfx, rbsx, TLS, parmDict, sigDict):
    ...
    if "T" in TLS:
        for _i, pt in enumerate(["T11:", "T22:", "T33:", "T12:", "T13:", "T23:"]):
    ...
    if "L" in TLS:
        ...
        valstr = "  values:"
        sigstr = "  esds  :"
        for _i, pt in enumerate(["L11:", "L22:", "L33:", "L12:", "L13:", "L23:"]):
            name = pfx + rbfx + pt + rbsx
            namstr += "%12s" % (pt[:3])
            valstr += f"{parmDict[name]:12.3f}"
            if name in sigDict:
                sigstr += f"{sigDict[name]:12.3f}"
            else:
                sigstr += 12 * " "
        out.append(namstr + "\n")
        out.append(valstr + "\n")
    ...
    if "S" in TLS:
        ...
        valstr = "  values:"
        sigstr = "  esds  :"
        for _i, pt in enumerate(
            ["S12:", "S13:", "S21:", "S23:", "S31:", "S32:", "SAA:", "SBB:"]
        ):
            name = pfx + rbfx + pt + rbsx
            namstr += "%12s" % (pt[:3])
            valstr += f"{parmDict[name]:12.4f}"
            if name in sigDict:
                sigstr += f"{sigDict[name]:12.4f}"
            else:
                sigstr += 12 * " "
        out.append(namstr + "\n")
        out.append(valstr + "\n")
    ...
    if "U" in TLS:
        ...
        valstr = "  values:" + f"{parmDict[name]:12.5f}"
        if name in sigDict:
            sigstr = "  esds  :" + f"{sigDict[name]:12.5f}"
        else:
            sigstr = "  esds  :" + 12 * " "
        out.append(namstr + "\n")
        out.append(valstr + "\n")
...
def WriteRBObjSHCAndSig(pfx, rbfx, rbsx, parmDict, sigDict, SHC):
    ...
    if name in parmDict:
        ...
        valstr = f"  values:{parmDict[name]:12.5f}"
        sigstr = "  esds  :"
        if name in sigDict:
            sigstr += f"{sigDict[name]:12.5f}"
        else:
...
def WriteResRBModel(RBModel):
    ...
    out.append(
        f"  Orientation defined by: {atNames[rbRef[0]]} -> {atNames[rbRef[1]]} & {atNames[rbRef[0]]} -> {atNames[rbRef[2]]}\n"
    )
    if rbSeq:
        for i, rbseq in enumerate(rbSeq):
            out.append(
                "  Torsion sequence %d Bond: %s - %s riding: %s\n"
                % (
                    i,
                    atNames[rbseq[0]],
                    atNames[rbseq[1]],
                    str([atNames[i] for i in rbseq[3]]),
                )
            )
    return out


def WriteVecRBModel(RBModel, sigDict=None, irb=None):
    ...
    for i in range(len(RBModel["VectMag"])):
        if sigDict and irb is not None:
            name = "::RBV;" + str(i) + ":" + str(irb)
            s = G2mth.ValEsd(RBModel["VectMag"][i], sigDict.get(name, -0.0001))
            out.append("Vector no.: %d Magnitude: %s\n" % (i, s))
        else:
            out.append(
                "Vector no.: %d Magnitude: %8.4f Refine? %s\n"
                % (i, RBModel["VectMag"][i], RBModel["VectRef"][i])
            )
        out.append("  No. Type     vx         vy         vz\n")
        for j, [name, xyz] in enumerate(
            zip(atTypes, RBModel["rbVect"][i], strict=False)
        ):
            out.append(
                "  %d   %2s %10.4f %10.4f %10.4f\n" % (j, name, xyz[0], xyz[1], xyz[2])
            )
