...
class FP_atan_windowed_convolver:
    ...
    def conv_emission(self):
        ...
        mono_width = (xx.b_mono / xx.diffractometer_radius) * math.tan(
            xx.two_theta_mono * math.pi / 360
        )
        dispersion = 2 * math.tan(theta) + mono_width
        relative_passwidth = xx.ibm_source_width / (
            xx.a_mono * math.tan(xx.two_theta_mono * math.pi / 360)
        )  # FW delta-lambda/lambda eq. 6 in paper
        ...
        center_wavelength = (
            xx.dominant_wavelength
        )  # fiducial wavelength from instrument global for mistune, etc.
        center_angle = math.asin(
            center_wavelength / (2.0 * xx.d)
        )  # nominal bragg angle
    ...
    def conv_crystallite_size(self):
        ...
        kwargs = {}
        kwargs.update(
            self.param_dicts["conv_emission"]
        )  # steal our parameters from conv_emission
        kwargs.update(self.param_dicts["conv_global"])
        ...
        widths = xx.strain_lor * (math.pi / 360) * math.tan(theta) + (
        ...
        gfwhm2s = (xx.strain_gauss * (math.pi / 360) * math.tan(theta)) ** 2 + (
...
if __name__ == "__main__":
    ...
    def fixphase(xfrm, second_try_bin=None):
        ...
        if second_try_bin:
            ...
            theta = (
                -math.atan2(zz.imag, zz.real) / second_try_bin
            )  # better average phase shift
    ...
    for twotheta_x in (
        ...
        plt.savefig(
            "realspace_window_{abs_window_pm:.2f}_tth_{twotheta_x:.0f}.pdf".format(
                **locals()
            ),
            bbox_inches="tight",
            transparent=True,
