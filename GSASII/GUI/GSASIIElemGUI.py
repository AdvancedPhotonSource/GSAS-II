...
class PickElement(wx.Dialog):
    ...
    def _init_ctrls(self, prnt, ifMag=False, ifOrbs=False):
        ...
        if self.multiple:
            b = wx.Button(
                self,
                wx.ID_CLOSE,
                pos=wxPoint(16.5 * self.butWid + 25, 7.75 * 24 + 24),
                label="Done",
            )
    ...
    def ElButton(self, name, pos, tip, color):
        ...
        if self.oneOnly:
            ...
            if (
                sum(color) / 3 < 150 and color[1] < 150
            ):  # background is mostly dark, use white letters
...
class PickElements(wx.Dialog):
    ...
    def _init_ctrls(self, prnt, list):
        ...
        for E in self.ElTable:
            PickElements.ElButton(
                self,
                parent=elPanel,
                name=E[0],
                pos=wxPoint(E[1] * 30 + 20, E[2] * 30 + 25),
                tip=E[3],
                color=E[4],
            )
            i += 1
